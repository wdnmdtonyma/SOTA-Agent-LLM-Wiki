...
> `Setup` 是 Claude Code hooks 系统的 hook event; 本节点覆盖它的触发入口、输入 JSON、响应 JSON 和 block 语义。

## 能回答的问题

- `Setup` hook 什么时候触发?
- `Setup` hook 的输入 JSON 有哪些字段?
- `Setup` hook 可以返回哪些 sync/async 响应字段?
- `Setup` hook 是否能 block 当前流程?

## 1 事件身份

`Setup` 的 fire 入口在 `utils/hooks.ts` 的 `executeSetupHooks()`; 该入口构造的 `hookInput.hook_event_name` 是 `Setup`。[E: utils/hooks.ts:3902][E: utils/hooks.ts:3910] `types/hooks.ts` 通过 `isHookEvent(value)` 调用 `HOOK_EVENTS.includes(...)` 校验字符串是否属于 hook event 集合。[E: types/hooks.ts:22][E: types/hooks.ts:23]

## 2 触发时机与 fire 点

初始化或维护 setup 阶段, 调用方把 trigger 交给 `executeSetupHooks()`。 [E: utils/hooks.ts:3902][E: utils/hooks.ts:3910] `executeSetupHooks()` 构造输入后调用 hook executor, 因此 `utils/hooks.ts:3914` 是本节点记录的源码 fire 点。[E: utils/hooks.ts:3914]
...
事件专属响应字段: `hookSpecificOutput.hookEventName` 必须是 `Setup`; 可返回 `additionalContext`, 并由 `processHookJSONOutput()` 写入结果上下文。[E: types/hooks.ts:93][E: types/hooks.ts:94][E: utils/hooks.ts:637][E: utils/hooks.ts:638]

## 6 是否能 block

能通过公共 hook 机制产生 blocking result。`Setup` 走 `executeHooks()` generator, command exit code 2 会产生 `outcome:'blocking'`, 聚合层会 yield blockingError; 调用方是否把该 yielded result 转成业务阻断取决于调用点。[E: utils/hooks.ts:3914][E: utils/hooks.ts:2648][E: utils/hooks.ts:2660][E: utils/hooks.ts:2759][I]
