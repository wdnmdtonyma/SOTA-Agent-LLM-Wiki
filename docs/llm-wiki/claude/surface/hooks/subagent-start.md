...
path: surface/hooks/subagent-start.md
title: SubagentStart Hook 事件
kind: hook-event
tier: T1
source: [types/hooks.ts, utils/hooks.ts]
symbols: [SubagentStart]
...
> `SubagentStart` 是 Claude Code hooks 系统的 hook event; 本节点覆盖它的触发入口、输入 JSON、响应 JSON 和 block 语义。

## 能回答的问题

- `SubagentStart` hook 什么时候触发?
- `SubagentStart` hook 的输入 JSON 有哪些字段?
- `SubagentStart` hook 可以返回哪些 sync/async 响应字段?
- `SubagentStart` hook 是否能 block 当前流程?

## 1 事件身份

`SubagentStart` 的 fire 入口在 `utils/hooks.ts` 的 `executeSubagentStartHooks()`; 该入口构造的 `hookInput.hook_event_name` 是 `SubagentStart`。[E: utils/hooks.ts:3932][E: utils/hooks.ts:3940] `types/hooks.ts` 通过 `isHookEvent(value)` 调用 `HOOK_EVENTS.includes(...)` 校验字符串是否属于 hook event 集合。[E: types/hooks.ts:22][E: types/hooks.ts:23]

## 2 触发时机与 fire 点

subagent 启动时, 调用方把 agentId 和 agentType 交给 `executeSubagentStartHooks()`。 [E: utils/hooks.ts:3932][E: utils/hooks.ts:3940] `executeSubagentStartHooks()` 构造输入后调用 hook executor, 因此 `utils/hooks.ts:3945` 是本节点记录的源码 fire 点。[E: utils/hooks.ts:3945]
...
事件专属响应字段: `hookSpecificOutput.hookEventName` 必须是 `SubagentStart`; 可返回 `additionalContext`, 并由 `processHookJSONOutput()` 写入结果上下文。[E: types/hooks.ts:97][E: types/hooks.ts:98][E: utils/hooks.ts:640][E: utils/hooks.ts:641]

## 6 是否能 block

能通过公共 hook 机制产生 blocking result。`SubagentStart` 走 `executeHooks()` generator, command exit code 2 会产生 `outcome:'blocking'`, 聚合层会 yield blockingError; 调用方是否把该 yielded result 转成业务阻断取决于调用点。[E: utils/hooks.ts:3945][E: utils/hooks.ts:2648][E: utils/hooks.ts:2660][E: utils/hooks.ts:2759][I]
