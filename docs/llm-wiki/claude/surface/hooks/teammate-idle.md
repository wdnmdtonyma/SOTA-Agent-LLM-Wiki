...
path: surface/hooks/teammate-idle.md
title: TeammateIdle Hook 事件
kind: hook-event
tier: T1
source: [types/hooks.ts, utils/hooks.ts]
symbols: [TeammateIdle]
...
> `TeammateIdle` 是 Claude Code hooks 系统的 hook event; 本节点覆盖它的触发入口、输入 JSON、响应 JSON 和 block 语义。

## 能回答的问题

- `TeammateIdle` hook 什么时候触发?
- `TeammateIdle` hook 的输入 JSON 有哪些字段?
- `TeammateIdle` hook 可以返回哪些 sync/async 响应字段?
- `TeammateIdle` hook 是否能 block 当前流程?

## 1 事件身份

`TeammateIdle` 的 fire 入口在 `utils/hooks.ts` 的 `executeTeammateIdleHooks()`; 该入口构造的 `hookInput.hook_event_name` 是 `TeammateIdle`。[E: utils/hooks.ts:3709][E: utils/hooks.ts:3718] `types/hooks.ts` 通过 `isHookEvent(value)` 调用 `HOOK_EVENTS.includes(...)` 校验字符串是否属于 hook event 集合。[E: types/hooks.ts:22][E: types/hooks.ts:23]

## 2 触发时机与 fire 点

teammate 将进入 idle 前, 调用方把 teammateName 和 teamName 交给 `executeTeammateIdleHooks()`。 [E: utils/hooks.ts:3709][E: utils/hooks.ts:3718] `executeTeammateIdleHooks()` 构造输入后调用 hook executor, 因此 `utils/hooks.ts:3723` 是本节点记录的源码 fire 点。[E: utils/hooks.ts:3723]
...
事件专属响应字段: `TeammateIdle` 在 `types/hooks.ts` 的 hookSpecificOutput union 中没有事件专属分支; 该事件仍可使用顶层 sync 字段和 async 字段。[I]

## 6 是否能 block

能 block。`TeammateIdle` 走 `executeHooks()` generator, 公共 blocking result 会被 yield; 文件还提供 TeammateIdle 的阻断反馈格式化函数。[E: utils/hooks.ts:3723][E: utils/hooks.ts:2759][E: utils/hooks.ts:1903][E: utils/hooks.ts:1906]
