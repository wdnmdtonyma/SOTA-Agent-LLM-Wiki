...
path: surface/hooks/worktree-create.md
title: WorktreeCreate Hook 事件
kind: hook-event
tier: T1
source: [types/hooks.ts, utils/hooks.ts]
symbols: [WorktreeCreate]
...
> `WorktreeCreate` 是 Claude Code hooks 系统的 hook event; 本节点覆盖它的触发入口、输入 JSON、响应 JSON 和 block 语义。

## 能回答的问题

- `WorktreeCreate` hook 什么时候触发?
- `WorktreeCreate` hook 的输入 JSON 有哪些字段?
- `WorktreeCreate` hook 可以返回哪些 sync/async 响应字段?
- `WorktreeCreate` hook 是否能 block 当前流程?

## 1 事件身份

`WorktreeCreate` 的 fire 入口在 `utils/hooks.ts` 的 `executeWorktreeCreateHook()`; 该入口构造的 `hookInput.hook_event_name` 是 `WorktreeCreate`。[E: utils/hooks.ts:4928][E: utils/hooks.ts:4933] `types/hooks.ts` 通过 `isHookEvent(value)` 调用 `HOOK_EVENTS.includes(...)` 校验字符串是否属于 hook event 集合。[E: types/hooks.ts:22][E: types/hooks.ts:23]

## 2 触发时机与 fire 点

需要由 hook 创建 worktree 时, 调用方把 name 交给 `executeWorktreeCreateHook()`。 [E: utils/hooks.ts:4928][E: utils/hooks.ts:4933] `executeWorktreeCreateHook()` 构造输入后调用 hook executor, 因此 `utils/hooks.ts:4937` 是本节点记录的源码 fire 点。[E: utils/hooks.ts:4937]
...
| 字段 | 含义 | 证据 |
| --- | --- | --- |
| `hook_event_name` | `WorktreeCreate` literal | [E: utils/hooks.ts:4933] |
| `name` | 要创建的 worktree 名称 | [E: utils/hooks.ts:4934] |

## 4 匹配与 IO

该事件没有事件特定 matcher query; `executeWorktreeCreateHook()` 调用 outside-REPL executor 时没有传入 `matchQuery`。[E: utils/hooks.ts:4937]
`WorktreeCreate` 使用 outside-REPL executor 或专用 outside-REPL wrapper; `executeHooksOutsideREPL()` 会把 hookInput 序列化后并行执行 matched hooks, 并返回 `HookOutsideReplResult[]` 或由 wrapper 消费该数组。[E: utils/hooks.ts:3003][E: utils/hooks.ts:3077][E: utils/hooks.ts:3380]

`WorktreeCreate` 在 outside-REPL 路径中由 `executeHooksOutsideREPL()` 直接序列化 `hookInput`; command hook 调用 `execCommandHook(..., jsonInput, ...)` 后通过 stdin 收到 `jsonInput + '\n'`, HTTP hook 调用 `execHttpHook(..., jsonInput, ...)`。[E: utils/hooks.ts:3077][E: utils/hooks.ts:3286][E: utils/hooks.ts:3290][E: utils/hooks.ts:1210][E: utils/hooks.ts:3193][E: utils/hooks.ts:3196] outside-REPL 的 prompt hook 和 agent hook 会返回 unsupported 结果, function hook 也被视为 outside-REPL 内部错误路径。[E: utils/hooks.ts:3153][E: utils/hooks.ts:3157][E: utils/hooks.ts:3163][E: utils/hooks.ts:3167][E: utils/hooks.ts:3174][E: utils/hooks.ts:3183]
...
事件专属响应字段: `hookSpecificOutput.hookEventName` 可以是 `WorktreeCreate`; 可返回 `worktreePath`。[E: types/hooks.ts:160][E: types/hooks.ts:161] outside-REPL callback path 会把该字段作为 output, HTTP path 也会把该字段作为 output。[E: utils/hooks.ts:3117][E: utils/hooks.ts:3121][E: utils/hooks.ts:3248][E: utils/hooks.ts:3253]

## 6 是否能 block

不是通过 `blocked` 标志阻断; `executeWorktreeCreateHook()` 查找第一个成功且非空的 output, 找不到就 throw `WorktreeCreate hook failed`。[E: utils/hooks.ts:4943][E: utils/hooks.ts:4944][E: utils/hooks.ts:4947][E: utils/hooks.ts:4952]
