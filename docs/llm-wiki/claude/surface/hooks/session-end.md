---
id: hook.session-end
path: surface/hooks/session-end.md
title: SessionEnd Hook 事件
kind: hook-event
tier: T1
...
> `SessionEnd` 是 Claude Code hooks 系统的 hook event; 本节点覆盖它的触发入口、输入 JSON、响应 JSON 和 block 语义。

## 能回答的问题

- `SessionEnd` hook 什么时候触发?
- `SessionEnd` hook 的输入 JSON 有哪些字段?
- `SessionEnd` hook 可以返回哪些 sync/async 响应字段?
- `SessionEnd` hook 是否能 block 当前流程?

## 1 事件身份

`SessionEnd` 的 fire 入口在 `utils/hooks.ts` 的 `executeSessionEndHooks()`; 该入口构造的 `hookInput.hook_event_name` 是 `SessionEnd`。[E: utils/hooks.ts:4097][E: utils/hooks.ts:4115] `types/hooks.ts` 通过 `isHookEvent(value)` 调用 `HOOK_EVENTS.includes(...)` 校验字符串是否属于 hook event 集合。[E: types/hooks.ts:22][E: types/hooks.ts:23]

## 2 触发时机与 fire 点

session 结束或清理时, 调用方把 exit reason 交给 `executeSessionEndHooks()`。 [E: utils/hooks.ts:4097][E: utils/hooks.ts:4115] `executeSessionEndHooks()` 构造输入后调用 hook executor, 因此 `utils/hooks.ts:4119` 是本节点记录的源码 fire 点。[E: utils/hooks.ts:4119]
...
| 字段 | 含义 | 证据 |
| --- | --- | --- |
| `hook_event_name` | `SessionEnd` literal | [E: utils/hooks.ts:4115] |
| `reason` | 退出原因 `ExitReason` | [E: utils/hooks.ts:4116] |

## 4 匹配与 IO

matcher query 使用 `reason`; wrapper 把 `reason` 作为 `matchQuery` 传入, `getMatchingHooks()` 在 `SessionEnd` case 中把 `hookInput.reason` 作为匹配值。[E: utils/hooks.ts:4122][E: utils/hooks.ts:1637][E: utils/hooks.ts:1638]
`SessionEnd` 使用 outside-REPL executor 或专用 outside-REPL wrapper; `executeHooksOutsideREPL()` 会把 hookInput 序列化后并行执行 matched hooks, 并返回 `HookOutsideReplResult[]` 或由 wrapper 消费该数组。[E: utils/hooks.ts:3003][E: utils/hooks.ts:3077][E: utils/hooks.ts:3380]

`SessionEnd` 在 outside-REPL 路径中由 `executeHooksOutsideREPL()` 直接序列化 `hookInput`; command hook 调用 `execCommandHook(..., jsonInput, ...)` 后通过 stdin 收到 `jsonInput + '\n'`, HTTP hook 调用 `execHttpHook(..., jsonInput, ...)`。[E: utils/hooks.ts:3077][E: utils/hooks.ts:3286][E: utils/hooks.ts:3290][E: utils/hooks.ts:1210][E: utils/hooks.ts:3193][E: utils/hooks.ts:3196] outside-REPL 的 prompt hook 和 agent hook 会返回 unsupported 结果, function hook 也被视为 outside-REPL 内部错误路径。[E: utils/hooks.ts:3153][E: utils/hooks.ts:3157][E: utils/hooks.ts:3163][E: utils/hooks.ts:3167][E: utils/hooks.ts:3174][E: utils/hooks.ts:3183]
...
事件专属响应字段: `SessionEnd` 在 `types/hooks.ts` 的 hookSpecificOutput union 中没有事件专属分支; 该事件仍可使用顶层 sync 字段和 async 字段。[I]

## 6 是否能 block

不能通过该 wrapper 阻断关闭流程。`executeSessionEndHooks()` 只对失败结果写 stderr, 然后清理 session hooks; 它不读取 `blocked` 字段。[E: utils/hooks.ts:4119][E: utils/hooks.ts:4128][E: utils/hooks.ts:4130][E: utils/hooks.ts:4137]
