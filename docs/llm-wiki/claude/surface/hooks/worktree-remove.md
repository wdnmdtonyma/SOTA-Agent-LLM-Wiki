...
path: surface/hooks/worktree-remove.md
title: WorktreeRemove Hook 事件
kind: hook-event
tier: T1
source: [types/hooks.ts, utils/hooks.ts]
symbols: [WorktreeRemove]
...
> `WorktreeRemove` 是 Claude Code hooks 系统的 hook event; 本节点覆盖它的触发入口、输入 JSON、响应 JSON 和 block 语义。

## 能回答的问题

- `WorktreeRemove` hook 什么时候触发?
- `WorktreeRemove` hook 的输入 JSON 有哪些字段?
- `WorktreeRemove` hook 可以返回哪些 sync/async 响应字段?
- `WorktreeRemove` hook 是否能 block 当前流程?

## 1 事件身份

`WorktreeRemove` 的 fire 入口在 `utils/hooks.ts` 的 `executeWorktreeRemoveHook()`; 该入口构造的 `hookInput.hook_event_name` 是 `WorktreeRemove`。[E: utils/hooks.ts:4967][E: utils/hooks.ts:4980] `types/hooks.ts` 通过 `isHookEvent(value)` 调用 `HOOK_EVENTS.includes(...)` 校验字符串是否属于 hook event 集合。[E: types/hooks.ts:22][E: types/hooks.ts:23]

## 2 触发时机与 fire 点

worktree 移除时, 调用方把 worktreePath 交给 `executeWorktreeRemoveHook()`。 [E: utils/hooks.ts:4967][E: utils/hooks.ts:4980] `executeWorktreeRemoveHook()` 构造输入后调用 hook executor, 因此 `utils/hooks.ts:4984` 是本节点记录的源码 fire 点。[E: utils/hooks.ts:4984]
...
| 字段 | 含义 | 证据 |
| --- | --- | --- |
| `hook_event_name` | `WorktreeRemove` literal | [E: utils/hooks.ts:4980] |
| `worktree_path` | 要移除的 worktree path | [E: utils/hooks.ts:4981] |

## 4 匹配与 IO

该事件没有事件特定 matcher query; `executeWorktreeRemoveHook()` 调用 outside-REPL executor 时没有传入 `matchQuery`。[E: utils/hooks.ts:4984]
`WorktreeRemove` 使用 outside-REPL executor 或专用 outside-REPL wrapper; `executeHooksOutsideREPL()` 会把 hookInput 序列化后并行执行 matched hooks, 并返回 `HookOutsideReplResult[]` 或由 wrapper 消费该数组。[E: utils/hooks.ts:3003][E: utils/hooks.ts:3077][E: utils/hooks.ts:3380]

`WorktreeRemove` 在 outside-REPL 路径中由 `executeHooksOutsideREPL()` 直接序列化 `hookInput`; command hook 调用 `execCommandHook(..., jsonInput, ...)` 后通过 stdin 收到 `jsonInput + '\n'`, HTTP hook 调用 `execHttpHook(..., jsonInput, ...)`。[E: utils/hooks.ts:3077][E: utils/hooks.ts:3286][E: utils/hooks.ts:3290][E: utils/hooks.ts:1210][E: utils/hooks.ts:3193][E: utils/hooks.ts:3196] outside-REPL 的 prompt hook 和 agent hook 会返回 unsupported 结果, function hook 也被视为 outside-REPL 内部错误路径。[E: utils/hooks.ts:3153][E: utils/hooks.ts:3157][E: utils/hooks.ts:3163][E: utils/hooks.ts:3167][E: utils/hooks.ts:3174][E: utils/hooks.ts:3183]
...
事件专属响应字段: `WorktreeRemove` 在 `types/hooks.ts` 的 hookSpecificOutput union 中没有事件专属分支; 该事件仍可使用顶层 sync 字段和 async 字段。[I]

## 6 是否能 block

不能通过该 wrapper 阻断移除流程。`executeWorktreeRemoveHook()` 对失败结果只写 debug log, 最后仍返回 `true` 表示 hooks 已运行。[E: utils/hooks.ts:4993][E: utils/hooks.ts:4996][E: utils/hooks.ts:5002]
