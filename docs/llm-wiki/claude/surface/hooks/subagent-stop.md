...
path: surface/hooks/subagent-stop.md
title: SubagentStop Hook 事件
kind: hook-event
tier: T1
source: [types/hooks.ts, utils/hooks.ts]
symbols: [SubagentStop]
...
> `SubagentStop` 是 Claude Code hooks 系统的 hook event; 本节点覆盖它的触发入口、输入 JSON、响应 JSON 和 block 语义。

## 能回答的问题

- `SubagentStop` hook 什么时候触发?
- `SubagentStop` hook 的输入 JSON 有哪些字段?
- `SubagentStop` hook 可以返回哪些 sync/async 响应字段?
- `SubagentStop` hook 是否能 block 当前流程?

## 1 事件身份

`SubagentStop` 的 fire 入口在 `utils/hooks.ts` 的 `executeStopHooks()`; 该入口构造的 `hookInput.hook_event_name` 是 `SubagentStop`。[E: utils/hooks.ts:3639][E: utils/hooks.ts:3673] `types/hooks.ts` 通过 `isHookEvent(value)` 调用 `HOOK_EVENTS.includes(...)` 校验字符串是否属于 hook event 集合。[E: types/hooks.ts:22][E: types/hooks.ts:23]

## 2 触发时机与 fire 点

subagent 即将 stop 时, `executeStopHooks()` 在存在 subagentId 的分支构造 `SubagentStop` 输入。 [E: utils/hooks.ts:3639][E: utils/hooks.ts:3673] `executeStopHooks()` 构造输入后调用 hook executor, 因此 `utils/hooks.ts:3688` 是本节点记录的源码 fire 点。[E: utils/hooks.ts:3688]
...
| 字段 | 含义 | 证据 |
| --- | --- | --- |
| `hook_event_name` | `SubagentStop` literal | [E: utils/hooks.ts:3673] |
| `stop_hook_active` | 是否已在 stop hook 内部 | [E: utils/hooks.ts:3674] |
| `agent_id` | subagent id | [E: utils/hooks.ts:3675] |
| `agent_transcript_path` | subagent transcript path | [E: utils/hooks.ts:3676] |
| `agent_type` | subagent 类型名 | [E: utils/hooks.ts:3677] |
| `last_assistant_message` | 可选的最后一条 assistant 文本 | [E: utils/hooks.ts:3678] |

## 4 匹配与 IO

matcher query 使用 `agent_type`; `executeStopHooks()` 不显式传 `matchQuery`, 但 `getMatchingHooks()` 在 `SubagentStop` case 中把 `hookInput.agent_type` 作为匹配值。[E: utils/hooks.ts:3688][E: utils/hooks.ts:1646][E: utils/hooks.ts:1647]
`SubagentStop` 使用 REPL generator path; `executeHooks()` 会 yield progress、message、blockingError、additional context、permission/update 等聚合结果。[E: utils/hooks.ts:1952][E: utils/hooks.ts:2096][E: utils/hooks.ts:2744][E: utils/hooks.ts:2759]

`SubagentStop` 在 REPL 路径中由 `executeHooks()` 惰性生成 `jsonInput`。[E: utils/hooks.ts:2133] command hook 通过 stdin 收到 `jsonInput + '\n'`。[E: utils/hooks.ts:1210] prompt hook 只在 `toolUseContext` guard 通过后把 `jsonInput` 传给 `execPromptHook()`。[E: utils/hooks.ts:2224][E: utils/hooks.ts:2230][E: utils/hooks.ts:2234] agent hook 只在 `toolUseContext` 和 `messages` guard 通过后把 `jsonInput` 传给 `execAgentHook()`。[E: utils/hooks.ts:2256][E: utils/hooks.ts:2262][E: utils/hooks.ts:2267][E: utils/hooks.ts:2271] HTTP hook 把 `jsonInput` 传给 `execHttpHook()`, 但 `SessionStart` 和 `Setup` 会在匹配后过滤 HTTP hooks。[E: utils/hooks.ts:2302][E: utils/hooks.ts:2305][E: utils/hooks.ts:1853][E: utils/hooks.ts:1854] stdout 以 `{` 开头时按 `HookJSONOutput` 解析, 非 JSON stdout 被当作 plain text; HTTP hook 的空 body 可当作空 JSON 对象, 非 JSON body 会成为 validation error。[E: utils/hooks.ts:404][E: utils/hooks.ts:411][E: utils/hooks.ts:459][E: utils/hooks.ts:469]
...
事件专属响应字段: `SubagentStop` 在 `types/hooks.ts` 的 hookSpecificOutput union 中没有事件专属分支; 该事件仍可使用顶层 sync 字段和 async 字段。[I]

## 6 是否能 block

能 block。`SubagentStop` 走 `executeHooks()` generator, command exit code 2 会产生 `outcome:'blocking'`, 聚合层会 yield blockingError。[E: utils/hooks.ts:3688][E: utils/hooks.ts:2648][E: utils/hooks.ts:2660][E: utils/hooks.ts:2759]
