...
path: surface/hooks/user-prompt-submit.md
title: UserPromptSubmit Hook 事件
kind: hook-event
tier: T1
...
> `UserPromptSubmit` 是 Claude Code hooks 系统的 hook event; 本节点覆盖它的触发入口、输入 JSON、响应 JSON 和 block 语义。

## 能回答的问题

- `UserPromptSubmit` hook 什么时候触发?
- `UserPromptSubmit` hook 的输入 JSON 有哪些字段?
- `UserPromptSubmit` hook 可以返回哪些 sync/async 响应字段?
- `UserPromptSubmit` hook 是否能 block 当前流程?

## 1 事件身份

`UserPromptSubmit` 的 fire 入口在 `utils/hooks.ts` 的 `executeUserPromptSubmitHooks()`; 该入口构造的 `hookInput.hook_event_name` 是 `UserPromptSubmit`。[E: utils/hooks.ts:3826][E: utils/hooks.ts:3843] `types/hooks.ts` 通过 `isHookEvent(value)` 调用 `HOOK_EVENTS.includes(...)` 校验字符串是否属于 hook event 集合。[E: types/hooks.ts:22][E: types/hooks.ts:23]

## 2 触发时机与 fire 点

用户 prompt 提交后进入模型处理前, 调用方把 prompt 和 permission mode 交给 `executeUserPromptSubmitHooks()`。 [E: utils/hooks.ts:3826][E: utils/hooks.ts:3843] `executeUserPromptSubmitHooks()` 构造输入后调用 hook executor, 因此 `utils/hooks.ts:3847` 是本节点记录的源码 fire 点。[E: utils/hooks.ts:3847]
...
事件专属响应字段: `hookSpecificOutput.hookEventName` 必须是 `UserPromptSubmit`; 可返回 `additionalContext`, 并由 `processHookJSONOutput()` 写入结果上下文。[E: types/hooks.ts:80][E: types/hooks.ts:81][E: utils/hooks.ts:624][E: utils/hooks.ts:625]

## 6 是否能 block

能 block。公共 blocking result 会被聚合层 yield; 文件还提供 `getUserPromptSubmitHookBlockingMessage()` 将阻断反馈格式化为 `UserPromptSubmit operation blocked by hook`。[E: utils/hooks.ts:2759][E: utils/hooks.ts:2761][E: utils/hooks.ts:1936][E: utils/hooks.ts:1939]
