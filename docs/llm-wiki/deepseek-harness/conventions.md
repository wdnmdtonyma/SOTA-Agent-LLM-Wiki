...
- **模型可见工具集** ground truth = `packages/*/tool-*`(含 `packages/fs/tool-present` 的 `present`)以及同样往 `ctx.tools` 注册的 `packages/plan/plan-mode`、`packages/core/tools`(PTC `run_code`)、`packages/schedule/schedule`、`packages/extensions/tool-cordis`、`packages/experimental/tool-agent-team`(opt-in)。完整性:官方生成器 glob `packages/*/tool-*` 并 **boot 真实 context** 读 `ctx.tools.schemas()`(`docs/tool-catalog.md` 是这份清单的生成物,只当查漏,不当 `[E]`)。多退少补。`str_replace_editor` 包仍在但出厂 preset/bundle 不挂;`present` 挂在 standard/ptc/cordis。
...
- **Profile 模板** = `PROFILE_TEMPLATES`:`web`(live)+ `headless` / `sdk` / `sdk-minimal` / `acp`(startup)。`sdk-minimal` 是唯一不叠 `dsh-base` 的 shipped bundle。`desktop` 是 Electron 应用独占目录,不是 `PROFILE_TEMPLATES` 成员;`dsh --profile desktop` 被拒绝。
- **Loop** = `packages/core/agent-loop`(默认可替换驱动 + `ReactLoopInbox` 在 `src/inbox.ts`)+ `packages/core/agent`(合同 / 事件)。Inbox 实现不在 `agent/src/inbox.ts`。新行为优先挂扩展点,改 loop 必须对照 `docs/architecture.md` 的地图——但 wiki 的 `[E]` 仍只认 loop 源码。
- **会话日志** = `packages/core/session` 的 `SessionEventMap` + `deriveMessages()`(`src/surface.ts`)。compaction 只有 `surfaceOp: replace`,没有 delete。`SESSION_FORMAT_VERSION` 现为 `3`;v0→v1→v2→v3 由 `dsh-session-format` + catalog + 三个 adjacent migrator 在 JSONL load 时执行。system prompt 是 `system/message` 事件,不在 `request/header`。`dsh-session-log-deepseek` 是 DeepSeek 方言投影,不是第二份日志。session 盘 shipped 只有 JSONL(`dsh-session-persistence-jsonl`);`dsh-session-persistence-sqlite` 已删除。
