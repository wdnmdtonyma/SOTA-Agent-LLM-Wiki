...
| 角色 | 包 | Cordis 插件名 | 做什么 |
|---|---|---|---|
| Definition | `@deepseek-ai/dsh-skill` | yml `id: skill`（default export `SkillRegistry`） | 进程级服务 `ctx.skills`：分层合并、`list` / `snapshot` / `get`、`registerProvider` / `register` [E: packages/skill/skill/package.json:2] [E: packages/skill/skill/src/index.ts:374] [E: packages/skill/skill/src/index.ts:868] |
| Provider | `@deepseek-ai/dsh-skill-filesystem` | `export const name = 'skill-filesystem'` | 扫本地根、解析 `SKILL.md` / 扁平 `.md`，`ctx.skills.registerProvider` [E: packages/skill/skill-filesystem/package.json:2] [E: packages/skill/skill-filesystem/src/index.ts:45] [E: packages/skill/skill-filesystem/src/index.ts:136] |
...
| 用户 | 在扫描根下放 `<name>/SKILL.md` 或扁平 `<name>.md`（YAML frontmatter 要 `name` + `description`）。[E: packages/skill/skill-filesystem/src/index.ts:729] [E: packages/skill/skill-filesystem/src/index.ts:814] |

读路径：`list` / `snapshot` / `get` 吃 `SkillViewOptions`。`scope` 选层；省略 `scope` 只读 global 层。[E: packages/skill/skill/src/index.ts:118] `list` 是 `snapshot` 的 `skills` 投影，不过滤 invocation。[E: packages/skill/skill/src/index.ts:471] `cwd` 传给 Provider 选项目根，也进 collect cache key。[E: packages/skill/skill/src/index.ts:500] [E: packages/skill/skill/src/index.ts:527]
...
| `'runtime'` 保留 | `registerProvider` 不得使用这个 provider 名。`register()` 默认 provider 就是 `'runtime'`。[E: packages/skill/skill/src/index.ts:24] [E: packages/skill/skill/src/index.ts:406] [E: packages/skill/skill/src/index.ts:450] |

### 层内 `rank`（越小越赢）

同层先比 `rank`，再 `providerOrder`，再 `localOrder`。[E: packages/skill/skill/src/index.ts:808] [E: packages/skill/skill/src/index.ts:809] [E: packages/skill/skill/src/index.ts:810]
...
| runtime `register()` | `250`（`RUNTIME_RANK`） | registry [E: packages/skill/skill/src/index.ts:25] [E: packages/skill/skill/src/index.ts:700] |
...
| `bundled` | `600`（`BUNDLED_SKILL_RANK`） | filesystem bundled 根与 `skill-badge` 共用 [E: packages/skill/skill/src/index.ts:28] [E: packages/skill/skill-filesystem/src/index.ts:262] |

**跨层不比 rank。** `collectFresh` 层序是 `[global, ...chainLayers(scope)]`（远祖在前、本 scope 在后），每一层赢家再 `merged.set(name, entry)`：近层同名**无论 rank 多差**都替换远层。[E: packages/skill/skill/src/index.ts:556] [E: packages/skill/skill/src/index.ts:562] 单测：global rank `10` 负于 preset rank `900`，带 scope 的 `list` 仍拿 preset；无 scope 的 `list()` 看不见 preset 层。[E: packages/skill/skill/tests/skill.spec.ts:1142] [E: packages/skill/skill/tests/skill.spec.ts:1167] [E: packages/skill/skill/tests/skill.spec.ts:1169]

`list()` 返回的摘要按名字码点排序，不按 rank。[E: packages/skill/skill/src/index.ts:486]
...
|---|---|---|---:|---|
| 1 | `<project>/.dsh/skills` | `project-dsh` | 100 | `cwd` 有值才压入。`project` = 向上找第一个 `.git`，找不到就用 `cwd`。[E: packages/skill/skill-filesystem/src/index.ts:258] [E: packages/skill/skill-filesystem/src/index.ts:948] [E: packages/skill/skill-filesystem/src/index.ts:952] |
| 2 | `<project>/.agents/skills` | `project-agents` | 200 | 与 `.dsh/skills` 并列的项目兼容根，不是产品主目录。[E: packages/skill/skill-filesystem/src/index.ts:251] |
| 3 | `customSkillDirs` 各项 | `custom` | 300 | schema 默认 `[]`。[E: packages/skill/skill-filesystem/src/index.ts:258] [E: packages/skill/skill-filesystem/src/index.ts:81] |
| 4 | `$DSH_HOME/skills`（否则 `~/.dsh/skills`） | `user-dsh` | 400 | `dshHome = resolveDshHome(config.dshHome)`。`skipSystem: true`，跳过名为 `.system` 的条目。[E: packages/skill/skill-filesystem/src/index.ts:167] [E: packages/skill/skill-filesystem/src/index.ts:257] [E: packages/skill/skill-filesystem/src/index.ts:727] |
...
`list()` **不过滤** invocation。[E: packages/skill/skill/src/index.ts:471] `disable-model-invocation: true` 的 `user-only-skill` 仍出现在 `ctx.skills.list` 里。[E: packages/skill/skill-filesystem/tests/skill-filesystem.spec.ts:277] [E: packages/skill/skill-filesystem/tests/skill-filesystem.spec.ts:282] 模型 catalog 与人手势各自在 Consumer 边界滤。
