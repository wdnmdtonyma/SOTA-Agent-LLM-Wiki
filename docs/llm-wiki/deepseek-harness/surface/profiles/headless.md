...
1. **Launcher**：`dsh --profile headless …`。`parseDshArgs` 只吃 `--profile` / `--patch` / `--dump-*` / `-V`；第一个它不认识的 token 起全部交给 app。[E: apps/cli/src/args.ts:119] [E: apps/cli/src/args.ts:131] `dsh --profile headless run the tests` 解析成 `{ mode: 'profile', profile: 'headless', patches: [], args: ['run', 'the', 'tests'] }`。[E: apps/cli/tests/args.spec.ts:41] launcher 把 `web` 做成子命令 alias（`dsh web` ≡ `--profile web`）。[E: apps/cli/src/args.ts:156] headless 没有对等 alias；help 例子写的是 `dsh --profile headless "run the tests"`。[E: apps/cli/src/args.ts:67]
2. **Dispatch**：`apps/cli/src/bin.ts` 在 `mode: 'profile'` 动态 `import('./profile-boot.ts')`，调用 `runProfile({ profile, patchFiles, args, environment })`。[E: apps/cli/src/bin.ts:27] [E: apps/cli/src/bin.ts:29]
3. **Boot**：`runProfile` → 文件内 `composeProfile` → `prepareProfile` → `loadProfile`。目录尚无 `package.json` 且名字在 `PROFILE_TEMPLATES` 里时，`initProfile(dir, template.bundles, template.patchReload)` 写出 `$DSH_HOME/profiles/headless/`（含 `patchReload: 'startup'`）。[E: packages/boot/app-boot/src/profile.ts:817] 未知名字（help 里的自定义 `tui`）第一次 `dsh --profile <unknown>` **不会**自动 init，会报错让你走 `dsh plugin --profile <name> add`。[E: packages/boot/app-boot/src/profile.ts:814]
4. **App 旗标**：inner args 经 `provideCmdline` 冻成 `ctx.cmdlineArgs`，并提供 `ctx.appExit`。[E: apps/cli/src/profile-boot.ts:258] [E: packages/boot/cmdline/src/index.ts:87] `headless-startup` 用 commander 程序名 `dsh --profile headless`，位置参数 `[task...]`，多词 `join(' ')`。[E: packages/bundle/headless/src/startup.ts:33] [E: packages/bundle/headless/src/startup.ts:52]
...
| `-h` / `--help` | `headless-startup` | 打 app help，**不** provide `headlessStartup`，runner 保持 pending，进程 `appExit(0)`。[E: packages/bundle/headless/tests/startup.spec.ts:105] |
...
| 符号 | 值 | 作用 |
|---|---|---|
| `PROFILE_TEMPLATES.headless` | bundles 两元组 + `patchReload: 'startup'` | 第一次 `loadProfile('headless')` 自动 `initProfile` 时写入。[E: packages/boot/app-boot/src/profile.ts:146] [E: packages/boot/app-boot/src/profile.ts:817] |
...
| 其它 list | 用户所有 | 三元组后再加 `custom-bundle`、或任何不同顺序 / 缺项，normalize **不动**。[E: packages/boot/app-boot/tests/profile.spec.ts:245] |
...
| `tools` | 覆盖 `config` | `mode: !!js process.env.DSH_TOOLS_MODE` | 进程级 PTC 呈现开关。unset 时 `!!js` 得到 `undefined`，`dsh-tools` schema 默认 `native`。[E: packages/bundle/headless/cordis.patch.yml:15] [E: packages/core/tools/src/index.ts:784] |
| `code-runtime` | insert | `name: '@deepseek-ai/dsh-code-runtime-worker-thread'` | PTC `run_code` 的 runtime provider，不是 Web 组件。[E: packages/bundle/headless/cordis.patch.yml:19] [E: packages/bundle/headless/cordis.patch.yml:20] |
...
headless **不**像 web 那样把 base 的 `tool-*` 整表 `disabled: true` 再交给每会话 preset。web 对 `tool-bash` 的 disable 从这里开始。[E: packages/bundle/web-app/cordis.patch.yml:320] 下列 id 继续挂在 headless host 面（平台 `!!js` 仍生效）：

| id | package / 要点 | 源 |
|---|---|---|
| `tool-bash` | `@deepseek-ai/dsh-tool-bash`；`disabled` 当 `win32` | [E: packages/bundle/base/cordis.patch.yml:252] |
| `tool-pwsh` | `@deepseek-ai/dsh-tool-pwsh`；`disabled` 当非 `win32` | [E: packages/bundle/base/cordis.patch.yml:256] |
| `tool-jobs` | `@deepseek-ai/dsh-tool-jobs` | [E: packages/bundle/base/cordis.patch.yml:260] |
| `tool-fs` | `@deepseek-ai/dsh-tool-fs` | [E: packages/bundle/base/cordis.patch.yml:266] |
| `tool-fs-search` | `@deepseek-ai/dsh-tool-fs-search` | [E: packages/bundle/base/cordis.patch.yml:269] |
| `tool-skill` | `@deepseek-ai/dsh-tool-skill` | [E: packages/bundle/base/cordis.patch.yml:289] |
| `tool-subagent-control` | `@deepseek-ai/dsh-tool-subagent-control` | [E: packages/bundle/base/cordis.patch.yml:349] |
...
**叠层（CLI `composeProfile`）**：`composeEntries([bundlePatches, profile.patches, homePatches, overlays])` 按这个数组顺序叠：bundle 层（`dsh.profile.bundles` 顺序，headless 默认先 `dsh-base` 再 `dsh-headless`）→ profile `$DSH_HOME/profiles/headless/cordis.patch.yml` → home `$DSH_HOME/cordis.patch.yml` → `--patch` overlays。[E: apps/cli/src/profile-boot.ts:166] `DSH_TELEMETRY_DISABLED` 非空且树里有 `session-telemetry-otel` 时再 push disable 该行。[E: apps/cli/src/profile-boot.ts:170] 默认 headless 树没有 `agent-presets` 行；`session-telemetry-otel` 行来自 `dsh-base`，env 开关仍然有效。

**旧三元组门控**：`loadProfile` 读完 manifest 必走 `normalizeShippedProfile`。[E: packages/boot/app-boot/src/profile.ts:819] 精确三元组被写成两元组并写入 `patchReload: 'startup'`；多一个用户 bundle 则保持原样。[E: packages/boot/app-boot/tests/profile.spec.ts:234] [E: packages/boot/app-boot/tests/profile.spec.ts:245]

**cmdline 门控**：`headless-startup.apply` 调 `parseCmdline`。[E: packages/bundle/headless/src/startup.ts:56] `task.trim() === ''`（无参数或纯空白）走 `program.error(...)`，**不** `provide(HEADLESS_STARTUP_SERVICE)`。[E: packages/bundle/headless/src/startup.ts:53] commander 拒绝被收成 `appExit`；测试里空 task 退出码 `1`，`--help` 退出码 `0`，两种情况 `runnerConfig` 都是 `undefined`。[E: packages/bundle/headless/tests/startup.spec.ts:96] [E: packages/bundle/headless/tests/startup.spec.ts:105] built-bin：`dsh --profile headless`（无 task）stderr 含 `a task is required`。[E: apps/cli/tests/built-bin.e2e.ts:397]
...
- [`surface.cli.overview`](../cli/overview.md)：launcher 三种 mode、`--profile` / `--patch` / dump 互斥。headless 的 task positional 是 **app** 参数，不是 launcher 旗标。`dsh --profile sdk|sdk-minimal|acp|headless` 与 `dsh web` 并列。
