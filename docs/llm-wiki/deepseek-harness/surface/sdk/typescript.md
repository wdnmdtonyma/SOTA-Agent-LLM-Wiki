...
| `DeepSeekHarness` 的 `provider` / `model` | SDK handshake 省略回落 `'deepseek-official'` / `'deepseek-v4-flash'`（这是客户端默认，不是 base `agent-default-model` 的 `deepseek-flash`）。[E: packages/sdk/client/src/api.ts:42] [E: packages/sdk/client/src/api.ts:43] |
...
2. **孩子必须挂 `sdk-jsonrpc-server`。** `dsh-sdk-app` 与 `dsh-sdk-minimal` 都 insert 该行，且 `inject: [sdkAppStartup, loader]`。[E: packages/bundle/sdk-app/cordis.patch.yml:18] [E: packages/bundle/sdk-app/cordis.patch.yml:19] [E: packages/bundle/sdk-minimal/cordis.patch.yml:11] [E: packages/bundle/sdk-minimal/cordis.patch.yml:13]
