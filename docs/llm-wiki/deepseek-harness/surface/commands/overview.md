...
包 `@deepseek-ai/dsh-commands` 发布 `CommandRuntime`，Loader 插件名 `export const name = 'commands'`，构造 `super(ctx, 'commands')`，键是 `ctx.commands`。 [E: packages/interaction/commands/package.json:2] [E: packages/interaction/commands/src/index.ts:30] [E: packages/interaction/commands/src/index.ts:277]

这是**人**面向 slash 行的注册表，不是模型可见工具。命中后 `execute` 调用 `command.definition.handler(invocation)`。 [E: packages/interaction/commands/src/index.ts:424] 空闲 session 上不会再包一层 `turn/start|end`，也不会多出 `user/message`。 [E: packages/interaction/commands/tests/commands.spec.ts:323] `ask_user_question` 是另一条缝：模型发起的工具，走 `ctx.userQuestions`，结果进 `tool/result`（[surface.tools.ask-user-question](../tools/ask-user-question.md)）。
...
**不是 `ctx.commands` 的斜杠。** `/model` 是 `commandUi.register` 的 client contribution（`popupSelect`），本仓没有 `ctx.commands.register({ name: 'model' })`。 [E: packages/client/ui-model-selection/src/client/index.ts:144] [E: packages/client/ui-model-selection/src/client/index.ts:145] contribution 与 host 命令撞名会抛。 [E: packages/client/ui-commands/src/client/service.ts:279]
...
| 字段 / 符号 | 人怎么碰到 | 机器合同 |
|---|---|---|
| 行首 `/` | 必须从行首写 `/name`；前导空格、裸 `name`、`/` 单独、`/Goal`、`/goal/path` 都不是命令 | `parseCommand`：`^\/([a-z][a-z0-9_-]*)(?=$\|[\t\n\r ])`。对不上返回 `undefined`。 [E: packages/interaction/commands/src/index.ts:126] [E: packages/interaction/commands/tests/commands.spec.ts:51] |
| `register` 的 `name` | 菜单里显示 `/compact`，登记时写 `'compact'` | `COMMAND_NAME = /^[a-z][a-z0-9_-]*$/u`；**不**带前导 `/`。带斜杠或大写过不了 `COMMAND_NAME.test`。 [E: packages/interaction/commands/src/index.ts:32] [E: packages/interaction/commands/src/index.ts:180] |
...
| `ScopedLayers` | 同一进程里，host 全局命令对每个会话可见；preset overlay 可以盖住同名 | `register` 用调用方 ctx 的 `scopeOf`：无 `kScope` 进 eager `global`；有标签进 overlay。`view(agent)` = `layers.merge(agent, …)`：先铺 global，再按祖先链 `set`，近层覆盖。 [E: packages/core/scope/src/index.ts:154] [E: packages/core/scope/src/store.ts:211] [E: packages/interaction/commands/src/index.ts:441] |
| 同层撞名 | 两个插件都想占 `/feedback` | 同一层 `NamedEntries.insert` 抛。global 诊断要求 per-agent 变体挂到 `agent.ctx`；scoped 层是 `already registered in this scope`。跨层允许阴影。 [E: packages/interaction/commands/src/index.ts:95] [E: packages/interaction/commands/tests/commands.spec.ts:128] [E: packages/interaction/commands/tests/commands.spec.ts:130] |
| `command/run` | 卡片出现「正在跑 `/name`」 | 命中后、handler **之前**写入。payload：`commandId` / `name` / 可选 `args` / `source: { kind: 'user' }`。`args` 就是 `rawInput`。 [E: packages/interaction/commands/src/types.ts:94] [E: packages/interaction/commands/src/index.ts:373] |
| `recordInput` | `/feedback` 的正文只该出现一次 | 缺省视为 true。`=== false` 时 `command/run` **省略** `args` 键；handler 仍收到完整 `rawInput`。 [E: packages/interaction/commands/src/index.ts:376] [E: packages/interaction/commands/tests/commands.spec.ts:360] [E: packages/feedback/command-feedback/src/index.ts:124] |
| `command/done` | 卡片结算文案 | 与 `command/run` 用同一 `commandId` 配对。`kind: 'success' \| 'error'`；success 可带 `sourceEventSeq`。 [E: packages/interaction/commands/src/types.ts:105] [E: packages/interaction/commands/src/index.ts:380] |
| admission miss | 打了不像命令的行，或 `/missing` | **不** mint id、**不**写 log。空闲 session 上 `execute('not a command')` / `execute('/missing')` 之后 `session.events` 仍是 `[]`。 [E: packages/interaction/commands/src/index.ts:367] [E: packages/interaction/commands/tests/commands.spec.ts:435] |
| `CommandId` | 把 run/done/RPC 回执对上 | 运行时 `cmd-${8 位 instanceToken}-${seq}`。品牌构造不校验。 [E: packages/interaction/commands/src/index.ts:448] [E: packages/interaction/commands/src/brand.ts:38] |
| `list` / `execute` vs `find` / `register` | 浏览器能发现和提交，拿不到 handler | 前两个带 `@Remote`；`find` 带着 handler，不是 Remote。 [E: packages/interaction/commands/src/index.ts:314] [E: packages/interaction/commands/src/index.ts:285] [E: packages/interaction/commands/src/index.ts:360] |
| 图片附件 | 带图提交 slash | 未声明 `input.attachments` 的命令在 handler 之前 `settle` error；声明了才 `admitCommandAttachments`。 [E: packages/interaction/commands/src/index.ts:391] |

已经 abort 的 `signal` 在 `command/run` **之前**就抛，log 里不会留下半对。 [E: packages/interaction/commands/src/index.ts:371] handler 开始之后的 abort：已有 `command/run`，再补 `command/done { kind: 'error' }`。 [E: packages/interaction/commands/src/index.ts:427]
