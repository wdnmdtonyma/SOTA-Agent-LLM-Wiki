...
| 键 / 符号 | 形状 | 门控 |
|---|---|---|
| `Config.providers` | `Record<route, PiAiProviderProfile>`，**键即 route** | 数组旧形 `Array.isArray` 直接拒。[E: packages/llm/llm-pi-ai/src/config.ts:386] `undefined` / `{}` 经 `Object.entries(providers ?? {})` 得到空 map。[E: packages/llm/llm-pi-ai/src/config.ts:386] |
...
| `declared` | catalog 成员？不是「有没有 profile」 | catalog 路由（含 `deepseek`、`openai-codex`）`false`；手写路由 `true`。 [E: packages/llm/llm-pi-ai/src/index.ts:135] [E: packages/llm/llm-pi-ai/tests/catalog.spec.ts:153] [E: packages/llm/llm-pi-ai/tests/catalog.spec.ts:1217] |
...
本页不枚举整张 pi-ai catalog。配置面只需要记住：dormant 时 directory 登记 **每一个** `catalogProviderIds()` 项（测试钉 `listConfigurableProviders().length > 30`，并含 `openai` **以及** OAuth-only 的 `openai-codex`）。 [E: packages/llm/llm-pi-ai/src/index.ts:138] [E: packages/llm/llm-pi-ai/src/catalog.ts:177] [E: packages/llm/llm-pi-ai/tests/dynamic-config.spec.ts:93] [E: packages/llm/llm-pi-ai/tests/catalog.spec.ts:1202]
...
2. **directory 与 route 拆开。** `apply` 立刻 `ensureDirectory()`：`directoryEntries` 先把 **全部** catalog id 写进 `registerConfigurableProviders`，再并上当前 profiles 的手写键。 [E: packages/llm/llm-pi-ai/src/index.ts:226] [E: packages/llm/llm-pi-ai/src/index.ts:138] [E: packages/llm/llm-pi-ai/src/index.ts:138] [E: packages/llm/llm-pi-ai/src/index.ts:242] 同一次 `apply` 还 `registerModelDiscovery`。所以 dormant 时 Models 页已经有卡可点，包括只走登录流的 catalog 项。

3. **`ensureRegistrationFacts`：空集不 `registerAdapter`。** `routes = [...profiles().keys()]`。还没有 handle 且 `routes.length === 0` 时只记下 facts 然后 `return`。 [E: packages/llm/llm-pi-ai/src/index.ts:280] [E: packages/llm/llm-pi-ai/src/index.ts:284] [E: packages/llm/llm-pi-ai/src/index.ts:286] 这是故意的：`LlmRuntime.registerAdapter([])` 会抛 `INVALID_ADAPTER`。 [E: packages/llm/llm/src/index.ts:387] 第一次非空才 `registerAdapter(routes, adapter)`；之后同一实例 `registration.replace(routes)`。 [E: packages/llm/llm-pi-ai/src/index.ts:288] [E: packages/llm/llm-pi-ai/src/index.ts:290]
