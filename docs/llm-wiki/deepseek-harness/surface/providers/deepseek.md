...
DeepSeek Harness 是 **Cordis 组合运行时**（`profile → bundle → agent preset`）。`dsh web` ≡ `--profile web`（唯一硬编码 profile 别名）。`PROFILE_TEMPLATES` 另有 `headless` / `sdk` / `sdk-minimal` / `acp`（startup）。`web` 把 `@deepseek-ai/dsh-base` 叠上 `@deepseek-ai/dsh-web-app`。本仓没有 shipped TUI。[E: apps/cli/src/args.ts:168] [E: packages/boot/app-boot/src/profile.ts:137] [E: packages/boot/app-boot/src/profile.ts:142]
...
| 入口 | 行为 |
|---|---|
| 新 Agent 默认选择 | `dsh-base` 的 `id: agent-default-model` 写 `provider: deepseek-official`、`model: deepseek-v4-flash`。settings ns `agent-default-model` 可覆盖。[E: packages/bundle/base/cordis.patch.yml:78] [E: packages/bundle/base/cordis.patch.yml:79] [E: packages/core/agent-default-model/src/index.ts:21] [E: packages/core/agent-default-model/tests/agent-default-model.spec.ts:47] |
| `dsh web` Models 页 | onboarding 步 `id: 'deepseek-official'`。directory 条目 `settingsNs: 'llm-deepseek'`、`settingsPath: []`。手写「自定义提供方」卡片写入的 ns 是 `llm-pi-ai`，不是本段。[E: packages/client/ui-settings-models/src/client/index.ts:150] [E: packages/llm/llm-deepseek/src/index.ts:472] [E: packages/client/ui-settings-models/src/client/CustomProviderCard.tsx:38] [E: packages/client/ui-settings-models/src/client/ProviderEditor.tsx:335] |
...
| 字段 | 默认 / 约束 | 用户可见含义 |
|---|---|---|
| `apiKeyEnv` | `DEEPSEEK_API_KEY`（`role('credential-ref')`） | 每请求解析的凭据名 [E: packages/llm/llm-deepseek/src/index.ts:178] |
| `baseURL` | 省略 → `$DEEPSEEK_BASE_URL` → `https://api.deepseek.com` | chat-completions 基址；后面拼 `/chat/completions` [E: packages/llm/llm-deepseek/src/index.ts:200] [E: packages/llm/llm-deepseek/src/adapter.ts:643] |
| `thinking` | 可选 `'enabled' \| 'disabled'` | `disabled` 时 advertised effort 只剩 `off` [E: packages/llm/llm-deepseek/src/index.ts:180] |
| `reasoningEffort` | 可选 `'off' \| 'low' \| 'high' \| 'max'`；advertised 缺省落到 `high` | `thinking: disabled` 时组合里只允许 `off`，否则 **load 失败、不注册** [E: packages/llm/llm-deepseek/src/index.ts:181] [E: packages/llm/llm-deepseek/src/adapter.ts:410] [E: packages/llm/llm-deepseek/src/adapter.ts:426] [E: packages/llm/llm-deepseek/src/index.ts:295] |
| `maxTokens` | `256_000` | 未列出 / 未单独封顶的模型的默认输出帽 [E: packages/llm/llm-deepseek/src/adapter.ts:142] [E: packages/llm/llm-deepseek/src/adapter.ts:409] |
| `defaultContextWindow` | `1_000_000` | catalog 没写容量、或 unlisted id 时的窗口 [E: packages/llm/llm-deepseek/src/adapter.ts:140] [E: packages/llm/llm-deepseek/src/adapter.ts:399] |
| `models` | 默认三条：`deepseek-v4-flash` / `deepseek-v4-pro` / `deepseek-v4-flash-vision-exp`（后一条 `inputModalities: ['text','image']`） | 选择器广告列表；`[]` 合法（广告为空）[E: packages/llm/llm-deepseek/src/index.ts:94] [E: packages/llm/llm-deepseek/src/index.ts:100] [E: packages/llm/llm-deepseek/src/index.ts:106] [E: packages/llm/llm-deepseek/src/index.ts:184] [E: packages/llm/llm-deepseek/tests/adapter.spec.ts:1921] |
| `streamIdleTimeoutMs` | `300_000` | 单次 stream 读空闲上限 [E: packages/llm/llm-deepseek/src/adapter.ts:138] |
| `retryPolicy` | 省略用 llm 普通默认 | **唯一**会触发 `registration.replace` 的字段 [E: packages/llm/llm-deepseek/src/index.ts:196] [E: packages/llm/llm-deepseek/src/index.ts:486] |

catalog **只是广告**。`listModels('deepseek-official')` 给 Models / ACP 选择器；`resolveModel` 对未列出的 id 仍返回 `inputModalities: ['text']`，`requestWithMessages` 把 `options.model` 原样写成 wire `model`。测试用未列入的 `'m'` 也能发出请求。[E: packages/llm/llm-deepseek/src/adapter.ts:381] [E: packages/llm/llm-deepseek/src/adapter.ts:406] [E: packages/llm/llm-deepseek/src/serialize.ts:358] [E: packages/llm/llm-deepseek/tests/adapter.spec.ts:205] [E: packages/llm/llm-deepseek/tests/adapter.spec.ts:1889]

图片：catalog 声明 `image` 的模型（默认 `deepseek-v4-flash-vision-exp`）走 Files API / inline `image_url`。**text-only** 模型（含默认 Flash/Pro 与 unlisted id）在 `assertTextOnly` / stream 入口抛 `LlmError` `UNSUPPORTED_CONTENT`，不会 flatten 成空文本。[E: packages/llm/llm-deepseek/src/serialize.ts:112] [E: packages/llm/llm-deepseek/tests/adapter.spec.ts:1055] [E: packages/llm/llm-deepseek/tests/adapter.spec.ts:1076] 非 `user` 角色带 image 同样 `UNSUPPORTED_CONTENT`。[E: packages/llm/llm-deepseek/src/serialize.ts:121]

**同名不同包。** `dsh-base` 的 `searchProvider: deepseek-official` 是 `@deepseek-ai/dsh-web-search-deepseek` 的 `DEEPSEEK_PROVIDER_ID`，走 Anthropic-compatible `/messages`，基址默认 `https://api.deepseek.com/anthropic/v1`。chat 的 `$DEEPSEEK_BASE_URL` **不**给搜索用；搜索用 `$DEEPSEEK_SEARCH_BASE_URL`。改 `llm-deepseek.baseURL` 换不了搜索。[E: packages/bundle/base/cordis.patch.yml:453] [E: packages/web/web-search-deepseek/src/provider.ts:27] [E: packages/web/web-search-deepseek/src/provider.ts:35] [E: packages/web/web-search-deepseek/src/index.ts:82] [E: packages/web/web-search-deepseek/src/index.ts:110]
...
3. **缺 key 不拒载。** 空 `DEEPSEEK_API_KEY` 时 `listProviders` / `listModels` 仍成功（默认三条：Flash / Pro / Vision-Exp）。第一次 `stream` 才 `MISSING_CREDENTIAL`，文案同时点名 credentials 服务和 `export DEEPSEEK_API_KEY`。存上 key 后下一请求就能打，不用重启。[E: packages/llm/llm-deepseek/tests/adapter.spec.ts:2140] [E: packages/llm/llm-deepseek/tests/adapter.spec.ts:2141] [E: packages/llm/llm-deepseek/tests/adapter.spec.ts:2143] [E: packages/llm/llm-deepseek/src/index.ts:449] [E: packages/llm/llm-deepseek/tests/dynamic-config.spec.ts:163] [E: packages/llm/llm-deepseek/tests/dynamic-config.spec.ts:165]
