...
symbols: [parseDshArgs, DshInvocation, runProfile, runDumpConfig, PROFILE_TEMPLATES]
...
- shipped `--profile` 有哪些?`tui` 是不是模板?未知名字第一次 `dsh --profile <name>` 会怎样?
...
包 `@deepseek-ai/dsh` 发布 bin 名 `dsh`,指向构建产物 `lib/bin.js`。[E: apps/cli/package.json:2][E: apps/cli/package.json:15] 发布 `files` 只含 `lib/*.js`,不再随 CLI 带 `config/` 树;shipped preset 目录在 `@deepseek-ai/dsh-agent-presets` 的 `presets/`。[E: apps/cli/package.json:17][E: packages/preset/agent-presets/src/discovery.ts:60] 源码入口是 `apps/cli/src/bin.ts`(`#!/usr/bin/env node`)。版本字符串从同包 `package.json` 的 `version` 字段读(当前冻结树为 `0.1.2-alpha.2`);`version` 不是 string 时回退 `'0.0.0'`。[E: apps/cli/package.json:4][E: apps/cli/src/bin.ts:21]

`parseDshArgs` 把 `process.argv.slice(2)` 收成导出类型 `DshInvocation` = `ProfileInvocation | DumpConfigInvocation | PluginInvocation`。help / version / 语法错误在 `parseDshArgs` **内部** `process.exit`,只有合法 mode 回到 `bin.ts` 的 `switch`。[E: apps/cli/src/args.ts:48][E: apps/cli/src/args.ts:186][E: apps/cli/src/bin.ts:26][E: apps/cli/src/bin.ts:27]
...
[E: apps/cli/src/bin.ts:27][E: apps/cli/src/bin.ts:37][E: apps/cli/src/bin.ts:42]

`dsh web` 在 commander 里是子命令,action 固定把 profile 写成 `'web'`,再走与默认命令相同的 `resolveBoot`;测试断言 `parse(['web'])` 得到 `{ mode: 'profile', profile: 'web', patches: [], args: [] }`。[E: apps/cli/src/args.ts:168][E: apps/cli/tests/args.spec.ts:28] 这是 launcher **唯一**硬编码的 profile alias。

**host 面 vs agent-preset 面。** launcher / `runProfile` 管的是 **host 面**(进程级):profile 目录、bundle 叠层、`$DSH_HOME` 用户层、`--patch`、`ctx.cmdlineArgs`、信号与 `fail-loud`、可选的 `session-telemetry-otel` 硬关。**agent-preset 面**(每会话的 tools / persona / isolate)由 web bundle 在树里 `insert` `id: agent-presets`(包 `@deepseek-ai/dsh-agent-presets`,`default: standard`);CLI **不再**往 `allPatches` 压一层 shipped root overlay。[E: packages/bundle/web-app/cordis.patch.yml:443][E: packages/bundle/web-app/cordis.patch.yml:445] headless overlay 只 insert `code-runtime` / `headless-startup` / `headless-runner`,没有 `agent-presets` 行。[E: packages/bundle/headless/cordis.patch.yml:19][E: packages/bundle/headless/cordis.patch.yml:22][E: packages/bundle/headless/cordis.patch.yml:26] shipped 四个 preset 目录名是 `minimal` / `standard` / `ptc` / `cordis`(旧名 `code` 即 PTC;wiki id `surface.presets.code` 仍是别名)。[E: packages/preset/agent-presets/src/display.ts:43][E: packages/preset/agent-presets/src/display.ts:44][E: packages/preset/agent-presets/src/display.ts:45][E: packages/preset/agent-presets/src/display.ts:46]

**五个 shipped profile,没有 shipped TUI。** `PROFILE_TEMPLATES` 键为 `acp` / `web` / `headless` / `sdk` / `sdk-minimal`。[E: packages/boot/app-boot/src/profile.ts:137] 其中只有 `web` 的 `patchReload` 是 `'live'`;其余四个是 `'startup'`。[E: packages/boot/app-boot/src/profile.ts:140][E: packages/boot/app-boot/src/profile.ts:144][E: packages/boot/app-boot/src/profile.ts:148][E: packages/boot/app-boot/src/profile.ts:152][E: packages/boot/app-boot/src/profile.ts:156] `sdk-minimal` 的 `bundles` **只有** `@deepseek-ai/dsh-sdk-minimal`,不叠 `dsh-base`。[E: packages/boot/app-boot/src/profile.ts:155] `HELP_EXAMPLES` 把 `tui` 写成 custom profile 示例,它不是模板键;built bin 的 `--help` 也不把 `tui` 列成子命令;光打 `dsh tui` 以缺 `--profile` 退出 1。[E: apps/cli/src/args.ts:68][E: apps/cli/tests/built-bin.e2e.ts:339][E: apps/cli/tests/args.spec.ts:75]
...
4. 合法 invocation 回到 `bin.ts`:`mode: 'profile'` 先 `loadLayeredEnv('dsh')` 再 `runProfile`;另两个 mode 不走 layered env。[E: apps/cli/src/bin.ts:30]

profile 目录在 `$DSH_HOME/profiles/<name>`(`PROFILES_DIR = 'profiles'`)。`$DSH_HOME` 由 `resolveDshHome` 解析:非空环境变量 `DSH_HOME`,否则 `~/.dsh`。[E: packages/boot/app-boot/src/profile.ts:41][E: packages/boot/app-boot/src/profile.ts:133][E: packages/util/home-paths/src/index.ts:18][E: packages/util/home-paths/src/index.ts:87]

`--host` / `--no-open` / `--port` / `--trusted-host` 属于 web app(节点 [`surface.profiles.web`](../profiles/web.md));headless 的 `[task...]` 属于 headless app(节点 [`surface.profiles.headless`](../profiles/headless.md));sdk / acp 的零 extra-flag stdio CLI 属于对应 bundle。launcher 把它们留在 `DshInvocation.args`,经 `provideCmdline` 冻成 `ctx.cmdlineArgs` 快照,由树内插件 `parseCmdline`。[E: apps/cli/tests/args.spec.ts:39][E: apps/cli/tests/args.spec.ts:41][E: packages/boot/cmdline/src/index.ts:84][E: apps/cli/src/profile-boot.ts:258]
...
| `-V, --version` | 根命令 | 打印 `readVersion()` 的字符串 | commander `.version`;built bin 断言 stdout 等于 manifest `version`。[E: apps/cli/src/args.ts:119][E: apps/cli/tests/built-bin.e2e.ts:599] |
...
| `web` | 子命令 | `--profile web` 的 alias,仍 `mode: 'profile'`(或带 dump 旗标时 `mode: 'dump-config'`) | 拒绝父级已解析的 `--profile` / `--patch` / `--dump-*`。[E: apps/cli/src/args.ts:167][E: apps/cli/src/args.ts:152] |
...
已删除、解析期就会失败的入口(测试按 exit 1 锁住):裸 `tui`、`--config`、`-p`、`run <task>`。这些不是 launcher 旗标。[E: apps/cli/tests/args.spec.ts:75][E: apps/cli/tests/args.spec.ts:76][E: apps/cli/tests/args.spec.ts:77][E: apps/cli/tests/args.spec.ts:78]
...
| 外观 | 真正的所有者 | 去哪读 |
|---|---|---|
| `--host` / `--no-open` / `--port` / `--trusted-host` | web 树里的 `web-startup` + `parseCmdline`;`--host 0.0.0.0` 仍拒绝 | [`surface.profiles.web`](../profiles/web.md)[E: packages/bundle/web-app/src/startup.ts:51][E: packages/bundle/web-app/src/startup.ts:52][E: packages/bundle/web-app/src/startup.ts:74] |
...
| sdk / sdk-minimal 零 extra-flag | `dsh --profile ${profile}` help 名;默认 `sdk` | [`surface.profiles.sdk`](../profiles/sdk.md)[E: packages/bundle/sdk-app/src/index.ts:40] |
| acp 零 extra-flag | `dsh --profile acp` | [`surface.profiles.acp`](../profiles/acp.md)[E: packages/bundle/acp-app/src/index.ts:27] |

`parse(['web', '--host', '127.0.0.1', '--port', '8080', '--no-open', '--future-web-flag'])` 的 `args` 原样含这些 token;`parse(['--profile', 'headless', 'run', 'the', 'tests'])` 的 `args` 是 `['run', 'the', 'tests']`。[E: apps/cli/tests/args.spec.ts:39][E: apps/cli/tests/args.spec.ts:41]
...
- 名字在 `PROFILE_TEMPLATES` 里且目录未初始化 → `initProfile(dir, template.bundles, template.patchReload)`。[E: packages/boot/app-boot/src/profile.ts:810][E: packages/boot/app-boot/src/profile.ts:817]
- 名字不在模板里 → 抛 `profile "<name>" does not exist; create it with 'dsh plugin --profile <name> add <package>'`。第一次 `dsh --profile tui`(或任何未知名)**不会**自动 init。[E: packages/boot/app-boot/src/profile.ts:813][E: packages/boot/app-boot/tests/profile.spec.ts:188][E: apps/cli/tests/built-bin.e2e.ts:610]
- `dsh plugin --profile <name> …` 在目录没有 `package.json` 时也会 `initProfile`;未知名用 `DEFAULT_PROFILE_BUNDLES = ['@deepseek-ai/dsh-base']` 与 `template?.patchReload`(未知名则省略,写出默认 live)。[E: apps/cli/src/plugin.ts:123][E: apps/cli/src/plugin.ts:126][E: packages/boot/app-boot/src/profile.ts:166][E: packages/boot/app-boot/src/profile.ts:169]
- 已存在的 `headless` 若 bundles 仍是退役三元组 `dsh-base` + `dsh-web-app` + `dsh-headless`,加载时会被 `normalizeShippedProfile` 写成当前两 bundle 模板;现行 headless **不含** `dsh-web-app`。[E: packages/boot/app-boot/src/profile.ts:162][E: packages/boot/app-boot/src/profile.ts:726]
- 手写 manifest 省略 `patchReload` 时,loader 回退 `'live'`(自定义默认),不是该名字当初模板值。[E: packages/boot/app-boot/src/profile.ts:828]

用户层文件名是 profile 目录内的 `cordis.patch.yml`(`PROFILE_PATCH_FILENAME`)。[E: packages/boot/app-boot/src/profile.ts:44] 细节与 pnpm reconcile 在 [`surface.cli.plugin`](plugin.md)。

### `runProfile` 叠层(host 面真树)

`prepareProfile` 调用 `loadProfile`,并把 profile 内 `cordis.yml` **整文件重写成**空数组 `[]`(Loader 需要真实 include 根;写回的组成行会在下次 boot 重复 insert)。[E: apps/cli/src/profile-boot.ts:119][E: apps/cli/src/profile-boot.ts:120] `composeProfile` 随后 `healProfilesModuleFallback`。[E: apps/cli/src/profile-boot.ts:160][E: apps/cli/src/profile-boot.ts:161]

`allPatches` 顺序:[E: apps/cli/src/profile-boot.ts:137]

1. `bundlePatches`:`dsh.profile.bundles` 各 bundle 的 `dsh.bundle.patch`,安装序。[E: apps/cli/src/profile-boot.ts:164]
2. `profile.patches`:该 profile 的 `cordis.patch.yml`。[E: apps/cli/src/profile-boot.ts:139]
3. `homePatches`:`homePatchPath()` = `$DSH_HOME/cordis.patch.yml`(每 profile 都叠这一层)。[E: apps/cli/src/profile-boot.ts:70][E: apps/cli/src/profile-boot.ts:162]
4. `--patch` overlays(argv 序;`loadOverlayPatches`,缺文件即失败)。[E: apps/cli/src/profile-boot.ts:163]
5. `DSH_TELEMETRY_DISABLED` 为**任意非空**字符串(含 `'0'` / `'false'`)且树里有 `session-telemetry-otel` 行 → `{ id: 'session-telemetry-otel', disabled: true }`;无该行则不生成 patch。[E: apps/cli/src/profile-boot.ts:101][E: apps/cli/src/profile-boot.ts:102][E: apps/cli/src/profile-boot.ts:170][E: apps/cli/tests/telemetry-switch.spec.ts:12]
...
`runDumpConfig` 同样 `prepareProfile`(因此 shipped 名的 `--dump-default-config` 也会触发模板 init),但只 `renderConfigDump` 写 stdout,不调用 `boot`。[E: apps/cli/src/dump-config.ts:31][E: apps/cli/src/dump-config.ts:51] `defaultOnly === false` 时才追加 profile `cordis.patch.yml`、home `$DSH_HOME/cordis.patch.yml`、以及 `--patch` 文件。[E: apps/cli/src/dump-config.ts:36] `defaultOnly` 把 `userLayer: false` 传进 `loadProfile`,避免坏的用户层搞挂默认 dump。[E: packages/boot/app-boot/src/profile.ts:840] dump **没有** `resolveTelemetryPatch` / `DSH_TELEMETRY_DISABLED` 那一层(该 patch 只在 `composeProfile` 里 push),也不求值 `!!js`。shipped preset roots 不会出现在 dump 里:它们是 web-bundle 行,不是 launcher overlay。
...
- [`surface.profiles.web`](../profiles/web.md) — `PROFILE_TEMPLATES.web` = `dsh-base` + `dsh-web-app`,`patchReload: live`;app 旗标含 `--no-open`;host 面挂 webserver / UI,并把 base 上的模型可见工具行 disable,改由每会话 preset 再挂。
