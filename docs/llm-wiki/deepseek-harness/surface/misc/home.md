...
`.env` **不能**改 `DSH_HOME`。`loadLayeredEnv` 先 `const home = resolveDshHome()` 再读项目 / home 的 `.env`；名字带 `DSH_` 前缀的键是 bootstrap-only，写进发现到的文件会抛，只能从继承的进程环境来。[E: packages/boot/app-boot/src/index.ts:120] [E: packages/boot/app-boot/src/index.ts:184] [E: packages/boot/app-boot/src/index.ts:160]
...
| `.agent-presets` | `AgentPresets` 在 `includeUserRoot`（schema 默认 `true`）时追加 `{ path: dshHomePath(USER_PRESET_DIR), trust: 'user' }`；`USER_PRESET_DIR = '.agent-presets'` | `$DSH_HOME/.agent-presets`。换 preset id **只换**扫描到的 composition，不换 home。[E: packages/preset/agent-presets/src/discovery.ts:51] [E: packages/preset/agent-presets/src/index.ts:111] [E: packages/preset/agent-presets/src/index.ts:181] |
...
同根下还有、但不在本表合同里的 join（各包自己拼）：home 级 `cordis.patch.yml`（`homePatchPath`）、用户 skills `join(dshHome, 'skills')`、attachment 后端 `dshHomePath('storages')`。[E: apps/cli/src/profile-boot.ts:70] [E: packages/skill/skill-filesystem/src/index.ts:253] [E: packages/bundle/base/cordis.patch.yml:151]
