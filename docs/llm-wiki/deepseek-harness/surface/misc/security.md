...
默认安装面是叠 `dsh-base` 的 profile：`dsh web` 以及 `dsh --profile headless|sdk|acp`。`--host 0.0.0.0` 被拒属于 [web profile](../profiles/web.md)（`surface.profiles.web`）的 launcher，不是 `ctx.sandbox` 行为。`sdk-minimal` 不叠 `dsh-base`，因此不自动带这三颗行。
...
| 值 | 产品含义 | 围栏怎么做 |
|---|---|---|
| `read-only` | 站桩不允许改文件 | `writeText` / `editText` 立刻 `FS_SANDBOX_DENIED`。[E: packages/fs/fs-sandbox/src/index.ts:127] `writableRoots` **空数组**。[E: packages/sandbox/sandbox/src/roots.ts:53] [E: packages/sandbox/sandbox/tests/roots.spec.ts:27] 读不被本缝挡住。 |
...
- [`surface.profiles.web`](../profiles/web.md)（`surface.profiles.web`）：默认 `dsh web` 组合；`--host 0.0.0.0` 拒绝写在那一页。其它叠 `dsh-base` 的入口是 `dsh --profile headless|sdk|acp`。
