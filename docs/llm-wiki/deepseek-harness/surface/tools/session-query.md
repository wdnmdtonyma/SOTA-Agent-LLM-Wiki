...
execute 还会**无条件**追加 `{ kind: 'cwd', values: [caller.header.cwd] }`。模型不能改 cwd，也不能把搜索扩到别的工作区。[E: packages/session-query/tool-session-query/src/operations.ts:90]
...
| `seq_from` / `seq_to` | `integer` | 否 | 无 | 非负安全整数 | schema 只声明闭区间。当前会话的截断在 execute：`range.to = min(用户 to, lastStepStartSeq - 1)`。[E: packages/session-query/tool-session-query/src/input.ts:72][E: packages/session-query/tool-session-query/src/operations.ts:138] |
