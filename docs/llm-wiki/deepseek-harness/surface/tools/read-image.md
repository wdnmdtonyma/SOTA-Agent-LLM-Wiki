...
- 成功结果是什么：canonical `ImageReadValue`、text envelope、image block？PTC 嵌套时谁把 image 转成 `additionalContexts`？
- `minimal` / `standard` / `ptc` / `cordis` 四个 shipped preset 谁装了 `dsh-tool-fs`，从而有机会露出 `read_image`？
...
它也不做 PDF / 音频 / 视频，不写文件，不改 schema 去广告 sandbox 升权。字节落点是 `ctx.attachments.saveImage`，不是把 base64 塞进 tool result 文本。[E: packages/fs/tool-fs/src/read-image.ts:220]

## 输入 schema

默认 `tool-fs` `Config` boot 之后，模型可见参数只有 `file_path`。`Config` 的四个键（`readLimit` / `readMaxLineLength` / `readMaxBytes` / `readStreamMinSize`）只喂给 `applyReadTool`，不进入 `read_image`。[E: packages/fs/tool-fs/src/index.ts:36][E: packages/fs/tool-fs/src/index.ts:61][E: packages/fs/tool-fs/src/read-image.ts:175]
...
- **`ctx.llm`（可选）**：`assertImageCapableRoute` 用 `ctx.get('llm')` + `resolveModelInfo(provider, model, exec.signal)`。没有 `llm`、或解析不出 provider/model，一律按「路由无法解析」拒绝，不读盘。[E: packages/fs/tool-fs/src/read-image.ts:91][E: packages/fs/tool-fs/src/read-image.ts:92][E: packages/fs/tool-fs/src/read-image.ts:93]
- **`ctx.tools`**：登记、`pre-execute` / `execute` / `post-execute`、把 canonical value render 成 blocks。PTC 嵌套的 image 转发在 `packages/core/tools/src/ptc.ts`。
- **`ctx.systemPrompt`**：插件 boot 需要它（`read` / `write` / `edit` 往里面挂 section）。`applyReadImageTool` 自己不挂 prompt section。

相对路径的 cwd 来自 `sessionResolveOptions` → `sessionCwd(exec, requestedPath)`：`exec.agent?.session.header.cwd`。没有 agent 时该值为 `undefined`，resolve 选项里就不带 `cwd` 字段。含 `..` 的 cwd/路径会先 `canonicalPath`。[E: packages/fs/tool-fs/src/session-cwd.ts:23][E: packages/fs/tool-fs/src/session-cwd.ts:40]

`fs/observed`：缺失目标在 `resolveRegularReadTarget` 里发 `{ kind: 'absent' }`；成功提交附件之后发 `{ kind: 'present', version }`。本工具只 `emit`，不在 `read-image.ts` 里写观察表。[E: packages/fs/tool-fs/src/read-target.ts:27][E: packages/fs/tool-fs/src/read-image.ts:257]

## 执行管线

入口是 `ToolRuntime.execute`：先 `prepareExecution`，再 `completeScheduledExecution` → `dispatchScheduledExecution`（`tools/execute` around-dispatch，内层跑 `definition.execute`），成功或可恢复失败再 `finalizeScheduledExecution` → `postExecute`（`tools/post-execute`）。[E: packages/core/tools/src/index.ts:1333][E: packages/core/tools/src/index.ts:1340][E: packages/core/tools/src/index.ts:1342][E: packages/core/tools/src/index.ts:1600]
...
成员资格只看 `packages/preset/agent-presets/presets/{minimal,standard,ptc,cordis}/agent.cordis.yml` 有没有 `@deepseek-ai/dsh-tool-fs`。`read_image` 不是单独一行：它跟着 `tool-fs` 走，再叠加 host 是否挂了 `attachments`。

shipped host `packages/bundle/base/cordis.patch.yml` 有 `attachment-local`，因此叠了 `dsh-base` 的产品（`web` / `headless` / `sdk` / `acp` profile）里 `attachments` 在。[E: packages/bundle/base/cordis.patch.yml:119] `sdk-minimal` 不叠 `dsh-base`。自定义 composition 拿掉 store，即使装了 `tool-fs`，catalog 里也不会有 `read_image`。[E: packages/fs/tool-fs/src/index.ts:70]
...
- `packages/fs/tool-fs/src/read-image.ts` — wire `read_image`、`imageMediaTypeForPath`、`assertImageCapableRoute`、`execute`、render、downscale envelope
...
- `packages/fs/tool-fs/tests/read-image.spec.ts` — 登记、路由门、admission、PTC 嵌套、HMR
- `packages/attachment/attachment/src/index.ts` — `ctx.attachments` / `AttachmentStore`
- `packages/attachment/attachment-local/src/index.ts` — 默认 byte / pixel / MIME / 归一化限额
- `packages/attachment/attachment-local/src/store.ts` — `IMAGE_TYPE_MISMATCH`、`sha256:` id、downscale `originalDimensions`
...
- `packages/core/tools/src/ptc.ts` — nested image → `deferContext`（legacy `'tools-code-mode'`）
- `packages/bundle/base/cordis.patch.yml` — host 挂 `attachment-local`
- `packages/preset/agent-presets/presets/minimal/agent.cordis.yml` — 无 `tool-fs`
- `packages/preset/agent-presets/presets/standard/agent.cordis.yml` — 装 `tool-fs`
- `packages/preset/agent-presets/presets/ptc/agent.cordis.yml` — 装 `tool-fs` + PTC `mode: ptc`
- `packages/preset/agent-presets/presets/cordis/agent.cordis.yml` — 装 `tool-fs`
...
- [surface.presets.code](../presets/code.md) — shipped PTC preset（目录 `presets/ptc/`）；本工具随 `tool-fs` 装入。
- [subsys.core.code-mode](../../subsystems/core/code-mode.md) — PTC `run_code` 把嵌套 image block 转成 `additionalContexts`。
