...
- 默认 `dsh web` / `dsh --profile sdk|sdk-minimal|acp|headless` 的模型工具表里有没有 `mcp__*`？`apps/cli` 声明了 `@deepseek-ai/dsh-mcp-client` 等于产品自带 MCP server 吗？
- 模型看见的名字怎么拼？`tools/call` 线上发 public name 还是 raw name？超长 / 非法字符会不会把两个 MCP 工具塌成一个？
- 本包桥 resources / prompts 吗？Native `schemas()` 里能看见 output schema 吗？PTC `run_code` SDK 里呢？
...
| 名字 | 谁用 | 例子（`apps/cli/config/examples/mcp-memory/mcp-reference-memory.cordis.yml`） |
|---|---|---|
| insert `id` | Cordis fiber | `memory-mcp-reference` [E: apps/cli/config/examples/mcp-memory/mcp-reference-memory.cordis.yml:4] |
| insert `name` | Loader 解析包 | `@deepseek-ai/dsh-mcp-client` [E: apps/cli/config/examples/mcp-memory/mcp-reference-memory.cordis.yml:5] |
| `config.serverName` | public 工具命名空间 | `reference_memory` [E: apps/cli/config/examples/mcp-memory/mcp-reference-memory.cordis.yml:7] |
...
用户要让模型看见 MCP 工具，必须在 **host overlay**（或 ACP session 的 `mcpServers`）里 insert / 动态 `plugin` 本包并给 `command` 或 `url`。默认产品路径含 `dsh web` 与 `dsh --profile sdk|sdk-minimal|acp|headless`；本仓没有 shipped TUI 模板。
...
CLI 测试把三份 example overlay 钉在 `apps/cli/config/examples/mcp-memory`，`name: '@deepseek-ai/dsh-mcp-client'`，并用 fixture 换成 `mcp__<serverName>__greet` 证明 Loader 能发现工具。[E: apps/cli/tests/memory-mcp-configs.spec.ts:33] [E: apps/cli/tests/memory-mcp-configs.spec.ts:90] [E: apps/cli/tests/memory-mcp-configs.spec.ts:130]

`apply` **await** 首次 connect + `tools/list`，所以 fiber 激活时，成功路径上 `ctx.tools` 已经有这一代工具。[E: packages/mcp/mcp-client/src/index.ts:184]

## 关键字段

`Config` 是 `z.union` 两支，用 `transport` 判别。[E: packages/mcp/mcp-client/src/index.ts:113]
...
- `subsys.core.tools` — `ctx.tools` 注册表与 `schemas()` / `register` / `tools/pre-execute`。本页只写 MCP 工具怎样进这张表、模型看见什么名字。PTC 模式下模型直调被收成 `run_code`，MCP 工具仍可作为 SDK 子调用。
