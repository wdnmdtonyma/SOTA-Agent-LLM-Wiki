...
- profile、bundle、agent preset、isolate 各自管哪一层？shipped profile / bundle / preset 各有哪些？`tui` 是不是 shipped 面？
...
| Consumer | composition / tool-or-policy | — | `inject` 已提供的 `ctx.*` 再做事的包。典型是模型可见工具：`dsh-tool-bash` inject `shell`，`dsh-tool-bash-persistent` inject `terminals`。 [E: packages/shell/tool-bash/src/index.ts:30] [E: packages/shell/tool-bash-persistent/src/index.ts:429] | 和 Provider 分开：Consumer 决定模型看见的名字与 schema。 | `packages/shell/tool-bash/src/index.ts:30` |
| profile | composition / host | `--profile`；shipped 模板 `web` / `headless` / `sdk` / `sdk-minimal` / `acp` | `$DSH_HOME/profiles/<name>/` 下一份目录：`package.json` 的 `dsh.profile.bundles` 排 bundle 层，再叠本目录 `cordis.patch.yml`，再叠 `--patch`。首次用 shipped 名时 `initProfile` 按 `PROFILE_TEMPLATES` 写盘。`web` 的 `patchReload` 是 `'live'`；其余四个 shipped 是 `'startup'`。 [E: packages/boot/app-boot/src/profile.ts:137] [E: packages/boot/app-boot/src/profile.ts:144] | 和 bundle 不同：profile 是用户/部署选的**栈名**；和 preset 不同：profile 是进程级组合，不是每会话 tools。入口不只 `dsh web`：也有 `dsh --profile sdk\|sdk-minimal\|acp\|headless`。 | `packages/boot/app-boot/src/profile.ts:137` |
| bundle | composition / host | `dsh.bundle.patch` | npm 包，`package.json` 里 `"dsh": { "bundle": { "patch": "./cordis.patch.yml" } }`，被 profile 按名推进空入口表。shipped 六个：`@deepseek-ai/dsh-base`、`dsh-web-app`、`dsh-headless`、`dsh-sdk-app`、`dsh-sdk-minimal`、`dsh-acp-app`。`sdk-minimal` 模板 **不**叠 `dsh-base`。 [E: packages/bundle/base/package.json:38] [E: packages/boot/app-boot/src/profile.ts:155] | 和 preset 不同：bundle 改的是 Loader 入口表（host 面为主）；preset 是 standing scope 上的第二棵树。 | `packages/bundle/base/package.json:37` · `packages/bundle/sdk-minimal/package.json:38` |
| agent preset | composition / per-session | preset；shipped `minimal` / `standard` / `ptc` / `cordis` | 目录名 = id，必有 `COMPOSITION_FILE = 'agent.cordis.yml'`。roster `ctx.agentPresets` 每个 id standing mount 一次，Agent 在 `setup` 里 `mount` / `composeFrom` 把 scope key parent 到该 mount。web 工程默认 `standard`。旧目录名 `code` 就是 `ptc`。 [E: packages/preset/agent-presets/src/discovery.ts:37] [E: packages/preset/agent-presets/src/discovery.ts:60] [E: packages/bundle/web-app/cordis.patch.yml:445] | 和 profile 不同：preset 管这个会话的 tools / persona / isolate；和「仓库里有这个包」不同：成员资格只认那四个 shipped `agent.cordis.yml`。 | `packages/preset/agent-presets/src/discovery.ts:60` · `packages/preset/agent-presets/src/index.ts:414` |
| isolate | composition / preset | `isolate: { <svc>: true }` | preset 里 **publish 服务** 的行必须进带 `isolate` 的 group，否则 `leakedServices` 看见它进了 root realm，`mountPreset` 直接抛。`true` = 这条 standing mount 的 entry-local realm。只 `register` 进 host registry 的 tool 行可以不带。 [E: packages/preset/agent-presets/src/mount.ts:407] [E: packages/preset/agent-presets/presets/standard/agent.cordis.yml:177] | 和 host 面 singleton 不同：isolate 阻止两个 preset 把同名服务打进进程全局。 | `packages/preset/agent-presets/src/mount.ts:210` |
| host 面 | composition / process | host plane | 进程级、会话出现之前就要 settle 的组合：webserver / persistence / sandbox / `ctx.fs`·`ctx.shell`·`ctx.subprocess` Provider / jobs·skills·tools **registry** / subagent backend。headless overlay 只 insert `code-runtime` + `headless-startup` + `headless-runner`，**不**挂 roster，模型可见行就坐在 base 全局。 [E: packages/bundle/headless/cordis.patch.yml:19] | 和 agent-preset 面不同：host 行 `inject` 时还没有 Agent 可 key。 | `packages/bundle/headless/cordis.patch.yml:17` · `packages/preset/agent-presets/src/index.ts:430` |
| agent-preset 面 | composition / per-session | agent plane | 每会话 join 的那一层：tools / persona / isolate 服务。web 在 factory `setup` 里 `AgentPresets.mount`；子 Agent 用 `composeFrom` 加入父会话那一代 standing mount，不重新读盘。五个 shipped profile 里**只有 web** 挂 `agent-presets` 行。 [E: packages/preset/agent-presets/src/index.ts:414] [E: packages/bundle/web-app/cordis.patch.yml:442] | 和 host 面不同：换 preset 换的是这个 Agent 看见的 Consumer 行，不换盘/壳 Provider。 | `packages/preset/agent-presets/src/index.ts:414` |
...
| ~/.dsh | util / default home | `DSH_HOME_DIR_NAME`；展示 `~/.dsh` | 未设 `$DSH_HOME` 时的 `join(homedir(), '.dsh')`。`dshHomeDisplay` 对这个默认路径永远印 `~/.dsh`，其它解析结果印 `$DSH_HOME`。 [E: packages/util/home-paths/src/index.ts:12] [E: packages/util/home-paths/src/index.ts:111] | 和 Claude/Pi 的配置根不同：产品数据只认这一根。 | `packages/util/home-paths/src/index.ts:12` |
...
| bash @ dsh-tool-bash-persistent | execution / PTY | wire `bash`；包 `@deepseek-ai/dsh-tool-bash-persistent` | Consumer of `ctx.terminals`：按 owner Agent 串行保 cwd/env。minimal preset 装这条，不装 one-shot。 [E: packages/shell/tool-bash-persistent/src/index.ts:401] [E: packages/preset/agent-presets/presets/minimal/agent.cordis.yml:37] | 和 one-shot `bash` 互斥同名：同一 scope 不能两套都 register。 | `packages/shell/tool-bash-persistent/src/index.ts:429` |
...
| subagent_fork | orchestration / tool | 同一包第二实例；`toolName: subagent_fork` | shipped 再 load 一次 `dsh-tool-subagent`，`provider: fork`。fork provider `inheritsParentContext = true`，seed 为父会话**已完成** turn 前缀。 [E: packages/preset/agent-presets/presets/standard/agent.cordis.yml:202] [E: packages/subagent/subagent-fork-in-process/src/index.ts:71] | 和 `subagent` 不同：名字是配置出来的，不是另一个包；child 看得到父历史。 | `packages/subagent/subagent-fork-in-process/src/index.ts:71` |
...
**两个 `bash`、两个 `pwsh`。** 同 wire 名、两个包、两条 seam。standard 用 one-shot `dsh-tool-bash` + `ctx.shell`；minimal 用 persistent `dsh-tool-bash-persistent` + `ctx.terminals`。`pwsh` 同样拆 one-shot 与 `dsh-tool-pwsh-persistent`。
