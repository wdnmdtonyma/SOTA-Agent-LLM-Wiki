...
- `standard` / `ptc` / `cordis` 为什么能重挂 filesystem 却**不必** `isolate`？`minimal` 还能不能 `list` 到全局层？
...
默认 GUI 入口是 `dsh web`（`--profile web`）。另外还有 shipped profile `headless` / `sdk` / `sdk-minimal` / `acp`（`dsh --profile …`）。本仓没有 shipped TUI 包。
...
| `packages/preset/agent-presets/presets/standard/agent.cordis.yml` | preset 面重挂 filesystem + `tool-skill`（无 isolate 组） |
| `packages/preset/agent-presets/presets/ptc/agent.cordis.yml` | 同上两行（旧 `code` preset；wiki id 仍是 `surface.presets.code`） |
| `packages/preset/agent-presets/presets/minimal/agent.cordis.yml` | 不挂 filesystem / `tool-skill` |
...
| 符号 | 要点 |
|---|---|
| `ctx.skills` / `SkillRegistry` | `super(ctx, 'skills')`。一份 host 服务，内含 `ScopedLayers<SkillLayer>`。 [E: packages/skill/skill/src/index.ts:374] [E: packages/skill/skill/src/index.ts:362] |
| `isSkillName` | `/^[a-z0-9]+(?:-[a-z0-9]+)*$/`。`Bad_Name`、空串、下划线非法。`get` 对非法名直接 `undefined`，不抛。 [E: packages/skill/skill/src/index.ts:21] [E: packages/skill/skill/src/index.ts:35] [E: packages/skill/skill/src/index.ts:501] |
| `SkillSource` | `'project-dsh' \| 'project-agents' \| 'runtime' \| 'user-dsh' \| 'user-agents' \| 'custom' \| 'bundled'`，外加开放 `string`。这是 prompt-visible 来源桶，**本身不是**优先级。 [E: packages/skill/skill/src/index.ts:40] |
...
| `RUNTIME_RANK` | `250`。`register()` 写入的 runtime 技能固定用这个 rank、默认 provider 名 `'runtime'`。 [E: packages/skill/skill/src/index.ts:25] [E: packages/skill/skill/src/index.ts:700] |
| `BUNDLED_SKILL_RANK` | `600`。打包技能与 filesystem 的 bundled 根共用。 [E: packages/skill/skill/src/index.ts:28] |
...
| `SkillInvocationPolicy` | `{ modelInvocable, userInvocable }`。registry 的 `list` **不过滤**；模型 / 人手势各自在边界滤。 [E: packages/skill/skill/src/index.ts:49] [E: packages/skill/skill/src/index.ts:126] [E: packages/skill/skill/src/index.ts:135] |
| `SkillViewOptions` | `cwd` 给 Provider；`scope` 给层选择；`signal` 取消。省略 `scope` 只读 global。 [E: packages/skill/skill/src/index.ts:116] [E: packages/skill/skill/src/index.ts:118] |
| `SkillCatalogSnapshot` | `{ skills, complete }`。不完整观察**不进** collect cache。 [E: packages/skill/skill/src/index.ts:231] [E: packages/skill/skill/src/index.ts:487] |
| `Config.collectCacheMaxEntries` | 默认 `128`。cache key 含 `cwd` + scope 链身份 + `revision`。 [E: packages/skill/skill/src/index.ts:22] [E: packages/skill/skill/src/index.ts:358] [E: packages/skill/skill/src/index.ts:644] |
| `'skills/change'` | **emit**，不是 waterfall。listener throw / reject 只 warn，不能否决登记。 [E: packages/skill/skill/src/index.ts:296] [E: packages/skill/skill/src/index.ts:656] |
...
| `'runtime'` 保留 | `registerProvider` 不得使用这个 provider 名。 [E: packages/skill/skill/src/index.ts:406] [E: packages/skill/skill/src/index.ts:407] |

同层同名 runtime：先到的赢，后来的 warn + no-op disposer，卸掉后来者不会拆赢家。 [E: packages/skill/skill/src/index.ts:444] [E: packages/skill/skill/src/index.ts:445] [E: packages/skill/skill/tests/skill.spec.ts:1039] [E: packages/skill/skill/tests/skill.spec.ts:1041]
...
flowchart TD
  ...
  Preset["standard/ptc/cordis: remount filesystem + tool-skill"] --> ScopeLayer["register into standing scope layer"]
  HostReg --> Merge
...
1. **host 面挂 Definition。** `dsh-base` 插入 `id: skill` / `name: '@deepseek-ai/dsh-skill'`。`SkillRegistry` 构造 `super(ctx, 'skills')`，键是 `ctx.skills`。这是进程级服务，不是 preset isolate 里的私有实例。 [E: packages/bundle/base/cordis.patch.yml:273] [E: packages/bundle/base/cordis.patch.yml:274] [E: packages/skill/skill/src/index.ts:374]

2. **base 同时挂三条 Provider / Consumer 行。** 同一份 patch 接着插 `skill-filesystem`、`skill-badge`（`disabled: true`）、`tool-skill`。badge 行在产品树上，但默认不 apply。 [E: packages/bundle/base/cordis.patch.yml:276] [E: packages/bundle/base/cordis.patch.yml:277] [E: packages/bundle/base/cordis.patch.yml:279] [E: packages/bundle/base/cordis.patch.yml:281] [E: packages/bundle/base/cordis.patch.yml:283] [E: packages/bundle/base/cordis.patch.yml:284]

3. **web-app 只关掉 filesystem 与 tool-skill，registry 留下。** overlay 写 `id: skill-filesystem` / `disabled: true` 与 `id: tool-skill` / `disabled: true`。同一文件**没有** `id: skill` 的 disable。默认 GUI 路径 `dsh web` 走这条树：host 仍解析 `ctx.skills`，本地根发现改由 preset 往 **该 standing scope 层** 投递。 [E: packages/bundle/web-app/cordis.patch.yml:393] [E: packages/bundle/web-app/cordis.patch.yml:393] [E: packages/bundle/web-app/cordis.patch.yml:395] [E: packages/bundle/web-app/cordis.patch.yml:396] [I]

4. **preset 重挂进 scope 层，无需 isolate。** `standard` / `ptc` 在 agent-preset 面再写 `skill-filesystem` + `tool-skill`，两行都是顶层 id，不在 `isolate:` 组里。`cordis` 同样两行，并给 filesystem `customSkillDirs` 指向本 preset 目录下的 `skills/`。`skill-filesystem.apply` 只 `ctx.skills.registerProvider(...)`，不 `provide` 新服务；`mountPreset` 的 `leakedServices` 只抓「子树把 process-global service publish 进 root realm」的行。 [E: packages/preset/agent-presets/presets/standard/agent.cordis.yml:84] [E: packages/preset/agent-presets/presets/standard/agent.cordis.yml:87] [E: packages/preset/agent-presets/presets/ptc/agent.cordis.yml:91] [E: packages/preset/agent-presets/presets/ptc/agent.cordis.yml:94] [E: packages/preset/agent-presets/presets/cordis/agent.cordis.yml:263] [E: packages/preset/agent-presets/presets/cordis/agent.cordis.yml:263] [E: packages/skill/skill-filesystem/src/index.ts:136] [E: packages/preset/agent-presets/src/mount.ts:210] [E: packages/preset/agent-presets/src/mount.ts:221]

5. **`minimal` 不挂 filesystem / tool-skill。** 该 composition 只挂 `persona` + `persistent-shell`，文件里没有 skill 两行。e2e：`minimal` agent 的 `ctx.skills.list({ scope: agent })` 仍能看见 host 全局层（测试 overlay 打开的 `dsh-badge`），但工具表只有 `bash`。 [E: packages/preset/agent-presets/presets/minimal/agent.cordis.yml:9] [E: packages/preset/agent-presets/presets/minimal/agent.cordis.yml:21] [E: apps/cli/tests/web-agent-presets.e2e.ts:474] [I]

6. **headless / sdk / acp 不搬 discovery。** `dsh-headless` 只 overlay persona / tools mode，再 insert `code-runtime` / startup / runner，**没有** disable `skill-filesystem` 或 `tool-skill`，也没有 `agent-presets`。叠 `dsh-base` 的 `headless` / `sdk` / `acp` 路径上 filesystem 留在 host 全局层。这不是 `dsh web` 默认树。`sdk-minimal` 不叠 base，另有自己的完整 insert。 [E: packages/bundle/headless/cordis.patch.yml:20] [I]

7. **登记进调用方那一层。** `registerProvider` / `register` 走 `this.layers.effect(this.ctx, ...)`；`ScopedLayers.effect` 读 `scopeOf(ctx)`：无标签写 `global`，有标签才懒创建 overlay。同层 provider 名唯一（global 报 `already registered`，scoped 报 `already registered in this scope`）。fiber dispose 卸登记并 `invalidateCache`。 [E: packages/skill/skill/src/index.ts:411] [E: packages/core/scope/src/store.ts:231] [E: packages/core/scope/src/store.ts:236] [E: packages/skill/skill/tests/skill.spec.ts:1206]

8. **读：global → 远祖 → 近层，近者覆盖。** `collectFresh` 的层序是 `[global, ...chainLayers(scope)]`；`chainLayers` 远祖在前、本 scope 在后。每一层先 `collectLayer`（层内按 rank 排序，同名只留第一），再 `merged.set(name, entry)` —— 近层同名**无论 rank 多差**都替换远层。测试：global rank `10` 负于 preset rank `900`，带 scope 的 `list` 仍拿 preset。无 scope 的 `list()` 看不见 preset 层。 [E: packages/skill/skill/src/index.ts:556] [E: packages/skill/skill/src/index.ts:562] [E: packages/core/scope/src/store.ts:192] [E: packages/skill/skill/tests/skill.spec.ts:1142] [E: packages/skill/skill/tests/skill.spec.ts:1167] [E: packages/skill/skill/tests/skill.spec.ts:1169]

9. **空白会话换 preset 靠 scope 链，不靠改 registry。** cache key 含 `scopeChainOf`。测试把 agent key `bindScopeParent` 到 preset A，再 `rebind` 到 B，下一次 `list({ scope: agentKey })` 换成 B 的技能，registry 零写入。 [E: packages/skill/skill/src/index.ts:527] [E: packages/skill/skill/tests/skill.spec.ts:1187] [E: packages/skill/skill/tests/skill.spec.ts:1192] [E: packages/skill/skill/tests/skill.spec.ts:1192]

10. **web 产品树上的可见性。** e2e 在临时工程写 `<proj>/.dsh/skills/project-proof/SKILL.md`，mount `standard` 后：无 scope 的 `list({ cwd })` **只有**测试打开的全局 `dsh-badge`；带 `scope: agent` 才同时有 `dsh-badge` 与 `project-proof`。这钉死「本地发现在 preset 层、registry 在 host」。 [E: apps/cli/tests/web-agent-presets.e2e.ts:435] [E: apps/cli/tests/web-agent-presets.e2e.ts:438] [E: apps/cli/tests/web-agent-presets.e2e.ts:441]

11. **filesystem 组根。** `FileSystemSkillProvider.roots(cwd)`：`includeDefaultRoots && cwd` 时，先 `findProjectRoot`（向上找第一个 `.git`，找不到就用 `cwd`），再压入 `<project>/.dsh/skills`（`project-dsh`）与 `<project>/.agents/skills`（`project-agents`）；然后 `customSkillDirs`（`custom`）；再 `includeDefaultRoots` 时压 `$DSH_HOME/skills`（`user-dsh`，`skipSystem: true`）与 `$DSH_AGENTS_HOME` 否则 `~/.agents` 下的 `skills`（`user-agents`）；最后可选 `bundledSkillDir` / `DSH_BUNDLED_SKILL_DIR`（`bundled`，`trustedHost: true`）。`dshHome` 走 `resolveDshHome`：显式配置、非空白 `$DSH_HOME`、否则 `~/.dsh`。 [E: packages/skill/skill-filesystem/src/index.ts:251] [E: packages/skill/skill-filesystem/src/index.ts:262] [E: packages/skill/skill-filesystem/src/index.ts:251] [E: packages/skill/skill-filesystem/src/index.ts:262] [E: packages/skill/skill-filesystem/src/index.ts:257] [E: packages/skill/skill-filesystem/src/index.ts:262] [E: packages/skill/skill-filesystem/src/index.ts:262] [E: packages/skill/skill-filesystem/src/index.ts:948] [E: packages/skill/skill-filesystem/src/index.ts:952] [E: packages/util/home-paths/src/index.ts:88] [E: packages/util/home-paths/src/index.ts:90] [E: packages/skill/skill-filesystem/src/index.ts:167] [E: packages/skill/skill-filesystem/src/index.ts:168]

12. **同层赢家（filesystem 测试钉死）。** 五个根都放同名 `same` 时，catalog 赢家 `source === 'project-dsh'`。`user-dsh` 的 `.system/` 不进表。project 压过 runtime，runtime 压过 custom / user（`RUNTIME_RANK=250` 夹在 `project-agents=200` 与 `custom=300` 之间）。无 `.git` 时 project 根回退到 `cwd` 自己。 [E: packages/skill/skill-filesystem/tests/skill-filesystem.spec.ts:202] [E: packages/skill/skill-filesystem/tests/skill-filesystem.spec.ts:203] [E: packages/skill/skill-filesystem/tests/skill-filesystem.spec.ts:231] [E: packages/skill/skill-filesystem/tests/skill-filesystem.spec.ts:231] [E: packages/skill/skill-filesystem/tests/skill-filesystem.spec.ts:209]

13. **发现形状与容错。** 每个根：目录条目 → `<dir>/SKILL.md`；`*.md` 文件 → 扁平技能（`resourceBase.directory` 是根）。缺 frontmatter、非法 YAML、缺 `name`/`description`、`!isSkillName`、旧键 `disableModelInvocation` / `modelInvocable` / `userInvocable`：该文件 warn 后跳过，不拖垮兄弟。invocation 只认 `disable-model-invocation` 与 `user-invocable`（缺省模型可调、用户可调）。 [E: packages/skill/skill-filesystem/src/index.ts:729] [E: packages/skill/skill-filesystem/src/index.ts:731] [E: packages/skill/skill-filesystem/src/index.ts:821] [E: packages/skill/skill-filesystem/src/index.ts:1001] [E: packages/skill/skill-filesystem/src/index.ts:1004] [E: packages/skill/skill-filesystem/src/index.ts:1008] [E: packages/skill/skill-filesystem/tests/skill-filesystem.spec.ts:277] [E: packages/skill/skill-filesystem/tests/skill-filesystem.spec.ts:302]

14. **加载。** `get(name, options)`：非法名 → `undefined`；`collect` 选出赢家后把 `locator` 交回**那个** provider 的 `get`。filesystem 把 locator 当成 `{ path, directory }`，再 `parseSkillFile`。`ctx.fs` 存在且根不是 `trustedHost` 时走 fs 服务；bundled 根与缺 fs 时走 Node `readFile`。返回体 `name` 若与 candidate 不一致，registry 作废该条 cache 并返回 `undefined`。 [E: packages/skill/skill/src/index.ts:501] [E: packages/skill/skill/src/index.ts:507] [E: packages/skill/skill/src/index.ts:512] [E: packages/skill/skill-filesystem/src/index.ts:211] [E: packages/skill/skill-filesystem/src/index.ts:755] [E: packages/skill/skill-filesystem/src/index.ts:849]

15. **失效不是 waterfall。** provider `control.invalidate()`、runtime 登记、层变更都会 `revision++` 并 `emit('skills/change')`。`collect` 最多重试 `MAX_COLLECT_ATTEMPTS = 2`；仍撞 revision 则交不完整、不缓存的结果。filesystem 默认 `watch: true`；`edit` / `write` 的 `fs/observed` 会同步 `observeHostMutation`。`includeDefaultRoots: false` 的隔离实例只看见 `customSkillDirs`，连 `DSH_BUNDLED_SKILL_DIR` 都不收。 [E: packages/skill/skill/src/index.ts:23] [E: packages/skill/skill/src/index.ts:622] [E: packages/skill/skill/src/index.ts:624] [E: packages/skill/skill-filesystem/src/index.ts:143] [E: packages/skill/skill-filesystem/src/index.ts:697] [E: packages/skill/skill-filesystem/tests/skill-filesystem.spec.ts:877]

16. **`skill-badge` 是 bundled Provider，base 默认关掉。** `apply` 登记名为 `dsh-badge` 的 provider，候选 `source: 'bundled'`、`rank: BUNDLED_SKILL_RANK`，`get` 读包装内 `dsh-badge.md`。单测：挂上能 `list`/`get`，dispose 后 catalog 空。产品树 `disabled: true`；e2e 为了测全局层才在测试 overlay 里写 `{ id: 'skill-badge', disabled: false }`。不要把它写成 Web 必开，也不要和 `@deepseek-ai/dsh-client-ui-skill` 混成一个包。 [E: packages/skill/skill-badge/src/index.ts:59] [E: packages/skill/skill-badge/src/index.ts:26] [E: packages/skill/skill-badge/src/index.ts:30] [E: packages/skill/skill-badge/src/index.ts:32] [E: packages/skill/skill-badge/tests/skill-badge.spec.ts:16] [E: packages/skill/skill-badge/tests/skill-badge.spec.ts:29] [E: packages/bundle/base/cordis.patch.yml:281] [E: apps/cli/tests/web-agent-presets.e2e.ts:84]
...
- **近层碾压 rank。** 不要按 `BUNDLED_SKILL_RANK = 600` 推断 preset 盖不过 host。600 只在同一层内垫底。 [E: packages/skill/skill/tests/skill.spec.ts:1142]
- **`SkillSource` 不是优先级。** 两个 `source: 'runtime'` 仍按层 + rank 比。开放 `string` 允许测试 / 第三方自报来源。 [E: packages/skill/skill/src/index.ts:40]
- **名字语法是闭环。** filesystem 非法名只 warn 跳过；`register()` 非法名抛；`get('Bad_Name')` 返回 `undefined`。三条路径都认同一份 `SKILL_NAME`。 [E: packages/skill/skill/src/index.ts:21] [E: packages/skill/skill/tests/skill.spec.ts:1026] [E: packages/skill/skill/tests/skill.spec.ts:1036]
...
- **`user-dsh` 跳过 `.system`。** 其它根不过这个滤镜。 [E: packages/skill/skill-filesystem/src/index.ts:257] [E: packages/skill/skill-filesystem/src/index.ts:727]
- **`list` 按名字码点排序，不按 rank。** rank 只决定谁进表。 [E: packages/skill/skill/src/index.ts:486]
- **invocation 中立。** `disable-model-invocation: true` 的技能仍在 `ctx.skills.list` 里；模型 catalog 与 `/name` 手势各自过滤。本页不写那两条边界的字段表。 [E: packages/skill/skill-filesystem/tests/skill-filesystem.spec.ts:282]
- **provider 名按层唯一。** 两个 preset 可以各挂一个名叫 `filesystem` 的 provider；同一层再挂一次抛错。 [E: packages/skill/skill/tests/skill.spec.ts:1203] [E: packages/skill/skill/tests/skill.spec.ts:1206]
- **`skill-badge` 默认关。** 看见 e2e 里的 `dsh-badge` 不要推论 `dsh web` 开箱就有。 [E: packages/bundle/base/cordis.patch.yml:281] [E: apps/cli/tests/web-agent-presets.e2e.ts:84]
- **`skills/change` 没有 `next()`。** 把它当成 waterfall、指望不调用就挡住别人，是错的。真 waterfall 在 Consumer 的 `agent/pre-step`。 [E: packages/skill/skill/src/index.ts:296] [E: vendor/cordis/src/events.ts:238]
- **隔离 Provider 必须关默认根。** `includeDefaultRoots: false` 才会丢掉环境 bundled 根，避免每个隔离实例用自己的 provider 名再发现一遍应用内置技能。 [E: packages/skill/skill-filesystem/src/index.ts:176] [E: packages/skill/skill-filesystem/tests/skill-filesystem.spec.ts:877]
- **preset 目录名是 `ptc`，不是 `code`。** 旧 Code Mode 预设现为 `packages/preset/agent-presets/presets/ptc/`；节点 id `surface.presets.code` 只是稳定别名。
...
| Provider | `@deepseek-ai/dsh-skill-filesystem`（`FileSystemSkillProvider`）；`@deepseek-ai/dsh-skill-badge`（`dsh-badge`）；任意 `registerProvider` / `register()` | 写入调用方 scope 层；`list(cwd, signal)` / `get(candidate)` | base 挂 filesystem（web 关掉）+ badge（`disabled: true`）。`standard` / `ptc` / `cordis` 重挂 filesystem 进 standing 层；`cordis` 另配 `customSkillDirs`。`minimal` 不挂 |
| Consumer | `@deepseek-ai/dsh-tool-skill`；人手势 / commands（本页不展开） | `inject: ['agents', 'tools', 'skills']`；`agent/pre-step` 必须 `next()` | base 挂 `tool-skill`；web-app disable；`standard` / `ptc` / `cordis` 重挂；`minimal` 不挂 |
...
- [surface.presets.code](../../surface/presets/code.md) — PTC 预设（`presets/ptc/`）同样重挂 filesystem + `tool-skill`。
- [surface.profiles.web](../../surface/profiles/web.md) — 默认 GUI 安装路径 `dsh web`。
