...
- **定义包 ≠ shipped 行。** 在 yml 里写 `@deepseek-ai/dsh-host-directory-picker` 得不到 backend。web 的 id 是 `directory-picker`，name 是 **auto**。[E: packages/bundle/web-app/cordis.patch.yml:80]
