...
- `@deepseek-ai/dsh-typert-protocol`：compiler-independent 的 Remote decorator、段字符校验、`RemoteError` / `remoteErrorOf`，以及把 `Context.typert: TypertRegistryContract` 写进 Cordis。[E: packages/typert/protocol/src/index.ts:20] [E: packages/typert/protocol/src/remote-error.ts:12] [E: packages/typert/protocol/src/types.ts:595]
- `@deepseek-ai/dsh-typert-registry`：`TypertRegistry` Service，构造时 `super(ctx, 'typert')`。`register` 只把传入的 `TypertContribution` 写入 map，不跑 analyzer、不 `z.object` 现场建模。[E: packages/typert/registry/src/service.ts:472] [E: packages/typert/registry/src/service.ts:516] [E: packages/typert/registry/src/service.ts:536]
...
- Host HTTP 业务控制器。`dsh-web-app` 挂 `id: session-controller` / `settings-controller` / `workspace-controller`（三个 `@deepseek-ai/dsh-api-*-controller`），以及 `id: api-remotes`。[E: packages/bundle/web-app/cordis.patch.yml:86] [E: packages/bundle/web-app/cordis.patch.yml:87] [E: packages/bundle/web-app/cordis.patch.yml:166] [E: packages/bundle/web-app/cordis.patch.yml:167] HTTP 面改写见 [`subsys.host.apiproxy`](../host/apiproxy.md)（稳定 id，包已不是 `dsh-host-apiproxy`）。
- `dsh-api-remotes` 的转发事件源：`inject = ['typertGateway']`，`apply` 调 `ctx.typertGateway.registerRemoteEvents`，不再在本包里 `lookups.configure`。[E: packages/api/remotes/src/index.ts:34] [E: packages/api/remotes/src/index.ts:39]
...
| `RemoteError` | `name = 'RemoteError'`；按 `code` 判别，不靠 `instanceof`。`remoteErrorOf` 认 `isDSHRemoteError === true`。[E: packages/typert/protocol/src/remote-error.ts:12] [E: packages/typert/protocol/src/remote-error.ts:29] [E: packages/typert/protocol/src/remote-error.ts:40] Gateway 见到已有 `RemoteError` 会原样再抛，不当成基础设施故障。[E: packages/api/gateway/src/index.ts:859] |
| `Remote` / `RemoteScope` | 标准 method decorator。标记进原型上的版本化 descriptor（属性名 `@deepseek-ai/dsh-typert-protocol/remote-methods`），**不是** emitDecoratorMetadata。`remoteMethods(service)` 按类声明序读快照。[E: packages/typert/protocol/src/index.ts:134] [E: packages/typert/protocol/src/index.ts:176] [E: packages/typert/protocol/src/index.ts:243] |
| `bindTypertRemote` / `typertRemote` | 显式 Service→Gateway 绑定：`service` + `serviceKey` + `namespace`（默认等于 key）。`TypertRemoteService` 构造里写好 `this.typertRemote`。[E: packages/typert/protocol/src/index.ts:143] [E: packages/typert/protocol/src/index.ts:157] Analyzer 失败文案仍写 `typertGateway` 字段名，但识别的成员名是 `typertRemote`。[E: packages/typert/generator/src/analyzer.ts:1147] [E: packages/typert/generator/src/analyzer.ts:967] |
...
| `ctx.typert` 合同 | protocol 钉四个视图：`local` / `remotes` / `lookups` / `contexts`。[E: packages/typert/protocol/src/types.ts:586] registry Host 入口再补 `register` / `get` / `resolve` / `list` / `getPackage` / `listPackages` / `toJSONSchema`。[E: packages/typert/registry/src/index.ts:19] |
...
3. **Registry 进 `ctx`。** Host 用 default export 的 `TypertRegistry` 当 Cordis plugin：`await ctx.plugin(TypertRegistry)`。[E: packages/typert/registry/src/service.ts:738] [E: packages/typert/registry/tests/typert.spec.ts:31] 构造函数 `super(ctx, 'typert')`，内部建 `localStore` / `remoteStore` / `lookupStore` / `contextStore`。[E: packages/typert/registry/src/service.ts:472] Client 面不 import Host 入口，而是 `apply@packages/typert/registry/src/client/index.ts` 里 `new TypertRegistry(ctx)`，`inject = []`。[E: packages/typert/registry/src/client/index.ts:13] [E: packages/typert/registry/src/client/index.ts:7]
...
8. **`TypertRegistry.register@packages/typert/registry/src/service.ts`。** 先 `validatePackage` / `validateSchemas` / `localStore.validate`：重名 package-face、schema key、invocation `id` 或 endpoint 整批拒绝、不落盘。[E: packages/typert/registry/src/service.ts:517] [E: packages/typert/registry/src/service.ts:520] [E: packages/typert/registry/tests/typert.spec.ts:150] 通过后 `ctx.effect('typert.register()')` 写入 packages / schemas / local descriptors；yield 时按 owner 撤回。[E: packages/typert/registry/src/service.ts:523] [E: packages/typert/registry/src/service.ts:536] 登记插件自己的 fiber dispose 也会带走 contribution。[E: packages/typert/registry/tests/typert.spec.ts:144]
...
- **`dsh-web-app` 的 controller 行不是 Typert。** `id: session-controller` 等是业务 Remote 实现；Typert dispatch 在 `dsh-base` 的 `id: typert-gateway`。[E: packages/bundle/web-app/cordis.patch.yml:87] [E: packages/bundle/base/cordis.patch.yml:46]
...
- **`configure` 不等于 `get`。** 只有 resolver、没有 provider 时 `lookups.get(key)` 是 `undefined`。wire 字段声明永远属于后来的 provider。[E: packages/typert/registry/tests/typert.spec.ts:395]
