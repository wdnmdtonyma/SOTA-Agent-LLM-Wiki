...
| `ImageAttachmentLimits` | `maxImageBytes` / `maxImagesPerMessage` / `maxMessageImageBytes` / `maxImagePixels` / `maxImageDimension` / `mediaTypes`。store 的 `validateImageBatch` 拦张数、合计字节、类型；单图字节 / 像素 / 边长在 `prepareImageFile`。`readImageFile` 不接收本结构。 [E: packages/attachment/attachment/src/types.ts:35] [E: packages/attachment/attachment/src/index.ts:63] [E: packages/attachment/attachment-local/src/store.ts:104] [E: packages/attachment/attachment-local/src/store.ts:281] |
| 默认限额（提交侧） | 20 MiB / 20 张 / 200 MiB / 64_000_000 像素 / 8192 边长。 [E: packages/attachment/attachment-local/src/index.ts:28] [E: packages/attachment/attachment-local/src/index.ts:30] [E: packages/attachment/attachment-local/src/index.ts:32] [E: packages/attachment/attachment-local/src/index.ts:34] [E: packages/attachment/attachment-local/src/index.ts:36] |
| 归一化默认 | 存储图 2048×2048 像素预算、长边 8192、编码目标 4 MiB；压缩并发默认 2、上限 8。 [E: packages/attachment/attachment-local/src/index.ts:44] [E: packages/attachment/attachment-local/src/index.ts:46] [E: packages/attachment/attachment-local/src/index.ts:48] [E: packages/attachment/attachment-local/src/index.ts:50] [E: packages/attachment/attachment-local/src/index.ts:52] |
...
| `SESSION_FORMAT_VERSION` | session event 盘为 `0`。shipped 默认 backend 是 jsonl。与 attachment `v1`、SQLite `SCHEMA_VERSION = 20` 正交。 [E: packages/core/session/src/types.ts:51] [E: packages/bundle/base/cordis.patch.yml:110] [E: packages/session/session-persistence-sqlite/src/schema.ts:19] |
| `SurfaceOp` | `'append'` 或 `{ op: 'replace', start, end }`。**没有 delete。** replace 不回收 attachment 对象。 [E: packages/core/session/src/types.ts:359] [E: packages/core/session/src/types.ts:361] |
...
- **批次限额现在在 Definition 的 `validateImageBatch`。** 张数 / 合计字节 / 类型不再是已删除的 ApiProxy 私有门。单图字节 / 像素 / 边长仍在 `prepareImageFile`。`read_image` 用 `min(maxImageBytes, maxMessageImageBytes)` 当读 cap。 [E: packages/attachment/attachment/src/index.ts:64] [E: packages/attachment/attachment/src/index.ts:68] [E: packages/fs/tool-fs/src/read-image.ts:214]
- **`validateImage` 成功 ≠ 已落盘。** 它保证这张图 *能* 写（含归一化能过字节帽）；真正的耐久边界是 `saveImage` / `saveImages` 返回之后。validate / 失败的 batch 不得留下 `attachments/v1`。 [E: packages/attachment/attachment-local/tests/index.spec.ts:150] [E: packages/attachment/attachment-local/tests/index.spec.ts:156]
...
- **`session/flush` 没有 `next()`。** 耐久否决发生在 `llm/stream` / `tools/execute`：那些 listener 先 `flush` 再决定要不要 `next()`。 [E: packages/core/session/src/index.ts:83] [E: packages/session/session-checkpoint-policy/src/index.ts:36]
- **waterfall 漏 `next()` 等于停整条链。** [E: vendor/cordis/src/events.ts:238]
- **export 缺缝是 500，不是空 ZIP。** 后端 `supportsRawArtifacts` 为假则 501：shipped JSONL 为 `true`，仓库里未 bundled 的 SQLite persistence 为 `false`。 [E: packages/session-query/session-log-export/src/index.ts:116] [E: packages/session-query/session-log-export/src/index.ts:124] [E: packages/session/session-persistence-jsonl/src/index.ts:124] [E: packages/session/session-persistence-sqlite/src/index.ts:55]
- **session event `version` ≠ SQLite `SCHEMA_VERSION` ≠ attachment 目录 `v1`。** 前者钉在 `0`；SQLite 表布局 20、非 0 且 ≠ 20 就拒盘。 [E: packages/core/session/src/types.ts:51] [E: packages/session/session-persistence-sqlite/src/schema.ts:19] [E: packages/session/session-persistence-sqlite/src/schema.ts:125]
...
- [subsys.persistence.sqlite](sqlite.md)：仓库有、bundle 没有；`SCHEMA_VERSION = 20` 非 0 且 ≠ 20 拒盘。
