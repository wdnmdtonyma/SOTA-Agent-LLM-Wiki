...
- 新 header 的 `version` 必须等于 `SESSION_FORMAT_VERSION`（现为 `2`）。JSONL catalog 有 adjacent v0→v1→v2；比 2 新仍拒。workspace bootstrap 只 `await this.ctx.sessionPersistence.list()`，把 snapshot 的 header 投影出来。JSONL `list()` 经 `listArtifacts` / `readGenerationHeader`：catalog 把旧 generation 翻成当前逻辑 header；`SessionFormatUnsupportedError`（比 current 新）在 listing 里 **skip**，open 同一 id 仍拒。 [E: packages/core/session/src/types.ts:86] [E: packages/workspace/workspace/src/index.ts:128] [E: packages/workspace/workspace/src/index.ts:592] [E: packages/session/session-persistence/src/index.ts:197] [E: packages/session/session-persistence-jsonl/src/index.ts:353] [E: packages/session/session-persistence-jsonl/src/index.ts:746] [E: packages/session/session-persistence-jsonl/src/index.ts:789]
- session persistence SQLite 包已删除。workspace domain version 2 ≠ session-query schema 8 ≠ storage-sqlite schema 1 ≠ event `version` 2。 [E: packages/session-query/session-query-sqlite/src/schema.ts:8] [E: packages/storage/storage-sqlite/src/schema.ts:20]
...
- compaction 的 surface 替换是 `{ op: 'replace', start, end }`，没有 delete。workspace 记账**不是** surface 节点，不走 `replace`。 [E: packages/core/session/src/types.ts:416] [E: packages/core/session/src/types.ts:418]
...
- **list 上外国/更新 generation 会被 skip。** JSONL listing 对 `SessionFormatUnsupportedError` `continue`；真正 `open` 才把拒绝交给调用方。 [E: packages/session/session-persistence-jsonl/src/index.ts:746] [E: packages/workspace/workspace/src/index.ts:128]
