...
- 新 header 的 `version` 必须等于 `SESSION_FORMAT_VERSION`（现为 `2`）。JSONL catalog 有 adjacent v0→v1→v2；比 2 新仍拒。这跟 FTS `user_version` 正交。 [E: packages/core/session/src/types.ts:86] [E: packages/session/session-persistence/src/storage-contract.ts:50]
...
- compaction 只有 `surfaceOp: { op: 'replace', start, end }`，没有 delete。`SurfaceOp` 联合只有 `'append'` 与 `{ op: 'replace'; start; end }`。检索把被 shadow 的 surface 标成 `shadowed`，不从 `this.log` 删事件。 [E: packages/core/session/src/types.ts:416] [E: packages/core/session/src/types.ts:418] [E: packages/session-query/session-query/src/documents.ts:71]
...
7. **lineage / event 关系不碰 FTS。** `traceSession` 沿 `header.parentSession` 往上走；环抛 `SESSION_QUERY_INVALID_LINEAGE`；缺父则 `complete: false` 并带 `unresolvedParentId`。`session/flush` 是 **parallel**（签名没有 `next`）[E: packages/core/session/src/index.ts:82]；实现是 `Promise.allSettled` [E: packages/core/session/src/index.ts:1121]。这些读路径也不在任何 waterfall 上，没有人必须 `next()`。
...
10. **schema 8：认得出就重建，认不出就拒。** `openSearchDatabase` 读 `PRAGMA application_id` / `user_version`。`application_id` 非 0 且不是 `0x44534851` → 「belongs to another application」，关句柄抛错。已是 `DSHQ` 但 `user_version !== 8` → `resetDerivedSchema`：`DROP TABLE` 所有已识别派生表，把 `user_version` 置 0，再 `ensurePersistentSchema` stamp 回 8。多了不认识的 user table → 拒，**不** DROP 别人的表。这与已删除的 session-persistence-sqlite「拒盘不迁」相反。 [E: packages/session-query/session-query-sqlite/src/schema.ts:58] [E: packages/session-query/session-query-sqlite/src/schema.ts:66] [E: packages/session-query/session-query-sqlite/tests/sqlite.spec.ts:1290]
...
15. **相关 waterfall 必须 `next()`；本缝不在那条链上。** Cordis `Events.waterfall` 把 innermost 从 `args.pop()` 取出 [E: vendor/cordis/src/events.ts:236]，`next()` 里才 `cbs.shift()` [E: vendor/cordis/src/events.ts:238]。checkpoint 在 `llm/stream` / `tools/execute` 上先 `flush` 再 `next()`。query 既不 veto adapter，也不挡 tool body。`session/event` 是 append 之后的 fire-and-forget emit [E: packages/core/session/src/index.ts:645]；漏听不会让 search 落后一整段——下一次 search 会重新 observe。
...
- **两套 version 不要混。** event header `SESSION_FORMAT_VERSION = 2`（JSONL catalog adjacent v0→v1→v2；比 2 新仍拒）；query 派生库 `SESSION_QUERY_SQLITE_SCHEMA_VERSION = 8`（丢表重建）。
...
- [subsys.core.session](../core/session.md)：`Session` / `SessionStore`、`session/flush` parallel、`SESSION_FORMAT_VERSION = 2`。
