...
- 插件名 `llm-pi-ai`，`inject = ['llm']`。 [E: packages/llm/llm-pi-ai/src/index.ts:89] [E: packages/llm/llm-pi-ai/src/index.ts:90]
- `apply` 里的 `PiAiAdapter` 实例、settings 命名空间 `llm-pi-ai`、本地函数 `ensureRegistrationFacts` / `ensureDirectory`。 [E: packages/llm/llm-pi-ai/src/index.ts:143] [E: packages/llm/llm-pi-ai/src/index.ts:271] [E: packages/llm/llm-pi-ai/src/index.ts:226]
...
| directory entry | `registerConfigurableProviders` | Models 页可选的 provider 卡。dormant 时含 **全部** 已安装 catalog id（含 OAuth-only 的 `openai-codex`），再并上手写路由。 [E: packages/llm/llm-pi-ai/src/index.ts:137] [E: packages/llm/llm-pi-ai/src/catalog.ts:177] |
...
3. **`ensureRegistrationFacts@packages/llm/llm-pi-ai/src/index.ts`：零 route 就不 `registerAdapter`。** 它读 `routes = [...profiles().keys()]`。若还没有 `AdapterRegistrationHandle` 且 `routes.length === 0`，只记下 `registeredFacts` 然后 `return`，**不** 调用 `registerAdapter`。 [E: packages/llm/llm-pi-ai/src/index.ts:280] [E: packages/llm/llm-pi-ai/src/index.ts:284] [E: packages/llm/llm-pi-ai/src/index.ts:286] 这是故意的：`LlmRuntime.registerAdapter([])` 会抛 `INVALID_ADAPTER`（「an adapter must register at least one provider」）。 [E: packages/llm/llm/src/index.ts:388] `replace([])` 才合法，留给已经拿过 handle 的清空路径。 [E: packages/llm/llm/src/index.ts:406]
