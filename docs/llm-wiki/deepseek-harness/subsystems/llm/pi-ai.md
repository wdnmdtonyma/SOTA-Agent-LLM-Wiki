...
- `apply` 里的 `PiAiAdapter` 实例、settings 命名空间 `llm-pi-ai`、本地函数 `ensureRegistrationFacts` / `ensureDirectory`。 [E: packages/llm/llm-pi-ai/src/index.ts:144] [E: packages/llm/llm-pi-ai/src/index.ts:271] [E: packages/llm/llm-pi-ai/src/index.ts:226]
- `Config.providers`：route 键 → `PiAiProviderProfile`；缺省 / 空 dict = dormant。 [E: packages/llm/llm-pi-ai/src/config.ts:222] [E: packages/llm/llm-pi-ai/src/config.ts:341]
...
- 默认 `provider` / `model` 选择 — [subsys.core.agent-default-model](../core/agent-default-model.md)。`dsh-base` 写的是 `deepseek-official` / `deepseek-v4-flash`。 [E: packages/bundle/base/cordis.patch.yml:78] [E: packages/bundle/base/cordis.patch.yml:79]
...
- credentials 文档、settings 文件后端、Models 页 UI — 本包只消费 `ctx.credentials` / `ctx.settings`。有 `authorization` 时另注册 pi-ai login flows；headless / ACP 无该 seam 时不提供 sign-in。 [E: packages/llm/llm-pi-ai/src/index.ts:220]
- Codex / Claude 子代理后端。那两家 **不是** 「像本包一样 dormant 加载」：`dsh-base` 没有对应行，manifest 也没有对应依赖。 [E: packages/bundle/base/tests/base.spec.ts:42] [E: packages/bundle/base/tests/base.spec.ts:43] [E: packages/bundle/base/tests/base.spec.ts:47] [E: packages/bundle/base/tests/base.spec.ts:48]
...
| 符号 / 键 | 落点 | 含义 |
|---|---|---|
| `Config.providers` | 插件 config + settings ns `llm-pi-ai` | dict 的 **键就是 route**。省略或 `{}` → `resolveProfiles` 得到空 map。 [E: packages/llm/llm-pi-ai/src/config.ts:222] [E: packages/llm/llm-pi-ai/src/config.ts:341] [E: packages/llm/llm-pi-ai/tests/adapter.spec.ts:820] |
...
| directory entry | `registerConfigurableProviders` | Models 页可选的 provider 卡。dormant 时含 **全部** 已安装 catalog id（含 OAuth-only 的 `openai-codex`），再并上手写路由。 [E: packages/llm/llm-pi-ai/src/index.ts:138] [E: packages/llm/llm-pi-ai/src/catalog.ts:177] |
...
| `PiAiSnapshot` | `PiAiAdapter.current` | 一份 immutable `profiles` + `Models`。配置变更建新 collection，飞行中的 stream 继续用它捕获的那份。 [E: packages/llm/llm-pi-ai/src/adapter.ts:232] [E: packages/llm/llm-pi-ai/src/adapter.ts:322] |
| catalog 名 `deepseek` | pi-ai 内置表 | 与官方路由 `deepseek-official` **刻意不同名**。前者只在本包写出 profile 之后才成为 adapter route。 |

## 控制流

1. **`dsh-base` 无条件挂上本包，且不给 `providers:`。** `cordis.patch.yml` 的行只有 `id: llm-pi-ai` 与 `name: '@deepseek-ai/dsh-llm-pi-ai'`，没有 `config:` 块。 [E: packages/bundle/base/cordis.patch.yml:107] [E: packages/bundle/base/cordis.patch.yml:108] manifest **依赖** `@deepseek-ai/dsh-llm-pi-ai`。 [E: packages/bundle/base/package.json:67] 对照：`subagent-codex` / `subagent-claude-code` 行数为 0，且不在 dependencies 里——那是「没装」，不是 dormant。 [E: packages/bundle/base/tests/base.spec.ts:42] [E: packages/bundle/base/tests/base.spec.ts:48] shipped preset（`minimal` / `standard` / `ptc` / `cordis`）的 `agent.cordis.yml` 没有本包行；本行留在 host root，无 `isolate`。叠 base 的 shipped profile 是 `web` / `headless` / `sdk` / `acp`；`sdk-minimal` 不叠 `dsh-base`。

2. **`apply@packages/llm/llm-pi-ai/src/index.ts` 先建 adapter，再铺 directory，最后才谈 route。** `new PiAiAdapter` 绑 `profiles` / `resolveApiKey` / `auth` / `attachments`。 [E: packages/llm/llm-pi-ai/src/index.ts:197] 立刻 `ensureDirectory()`：`directoryEntries` 把 **`catalogProviderIds()` 全部** 登记进 `registerConfigurableProviders`，再并上当前 profiles 里的手写路由。 [E: packages/llm/llm-pi-ai/src/index.ts:242] [E: packages/llm/llm-pi-ai/src/catalog.ts:177] 裸挂 `{}` 时测试钉死 `listProviders() === []`，同时 `listConfigurableProviders().length > 30`（含 `openai` 等 catalog 卡）。 [E: packages/llm/llm-pi-ai/tests/dynamic-config.spec.ts:89] [E: packages/llm/llm-pi-ai/tests/dynamic-config.spec.ts:93] OAuth-only 的 `openai-codex` **也** 进 directory：collection 带 durable credential store + login flow。 [E: packages/llm/llm-pi-ai/tests/catalog.spec.ts:1202] 同一次 `apply` 还 `registerModelDiscovery(NS, discoverModels)`，与有没有 route 无关。 [E: packages/llm/llm-pi-ai/src/index.ts:261]

3. **`ensureRegistrationFacts@packages/llm/llm-pi-ai/src/index.ts`：零 route 就不 `registerAdapter`。** 它读 `routes = [...profiles().keys()]`。若还没有 `AdapterRegistrationHandle` 且 `routes.length === 0`，只记下 `registeredFacts` 然后 `return`，**不** 调用 `registerAdapter`。 [E: packages/llm/llm-pi-ai/src/index.ts:280] [E: packages/llm/llm-pi-ai/src/index.ts:284] [E: packages/llm/llm-pi-ai/src/index.ts:286] 这是故意的：`LlmRuntime.registerAdapter([])` 会抛 `INVALID_ADAPTER`（「an adapter must register at least one provider」）。 [E: packages/llm/llm/src/index.ts:387] `replace([])` 才合法，留给已经拿过 handle 的清空路径。 [E: packages/llm/llm/src/index.ts:407]

4. **profiles 从哪来：composition `config.providers` 或 settings `llm-pi-ai:`。** `Config` schema 把缺省 `providers` 收成 `{}`。 [E: packages/llm/llm-pi-ai/src/config.ts:341] `installSection` 把 composition 行当 `base`，用户文档叠在上面；`settings.update` 走 `mergeLayers`（plain object 递归合并，所以 dict 按 route 键叠加），`settings.replace({})` 退回 composition `base`。 [E: packages/settings/settings/src/index.ts:480] [E: packages/settings/settings/src/index.ts:287] [E: packages/settings/settings/src/index.ts:671] shipped base 没有 `config.providers`，所以用户层一空就回到空 map。真 Loader 测试：裸 `id: llm-pi-ai` 启动后 `listProviders()` 为空；往 `settings.yaml` 写 `llm-pi-ai.providers.deepseek` 后出现 route `deepseek`。 [E: packages/llm/llm-pi-ai/tests/loader-composition.spec.ts:106] [E: packages/llm/llm-pi-ai/tests/loader-composition.spec.ts:118]

5. **有 route 之后：同一 adapter 实例 `registerAdapter` 或 `replace`。** 第一次非空 `routes` 走 `ctx.llm.registerAdapter(routes, adapter)`。 [E: packages/llm/llm-pi-ai/src/index.ts:288] 之后只 `registration.replace(routes)`——同一实例、先校验再一次性 commit，观察者看不到空窗。 [E: packages/llm/llm-pi-ai/src/index.ts:290] 捕获进 registry 的事实是 route 集合 + 每条 `displayName` + `retryPolicy`；仅重排 YAML 键不会 swap。 [E: packages/llm/llm-pi-ai/src/index.ts:105] [E: packages/llm/llm-pi-ai/src/index.ts:272] Settings 把 ns 清成 `{}` 时，shipped 姿态下 `routes` 变空，已有 handle 就 `replace([])`，`listProviders()` 回到 `[]`（dormant 活 registration）。 [E: packages/llm/llm-pi-ai/tests/dynamic-config.spec.ts:112] [E: packages/llm/llm-pi-ai/tests/dynamic-config.spec.ts:113] 若 composition 自己带了 `providers`（测试里的 `openai` 行），`replace(NS, {})` 只撤用户层，composition 那条继续服务。 [E: packages/llm/llm-pi-ai/tests/dynamic-config.spec.ts:140] [E: packages/llm/llm-pi-ai/tests/dynamic-config.spec.ts:141]

6. **撞名：整次 swap 拒绝，旧 routes / 旧 directory 继续服务。** `prepareRoutes` 发现别的 registration 已占同一 provider 字符串，抛 `DUPLICATE_ADAPTER`。 [E: packages/llm/llm/src/index.ts:422] `onChange` 包住 `ensureRegistrationFacts`：打 error 日志，**不** 推进 `registeredFacts`。 [E: packages/llm/llm-pi-ai/src/index.ts:315] 测试：外来 adapter 占着 `anthropic` 时，settings 再声明 `anthropic`，结果仍是 `['anthropic', 'openai']`，openai 还能打到原 endpoint。 [E: packages/llm/llm-pi-ai/tests/dynamic-config.spec.ts:216] directory 同理：已有 `llm-deepseek` 声明 `deepseek-official` 时，本包再声明同名只耗一条诊断，`settingsNs` 仍是 `llm-deepseek`，directory 长度不变。 [E: packages/llm/llm-pi-ai/tests/catalog.spec.ts:1166] [E: packages/llm/llm-pi-ai/tests/catalog.spec.ts:1168] 不可服务的 profile（catalog 不认识且没列 `models`）在 `assertServiceable` / `validate` 处就拒写，旧 routes 不动。 [E: packages/llm/llm-pi-ai/tests/dynamic-config.spec.ts:191] [E: packages/llm/llm-pi-ai/src/index.ts:301]

7. **请求路径：`GenerateOptions.provider` → 私有 `adapters` → `llm/stream` waterfall → `PiAiAdapter.stream`。** `LlmRuntime.registration` 按 provider 字符串取 map；没有就 `NO_ADAPTER`。 [E: packages/llm/llm/src/index.ts:940] `streamWithRegistration` 把 innermost 设成 `adapterStream`，外包 `ctx.waterfall(..., 'llm/stream', …)`。 [E: packages/llm/llm/src/index.ts:1059] [E: packages/llm/llm/src/index.ts:1063] Cordis `Events.waterfall` 把最后一个参数当 innermost `next`：监听器必须调用传入的 `next()` 才会 `shift` 到下一层；不调用就停在本层，adapter 根本不会跑。 [E: vendor/cordis/src/events.ts:237] [E: vendor/cordis/src/events.ts:238] 本包 **不** 自己挂 `llm/stream`。重试执行在 [subsys.llm.retry](./retry.md) 的 `agent/request-error`；本 adapter 每次 SDK 调用钉 `maxRetries: 0`。 [E: packages/llm/llm-pi-ai/src/adapter.ts:130] [E: packages/llm/llm-pi-ai/tests/sdk-options.spec.ts:52] `stream` 在任何 `await` 之前 `current()` 冻结 snapshot；`GenerateOptions.stop` 直接 `UNSUPPORTED_OPTION`。 [E: packages/llm/llm-pi-ai/src/adapter.ts:322] [E: packages/llm/llm-pi-ai/src/adapter.ts:330] 缺 `apiKeyEnv` 命中值是请求时 `MISSING_CREDENTIAL`，不是 load 失败。 [E: packages/llm/llm-pi-ai/src/index.ts:185]

8. **默认对话不走本包。** 兄弟插件写死 `PROVIDER = 'deepseek-official'`，load 时就 `registerAdapter([PROVIDER], adapter)`。 [E: packages/llm/llm-deepseek/src/index.ts:90] [E: packages/llm/llm-deepseek/src/index.ts:476] `agent-default-model` 的 composition 默认也是这对。dormant 的本包不会把 `deepseek` 或任何 catalog 名变成已注册 route。
...
- **SDK 不重试。** 可见次数与 backoff 属于 `retryPolicy` + `dsh-llm-retry`。profile 上残留的 `maxRetries` / `maxRetryDelayMs` 在 `resolveProfiles` 就拒绝，避免静默叠乘。 [E: packages/llm/llm-pi-ai/src/config.ts:370]
...
- **图片要 `attachments`。** 模型 `input` 含 `image` 且消息带图时，没有 durable attachment service 会 `UNSUPPORTED_CONTENT`。这与官方 adapter 拒绝图片是两条路。 [E: packages/llm/llm-pi-ai/src/adapter.ts:359]
