...
| `contextPressure` | last-wins：`pressureTokens`（prompt 侧，不含 output）、`projectedTokens`、`contextWindow`。不是一次原子请求观察。[E: packages/llm/token-meter/src/usage-projection.ts:153] [E: packages/llm/token-meter/src/usage-projection.ts:78] |
...
4. **`session/event` 不是 waterfall。** 事件签名是 `(session, event) => void`，没有 `next` 参数。append 先 `this.log.push` 再 `invokeContainedSessionObservers`；listener 失败不能回滚已提交事件。`TokenMeter` 只在 `this.states.has(session)` 时追赶，避免给从未被读过的 session 建 fold。[E: packages/core/session/src/index.ts:73] [E: packages/core/session/src/index.ts:638] [E: packages/core/session/src/index.ts:645] [E: packages/llm/token-meter/src/index.ts:109]

5. **拉式计量：`measure@index.ts`。** 调用方传入 `Session` 与可选的有效信封 `requestHeader`。`_sync` 把私有 `ReplayState` 追到 `session.events` 尾；`logRevision` 等于已消费条数。`requestHeader` 只改请求压力（选哪条 baseline / 路由计价）；返回的 `nodes` 永远描述当前 surface。生产 Consumer 只传 `session`。[E: packages/llm/token-meter/src/index.ts:144] [E: packages/llm/token-meter/src/index.ts:144]
...
8. **usage 锚只在「不比全量启发式更小」且信封匹配时采用。** `assistant/message` 若带 `usage` 且已有 header：记下 `MeasurementAnchor`（header、stepStart 节点、provider assistant 启发式、usage）。`measure` 时若当前信封与锚 header 相等，用同一路由 `priceSurface` 重算锚 surface，再算 `estimatedAnchorTokens = estimateHeader + anchorSurfaceTokens`。仅当 `usageTokens(usage) >= estimatedAnchorTokens` 才把 baseline 写成 `{ kind: 'usage', … }`；否则写 `estimated`。之后 surface 的 signed delta 加在这条锚上；`totalTokens` 下限为 0。[E: packages/llm/token-meter/src/index.ts:154] [E: packages/llm/token-meter/src/index.ts:150] [E: packages/llm/token-meter/tests/token-meter.spec.ts:287]
...
- **usage 偏小会被丢掉。** provider 报 27、启发式锚更大时，baseline 是 `estimated`，不是 `usage`。测试里随后 shrink surface，若误用偏小 usage 当锚，`27 + surfaceDelta` 会小于 0。[E: packages/llm/token-meter/tests/token-meter.spec.ts:287]
- **`sourceEventSeqs` 空列表 ≠ 缺省。** `[]` 表示已知空流，provider 侧 assistant 价为 0，durable 改写会拉大 `surfaceDeltaTokens`。缺省列表把 durable 文本当成当时的 provider 输出，delta 为 0。[E: packages/llm/token-meter/src/index.ts:305]
