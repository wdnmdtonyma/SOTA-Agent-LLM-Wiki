...
- shipped 工作台谁占 `root` / `sidebar` / `main` / `rightbar` / `shell.overlay`？为什么在 `root` 上再 `register` 会拆掉整框？
...
组合画像：`PROFILE_TEMPLATES.web` 叠 `dsh-base` + `dsh-web-app`。web-app 插入 `ui-layout`、`ui-renderer`、`ui-conversation` 等 `dsh.client` 行，**没有** `id: ui-slots`。[E: packages/bundle/web-app/cordis.patch.yml:207] [E: packages/bundle/web-app/cordis.patch.yml:210] [E: packages/bundle/web-app/cordis.patch.yml:249] launcher 在 `provide('webStartup')` 之前拒绝 `--host 0.0.0.0`；那条旗标门在 [`surface.profiles.web`](../../surface/profiles/web.md)。非 GUI 入口走 `dsh --profile headless|sdk|sdk-minimal|acp`，不装这条座位缝。
...
| `SlotKind` | `'single' \| 'list' \| 'keyed' \| 'chain'`：单座 / 有序列表 / 键分发 / selector 选举链。[E: packages/client/ui-slots/src/index.ts:98] |
| `SlotScope` | `'root' \| 'session-maybe' \| 'session'`：全局、当前会话可空、严格绑会话。[E: packages/client/ui-slots/src/index.ts:101] `InjectParams`：`session` 带确定 `sessionId`，`session-maybe` 允许 `undefined`，`root` 不传 session 轴。[E: packages/client/ui-slots/src/index.ts:467] [E: packages/client/ui-slots/src/index.ts:469] [E: packages/client/ui-slots/src/index.ts:471] `PropsRuntime` 按 scope 交叉 `SessionStandardProps` 或 `SessionMaybeStandardProps`。[E: packages/client/ui-slots/src/index.ts:229] [E: packages/client/ui-slots/src/index.ts:230] |
...
| `StoreDecl` | 共享 `StoreHandle` 或 exclusive `StoreFactory`（类型来自 `dsh-client-store`）。[E: packages/client/ui-slots/src/store.ts:12] 核只钉非函数 handle 的 scope；factory 走 `typeof store !== 'function'` 的反面豁免。[E: packages/client/ui-slots/src/index.ts:841] |
...
ui-renderer 把 `'root'` 合并进 `SlotMap`（`single`/`root`）。[E: packages/client/ui-renderer/src/client/registry.ts:43] 这里再 register 会阴影整框；加法面是 ui-layout 声明的 `shell.overlay` list。[E: packages/client/ui-layout/src/client/index.ts:91]

## 控制流

1. **组合层不插 `ui-slots` 行。** `dsh-web-app` insert 有 `id: cordis-client-runner`、`id: ui-layout`、`id: ui-renderer`、`id: ui-conversation`。核以 library 被 renderer `import { SlotCore }`。`dsh-base` 的 insert 从 `timer` / `hmr` / `llm` 起，没有浏览器 roster。[E: packages/bundle/web-app/cordis.patch.yml:198] [E: packages/bundle/web-app/cordis.patch.yml:207] [E: packages/bundle/base/cordis.patch.yml:16] `dsh-headless` 只插 `code-runtime` / `headless-startup` / `headless-runner`，没有 `webserver`、没有 `agent-presets`、也没有 client 座位图。[E: packages/bundle/headless/cordis.patch.yml:23] shipped preset 目录是 `minimal` / `standard` / `ptc` / `cordis`（旧 `code` = PTC），与座位图无关。
...
3. **`SlotCore` 构造即声明 `'root'`。** 记录 `kind: 'single', scope: 'root'`，`declaredBy: '(built-in)'`。构造不 `markDirty`。之后任何贡献必须先被某条 `children` 表声明，否则 `register` 抛 `slot "…" is not declared`。[E: packages/client/ui-slots/src/index.ts:698] [E: packages/client/ui-slots/src/index.ts:796] 测试钉死：新核 `specDynamic('root')` 已是 `{ kind: 'single', scope: 'root' }`；对未声明键 `register` 抛 `not declared`。[E: packages/client/ui-slots/tests/core.client.spec.ts:50] [E: packages/client/ui-slots/tests/core.client.spec.ts:55]
...
5. **shipped 工作台一次 `register` 坐下整框。** `ui-layout` `inject = ['slots', 'theme', 'locale']`。[E: packages/client/ui-layout/src/client/index.ts:123] `apply` 提供 `ctx.layout`，然后 `ctx.slots.register({ name: 'root', children: { sidebar, main, rightbar, shell.overlay } })`，component 是 `AppFrame`。[E: packages/client/ui-layout/src/client/index.ts:148] [E: packages/client/ui-layout/src/client/index.ts:152] [E: packages/client/ui-layout/src/client/index.ts:153] [E: packages/client/ui-layout/src/client/index.ts:154] 同一声 `children` 既声明规格又拿走这四个键的 `renderSlot` 独占权。重复声明同一子键抛 `already declared`，并点名先声明者。[E: packages/client/ui-slots/src/index.ts:835]
...
6. **Consumer 占子座，不再给每个 `ui-*` 开节。**
   ...
   | `main` | `keyed` / `root` | ui-layout | `ui-conversation` 占 `key: 'conversation'`（内部再声明 `main.conversation` / composer 等子座） |
   | `rightbar` | `single` / `root` | ui-layout | 右侧栏栈（`ui-sidebar-right` / documentpreview；fold 进 workbench / ui-layout，不为单个 `ui-*` 另开节点） |
   | `shell.overlay` | `list` / `root` | ui-layout | 无人独占；新 `id` 是加法 |

   `AppFrame` 在固定树位分别调用 `renderSlot('sidebar', …)`、`renderSlot('main', …)`（经 `MainPanel`）、`renderSlot('rightbar', …)`、`renderSlot('shell.overlay', {})`。[E: packages/client/ui-layout/src/client/AppFrame.tsx:193] [E: packages/client/ui-layout/src/client/AppFrame.tsx:42] [E: packages/client/ui-layout/src/client/AppFrame.tsx:227] [E: packages/client/ui-layout/src/client/AppFrame.tsx:200] `ui-sidebar` 占 `sidebar`。[E: packages/client/ui-sidebar/src/client/index.ts:72] `ui-conversation` 占 `main`。[E: packages/client/ui-conversation/src/client/apply.ts:411] `ui-sidebar-right` 占 `rightbar`。[E: packages/client/ui-sidebar-right/src/client/index.ts:149] `ui-chat` **不再**占根子槽；详情/预览落到右侧栏。
...
8. **`register` 的 cell 规则（核，所有调用方共用）。** `priority` 缺省 `0`。`single` / 同 `key` / 同 `id` 在**同一 priority** 再登记抛错（`already has a registration` / `already has an entry`）；缺 `key` / `id` / `select` 也按 kind 抛。[E: packages/client/ui-slots/src/index.ts:807] [E: packages/client/ui-slots/src/index.ts:808] 不同 priority 共存，台账按 priority 升序（list 再按 `order`）；`entriesOfSlot` 跳过 abdicate 条目，每个 cell 只收第一次出现的赢家，因此**数字更小的赢**。[E: packages/client/ui-slots/src/index.ts:874] [E: packages/client/ui-slots/src/index.ts:948] `chain` 不做阴影：选举吃完全部条目。同优二次占 `root` 的测试期望抛 `already has a registration`。[E: packages/client/ui-slots/tests/core.client.spec.ts:61]
...
10. **渲染读 `entriesOfSlot`，不读生 ledger。** `createSlotRenderer` 的 `RootOutlet` 取 `host.entriesOfSlot('root')[0]`；`single` 取 cell 赢家，`chain` 按 `select(owner)` 第一个非 `null` 当选，`overlay` 保活 fallback。[E: packages/client/ui-renderer/src/client/scoped-slots.tsx:897] [E: packages/client/ui-renderer/src/client/scoped-slots.tsx:799] [E: packages/client/ui-renderer/src/client/scoped-slots.tsx:808] [E: packages/client/ui-renderer/src/client/scoped-slots.tsx:876] 入口 crash 对阴影 kind 带 `abdicate: true`：条目仍在 `entries()` 里，但被放进 `abdicated` WeakSet，下一个 survivor 上场。[E: packages/client/ui-slots/src/index.ts:1105] `boundRenderSlot` 在条目已死时抛 `StaleAuthorizationError`，键不在 `children` 表时抛 `SlotOwnershipError`；两个类在 `renderer.ts` 是空 `Error` 子类。[E: packages/client/ui-renderer/src/client/scoped-slots.tsx:46] [E: packages/client/ui-renderer/src/client/scoped-slots.tsx:51] [E: packages/client/ui-slots/src/renderer.ts:219] [E: packages/client/ui-slots/src/renderer.ts:226]

11. **一条生命周期轴：disposer = 贡献 + 声明树。** `register` 返回的函数若条目已不在 ledger 则直接 return（idempotent / stale no-op）。[E: packages/client/ui-slots/src/index.ts:897] 仍在则滤掉自己、`markDirty`、`releaseEntry`。`releaseEntry` 把每个子槽 `spec` 清空、entries 置空、递归再 `releaseEntry`。[E: packages/client/ui-slots/src/index.ts:1144] 测例：dispose 占 `root` 的 frame 后，子槽 `specDynamic` 变 `undefined`。[E: packages/client/ui-slots/tests/core.client.spec.ts:92] `SlotRegistry` 把该 disposer 挂进 `ctx.effect(..., 'slots.register()')`：插件 fiber `dispose` 即卸座。测例：带 `inject: ['slots']` 的插件 unload 后 `t.host` 条目数为 0。[E: packages/client/ui-renderer/tests/registry.client.spec.ts:241]

12. **变更通知两档。** `markDirty` 同步 bump `version` 并调用 `onMutate`（→ `slots/changed`），`subscribe` 则 `queueMicrotask` 按 record 批一次。[E: packages/client/ui-slots/src/index.ts:1177] [E: packages/client/ui-slots/src/index.ts:1181] 同 tick 两次 `register`：version +2，listener 只响 1 次。[E: packages/client/ui-slots/tests/core.client.spec.ts:263] `subscribeDeclaration` 同步，且整张 `children` 表先全部落声明再 `notifyDeclaration`。[E: packages/client/ui-slots/src/index.ts:1056] [E: packages/client/ui-slots/src/index.ts:893]

13. **store：核钉 scope，registry 管实例，引擎在 `dsh-client-store`。** 共享 handle 第一次挂上记下 `SlotScope`，跨 scope 再挂抛 `one handle, one scope`；factory 豁免。[E: packages/client/ui-slots/src/index.ts:845] [E: packages/client/ui-slots/tests/core.client.spec.ts:191] `resolveStore`：root 用字面量 `'root'` 作 key 且 `handle.create()`；session 用 `scopeBinding.key` 且 `handle.create(key)`。会话死时 `clearStoreScope` 对 session 轴 `clearPersisted` 再删实例（含从未渲染过、但上次页面留下 persist key 的会话）。[E: packages/client/ui-renderer/src/client/registry.ts:535] [E: packages/client/ui-renderer/src/client/registry.ts:556] 组件只看见 `useStore` + 已烘焙 `actions`，看不见引擎 store。[E: packages/client/store/src/index.ts:179] `defineStore` 实现在 store 包。[E: packages/client/store/src/index.ts:217]
...
- **直接 `ctx.slots.register({ name: 'root' })` 在 AppFrame 之后会抛，不是覆盖。** 覆盖只发生在调用方给出**更低** `priority`，或走 cordis-client-runner facade（自动 `--nextPriority`）。[E: packages/client/ui-slots/src/index.ts:808] [E: packages/extensions/cordis-client-runner/src/client/runtime.ts:424]
...
- **共享 store handle 一 scope。** 同一 handle 不能同时坐 `root` 槽和 `session` 槽；要跨 scope 用 factory。[E: packages/client/ui-slots/src/index.ts:845]
...
- **不要给 30 个 `ui-*` 各开子系统。** 本页的权威装配点就是 `root` 的 `sidebar` / `main` / `rightbar` / `shell.overlay`；更里面的 composer / toolview / settings.section 仍是同一次 `register` 语义。
...
- [`subsys.client.ui-layout`](ui-layout.md) — `AppFrame` 占 `root`、四子槽（`sidebar` / `main` / `rightbar` / `shell.overlay`）与 `ctx.layout`。
- [`subsys.client.ui-conversation`](ui-conversation.md) — 占 keyed `main`（`key: 'conversation'`）；详情/预览在右侧栏。
