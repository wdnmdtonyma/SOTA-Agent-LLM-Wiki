...
8. **初始选择与 New Session 在 `ui-workspace`，不在 WorkspaceController。** `watchNavigation`：两条 list `phase !== 'ready'` 则等；已有 `sessions.current` 或没有 recent workspace 则 `initial='done'`，**不** `create`；否则 `connectWorkspace(target)`，回来若 current 仍空才 `sessions.open`。[E: packages/client/ui-workspace/src/client/navigation.ts:167] [E: packages/client/ui-workspace/src/client/navigation.ts:172] [E: packages/client/ui-workspace/src/client/navigation.ts:177] `connectWorkspace`：未知 id 抛；复用条件是 `summary.blank`、`cwd === workspace.path`、成员 `sessionIds`、且不在 `archivedSessionIds`；否则 `sessions.create({ workspaceId })`，同 workspace 单飞。[E: packages/client/ui-workspace/src/client/navigation.ts:94] [E: packages/client/ui-workspace/src/client/navigation.ts:103] [E: packages/client/ui-workspace/src/client/navigation.ts:108] `startSession`：显式 workspace → 当前 session 所属 → recent；一个都没有则 `sessions.clear()`，仍不凭空 create。[E: packages/client/ui-workspace/src/client/navigation.ts:124] [E: packages/client/ui-workspace/src/client/navigation.ts:126]
