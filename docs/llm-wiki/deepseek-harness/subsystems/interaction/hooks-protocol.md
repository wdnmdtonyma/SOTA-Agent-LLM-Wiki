...
DSH 是 Cordis 组合运行时（`profile → bundle → agent preset`）。五个 shipped profile 是 `web`（live）与 `headless` / `sdk` / `sdk-minimal` / `acp`（startup）；六个 bundle 是 `dsh-base`、`dsh-web-app`、`dsh-headless`、`dsh-sdk-app`、`dsh-sdk-minimal`、`dsh-acp-app`。四个 shipped preset 目录是 `minimal` / `standard` / `ptc` / `cordis`。本库**不坐** host 面也不坐 agent-preset 面：它没有组合行。两座桥才是 overlay 插件（`inject = ['shell', 'sessionProjections']`），出现在测试夹具如 `snapshots/session/text-turn/cordis.yml`，不在上述 shipped bundle 的 `cordis.patch.yml`。宿主入口是 `dsh web` 以及 `dsh --profile sdk|sdk-minimal|acp|headless`（及自定义 profile）。
...
| `snapshots/session/text-turn/cordis.yml` | overlay 只挂两座**桥**，不挂本库 |
...
| `hook/result` | `{ turn, point, handlerId, decision, exitCode?, stderrSummary?, durationMs }`。与 invoked 靠 `handlerId`（invariant 还加上 turn+point）配对。 [E: packages/hooks/hook-protocol/src/types.ts:31] [E: packages/core/session/src/known-event-types.ts:39] |
...
2. **组合树上没有本库行。** shipped 六个 bundle 的 `cordis.patch.yml` 都不含 `@deepseek-ai/dsh-hook-protocol`、也不含两座桥。overlay 只挂桥：`id: hooks-claude-code` / `id: hooks-codex`。桥再 import 本库。 [E: snapshots/session/text-turn/cordis.yml:72] [E: snapshots/session/text-turn/cordis.yml:77]
...
- **桥的 `inject` 不止 `shell`。** 两座桥都是 `['shell', 'sessionProjections']`：shell 跑命令，projection 给 turn 号。 [E: packages/hooks/hooks-claude-code/src/index.ts:41] [E: packages/hooks/hooks-codex/src/index.ts:40]
...
| Consumer | `@deepseek-ai/dsh-hooks-claude-code`、`@deepseek-ai/dsh-hooks-codex` | `inject = ['shell', 'sessionProjections']`；import 本库；自己挂 Cordis 监听点并必须 `next()` | 不在 shipped bundle。overlay：`snapshots/session/text-turn/cordis.yml` 的 `id: hooks-claude-code` / `id: hooks-codex` |
...
- [subsys.interaction.hooks-claude](hooks-claude.md)（`subsys.interaction.hooks-claude`）：Claude 桥；CC 事件 → Cordis 监听点；`updatedInput` 只 warn。
- [subsys.interaction.hooks-codex](hooks-codex.md)（`subsys.interaction.hooks-codex`）：Codex 桥；5 个点；regex-only；stdin 无尾换行。
- [surface.hooks.bridges](../../surface/hooks/bridges.md)（`surface.hooks.bridges`）：两座桥的产品面装配。
