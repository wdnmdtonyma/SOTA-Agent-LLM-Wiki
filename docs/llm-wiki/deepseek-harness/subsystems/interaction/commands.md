...
| 符号 | 要点 |
|---|---|
| `COMMAND_NAME` | `^[a-z][a-z0-9_-]*$`。`register` 的 `name` 必须整串匹配，**不**带前导 `/`。 [E: packages/interaction/commands/src/index.ts:30] |
| `parseCommand` | 行首 `/` + 同上名字字符，其后必须是行尾或空白（`\t\n\r `）。返回 `{ name, rawInput }`；`rawInput` 是名字后的原文，含分隔空白，不做 trim。对不上返回 `undefined`。 [E: packages/interaction/commands/src/index.ts:125] [E: packages/interaction/commands/src/index.ts:126] |
| `CommandDefinition` | `name` / `description` / 可选 `input`（`hint` + 可选 `images`）/ 可选 `recordInput` / `handler`。`description` 与 hint 都必须是非空（trim 后）字符串。 [E: packages/interaction/commands/src/index.ts:57] [E: packages/interaction/commands/src/index.ts:181] |
| `input.images` | `true` 时 descriptor 带上 `images: true`；`false` 或省略不写进 descriptor。非 boolean 登记期抛。 [E: packages/interaction/commands/src/index.ts:197] [E: packages/interaction/commands/src/index.ts:202] |
| `recordInput` | 缺省视为 true。`=== false` 时 `command/run` **省略** `args`；handler 仍收到完整 `rawInput`。 [E: packages/interaction/commands/src/index.ts:360] |
| `CommandInvocation` | `{ commandId, agent, rawInput, attachments, signal }`。`attachments` 是已 admit 的 `ImageBlock[]`；不经模型。 [E: packages/interaction/commands/src/index.ts:35] [E: packages/interaction/commands/src/index.ts:47] |
| `CommandResult` | `{ kind: 'success', text?, sourceEventSeq? }` 或 `{ kind: 'error', text }`。error 的 `text` 必须非空。`sourceEventSeq` 只允许出现在 success。 [E: packages/interaction/commands/src/types.ts:30] [E: packages/interaction/commands/src/index.ts:243] |
| `CommandDescriptor` | `list()` 返回的不可变视图：`name` / `description` / 可选 `input`。**没有** handler。 [E: packages/interaction/commands/src/types.ts:51] |
| `CommandExecution` | `{ commandId, result }`。`execute` 在 handler 正常结算后返回；admission miss 返回 `undefined`。 [E: packages/interaction/commands/src/types.ts:41] [E: packages/interaction/commands/src/index.ts:329] [E: packages/interaction/commands/src/index.ts:329] |
| `CommandId` | `Branded<'CommandId'>`。构造函数只做类型品牌，不校验。运行时形状 `cmd-${8 位 instanceToken}-${seq}`。 [E: packages/interaction/commands/src/brand.ts:31] [E: packages/interaction/commands/src/index.ts:418] |
| `CommandSource` | 今天只有 `{ kind: 'user' }`。 [E: packages/interaction/commands/src/types.ts:65] [E: packages/interaction/commands/src/types.ts:74] |
| `command/run` | log-only。`{ commandId, name, args?, source }`。`args` 是 `parseCommand` 的 `rawInput`。 [E: packages/interaction/commands/src/types.ts:94] |
| `command/done` | log-only。`{ commandId, kind, text?, sourceEventSeq? }`。与 `command/run` 用同一 `commandId` 配对。 [E: packages/interaction/commands/src/types.ts:105] |
| `commands/change` | Cordis **emit**（不是 waterfall）。登记或卸载后广播；observer 失败不能否决突变。 [E: packages/interaction/commands/src/types.ts:82] |

`command/*` 走两参 `session.append(type, data)`，不开 turn、不带 `surfaceOp`。 [E: packages/interaction/commands/src/index.ts:437] 不进 `surface.nodes` / `deriveMessages()` 是因为 `SurfaceEventType` 只有 `user/message` / `assistant/message` / `tool/result`。[I]
...
3. **`register@CommandRuntime` 写哪一层，由调用方 ctx 的 `scopeOf` 决定。** Cordis `Service` tracker 把 `this.ctx` 重绑到调用方：读 `this.ctx` 时 proxy 返回调用 context，不是构造时那份。`ScopedLayers.effect` 再 `scopeOf(ctx)`：无 `kScope` 进 eager `global`；有标签才懒创建 overlay。`NamedEntries.insert` 在**同一层**撞名就抛。global 层的诊断要求「per-agent 变体请挂到该 agent 的 `agent.ctx`」；scoped 层则是 `already registered in this scope`。 [E: vendor/cordis/src/service.ts:50] [E: vendor/cordis/src/utils.ts:176] [E: packages/core/scope/src/store.ts:231] [E: packages/core/scope/src/index.ts:154] [E: packages/core/scope/src/store.ts:45] [E: packages/interaction/commands/src/index.ts:95] [E: packages/interaction/commands/src/index.ts:101] [E: packages/interaction/commands/src/index.ts:276]

4. **同层冲突，跨层允许阴影。** 两次 `ctx.commands.register(command('same'))` 抛 `/agent\.ctx/`。`scope.ctx.commands.register(command('same'))` 可以盖住 global；对该 agent 的 `execute('/shared')` 跑 scoped handler，`scope.dispose()` 之后回落到 global。fiber dispose 卸掉该 fiber 贡献的登记。 [E: packages/interaction/commands/tests/commands.spec.ts:127] [E: packages/interaction/commands/tests/commands.spec.ts:128] [E: packages/interaction/commands/tests/commands.spec.ts:103] [E: packages/interaction/commands/tests/commands.spec.ts:108] [E: packages/interaction/commands/tests/commands.spec.ts:119]

5. **有效视图：global + scope 祖先链，近层覆盖。** `view(agent)` = `layers.merge(agent, layer => layer.commands)`。`merge` 先铺 global，再按 `chainLayers`（远祖在前、本 scope 在后）`set`。live `Agent` 自己是 `ScopeKey`；web 上 `bindScopeParent(agent, standingKey)` 之后，preset standing 上登记的名字对该 agent 可见。`list` 返回按 `name` 排序的冻结 `CommandDescriptor`；`find` 返回带 handler 的 `CommandDefinition`。 [E: packages/interaction/commands/src/index.ts:441] [E: packages/core/scope/src/store.ts:212] [E: packages/interaction/commands/src/index.ts:290] [E: packages/interaction/commands/src/index.ts:304] [E: packages/interaction/commands/tests/commands.spec.ts:89]

6. **`list` / `execute` 是 `@Remote`；`find` / `register` 不是。** 浏览器半边只看 descriptor 和结算结果，拿不到 handler 函数。host `execute(agent, line, images, signal)` 仍要 live `Agent` 才能选层、往该 session 写 log。client 调的是 `remote.commands.execute`；session 身份 → Agent、request abort → `signal`、composer 图片 → `images`，属于 Typert / session-controller，不是本包。 [E: packages/interaction/commands/src/index.ts:290] [E: packages/interaction/commands/src/index.ts:329] [E: packages/interaction/commands/src/index.ts:329]

7. **admission miss 静默返回，不写 log。** `parseCommand` 失败或 `view(agent)` 没有该名 → `undefined`。空闲 session 上 `execute('not a command')` / `execute('/missing')` 之后 `session.events` 仍是 `[]`。调用方（composer）把这当成「不是命令」，不要当成 handler 失败。 [E: packages/interaction/commands/src/index.ts:329] [E: packages/interaction/commands/src/index.ts:329] [E: packages/interaction/commands/tests/commands.spec.ts:433]

8. **已 abort 的 signal 在 `command/run` 之前就抛。** 解析成功且名字命中之后，若 `signal.aborted`，立刻 `throw abortError(signal)`，**还没有** mint id、也没有 append。挂起中的 handler 被 abort 则已经有 `command/run`，再补 `command/done { kind: 'error' }` 后把原错误抛出。 [E: packages/interaction/commands/src/index.ts:360] [E: packages/interaction/commands/src/index.ts:398] [E: packages/interaction/commands/tests/commands.spec.ts:216] [E: packages/interaction/commands/tests/commands.spec.ts:420]

9. **命中后：先 `command/run`，再 admit 图片，再跑 handler，再 `command/done`。** `mintCommandId` 之后同步 `appendLifecycle(..., 'command/run', { commandId, name, args?, source: { kind: 'user' } })`。`command/run` append 失败会让整次 `execute` 响亮失败。非空 `images` 且定义未声明 `input.images === true` 时 **不进 handler**，结算 `{ kind: 'error', text: '/<name> does not accept image attachments' }`。声明了则 `admitEncodedImages`；无 `attachments` 服务或 `AttachmentError` 同样在 handler 前结算 error。handler 的返回值经 `withAbort` + `normalizeResult`；成功路径再 append `command/done`（success 才抄 `sourceEventSeq`）并返回冻结的 `{ commandId, result }`。handler 抛错 / abort：先尽量 append `command/done { kind: 'error', text }`，这次 append 失败只 `logger.warn`，然后把 handler 自己的错误继续抛出。 [E: packages/interaction/commands/src/index.ts:360] [E: packages/interaction/commands/src/index.ts:365] [E: packages/interaction/commands/src/index.ts:366] [E: packages/interaction/commands/src/index.ts:360] [E: packages/interaction/commands/src/index.ts:408] [E: packages/interaction/commands/src/index.ts:415] [E: packages/interaction/commands/tests/commands.spec.ts:538]

10. **不开 turn，也不包一层假 turn。** 空闲 log 上成功执行后事件序就是 `command/run`、`command/done`。已经有 `turn/start` 时，这对事件直接插在开 turn 里，不会再写合成的 `turn/start|end`。persistence 走普通 `session/event` + checkpoint，本包不强制 `flush`。 [E: packages/interaction/commands/tests/commands.spec.ts:324] [E: packages/interaction/commands/tests/commands.spec.ts:444] [E: packages/interaction/commands/src/index.ts:428]

11. **`recordInput: false` 让领域事件独占 payload。** `/private keep this once` 的 handler 仍看见 `rawInput: ' keep this once'`，但 `command/run.data` 没有 `args` 键。`dsh-command-feedback` 这样登记：它自己 `session.append('feedback/record', { text })`，避免命令行与领域事件各记一份。缺省（含 `/compact` 的空输入）会记下 `args: ''`。 [E: packages/interaction/commands/src/index.ts:360] [E: packages/interaction/commands/tests/commands.spec.ts:360] [E: packages/feedback/command-feedback/src/index.ts:124] [E: packages/feedback/command-feedback/src/index.ts:60]
...
- **人命令不经模型 turn。** 不要把 `execute` 理解成「先 append `user/message` 再让 loop 看」。空闲 log 上只有 `command/run` + `command/done`。 [E: packages/interaction/commands/tests/commands.spec.ts:324]
- **`register` 的 `name` 没有 `/`；`execute` 的 `line` 必须有。** `execute(agent, 'run', [], signal)` 是 admission miss。 [E: packages/interaction/commands/tests/commands.spec.ts:195]
- **大小写与路径都不是命令。** `/Goal`、`/goal/path`、` /goal` 都是 `undefined`。 [E: packages/interaction/commands/tests/commands.spec.ts:51]
- **同层同名会抛；阴影只能来自更近的 scope。** 不要在 host 上登记第二个 `feedback`。要做 per-agent 变体，挂到该 agent 的 scoped ctx。 [E: packages/interaction/commands/tests/commands.spec.ts:127]
- **已经 abort 的 signal 不会留下 `command/run`。** 只有 handler 开始之后的 abort 才保证成对。 [E: packages/interaction/commands/src/index.ts:360]
- **handler 返回 `{ kind: 'error' }` 不抛；handler throw / abort 才抛。** 两条路径都会写 `command/done kind: 'error'`，但 RPC 调用方看到的是「结算结果」对「rejected promise」。 [E: packages/interaction/commands/tests/commands.spec.ts:281] [E: packages/interaction/commands/tests/commands.spec.ts:398]
- **`recordInput` 默认 true。** 只有显式 `false` 才省略 `args`。空 `/compact` 仍记下 `args: ''`。
- **`CommandId(...)` 不校验。** 品牌构造接受任意字符串；运行时 mint 才是 `cmd-` 前缀。 [E: packages/interaction/commands/src/brand.ts:38]
- **`find` 带着 handler，不要放到 Remote 上。** 远程面只有 `list` / `execute`。
- **未声明 `input.images` 时带图会先写 `command/run` 再 error 结算，handler 不跑。** [E: packages/interaction/commands/tests/commands.spec.ts:539]
