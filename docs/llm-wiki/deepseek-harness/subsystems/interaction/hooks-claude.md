...
| 符号 | 要点 |
|---|---|
| `hooks-claude-code` | 插件 `name`。`inject = ['shell', 'sessionProjections']`：`shell` 跑 hook 命令，`sessionProjections` 给 `lastTurn`；`agents` / `sessionPersistence` 走 `ctx.get`，缺席不挡 load。[E: packages/hooks/hooks-claude-code/src/index.ts:39] [E: packages/hooks/hooks-claude-code/src/index.ts:42] |
