...
11. **生命周期属 `ctx.subprocess`。** 卸掉执行器 fiber，后台进程继续跑；卸掉 `LocalSubprocessRuntime` 才杀树并 join。local 前台坏 workdir 是 reject（基础设施失败）；后台坏 workdir 是 `killed` + stderr 笔记——这是 `ShellExecutor` 合同，sandbox 包装在可归因 runner 失败时把它升级成 `SANDBOX_UNAVAILABLE` / `runnerFailed`。[E: packages/shell/pwsh-local/tests/executor.spec.ts:467] [E: packages/shell/pwsh-local/tests/executor.spec.ts:471] [E: packages/shell/pwsh-local/tests/executor.spec.ts:443]
