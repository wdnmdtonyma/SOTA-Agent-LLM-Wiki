...
**host 面 vs agent-preset 面。** `ctx.codeRuntime` 是进程级 host 服务：session 出现之前就要占键。Shipped profiles 是 `web`（live）与 `headless` / `sdk` / `sdk-minimal` / `acp`（startup）；`dsh web` 只是 `web` 的硬编码别名，不是唯一宿主入口。`dsh-web-app` 与 `dsh-headless` 各 `insert` 一行 `id: code-runtime` → `@deepseek-ai/dsh-code-runtime-worker-thread`。[E: packages/bundle/web-app/cordis.patch.yml:49] [E: packages/bundle/web-app/cordis.patch.yml:50] [E: packages/bundle/headless/cordis.patch.yml:19] [E: packages/bundle/headless/cordis.patch.yml:20] 对应 manifest 都依赖该包。[E: packages/bundle/web-app/package.json:93] [E: packages/bundle/headless/package.json:49]
...
- **有 runtime ≠ 模型在用 PTC。** web/headless 总是 insert 本缝；web 默认 preset 仍是 `standard`。headless **不**挂 `agent-presets`，shipped PTC yml 不会自动上树。进程级临时开关是同一 overlay 里的 `DSH_TOOLS_MODE`，与本缝是否 loaded 正交。[E: packages/bundle/web-app/cordis.patch.yml:37] [E: packages/bundle/headless/cordis.patch.yml:15]
