...
- host 上的 `fs-sandbox` 与 agent-preset 面上的 `tool-fs` 怎么切？`minimal` 还会 isolate 一份 `ctx.fs` 吗？
...
- **同一 realm 只能一份 `ctx.fs`。** 再挂一个 `FileSystem` 子类会 duplicate-service 抛。要给某个 preset 另一份 fs，必须 `isolate.fs`，不能在 root 再插一行 `fs-local`。shipped `minimal` **不再**这么做。
...
- [`surface.tools.str-replace-editor`](../../surface/tools/str-replace-editor.md) — 包仍在，出厂 preset / bundle **不挂**。
