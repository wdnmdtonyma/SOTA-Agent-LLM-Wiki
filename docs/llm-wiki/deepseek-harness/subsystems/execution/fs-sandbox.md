...
- `minimal` 还会 isolate 一份 `ctx.fs` 吗？host 上的 `fs-sandbox` 还在吗？
...
6. **`checkedTarget` 三分支。** `policy = sandboxPolicy ?? this.ctx.sandboxPolicy.resolve()`（无 session → 部署 default，**不**读该会话的 `sandbox/mode` 覆盖）。[E: packages/fs/fs-sandbox/src/index.ts:123] [E: packages/sandbox/sandbox-policy/src/index.ts:166]
   - `danger-full-access`：原样 `return target`，不重 resolve，不看 `writableRoots`。[E: packages/fs/fs-sandbox/src/index.ts:125] 测例：workspace 外的绝对路径写成功。[E: packages/fs/fs-sandbox/tests/fs-sandbox.spec.ts:194]
...
9. **拒绝是结构化 `FsError`；模型文案在 tool 层。** 围栏抛出的对象 `instanceof FsError` 且 `code === 'FS_SANDBOX_DENIED'`。[E: packages/fs/fs-sandbox/tests/fs-sandbox.spec.ts:236] [E: packages/fs/fs-sandbox/tests/fs-sandbox.spec.ts:236] `FsSandboxController.mapError` 把它换成带 `[sandbox: …]` + 升权 hint 的同码 `FsError`；其它错误原样返回。[E: packages/fs/tool-fs/src/sandbox.ts:125] [E: packages/fs/tool-fs/src/sandbox.ts:129] 本包**不**抛 `SANDBOX_UNAVAILABLE`：那个码属于 `ctx.sandbox.confine` 找不到 runner。本包也**从不**调用 `confine`。
...
- **同一 realm 不能同时挂 `fs-sandbox` 与 `fs-local`。** overlay 要另开一份必须 `isolate.fs`；漏 isolate 会被 `mountPreset` 当泄漏拒绝。shipped `minimal` **不再**这么做。`sdk-minimal` 整棵树从一开始就不挂 `fs-sandbox`，也没有 `fs-local`。
...
| **例外 Provider（overlay）** | `@deepseek-ai/dsh-fs-local` | 该 realm 的 `ctx.fs` | 必须 `isolate.fs`；shipped `minimal` **不再**挂 |
| **例外（sdk-minimal）** | 无 fs Provider | — | `dsh-sdk-minimal` 不叠 `dsh-base`，insert 也无 fs 行 |
...
- [surface.presets.minimal](../../surface/presets/minimal.md)（`surface.presets.minimal`）：complete persona + persistent shell；不换 host `ctx.fs`。
