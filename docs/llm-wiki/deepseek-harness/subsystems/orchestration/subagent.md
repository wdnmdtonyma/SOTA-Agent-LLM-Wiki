...
DSH 是 Cordis 组合运行时（`profile → bundle → agent preset`），不是把子代理焊进某一个 coding agent。宿主入口是 `dsh web` 以及 `dsh --profile headless|sdk|sdk-minimal|acp`（`sdk-minimal` 不叠 `dsh-base`）。`desktop` 不是第六个 CLI profile。本仓没有 shipped TUI 模板。`ctx.subagents` 是进程级 host 单例：provider 名全局唯一，跨会话 `listChildren` / `followup` 也挂在这里，不能按会话复制。
...
| `ContinuableStart` | `{ childId, messageId }`：inbox **接受**初始 prompt 即返回，不等 turn、不等消息落盘。[E: packages/subagent/subagent/src/continuation.ts:56] |
