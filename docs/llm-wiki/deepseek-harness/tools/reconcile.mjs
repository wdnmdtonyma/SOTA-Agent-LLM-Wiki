#!/usr/bin/env node
// 登记/同步 —— deepseek-harness 源码 LLM wiki。无外部依赖,跑:node tools/reconcile.mjs
// 1) 各 node .md 的 frontmatter 全量同步回 index.json;新节点登记进 index.json。
...
const SYNC_KEYS = ["title", "kind", "tier", "pkg", "source", "symbols", "related", "evidence", "status", "updated"]
...
for (const dir of NODE_DIRS) {
  for (const file of walk(path.join(WIKI, dir))) {
    ...
    if (existing) {
      let changed = false
      if (existing.path !== rel) { existing.path = rel; changed = true }
...
if (stage.length) {
  let sha = ""
  try { sha = execSync(`git -C "${SRC}" rev-parse --short=10 HEAD`, { stdio: ["ignore", "pipe", "ignore"] }).toString().trim() } catch {}
  const fmLines = [
