#!/usr/bin/env node
// L1 机械校验 —— deepseek-harness 源码 LLM wiki。无外部依赖,跑:node tools/lint.mjs
...
const PKGS = new Set([
  "composition", "core", "llm", "execution", "orchestration", "context",
  "persistence", "interaction", "host", "client", "integration", "vendor", "util", "cross",
])
...
for (const n of nodes) {
  ...
  for (const s of n.source || []) if (!srcExists(s)) err(w, `source 路径不存在于 deepseek-harness/: ${s}`)
}
...
const SRC_TOKEN = /(?:packages|apps|vendor|python|native|docs|scripts|examples|\.agents)\/[^\s`)\]]+|(?:AGENTS|CLAUDE|CONTRIBUTING|BENCHMARK|README)(?:\.\w+)?\.md|package\.json|pnpm-workspace\.yaml|tsconfig(?:\.[\w.]+)?\.json/g
for (const n of nodes) {
  ...
  if (fm.updated) {
    ...
    catch { err(w, `updated 不是合法 deepseek-harness commit SHA: ${fm.updated}`) }
  }
}
