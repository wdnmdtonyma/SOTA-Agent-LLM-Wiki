...
> CLI 全局 flag 是 `codex` 根命令由 `MultitoolCli` 拼出的 option surface：root config override、feature toggle、remote app-server 连接、interactive TUI flags，以及 interactive/exec 共享的 model/sandbox/workdir flags。[E: codex-rs/cli/src/main.rs:129][E: codex-rs/cli/src/main.rs:143]
...
`MultitoolCli` 依次 flatten `CliConfigOverrides`、`FeatureToggles`、`InteractiveRemoteOptions` 和 `codex_tui::Cli`，最后挂载 optional `Subcommand`。[E: codex-rs/cli/src/main.rs:129][E: codex-rs/cli/src/main.rs:130][E: codex-rs/cli/src/main.rs:133][E: codex-rs/cli/src/main.rs:136][E: codex-rs/cli/src/main.rs:139][E: codex-rs/cli/src/main.rs:142] 因为 `codex_tui::Cli` 又 flatten `TuiSharedCliOptions`，root interactive surface 同时包含 `SharedCliOptions` 里的 model/sandbox/workdir flags。[E: codex-rs/tui/src/cli.rs:10][E: codex-rs/tui/src/cli.rs:62][E: codex-rs/tui/src/cli.rs:97][E: codex-rs/tui/src/cli.rs:120]

## Catalog

| flag / arg | 定义字段 | 类型 / 默认 | 语义 | 源 |
|---|---|---|---|---|
| `PROMPT` | `codex_tui::Cli::prompt` | optional positional | 不带 subcommand 时作为 initial user prompt 进入 interactive TUI。`codex agents` 拒绝 initial prompt/images。[E: codex-rs/tui/src/cli.rs:13][E: codex-rs/cli/src/main.rs:1174][E: codex-rs/cli/src/main.rs:1180] | `codex-rs/tui/src/cli.rs:13` |
| `--strict-config` | `codex_tui::Cli::strict_config` | bool false | interactive root 可用；有 subcommand 时会先走 `reject_root_strict_config_for_subcommand()`，只让 allow-list 继承 root strict config。[E: codex-rs/tui/src/cli.rs:17][E: codex-rs/cli/src/main.rs:1163][E: codex-rs/cli/src/main.rs:2511][E: codex-rs/cli/src/main.rs:2544] | `codex-rs/tui/src/cli.rs:17` |
...
| `--enable FEATURE` | `FeatureToggles::enable` | repeatable string | 转为 `features.<feature>=true` root config override；unknown feature 通过 `is_known_feature_key()` 报错。[E: codex-rs/cli/src/main.rs:1041][E: codex-rs/cli/src/main.rs:1066][E: codex-rs/cli/src/main.rs:1068][E: codex-rs/cli/src/main.rs:1078] | `codex-rs/cli/src/main.rs:1041` |
| `--disable FEATURE` | `FeatureToggles::disable` | repeatable string | 转为 `features.<feature>=false` root config override，和 `--enable` 一样先校验 feature key。[E: codex-rs/cli/src/main.rs:1045][E: codex-rs/cli/src/main.rs:1070][E: codex-rs/cli/src/main.rs:1072][E: codex-rs/cli/src/main.rs:1078] | `codex-rs/cli/src/main.rs:1045` |
| `--remote ADDR` | `InteractiveRemoteOptions::remote` | optional string | 连接 remote app-server endpoint；interactive root 直接传入 `run_interactive_tui()`，`agents`/`resume`/`queue`/`fork` 用 subcommand 值覆盖或合并 root 值，`archive`/`delete`/`unarchive` 也通过 session path 合并。[E: codex-rs/cli/src/main.rs:1054][E: codex-rs/cli/src/main.rs:1153][E: codex-rs/cli/src/main.rs:1217][E: codex-rs/cli/src/main.rs:2450] | `codex-rs/cli/src/main.rs:1054` |
| `--remote-auth-token-env ENV_VAR` | `InteractiveRemoteOptions::remote_auth_token_env` | optional string | 指定 bearer token 所在环境变量；interactive/session wrappers 会合并 root/subcommand 值，非 interactive/session path 会被 `reject_remote_mode_for_subcommand()` 拒绝。[E: codex-rs/cli/src/main.rs:1059][E: codex-rs/cli/src/main.rs:1156][E: codex-rs/cli/src/main.rs:2450][E: codex-rs/cli/src/main.rs:2460] | `codex-rs/cli/src/main.rs:1059` |
...
| `-m, --model MODEL` | `SharedCliOptions::model` | optional string | 指定 agent model；exec path destructure 后写入 `ConfigOverrides.model`。[E: codex-rs/utils/cli/src/shared_options.rs:23][E: codex-rs/exec/src/lib.rs:283] | `codex-rs/utils/cli/src/shared_options.rs:23` |
| `--oss` | `SharedCliOptions::oss` | bool false | 启用 open-source provider path；exec 只在 `oss` 为 true 时解析 OSS provider。[E: codex-rs/utils/cli/src/shared_options.rs:27][E: codex-rs/exec/src/lib.rs:284] | `codex-rs/utils/cli/src/shared_options.rs:27` |
| `--local-provider PROVIDER` | `SharedCliOptions::oss_provider` | optional string | 指定 OSS/local provider id。[E: codex-rs/utils/cli/src/shared_options.rs:32][E: codex-rs/exec/src/lib.rs:285] | `codex-rs/utils/cli/src/shared_options.rs:32` |
| `-p, --profile PROFILE` | `SharedCliOptions::config_profile_v2` | optional `ProfileV2Name` | 选择 `$CODEX_HOME/<name>.config.toml` profile；exec loader overrides 用它构造 user config path/profile。[E: codex-rs/utils/cli/src/shared_options.rs:36][E: codex-rs/exec/src/lib.rs:286] | `codex-rs/utils/cli/src/shared_options.rs:36` |
| `-s, --sandbox MODE` | `SharedCliOptions::sandbox_mode` | optional enum | CLI enum 值是 `read-only`、`workspace-write`、`danger-full-access`；exec 映射为 `ConfigOverrides.sandbox_mode`。[E: codex-rs/utils/cli/src/shared_options.rs:41][E: codex-rs/exec/src/lib.rs:287][E: codex-rs/exec/src/lib.rs:327] | `codex-rs/utils/cli/src/shared_options.rs:41` |
...
| `--dangerously-bypass-approvals-and-sandbox` / `--yolo` | `SharedCliOptions::dangerously_bypass_approvals_and_sandbox` | bool false | 跳过 confirmation prompts 且不使用 sandbox；exec 将其映射为 `SandboxMode::DangerFullAccess`。[E: codex-rs/utils/cli/src/shared_options.rs:59][E: codex-rs/exec/src/lib.rs:327] | `codex-rs/utils/cli/src/shared_options.rs:59` |
| `--dangerously-bypass-hook-trust` | `SharedCliOptions::bypass_hook_trust` | bool false | 仅本次 invocation 绕过 persisted hook trust；exec destructure 该字段并写入 `ConfigOverrides.bypass_hook_trust`。[E: codex-rs/utils/cli/src/shared_options.rs:64][E: codex-rs/exec/src/lib.rs:290] | `codex-rs/utils/cli/src/shared_options.rs:64` |
| `-C, --cd DIR` | `SharedCliOptions::cwd` | optional `PathBuf` | 指定 working root；exec 用该值作为 `ConfigOverrides.cwd`，未指定时取 current dir。[E: codex-rs/utils/cli/src/shared_options.rs:68][E: codex-rs/exec/src/lib.rs:291] | `codex-rs/utils/cli/src/shared_options.rs:68` |
| `--worktree` | `SharedCliOptions::worktree` | bool false | 在新的 managed Git worktree 中运行 session；exec 还把它标成 exec-global arg，并要求 `Feature::Worktrees`。[E: codex-rs/utils/cli/src/shared_options.rs:76][E: codex-rs/exec/src/cli.rs:146][E: codex-rs/exec/src/lib.rs:407] | `codex-rs/utils/cli/src/shared_options.rs:72` |
...
| `--search` | `codex_tui::Cli::web_search` | bool false | 开启 interactive native web search；`features list` 也会把它作为 canonical `web_search="live"` override 来计算 effective feature rows。[E: codex-rs/tui/src/cli.rs:70][E: codex-rs/cli/src/main.rs:1842][E: codex-rs/cli/src/main.rs:1845] | `codex-rs/tui/src/cli.rs:70` |
...
root config overrides 通过 `prepend_root_overrides()` 放到 subcommand-local overrides 前面，因此 subcommand 后面解析到的 `-c` 有更高优先级。[E: codex-rs/utils/cli/src/config_override.rs:42][E: codex-rs/cli/src/main.rs:2443][E: codex-rs/cli/src/main.rs:2447]

`codex exec` 入口会调用 `SharedCliOptions::inherit_exec_root_options()` 继承 root interactive shared options；该函数对 unset 的 scalar 才复制 root 值，对 images/add-dir 采用合并策略，并把 sandbox、approve-for-me 与 dangerous bypass 视为互斥的一组选择，保留 subcommand 自己显式选择的那一组。[E: codex-rs/cli/src/main.rs:1234][E: codex-rs/utils/cli/src/shared_options.rs:95][E: codex-rs/utils/cli/src/shared_options.rs:140][E: codex-rs/utils/cli/src/shared_options.rs:153]
