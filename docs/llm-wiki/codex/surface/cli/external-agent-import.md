...
```mermaid
flowchart TD
    TUI["TUI /import"] --> RPC["externalAgentConfig detect/import/readHistories/recordHistory"]
...
`SlashCommand::Import` 的 wire name 是 `import`，popup description 仍写作从 Claude Code 导入；该命令不能在 task 运行期间执行。[E: codex-rs/tui/src/slash_command.rs:11][E: codex-rs/tui/src/slash_command.rs:103][E: codex-rs/tui/src/slash_command.rs:190][E: codex-rs/tui/src/slash_command.rs:202]

app-server 暴露 `externalAgentConfig/detect`、`externalAgentConfig/import`、`externalAgentConfig/import/recordHistory`、`externalAgentConfig/import/readHistories`；detect/import/recordHistory 按 global `config` 串行，history read 使用 shared-read serialization。服务器发送 `externalAgentConfig/import/progress` 与 `externalAgentConfig/import/completed` 两种 notification。[E: codex-rs/app-server-protocol/src/protocol/common.rs:1235][E: codex-rs/app-server-protocol/src/protocol/common.rs:1238][E: codex-rs/app-server-protocol/src/protocol/common.rs:1244][E: codex-rs/app-server-protocol/src/protocol/common.rs:1250][E: codex-rs/app-server-protocol/src/protocol/common.rs:1808][E: codex-rs/app-server-protocol/src/protocol/common.rs:1810]

migration item 有 `AGENTS_MD`、`CONFIG`、`SKILLS`、`PLUGINS`、`MCP_SERVER_CONFIG`、`SUBAGENTS`、`HOOKS`、`COMMANDS`、`MEMORY`、`SESSIONS` 十类。[E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:590][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:620] detect 接受 `includeHome`、repo `cwds`、session age/count limits 和 `migrationSource`；协议仍保留但忽略旧 `source` 字段。[E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:724][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:729][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:733][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:736][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:740][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:742] processor 把新 limits 写入 session import limits，并用 `migrationSource`、`includeHome` 和 `cwds` 构造 service/options。[E: codex-rs/app-server/src/external_agent_migration/processor.rs:121][E: codex-rs/app-server/src/external_agent_migration/processor.rs:128][E: codex-rs/app-server/src/external_agent_migration/processor.rs:140] import 同样用 `migrationSource` 选择 service，并返回新建的 `importId`。[E: codex-rs/app-server/src/external_agent_migration/processor.rs:177][E: codex-rs/app-server/src/external_agent_migration/processor.rs:202]

detect response 的协议含 `connectors`，source 可为 `RemoteMcpServersConfig`/`SessionToolUse`。processor 现在从 detected sessions 计算 connector candidates，再交给 `detect_response()` 映射进 response，不再固定返回空列表。[E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:746][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:749][E: codex-rs/app-server/src/external_agent_migration/processor.rs:159][E: codex-rs/app-server/src/external_agent_migration/processor.rs:170][E: codex-rs/app-server/src/external_agent_migration/protocol.rs:34]

## Crate 重组后的职责边界

旧 `external-agent-sessions` 已并入 `external-agent-migration::sessions`；crate 根现在只组织 source adapters、detect、service、memory、plugins、rewrite 与 sessions，并从 `service`/`sessions` 重导出公共 API。[E: codex-rs/external-agent-migration/src/lib.rs:3][E: codex-rs/external-agent-migration/src/lib.rs:18][E: codex-rs/external-agent-migration/src/lib.rs:50][E: codex-rs/external-agent-migration/src/lib.rs:61][E: codex-rs/external-agent-migration/src/lib.rs:69]

app-server 旧 `config/external_agent_config.rs` 和 `request_processors/external_agent_config_processor.rs` 的职责重定位为 `external_agent_migration/processor.rs`：processor 构造 `ExternalAgentConfigService` 与 `ExternalAgentSessionImporter`，负责 RPC 参数校验、响应、后台任务和通知；core/protocol 类型转换独立在 `protocol.rs`，session persistence 独立在 `session_importer.rs`。[E: codex-rs/app-server/src/external_agent_migration/processor.rs:60][E: codex-rs/app-server/src/external_agent_migration/processor.rs:83][E: codex-rs/app-server/src/external_agent_migration/processor.rs:96][E: codex-rs/app-server/src/external_agent_migration/protocol.rs:30][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:61]

## Source adapter 与 scope

`ExternalAgentSource::from_migration_source` 仅把大小写不敏感的 `cursor` 选成 Cursor，其余值都回落到 Claude Code。Claude source 用 `.claude/settings.json`，Cursor source 用 home `~/.cursor/cli-config.json` 或 repo `.cursor/cli.json`；memory 只支持 Claude，recent sessions 两者都支持。[E: codex-rs/external-agent-migration/src/migration_source.rs:55][E: codex-rs/external-agent-migration/src/migration_source.rs:80][E: codex-rs/external-agent-migration/src/migration_source.rs:87][E: codex-rs/external-agent-migration/src/migration_source.rs:130]

`ExternalAgentConfigService::detect` 可扫描 home 与多个 repo scope；memory 额外要求 `include_home`、`include_memory` 且 source 支持 memory。每个 scope 都只为目标中尚缺的内容生成 migration item。[E: codex-rs/external-agent-migration/src/detect/mod.rs:38][E: codex-rs/external-agent-migration/src/detect/mod.rs:43][E: codex-rs/external-agent-migration/src/detect/mod.rs:46][E: codex-rs/external-agent-migration/src/detect/mod.rs:56][E: codex-rs/external-agent-migration/src/detect/mod.rs:83][E: codex-rs/external-agent-migration/src/detect/mod.rs:99]

home scope 把 config/MCP 写入 `CODEX_HOME/config.toml`、hooks 写入 `CODEX_HOME/hooks.json`、subagents 写入 `CODEX_HOME/agents`、instructions 写入 `CODEX_HOME/AGENTS.md`；skills 与 migrated commands 写入 `CODEX_HOME` 的 parent 下 `.agents/skills`。repo scope 对应写入 `<repo>/.codex/config.toml`、`.codex/hooks.json`、`.codex/agents`、`<repo>/AGENTS.md` 与 `<repo>/.agents/skills`。[E: codex-rs/external-agent-migration/src/service.rs:444][E: codex-rs/external-agent-migration/src/service.rs:537][E: codex-rs/external-agent-migration/src/service.rs:587][E: codex-rs/external-agent-migration/src/service.rs:623][E: codex-rs/external-agent-migration/src/service.rs:641][E: codex-rs/external-agent-migration/src/service.rs:666][E: codex-rs/external-agent-migration/src/service.rs:646][E: codex-rs/external-agent-migration/src/service.rs:732]
...
subagent importer 只枚举非 README 的 `*.md`，跳过缺有效 metadata 或目标已存在的文件，并输出同 stem 的 `.toml`；service 对 skills 也采用“只复制目标不存在的目录”语义。[E: codex-rs/external-agent-migration/src/subagents.rs:54][E: codex-rs/external-agent-migration/src/subagents.rs:70][E: codex-rs/external-agent-migration/src/subagents.rs:86][E: codex-rs/external-agent-migration/src/subagents.rs:101][E: codex-rs/external-agent-migration/src/subagents.rs:109][E: codex-rs/external-agent-migration/src/service.rs:705][E: codex-rs/external-agent-migration/src/service.rs:716]
...
app-server 只有在 `Feature::ExternalAgentMemoryImport` 启用时才检测/接受 `MEMORY`；请求缺少所选 memory 时直接返回 invalid request。[E: codex-rs/app-server/src/external_agent_migration/processor.rs:158][E: codex-rs/app-server/src/external_agent_migration/processor.rs:166][E: codex-rs/app-server/src/external_agent_migration/processor.rs:335][E: codex-rs/app-server/src/external_agent_migration/processor.rs:350]

## JSONL session migration

Claude detector 扫描 `.claude/projects/*/*.jsonl` 并要求解析出的 cwd 仍是目录；Cursor detector 扫描 `.cursor/projects/*/agent-transcripts`，排除 `subagents` 子树，并允许用 encoded project path 作为 fallback cwd。[E: codex-rs/external-agent-migration/src/detect/sessions/cla.rs:10][E: codex-rs/external-agent-migration/src/detect/sessions/cla.rs:32][E: codex-rs/external-agent-migration/src/detect/sessions/cla.rs:45][E: codex-rs/external-agent-migration/src/detect/sessions/cur.rs:11][E: codex-rs/external-agent-migration/src/detect/sessions/cur.rs:41][E: codex-rs/external-agent-migration/src/detect/sessions/cur.rs:38][E: codex-rs/external-agent-migration/src/detect/sessions/cur.rs:68]

共享 detector 默认只收最近 30 天、最多 50 条候选；RPC 可用 `maxSessionAgeDays` / `maxSessions` 覆盖这两个上限。它用 ledger 的 modified time/content state 跳过未变化的已导入文件。session parser 已拆为 Claude/Cursor record adapters 与共享 content-block helpers：两者都逐行容忍空行和坏 JSON，提取 cwd/messages/content hash；Claude 还提取 custom/AI title 和 MCP attribution ids，Cursor 允许 detector cwd/timestamp fallback。[E: codex-rs/external-agent-migration/src/model.rs:5][E: codex-rs/external-agent-migration/src/model.rs:6][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:733][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:736][E: codex-rs/app-server/src/external_agent_migration/processor.rs:128][E: codex-rs/app-server/src/external_agent_migration/processor.rs:131][E: codex-rs/app-server/src/external_agent_migration/processor.rs:135][E: codex-rs/external-agent-migration/src/detect/sessions/common.rs:16][E: codex-rs/external-agent-migration/src/detect/sessions/common.rs:22][E: codex-rs/external-agent-migration/src/detect/sessions/common.rs:44][E: codex-rs/external-agent-migration/src/detect/sessions/common.rs:52][E: codex-rs/external-agent-migration/src/detect/sessions/common.rs:70][E: codex-rs/external-agent-migration/src/sessions/records_cla.rs:23][E: codex-rs/external-agent-migration/src/sessions/records_cla.rs:40][E: codex-rs/external-agent-migration/src/sessions/records_cla.rs:97][E: codex-rs/external-agent-migration/src/sessions/records_cla.rs:117][E: codex-rs/external-agent-migration/src/sessions/records_cla.rs:120][E: codex-rs/external-agent-migration/src/sessions/records_cla.rs:144][E: codex-rs/external-agent-migration/src/sessions/records_cur.rs:23][E: codex-rs/external-agent-migration/src/sessions/records_cur.rs:42][E: codex-rs/external-agent-migration/src/sessions/records_cur.rs:94][E: codex-rs/external-agent-migration/src/sessions/records_cur.rs:115][E: codex-rs/external-agent-migration/src/sessions/records_cur.rs:131][E: codex-rs/external-agent-migration/src/sessions/records_common.rs:14][E: codex-rs/external-agent-migration/src/sessions/records_common.rs:30]

`prepare_validated_session_import_with_metadata_mode` 先以 ledger 去重，再按 `Embedded` 或 `MigrationFallback` 决定是否接纳 detector 的 cwd；只有 cwd 仍存在且 rollout 非空才产出 `PendingSessionImport`。[E: codex-rs/external-agent-migration/src/sessions/mod.rs:60][E: codex-rs/external-agent-migration/src/sessions/mod.rs:112][E: codex-rs/external-agent-migration/src/sessions/mod.rs:118][E: codex-rs/external-agent-migration/src/sessions/mod.rs:122][E: codex-rs/external-agent-migration/src/sessions/mod.rs:146][E: codex-rs/external-agent-migration/src/sessions/mod.rs:151][E: codex-rs/external-agent-migration/src/sessions/mod.rs:156]

exporter 把每条 user message 变成 synthetic turn start/user event/response item，assistant message 只在已有 turn 时加入；末尾追加 `<EXTERNAL SESSION IMPORTED>`、估算 token count 和 turn complete。[E: codex-rs/external-agent-migration/src/sessions/export.rs:77][E: codex-rs/external-agent-migration/src/sessions/export.rs:95][E: codex-rs/external-agent-migration/src/sessions/export.rs:106][E: codex-rs/external-agent-migration/src/sessions/export.rs:126][E: codex-rs/external-agent-migration/src/sessions/export.rs:141][E: codex-rs/external-agent-migration/src/sessions/export.rs:152][E: codex-rs/external-agent-migration/src/sessions/export.rs:182][E: codex-rs/external-agent-migration/src/sessions/export.rs:196]

`ExternalAgentSessionImporter` 串行化整个 import batch，但 batch 内最多并发 5 个 session；成功后把 source path/hash、thread id、modified time 与 detected connector names 写入 `external_agent_session_imports.json`。单个 session 通过 `ThreadStore::create_thread`、`append_items`、`update_thread_metadata`、`persist_thread`、`shutdown_thread` 落盘，append 失败会 discard 新 thread。[E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:44][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:84][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:112][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:121][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:203][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:465][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:474][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:483][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:490][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:502][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:512][E: codex-rs/external-agent-migration/src/sessions/ledger.rs:16][E: codex-rs/external-agent-migration/src/sessions/ledger.rs:159]

## 同步、后台任务与 history

processor 先验证 sessions、同步执行普通迁移、必要时刷新 config runtime，然后立即回 `importId`。remote plugins 与 sessions 放进同一个 spawned background task 并并行等待；每类结果发送 progress，最终合并成 completed notification，plugin import 后清 plugins/skills cache。[E: codex-rs/app-server/src/external_agent_migration/processor.rs:191][E: codex-rs/app-server/src/external_agent_migration/processor.rs:193][E: codex-rs/app-server/src/external_agent_migration/processor.rs:194][E: codex-rs/app-server/src/external_agent_migration/processor.rs:199][E: codex-rs/app-server/src/external_agent_migration/processor.rs:222][E: codex-rs/app-server/src/external_agent_migration/processor.rs:252][E: codex-rs/app-server/src/external_agent_migration/processor.rs:309][E: codex-rs/app-server/src/external_agent_migration/processor.rs:317][E: codex-rs/app-server/src/external_agent_migration/processor.rs:324]

`readHistories` 从 state DB 读取 import success/failure records，同时从 session ledger 聚合 connector candidate 与 session count；response 的 `connectors` source 固定为 `RemoteMcpServersConfig`。[E: codex-rs/app-server/src/external_agent_migration/processor.rs:352][E: codex-rs/app-server/src/external_agent_migration/processor.rs:360][E: codex-rs/app-server/src/external_agent_migration/processor.rs:368][E: codex-rs/app-server/src/external_agent_migration/processor.rs:373][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:861][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:865]

`recordHistory` 不执行 migration，而是记录外部已完成结果；其 payload 已与 runtime import progress result 拆开，使用专门的 history-record type，success 还可附原 session `title`。普通 import 的 success response 同样保留 nullable title，供 histories 展示。[E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:787][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:809][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:824][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:831][E: codex-rs/app-server/src/external_agent_migration/processor.rs:385]
...
- `codex-rs/external-agent-migration/src/sessions/records_cla.rs`
- `codex-rs/external-agent-migration/src/sessions/records_cur.rs`
- `codex-rs/external-agent-migration/src/sessions/records_common.rs`
- `codex-rs/external-agent-migration/src/sessions/append.rs`
