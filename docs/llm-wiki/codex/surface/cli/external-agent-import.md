...
> external-agent migration 把 Claude Code 或 Cursor 的配置、instructions、skills、commands、subagents、hooks、plugins、MCP servers 和 recent JSONL sessions 导入 Codex；Claude Code 还可迁移 memory。TUI 入口是 `/import`，app-server 入口是 `externalAgentConfig/*`。[E: codex-rs/external-agent-migration/src/migration_source.rs:47][E: codex-rs/external-agent-migration/src/migration_source.rs:54][E: codex-rs/external-agent-migration/src/model.rs:34][E: codex-rs/tui/src/slash_command.rs:29][E: codex-rs/app-server-protocol/src/protocol/common.rs:1148]
...
`SlashCommand::Import` 的 wire name 是 `import`，popup description 仍写作从 Claude Code 导入；该命令不能在 task 运行期间执行。[E: codex-rs/tui/src/slash_command.rs:11][E: codex-rs/tui/src/slash_command.rs:103][E: codex-rs/tui/src/slash_command.rs:190][E: codex-rs/tui/src/slash_command.rs:204]

app-server 暴露 `externalAgentConfig/detect`、`externalAgentConfig/import`、`externalAgentConfig/import/readHistories`；前两者按 global `config` 串行，history read 使用 shared-read serialization。服务器发送 `externalAgentConfig/import/progress` 与 `externalAgentConfig/import/completed` 两种 notification。[E: codex-rs/app-server-protocol/src/protocol/common.rs:1148][E: codex-rs/app-server-protocol/src/protocol/common.rs:1153][E: codex-rs/app-server-protocol/src/protocol/common.rs:1163][E: codex-rs/app-server-protocol/src/protocol/common.rs:1709][E: codex-rs/app-server-protocol/src/protocol/common.rs:1710]

migration item 有 `AGENTS_MD`、`CONFIG`、`SKILLS`、`PLUGINS`、`MCP_SERVER_CONFIG`、`SUBAGENTS`、`HOOKS`、`COMMANDS`、`MEMORY`、`SESSIONS` 十类。[E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:590][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:593][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:596][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:599][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:602][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:605][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:608][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:611][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:614][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:617][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:620] detect 接受 `includeHome`、repo `cwds` 和 `migrationSource`；协议仍保留旧 `source` 字段。[E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:722][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:725][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:728][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:738][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:741] detect processor 只用 `migrationSource`、`includeHome` 和 `cwds` 构造 service/options。[E: codex-rs/app-server/src/external_agent_migration/processor.rs:122][E: codex-rs/app-server/src/external_agent_migration/processor.rs:126][E: codex-rs/app-server/src/external_agent_migration/processor.rs:139][E: codex-rs/app-server/src/external_agent_migration/processor.rs:140][E: codex-rs/app-server/src/external_agent_migration/processor.rs:142] 据此，旧 `source` 不参与 detection。[I] import 也用 `migrationSource` 选择 service，并返回新建的 `importId`。[E: codex-rs/app-server/src/external_agent_migration/processor.rs:176][E: codex-rs/app-server/src/external_agent_migration/processor.rs:179][E: codex-rs/app-server/src/external_agent_migration/processor.rs:181][E: codex-rs/app-server/src/external_agent_migration/processor.rs:201][E: codex-rs/app-server/src/external_agent_migration/processor.rs:202]
...
`ExternalAgentSessionImporter` 串行化整个 import batch，但 batch 内最多并发 5 个 session；成功后把 source path/hash、thread id、modified time 与 detected connector names 写入 `external_agent_session_imports.json`。单个 session 通过 `ThreadStore::create_thread`、`append_items`、`update_thread_metadata`、`persist_thread`、`shutdown_thread` 落盘，append 失败会 discard 新 thread。[E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:40][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:70][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:97][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:106][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:174][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:381][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:390][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:399][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:406][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:419][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:428][E: codex-rs/external-agent-migration/src/sessions/ledger.rs:16][E: codex-rs/external-agent-migration/src/sessions/ledger.rs:81]

## 同步、后台任务与 history

processor 先验证 sessions、同步执行普通迁移、必要时刷新 config runtime，然后立即回 `importId`。remote plugins 与 sessions 放进同一个 spawned background task 并并行等待；每类结果发送 progress，最终合并成 completed notification，plugin import 后清 plugins/skills cache。[E: codex-rs/app-server/src/external_agent_migration/processor.rs:190][E: codex-rs/app-server/src/external_agent_migration/processor.rs:192][E: codex-rs/app-server/src/external_agent_migration/processor.rs:195][E: codex-rs/app-server/src/external_agent_migration/processor.rs:198][E: codex-rs/app-server/src/external_agent_migration/processor.rs:221][E: codex-rs/app-server/src/external_agent_migration/processor.rs:252][E: codex-rs/app-server/src/external_agent_migration/processor.rs:308][E: codex-rs/app-server/src/external_agent_migration/processor.rs:315][E: codex-rs/app-server/src/external_agent_migration/processor.rs:319]

`readHistories` 从 state DB 读取 import success/failure records，同时从 session ledger 聚合 connector candidate 与 session count；response 的 `connectors` source 固定为 `RemoteMcpServersConfig`。[E: codex-rs/app-server/src/external_agent_migration/processor.rs:352][E: codex-rs/app-server/src/external_agent_migration/processor.rs:359][E: codex-rs/app-server/src/external_agent_migration/processor.rs:367][E: codex-rs/app-server/src/external_agent_migration/processor.rs:374][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:832][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:840]
