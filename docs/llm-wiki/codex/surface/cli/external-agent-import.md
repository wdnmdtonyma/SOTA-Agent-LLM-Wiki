...
`SlashCommand::Import` 的 wire name 是 `import`，popup description 仍写作从 Claude Code 导入；该命令不能在 task 运行期间执行。[E: codex-rs/tui/src/slash_command.rs:11][E: codex-rs/tui/src/slash_command.rs:29][E: codex-rs/tui/src/slash_command.rs:110][E: codex-rs/tui/src/slash_command.rs:223]

app-server 暴露 `externalAgentConfig/detect`、`externalAgentConfig/import`、`externalAgentConfig/import/recordHistory`、`externalAgentConfig/import/readHistories`；detect 用 `external-agent-detect` 串行，import/recordHistory 按 global `config` 串行，history read 使用 shared-read serialization。服务器发送 `externalAgentConfig/import/progress` 与 `externalAgentConfig/import/completed` 两种 notification。[E: codex-rs/app-server-protocol/src/protocol/common.rs:1324][E: codex-rs/app-server-protocol/src/protocol/common.rs:1329][E: codex-rs/app-server-protocol/src/protocol/common.rs:1334][E: codex-rs/app-server-protocol/src/protocol/common.rs:1339][E: codex-rs/app-server-protocol/src/protocol/common.rs:1905][E: codex-rs/app-server-protocol/src/protocol/common.rs:1906]

migration item 有 `AGENTS_MD`、`CONFIG`、`SKILLS`、`PLUGINS`、`MCP_SERVER_CONFIG`、`SUBAGENTS`、`HOOKS`、`COMMANDS`、`MEMORY`、`SESSIONS` 十类。[E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:712][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:742] detect 接受 `includeHome`、repo `cwds`、session age/count limits 和 `migrationSource`；协议仍保留但忽略旧 `source` 字段。[E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:849][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:852][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:855][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:858][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:862][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:865] processor 把新 limits 写入 session import limits，并用 `migrationSource`、`includeHome` 和 `cwds` 构造 service/options。[E: codex-rs/app-server/src/external_agent_migration/processor.rs:125][E: codex-rs/app-server/src/external_agent_migration/processor.rs:131][E: codex-rs/app-server/src/external_agent_migration/processor.rs:144] import 同样用 `migrationSource` 选择 service，并返回新建的 `importId`。[E: codex-rs/app-server/src/external_agent_migration/processor.rs:197][E: codex-rs/app-server/src/external_agent_migration/processor.rs:202]

detect response 的协议含 `connectors`。processor 现在从 detected sessions 计算 connector candidates，再交给 `detect_response()` 映射进 response，不再固定返回空列表。[E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:837][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:840][E: codex-rs/app-server/src/external_agent_migration/processor.rs:159][E: codex-rs/app-server/src/external_agent_migration/processor.rs:170][E: codex-rs/app-server/src/external_agent_migration/protocol.rs:34]
...
`prepare_validated_session_import_with_metadata_mode` 先以 ledger 去重，再按 `Embedded` 或 `MigrationFallback` 决定是否接纳 detector 的 cwd；只有 cwd 仍存在且 rollout 非空才产出 `PendingSessionImport`。[E: codex-rs/external-agent-migration/src/sessions/mod.rs:116][E: codex-rs/external-agent-migration/src/sessions/mod.rs:125][E: codex-rs/external-agent-migration/src/sessions/mod.rs:150][E: codex-rs/external-agent-migration/src/sessions/mod.rs:151][E: codex-rs/external-agent-migration/src/sessions/mod.rs:160]

exporter 把每条 user message 变成 synthetic turn start/user event/response item，assistant message 只在已有 turn 时加入；末尾追加 `<EXTERNAL SESSION IMPORTED>`、估算 token count 和 turn complete。[E: codex-rs/external-agent-migration/src/sessions/export.rs:27][E: codex-rs/external-agent-migration/src/sessions/export.rs:95][E: codex-rs/external-agent-migration/src/sessions/export.rs:106][E: codex-rs/external-agent-migration/src/sessions/export.rs:126][E: codex-rs/external-agent-migration/src/sessions/export.rs:141][E: codex-rs/external-agent-migration/src/sessions/export.rs:182]

`ExternalAgentSessionImporter` 串行化整个 import batch，但 batch 内最多并发 5 个 session；成功后把 source path/hash、thread id、modified time 与 detected connector names 写入 `external_agent_session_imports.json`。单个 session 通过 `ThreadStore::create_thread`、`append_items`、`update_thread_metadata`、`persist_thread`、`shutdown_thread` 落盘，append 失败会 discard 新 thread。[E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:49][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:91][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:129][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:542][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:553][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:559][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:567][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:580][E: codex-rs/app-server/src/external_agent_migration/session_importer.rs:589][E: codex-rs/external-agent-migration/src/sessions/ledger.rs:16]

## 同步、后台任务与 history

processor 先验证 sessions、同步执行普通迁移、必要时刷新 config runtime，然后立即回 `importId`。remote plugins 与 sessions 放进同一个 spawned background task 并并行等待；每类结果发送 progress，最终合并成 completed notification，plugin import 后清 plugins/skills cache。[E: codex-rs/app-server/src/external_agent_migration/processor.rs:211][E: codex-rs/app-server/src/external_agent_migration/processor.rs:214][E: codex-rs/app-server/src/external_agent_migration/processor.rs:217][E: codex-rs/app-server/src/external_agent_migration/processor.rs:222][E: codex-rs/app-server/src/external_agent_migration/processor.rs:273][E: codex-rs/app-server/src/external_agent_migration/processor.rs:332][E: codex-rs/app-server/src/external_agent_migration/processor.rs:340][E: codex-rs/app-server/src/external_agent_migration/processor.rs:341][E: codex-rs/app-server/src/external_agent_migration/processor.rs:343]

`readHistories` 从 state DB 读取 import success/failure records，同时从 session ledger 聚合 connector candidate 与 session count；response 的 `connectors` source 固定为 `RemoteMcpServersConfig`。[E: codex-rs/app-server/src/external_agent_migration/processor.rs:376][E: codex-rs/app-server/src/external_agent_migration/processor.rs:384][E: codex-rs/app-server/src/external_agent_migration/processor.rs:391][E: codex-rs/app-server/src/external_agent_migration/processor.rs:401]

`recordHistory` 不执行 migration，而是记录外部已完成结果；其 payload 已与 runtime import progress result 拆开，使用专门的 history-record type，success 还可附原 session `title`。普通 import 的 success response 同样保留 nullable title，供 histories 展示。[E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:931][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:939][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:954][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:915][E: codex-rs/app-server/src/external_agent_migration/processor.rs:408]
