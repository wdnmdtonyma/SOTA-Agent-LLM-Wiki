...
detect response 的协议含 `connectors`。processor 现在从 detected sessions 计算 connector candidates，再交给 `detect_response()` 映射进 response，不再固定返回空列表。[E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:865][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:868][E: codex-rs/app-server/src/external_agent_migration/processor.rs:159][E: codex-rs/app-server/src/external_agent_migration/processor.rs:170][E: codex-rs/app-server/src/external_agent_migration/protocol.rs:34]
...
`recordHistory` 不执行 migration，而是记录外部已完成结果；其 payload 已与 runtime import progress result 拆开，使用专门的 history-record type，success 还可附原 session `title`。普通 import 的 success response 同样保留 nullable title，供 histories 展示。[E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:959][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:967][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:982][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:943][E: codex-rs/app-server/src/external_agent_migration/processor.rs:408]
