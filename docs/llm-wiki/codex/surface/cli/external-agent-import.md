...
`SlashCommand::Import` 的 wire name 是 `import`，popup description 仍写作从 Claude Code 导入；该命令不能在 task 运行期间执行。[E: codex-rs/tui/src/slash_command.rs:11][E: codex-rs/tui/src/slash_command.rs:103][E: codex-rs/tui/src/slash_command.rs:191][E: codex-rs/tui/src/slash_command.rs:205]

app-server 暴露 `externalAgentConfig/detect`、`externalAgentConfig/import`、`externalAgentConfig/import/recordHistory`、`externalAgentConfig/import/readHistories`；detect/import/recordHistory 按 global `config` 串行，history read 使用 shared-read serialization。服务器发送 `externalAgentConfig/import/progress` 与 `externalAgentConfig/import/completed` 两种 notification。[E: codex-rs/app-server-protocol/src/protocol/common.rs:1172][E: codex-rs/app-server-protocol/src/protocol/common.rs:1177][E: codex-rs/app-server-protocol/src/protocol/common.rs:1182][E: codex-rs/app-server-protocol/src/protocol/common.rs:1187][E: codex-rs/app-server-protocol/src/protocol/common.rs:1742][E: codex-rs/app-server-protocol/src/protocol/common.rs:1743]

migration item 有 `AGENTS_MD`、`CONFIG`、`SKILLS`、`PLUGINS`、`MCP_SERVER_CONFIG`、`SUBAGENTS`、`HOOKS`、`COMMANDS`、`MEMORY`、`SESSIONS` 十类。[E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:590][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:620] detect 接受 `includeHome`、repo `cwds`、session age/count limits 和 `migrationSource`；协议仍保留但忽略旧 `source` 字段。[E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:724][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:730][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:733][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:736][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:740][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:743] processor 把新 limits 写入 session import limits，并用 `migrationSource`、`includeHome` 和 `cwds` 构造 service/options。[E: codex-rs/app-server/src/external_agent_migration/processor.rs:121][E: codex-rs/app-server/src/external_agent_migration/processor.rs:128][E: codex-rs/app-server/src/external_agent_migration/processor.rs:140] import 同样用 `migrationSource` 选择 service，并返回新建的 `importId`。[E: codex-rs/app-server/src/external_agent_migration/processor.rs:177][E: codex-rs/app-server/src/external_agent_migration/processor.rs:202]

detect response 的协议新增 `connectors` 以及 `RemoteMcpServersConfig`/`SessionToolUse` source，但当前 `detect_response()` 仍固定返回空 `Vec`；detector/schema 面已存在，app-server 对外接线尚未完成。[E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:715][E: codex-rs/app-server-protocol/src/protocol/v2/config.rs:884][E: codex-rs/app-server/src/external_agent_migration/protocol.rs:30]
