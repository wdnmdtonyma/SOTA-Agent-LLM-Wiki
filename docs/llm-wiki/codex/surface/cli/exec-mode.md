...
|---|---|---|---|---|
| `PROMPT` | `Cli::prompt` | optional positional | root initial instructions；有 prompt 且 stdin piped 时，`resolve_root_prompt()` 会把 stdin 追加为 `<stdin>` block。[E: codex-rs/exec/src/cli.rs:80][E: codex-rs/exec/src/lib.rs:2292] | `codex-rs/exec/src/cli.rs:80` |
...
| `review` | `Command::Review(ReviewArgs)` | nested subcommand | 非交互 code review；先构造 `ReviewRequest`，再发送 `review/start`。[E: codex-rs/exec/src/cli.rs:158][E: codex-rs/exec/src/lib.rs:872][E: codex-rs/exec/src/lib.rs:873][E: codex-rs/exec/src/lib.rs:1188][E: codex-rs/exec/src/lib.rs:2305] | `codex-rs/exec/src/cli.rs:157` |
...
| `--ephemeral` | `Cli::ephemeral` | bool global false | 不持久化 session 文件的 CLI 意图；exec 写入 `ConfigOverrides.ephemeral`，thread start params 也带 `ephemeral: Some(config.ephemeral)`。[E: codex-rs/exec/src/cli.rs:37][E: codex-rs/exec/src/lib.rs:270][E: codex-rs/exec/src/lib.rs:1366] | `codex-rs/exec/src/cli.rs:37` |
...
| `review --uncommitted` | `ReviewArgs::uncommitted` | bool false | review staged、unstaged、untracked changes；与 base/commit/prompt 冲突，映射到 `ReviewTarget::UncommittedChanges`。[E: codex-rs/exec/src/cli.rs:281][E: codex-rs/exec/src/lib.rs:2306][E: codex-rs/exec/src/lib.rs:2307] | `codex-rs/exec/src/cli.rs:280` |
| `review --base BRANCH` | `ReviewArgs::base` | optional string | review 相对 base branch 的 changes；映射到 `ReviewTarget::BaseBranch`。[E: codex-rs/exec/src/cli.rs:289][E: codex-rs/exec/src/lib.rs:2308][E: codex-rs/exec/src/lib.rs:2309] | `codex-rs/exec/src/cli.rs:288` |
| `review --commit SHA` / `--title TITLE` | `ReviewArgs::commit`, `commit_title` | optional string | review 指定 commit 引入的 changes；title 只在 commit target 上可用。[E: codex-rs/exec/src/cli.rs:297][E: codex-rs/exec/src/cli.rs:301][E: codex-rs/exec/src/lib.rs:2310][E: codex-rs/exec/src/lib.rs:2313] | `codex-rs/exec/src/cli.rs:296` |
| `review PROMPT` | `ReviewArgs::prompt` | optional positional | 自定义 review instructions；`-` 会从 stdin 读，空 prompt 报错。[E: codex-rs/exec/src/cli.rs:305][E: codex-rs/exec/src/lib.rs:2315][E: codex-rs/exec/src/lib.rs:2318] | `codex-rs/exec/src/cli.rs:304` |
...
exec 模式启动 `InProcessAppServerClient` 后，resume path 先尝试 `thread/resume`，fork path 发 `thread/fork`，否则使用 `thread/start`；随后 user turn 发 `turn/start`，review 发 `review/start`。[E: codex-rs/exec/src/lib.rs:973][E: codex-rs/exec/src/lib.rs:980][E: codex-rs/exec/src/lib.rs:1013][E: codex-rs/exec/src/lib.rs:1139][E: codex-rs/exec/src/lib.rs:1169][E: codex-rs/exec/src/lib.rs:1188][E: codex-rs/exec/src/lib.rs:1316]

非 ephemeral persistent exec thread 在 `thread/start` 时默认请求 `history_mode: Paginated`。如果当前 in-process app-server 返回 `-32600` 且 message 是 `paginated threads require thread/turns/list and thread/items/list support`，exec 会清掉 `history_mode` 再重试一次。[E: codex-rs/exec/src/lib.rs:1366][E: codex-rs/exec/src/lib.rs:1367][E: codex-rs/exec/src/lib.rs:1333][E: codex-rs/exec/src/lib.rs:1336][E: codex-rs/exec/src/lib.rs:1338]

新 thread 总是把 resolved `config.approvals_reviewer` 传入 `thread/start`；resume 则只在 CLI config overrides 显式包含 `approvals_reviewer` 时向 `thread/resume` 传该 override。[E: codex-rs/exec/src/lib.rs:609][E: codex-rs/exec/src/lib.rs:1362][E: codex-rs/exec/src/lib.rs:1392]

事件循环同时监听 Ctrl+C 和 app-server events；Ctrl+C 发送 `turn/interrupt`。[E: codex-rs/exec/src/lib.rs:1227]
