...
`AgentsToml` exposes `enabled`、per-session `max_concurrent_threads_per_session`（`max_threads` alias）、V1-only `max_depth`、默认 subagent model/reasoning effort、interrupt-message policy 与 flattened role declarations。`job_max_runtime_seconds` 只为旧配置可解析而保留，是 schema-hidden no-op；当前没有 agent-job runtime。[E: codex-rs/config/src/config_toml.rs:673][E: codex-rs/config/src/config_toml.rs:676][E: codex-rs/config/src/config_toml.rs:679][E: codex-rs/config/src/config_toml.rs:681][E: codex-rs/config/src/config_toml.rs:690][E: codex-rs/config/src/config_toml.rs:705]
