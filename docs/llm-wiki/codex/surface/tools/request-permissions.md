...
> `request_permissions` 是 Codex 暴露给模型的增量权限申请工具：模型提交 filesystem/network permission profile，可选指定 `environment_id`，handler 按选中 environment cwd 解析并规范化请求，等待 guardian 或客户端返回授权子集，再把 turn/session scope 的获批权限记录下来。[E: codex-rs/core/src/tools/handlers/shell_spec.rs:227][E: codex-rs/core/src/tools/handlers/shell_spec.rs:245][E: codex-rs/core/src/tools/handlers/request_permissions.rs:66][E: codex-rs/core/src/tools/handlers/request_permissions.rs:84][E: codex-rs/core/src/session/mod.rs:2440][E: codex-rs/core/src/session/mod.rs:2553][E: codex-rs/core/src/session/mod.rs:2804]
...
| `reason` | string | 否 | 无 | schema 描述为可选短说明。[E: codex-rs/core/src/tools/handlers/shell_spec.rs:229][E: codex-rs/core/src/tools/handlers/shell_spec.rs:232] | protocol 字段是 `Option<String>`；client event 携带 reason，guardian request 也把 `args.reason` 放入 approval request。[E: codex-rs/protocol/src/request_permissions.rs:60][E: codex-rs/protocol/src/request_permissions.rs:94][E: codex-rs/core/src/session/mod.rs:2492][E: codex-rs/core/src/session/mod.rs:2495][E: codex-rs/core/src/session/mod.rs:2595] |
...
| `permissions.file_system.write` | array<string> | 否 | none | write 字段同样是 absolute path 数组。[E: codex-rs/core/src/tools/handlers/shell_spec.rs:387][E: codex-rs/core/src/tools/handlers/shell_spec.rs:392] | response 最终会和 requested permissions 求交，避免返回超出请求集合的路径。[E: codex-rs/core/src/session/mod.rs:2775][E: codex-rs/core/src/session/mod.rs:2792][E: codex-rs/core/src/session/mod.rs:2793] |
...
`add_core_utility_tools` 只在 `Feature::RequestPermissionsTool` 开启且 current tool environment mode 有 environment 时注册 `RequestPermissionsHandler`。[E: codex-rs/core/src/tools/spec_plan.rs:731][E: codex-rs/core/src/tools/spec_plan.rs:733][E: codex-rs/core/src/tools/spec_plan.rs:704][E: codex-rs/core/src/tools/spec_plan.rs:761][E: codex-rs/core/src/tools/spec_plan.rs:762]

visible spec 构建沿用普通 runtime 流程：direct exposure 的 runtime spec 会加入 model-visible specs；本 handler 没有额外 hidden/deferred exposure。[E: codex-rs/core/src/tools/spec_plan.rs:243][E: codex-rs/core/src/tools/spec_plan.rs:253][E: codex-rs/core/src/tools/spec_plan.rs:258][E: codex-rs/core/src/tools/spec_plan.rs:259][E: codex-rs/core/src/tools/spec_plan.rs:261]
...
5. session 在 approval policy 为 `Never` 或 granular policy 不允许 request permissions 时，立即返回空权限 turn-scope response。[E: codex-rs/core/src/session/mod.rs:2448][E: codex-rs/core/src/session/mod.rs:2450][E: codex-rs/core/src/session/mod.rs:2456][E: codex-rs/core/src/session/mod.rs:2459]
6. guardian path 会构造 `GuardianApprovalRequest::RequestPermissions`，把 review decision 映射成 turn/session 授权或空权限，然后 normalize 并记录。[E: codex-rs/core/src/session/mod.rs:2484][E: codex-rs/core/src/session/mod.rs:2492][E: codex-rs/core/src/session/mod.rs:2518][E: codex-rs/core/src/session/mod.rs:2526][E: codex-rs/core/src/session/mod.rs:2384][E: codex-rs/core/src/session/mod.rs:2553][E: codex-rs/core/src/session/mod.rs:2558]
7. 非 guardian path 创建 pending request entry，发出 `EventMsg::RequestPermissions`，等待客户端通过 call id 返回 response。[E: codex-rs/core/src/session/mod.rs:2568][E: codex-rs/core/src/session/mod.rs:2574][E: codex-rs/core/src/session/mod.rs:2590][E: codex-rs/core/src/session/mod.rs:2599][E: codex-rs/core/src/session/mod.rs:2600]
8. 客户端 `Op::RequestPermissionsResponse` 先分发到 helper，再由 helper 调用 `notify_request_permissions_response`；该函数移除 pending entry、normalize response，再发送给等待中的 handler。[E: codex-rs/core/src/session/handlers.rs:414][E: codex-rs/core/src/session/handlers.rs:416][E: codex-rs/core/src/session/handlers.rs:776][E: codex-rs/core/src/session/handlers.rs:777][E: codex-rs/core/src/session/mod.rs:2722][E: codex-rs/core/src/session/mod.rs:2732][E: codex-rs/core/src/session/mod.rs:2761][E: codex-rs/core/src/session/mod.rs:2767]
9. normalize 会拒绝 session-scope strict auto review，并把 granted permissions 与 requested permissions 求交；空 permissions 原样返回。[E: codex-rs/core/src/session/mod.rs:2775][E: codex-rs/core/src/session/mod.rs:2780][E: codex-rs/core/src/session/mod.rs:2788][E: codex-rs/core/src/session/mod.rs:2792][E: codex-rs/core/src/session/mod.rs:2793]
10. turn scope 写入 originating turn state，session scope 写入 session state；strict auto review 只在 turn scope 下启用。[E: codex-rs/core/src/session/mod.rs:2804][E: codex-rs/core/src/session/mod.rs:2813][E: codex-rs/core/src/session/mod.rs:2815][E: codex-rs/core/src/session/mod.rs:2819][E: codex-rs/core/src/session/mod.rs:2820][E: codex-rs/core/src/session/mod.rs:2825][E: codex-rs/core/src/session/mod.rs:2827]
