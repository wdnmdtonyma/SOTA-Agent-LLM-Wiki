...
| ToolSpec | `create_shell_command_tool` 返回 `ToolSpec::Function(ResponsesApiTool { ... })`，且 `output_schema: None`。[E: codex-rs/core/src/tools/handlers/shell_spec.rs:157][E: codex-rs/core/src/tools/handlers/shell_spec.rs:213][E: codex-rs/core/src/tools/handlers/shell_spec.rs:223][E: codex-rs/tools/src/tool_spec.rs:17][E: codex-rs/tools/src/tool_spec.rs:19] |
...
|---|---|---:|---|---|---|
| `command` | `string` | 是 | 无 | schema properties 固定包含 `command`，required 列表只要求 `command`。[E: codex-rs/core/src/tools/handlers/shell_spec.rs:158][E: codex-rs/core/src/tools/handlers/shell_spec.rs:160][E: codex-rs/core/src/tools/handlers/shell_spec.rs:218][E: codex-rs/core/src/tools/handlers/shell_spec.rs:220] | protocol struct `ShellCommandToolCallParams.command` 是 string；handler 解析后用它触发 implicit skill invocation 并派生 argv。[E: codex-rs/protocol/src/models.rs:1806][E: codex-rs/protocol/src/models.rs:1807][E: codex-rs/core/src/tools/handlers/shell/shell_command.rs:197][E: codex-rs/core/src/tools/handlers/shell/shell_command.rs:198] |
...
| `timeout_ms` | `number` | 否 | runtime expiration default | schema 描述最大运行时间默认 10000ms。[E: codex-rs/core/src/tools/handlers/shell_spec.rs:172][E: codex-rs/core/src/tools/handlers/shell_spec.rs:174] | protocol struct 支持 `timeout_ms`，并通过 serde alias 接受 `timeout`；`to_exec_params` 把它转为 `ExecParams.expiration`。[E: codex-rs/protocol/src/models.rs:1814][E: codex-rs/protocol/src/models.rs:1815][E: codex-rs/core/src/tools/handlers/shell/shell_command.rs:113] |
...
| `sandbox_permissions` | enum string | 否 | `UseDefault` | approval helper 插入 sandbox override 字段。[E: codex-rs/core/src/tools/handlers/shell_spec.rs:187][E: codex-rs/core/src/tools/handlers/shell_spec.rs:298][E: codex-rs/core/src/tools/handlers/shell_spec.rs:314] | protocol 字段是 optional；`to_exec_params` 使用 `unwrap_or_default()`。[E: codex-rs/protocol/src/models.rs:1816][E: codex-rs/protocol/src/models.rs:1818][E: codex-rs/core/src/tools/handlers/shell/shell_command.rs:118] |
...
| `justification` | `string` | 否 | 无 | approval helper 插入用户可见说明字段。[E: codex-rs/core/src/tools/handlers/shell_spec.rs:320][E: codex-rs/core/src/tools/handlers/shell_spec.rs:323] | `to_exec_params` 把 `params.justification` 放进 `ExecParams`。[E: codex-rs/protocol/src/models.rs:1825][E: codex-rs/protocol/src/models.rs:1826][E: codex-rs/core/src/tools/handlers/shell/shell_command.rs:124] |
| `prefix_rule` | `array<string>` | 否 | 无 | approval helper 插入可复用 approval prefix 字段；当前 schema 文案仍写 `cmd`。[E: codex-rs/core/src/tools/handlers/shell_spec.rs:326][E: codex-rs/core/src/tools/handlers/shell_spec.rs:329] | handler 克隆 `prefix_rule` 并传入 shared run path，最后进入 exec approval request。[E: codex-rs/protocol/src/models.rs:1819][E: codex-rs/protocol/src/models.rs:1821][E: codex-rs/core/src/tools/handlers/shell/shell_command.rs:205][E: codex-rs/core/src/tools/handlers/shell/shell_command.rs:227][E: codex-rs/core/src/tools/handlers/shell.rs:181] |
...
执行输出的捕获策略在 `to_exec_params` 和 runtime options 中都是 `ExecCapturePolicy::ShellTool`。[E: codex-rs/core/src/tools/handlers/shell/shell_command.rs:114][E: codex-rs/core/src/tools/runtimes/shell.rs:324][E: codex-rs/core/src/tools/runtimes/shell.rs:326]

## 5 注册与门控

`add_shell_tools` 没有 environment 时直接返回。[E: codex-rs/core/src/tools/spec_plan.rs:636][E: codex-rs/core/src/tools/spec_plan.rs:639][E: codex-rs/core/src/tools/spec_plan.rs:640] shell command options 中的 backend 来自 `shell_command_backend_for_features(features)`，该函数在 `ShellTool` 与 `ShellZshFork` 同开时选择 `ZshFork`，否则选择 `Classic`。[E: codex-rs/core/src/tools/spec_plan.rs:647][E: codex-rs/core/src/tools/spec_plan.rs:648][E: codex-rs/tools/src/tool_config.rs:11][E: codex-rs/tools/src/tool_config.rs:12][E: codex-rs/tools/src/tool_config.rs:13][E: codex-rs/tools/src/tool_config.rs:49][E: codex-rs/tools/src/tool_config.rs:50][E: codex-rs/tools/src/tool_config.rs:51][E: codex-rs/tools/src/tool_config.rs:53]

当 shell 类型是 `UnifiedExec` 时，`shell_command` 不进入 model-visible specs，而是通过 `add_dispatch_only` 以 hidden exposure 注册。[E: codex-rs/core/src/tools/spec_plan.rs:653][E: codex-rs/core/src/tools/spec_plan.rs:654][E: codex-rs/core/src/tools/spec_plan.rs:668][E: codex-rs/core/src/tools/spec_plan.rs:127][E: codex-rs/core/src/tools/spec_plan.rs:131] 当 shell 类型是 `Default`、`Local` 或 `ShellCommand` 时，`spec_plan.rs` 直接 `planned_tools.add(ShellCommandHandler::new(...))`；普通 handler 默认 exposure 是 `Direct`，visible-spec 构建会收集 direct runtime specs。[E: codex-rs/core/src/tools/spec_plan.rs:671][E: codex-rs/core/src/tools/spec_plan.rs:672][E: codex-rs/core/src/tools/spec_plan.rs:673][E: codex-rs/core/src/tools/spec_plan.rs:674][E: codex-rs/tools/src/tool_executor.rs:55][E: codex-rs/tools/src/tool_executor.rs:56][E: codex-rs/core/src/tools/spec_plan.rs:239][E: codex-rs/core/src/tools/spec_plan.rs:245][E: codex-rs/core/src/tools/spec_plan.rs:247]
...
`ShellRuntimeBackend` 只有 `ShellCommandClassic` 和 `ShellCommandZshFork` 两种 shell-command backend；ZshFork shell-command backend 调用 `zsh_fork_backend::maybe_run_shell_command`,该 backend 的 Unix `imp` 分支委托 `unix_escalation::try_run_zsh_fork`,并使用 `codex_shell_escalation` 的 escalation session/server；条件不满足会 fallback 到 standard flow。[E: codex-rs/core/src/tools/runtimes/shell.rs:76][E: codex-rs/core/src/tools/runtimes/shell.rs:81][E: codex-rs/core/src/tools/runtimes/shell.rs:87][E: codex-rs/core/src/tools/runtimes/shell.rs:306][E: codex-rs/core/src/tools/runtimes/shell.rs:307][E: codex-rs/core/src/tools/runtimes/shell.rs:311][E: codex-rs/core/src/tools/runtimes/shell/zsh_fork_backend.rs:46][E: codex-rs/core/src/tools/runtimes/shell/zsh_fork_backend.rs:51][E: codex-rs/core/src/tools/runtimes/shell/zsh_fork_backend.rs:52][E: codex-rs/core/src/tools/runtimes/shell/zsh_fork_backend.rs:74][E: codex-rs/core/src/tools/runtimes/shell/zsh_fork_backend.rs:80][E: codex-rs/core/src/tools/runtimes/shell/unix_escalation.rs:54][E: codex-rs/core/src/tools/runtimes/shell/unix_escalation.rs:60][E: codex-rs/core/src/tools/runtimes/shell/unix_escalation.rs:339][E: codex-rs/core/src/tools/runtimes/shell/unix_escalation.rs:344]
