...
> `write_stdin` 是 unified-exec 的续写/轮询工具：模型用 `session_id` 指向已有 `exec_command` process，传入 `chars` 可向 TTY session 写 stdin；未传或传空字符串时，它也可作为 poll，等待已有输出或最终完成。[E: codex-rs/core/src/tools/handlers/shell_spec.rs:110][E: codex-rs/core/src/tools/handlers/shell_spec.rs:113][E: codex-rs/core/src/tools/handlers/shell_spec.rs:119][E: codex-rs/core/src/tools/handlers/unified_exec/write_stdin.rs:69][E: codex-rs/core/src/tools/handlers/unified_exec/write_stdin.rs:73][E: codex-rs/core/src/tools/handlers/unified_exec/write_stdin.rs:75][E: codex-rs/core/src/unified_exec/process_manager.rs:665][E: codex-rs/core/src/unified_exec/process_manager.rs:673]
...
| concrete handler | `WriteStdinHandler` 是独立 handler，由 `spec_plan.rs` 在 unified-exec 分支注册。[E: codex-rs/core/src/tools/handlers/unified_exec/write_stdin.rs:32][E: codex-rs/core/src/tools/spec_plan.rs:669] |
...
`write_stdin` 依赖 `exec_command` 的 running session id：unified-exec output schema 把 `session_id` 描述为 process still running 时传给 `write_stdin` 的 identifier。[E: codex-rs/core/src/tools/handlers/shell_spec.rs:277][E: codex-rs/core/src/tools/handlers/shell_spec.rs:279] 文本输出同样在 `process_id` 存在时写出 running session id。[E: codex-rs/core/src/tools/context.rs:426][E: codex-rs/core/src/tools/context.rs:427]
...
非空 stdin 受 session 形态约束：manager 只有在 TTY process 上执行 `process.write(...)`；非 TTY 时仅 Ctrl-C interrupt 特例会触发 `process.interrupt()`，其它输入返回 `StdinClosed`。因此需要交互式 stdin 的 `exec_command` 应以 `tty: true` 创建，而 `tty` schema 默认是 false。[E: codex-rs/core/src/unified_exec/process_manager.rs:665][E: codex-rs/core/src/unified_exec/process_manager.rs:666][E: codex-rs/core/src/unified_exec/process_manager.rs:667][E: codex-rs/core/src/unified_exec/process_manager.rs:668][E: codex-rs/core/src/unified_exec/process_manager.rs:670][E: codex-rs/core/src/unified_exec/process_manager.rs:673][E: codex-rs/core/src/tools/handlers/shell_spec.rs:39][E: codex-rs/core/src/tools/handlers/shell_spec.rs:41]

## 3 输入 schema 表

| 字段 | 类型 | 必填 | 默认 | 说明 | 校验/运行时 |
|---|---|---:|---|---|---|
| `session_id` | `number` | 是 | 无 | schema properties 包含 `session_id`，required 列表只要求 `session_id`。[E: codex-rs/core/src/tools/handlers/shell_spec.rs:111][E: codex-rs/core/src/tools/handlers/shell_spec.rs:113][E: codex-rs/core/src/tools/handlers/shell_spec.rs:145][E: codex-rs/core/src/tools/handlers/shell_spec.rs:147] | runtime struct 字段类型是 `i32`，请求中映射为 `process_id`。[E: codex-rs/core/src/tools/handlers/unified_exec/write_stdin.rs:21][E: codex-rs/core/src/tools/handlers/unified_exec/write_stdin.rs:23][E: codex-rs/core/src/tools/handlers/unified_exec/write_stdin.rs:74] |
...
| `yield_time_ms` | `number` | 否 | serde default `250`; empty poll effective minimum `5000` | schema 描述里同时写了非空写默认 250ms 和空 poll 的 5000-300000ms 等待范围。[E: codex-rs/core/src/tools/handlers/shell_spec.rs:125][E: codex-rs/core/src/tools/handlers/shell_spec.rs:127] | Rust args 的 serde default 统一返回 `250`；manager 随后把非空写限制在普通 yield cap 内，把空 poll clamp 到 `MIN_EMPTY_YIELD_TIME_MS` 与 background timeout 上限之间。[E: codex-rs/core/src/tools/handlers/unified_exec/write_stdin.rs:27][E: codex-rs/core/src/tools/handlers/unified_exec.rs:64][E: codex-rs/core/src/tools/handlers/unified_exec.rs:65][E: codex-rs/core/src/unified_exec/mod.rs:64][E: codex-rs/core/src/unified_exec/mod.rs:67][E: codex-rs/core/src/unified_exec/mod.rs:69][E: codex-rs/core/src/unified_exec/process_manager.rs:695][E: codex-rs/core/src/unified_exec/process_manager.rs:698][E: codex-rs/core/src/unified_exec/process_manager.rs:699][E: codex-rs/core/src/unified_exec/process_manager.rs:700][E: codex-rs/core/src/unified_exec/process_manager.rs:702] |
| `max_output_tokens` | `number` | 否 | `10000 tokens` | schema 描述输出 token 预算默认 10000 tokens。[E: codex-rs/core/src/tools/handlers/shell_spec.rs:131][E: codex-rs/core/src/tools/handlers/shell_spec.rs:133] | 请求把该值传给 manager；runtime 默认常量是 `10_000`，输出再受 truncation policy cap 并按 token policy 截断。[E: codex-rs/core/src/tools/handlers/unified_exec/write_stdin.rs:77][E: codex-rs/core/src/unified_exec/mod.rs:70][E: codex-rs/core/src/unified_exec/mod.rs:177][E: codex-rs/core/src/unified_exec/mod.rs:178][E: codex-rs/core/src/tools/context.rs:404][E: codex-rs/core/src/tools/context.rs:407][E: codex-rs/core/src/tools/context.rs:409] |
...
`UnifiedExecProcessManager::write_stdin` 返回 `ExecCommandToolOutput`；handler 将该 response boxed 返回，因此后续使用同一个 `ExecCommandToolOutput` 文本/code-mode 输出逻辑。[E: codex-rs/core/src/unified_exec/process_manager.rs:641][E: codex-rs/core/src/unified_exec/process_manager.rs:644][E: codex-rs/core/src/tools/handlers/unified_exec/write_stdin.rs:70][E: codex-rs/core/src/tools/handlers/unified_exec/write_stdin.rs:101][E: codex-rs/core/src/tools/context.rs:369][E: codex-rs/core/src/tools/context.rs:412]

## 5 注册与门控

`add_tool_sources` 调用 `add_shell_tools`；后者没有 environment 时直接返回，不会注册 shell family。[E: codex-rs/core/src/tools/spec_plan.rs:613][E: codex-rs/core/src/tools/spec_plan.rs:614][E: codex-rs/core/src/tools/spec_plan.rs:641][E: codex-rs/core/src/tools/spec_plan.rs:644][E: codex-rs/core/src/tools/spec_plan.rs:645] 当 shell 类型是 `ConfigShellToolType::UnifiedExec` 时，`spec_plan.rs` 连续注册 `ExecCommandHandler` 和 `WriteStdinHandler`，并把 legacy `ShellCommandHandler` hidden dispatch-only 注册。[E: codex-rs/core/src/tools/spec_plan.rs:658][E: codex-rs/core/src/tools/spec_plan.rs:659][E: codex-rs/core/src/tools/spec_plan.rs:660][E: codex-rs/core/src/tools/spec_plan.rs:669][E: codex-rs/core/src/tools/spec_plan.rs:673]
...
`write_stdin` 是已有 exec session 的传输层，不发新的 pre hook；`pre_tool_use_payload` 当前直接返回 `None`。[E: codex-rs/core/src/tools/handlers/unified_exec/write_stdin.rs:105][E: codex-rs/core/src/tools/handlers/unified_exec/write_stdin.rs:110][E: codex-rs/core/src/tools/handlers/unified_exec/write_stdin.rs:114]
