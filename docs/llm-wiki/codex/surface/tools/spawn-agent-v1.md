...
| handler | `multi_agents.rs` re-export `spawn::Handler as SpawnAgentHandler`；handler 只匹配 function payload。[E: codex-rs/core/src/tools/handlers/multi_agents.rs:77][E: codex-rs/core/src/tools/handlers/multi_agents/spawn.rs:220] |

## 注册与门控

`add_collaboration_tools` 在 `collab_tools_enabled` 为 true 后分流：`multi_agent_v2_enabled` 为 false 时进入 V1 注册路径，注册 `SpawnAgentHandler`、`SendInputHandler`、`ResumeAgentHandler`、`WaitAgentHandler` 和 `CloseAgentHandler`。[E: codex-rs/core/src/tools/spec_plan.rs:1275][E: codex-rs/core/src/tools/spec_plan.rs:1334][E: codex-rs/core/src/tools/spec_plan.rs:1342][E: codex-rs/core/src/tools/spec_plan.rs:1358]

V1 exposure 在 search tool 开启时是 `Deferred`，否则是 `Direct`。[E: codex-rs/core/src/tools/spec_plan.rs:1337][E: codex-rs/core/src/tools/spec_plan.rs:640]

V1 spawn options 硬编码 `hide_agent_type_model_reasoning: false`、`expose_spawn_agent_model_overrides: true`，并仍会把 `multi_agent_v2.usage_hint_text` 写进 V1 tool description。[E: codex-rs/core/src/tools/spec_plan.rs:1347][E: codex-rs/core/src/tools/spec_plan.rs:1348][E: codex-rs/core/src/tools/spec_plan.rs:1350]
...
| `agent_type` | 否 | 非空白 agent type 会在非 full-history fork 分支应用 role config；`fork_context=true` 会调用 `reject_full_fork_agent_type_override`，显式 `agent_type` 被拒绝。当配置没有 agent roles 时，spec 会隐藏该字段。[E: codex-rs/core/src/tools/handlers/multi_agents_spec.rs:74][E: codex-rs/core/src/tools/handlers/multi_agents/spawn.rs:95][E: codex-rs/core/src/tools/handlers/multi_agents/spawn.rs:106][E: codex-rs/core/src/tools/handlers/multi_agents_common.rs:220] |
| `fork_context` | 否 | 默认 false；true 表示 full-history fork。此时继承父 agent type 并拒绝显式 `agent_type`，但 model / reasoning override 仍可按请求应用。[E: codex-rs/core/src/tools/handlers/multi_agents_spec.rs:598][E: codex-rs/core/src/tools/handlers/multi_agents/spawn.rs:232][E: codex-rs/core/src/tools/handlers/multi_agents/spawn.rs:97] |
...
spawn config 来自父 turn 的 effective config；公共 helper 会刷新 model/provider/reasoning/developer instructions，并复制 approval policy、cwd、permission profile 等 runtime state。[E: codex-rs/core/src/tools/handlers/multi_agents/spawn.rs:93][E: codex-rs/core/src/tools/handlers/multi_agents_common.rs:177][E: codex-rs/core/src/tools/handlers/multi_agents_common.rs:235]

V1 spawn 调用 `spawn_agent_with_metadata`，但传给 `thread_spawn_source` 的 `task_name` 是 `None`，所以它返回 thread id 风格的 `agent_id`，不是 V2 canonical task path。V1 也不会解析 catalog usage hints。[E: codex-rs/core/src/tools/handlers/multi_agents/spawn.rs:111][E: codex-rs/core/src/tools/handlers/multi_agents/spawn.rs:128]

输出 schema 是 `{ agent_id, nickname }`；handler 的 `ToolOutput` 以 success true 写回 function output。handler 没有覆写 `supports_parallel_tool_calls`，按默认 trait 不是 parallel-safe。[E: codex-rs/core/src/tools/handlers/multi_agents_spec.rs:399][E: codex-rs/core/src/tools/handlers/multi_agents/spawn.rs:212][E: codex-rs/core/src/tools/handlers/multi_agents/spawn.rs:250][E: codex-rs/tools/src/tool_executor.rs:122]
