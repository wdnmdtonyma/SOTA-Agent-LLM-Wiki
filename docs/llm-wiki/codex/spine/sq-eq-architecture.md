...
3. Protocol 层把 EQ entry 定义为 `Event { id, msg }`，`EventMsg` 是 tagged enum（**83** 个变体），包含 warning、auth recovery、realtime、model routing、verification，以及 `ThreadRolledBack`、`ThreadSettingsApplied`、`ThreadQueueChanged`。[E: codex-rs/protocol/src/protocol.rs:1337][E: codex-rs/protocol/src/protocol.rs:1346][E: codex-rs/protocol/src/protocol.rs:1382][E: codex-rs/protocol/src/protocol.rs:1391][E: codex-rs/protocol/src/protocol.rs:1430]
