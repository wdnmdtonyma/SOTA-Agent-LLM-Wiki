#!/usr/bin/env node
// Rebase existing [E: path:line] anchors between two Codex commits.
// Usage: node tools/rebase-evidence.mjs <base-codex-sha> <target-codex-sha> [wiki-git-ref] [--write] [--safe-only]
//
...
for (const file of nodeFiles) {
  ...
  if (rel === "reference/__none__") continue
  let baselineText
  try {
    baselineText = git(["show", `${wikiRef}:docs/llm-wiki/codex/${oldWikiPath(rel)}`])
  } catch {
