#!/usr/bin/env node
// L1 机械 lint —— 实现 conventions.md 第 5 节的结构护栏(codex wiki)。
// 用法: node tools/lint.mjs   (路径基于脚本位置)
...
const WIKI = resolve(HERE, '..')                 // docs/llm-wiki/codex
const SRC = resolve(WIKI, '../../../codex')       // Best/codex  (源仓)
const TIER_DIRS = ['spine', 'surface', 'subsystems', 'reference']
const KINDS = ['tool', 'command', 'cli', 'config', 'rpc', 'sdk', 'subsystem', 'flow', 'reference']
const TIERS = ['T0', 'T1', 'T2', 'T3']
const EVID = ['explicit', 'inferred', 'unknown']
const STATUS = ['planned', 'draft', 'verified']
const NS_DIR = { spine: 'spine/', tool: 'surface/tools/', command: 'surface/slash-commands/', cli: 'surface/cli/', config: 'surface/config/', rpc: 'surface/app-server/', sdk: 'surface/sdk/', subsys: 'subsystems/', ref: 'reference/' }
...
if (existsSync(join(WIKI, 'llms.txt'))) {
  ...
  for (const m of llms.matchAll(/\]\(([^)]+)\)/g)) {
    const p = m[1]
    if (p.endsWith('.md')) { if (!nodeByPath.has(p) && !groupDirs.some(d => p.startsWith(d)) && !p.startsWith('tools/')) warn(`llms.txt: 链接 "${p}" 不对应任何 index 节点`) }
