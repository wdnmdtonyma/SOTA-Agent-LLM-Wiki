...
> `codex_diagnostics` 提供进程内、无内容的 gauge/process snapshot；`codex doctor` 用只读检查拼出人读/JSON 报告；app-server 的 experimental `server/diagnostics` RPC 把同一 snapshot 映射成协议响应。三者都不修复环境。[E: codex-rs/diagnostics/src/lib.rs:90][E: codex-rs/cli/src/doctor.rs:328][E: codex-rs/cli/src/doctor.rs:348][E: codex-rs/app-server/src/request_processors/diagnostics.rs:5][E: codex-rs/app-server-protocol/src/protocol/common.rs:505]
...
`codex doctor` 是 CLI 只读报告：检查安装、配置、auth、终端、state 路径和有界可达性，失败写入报告并以非零退出；它不修复问题，也不启动长生命周期服务。[E: codex-rs/cli/src/doctor.rs:328][E: codex-rs/cli/src/doctor.rs:334][E: codex-rs/cli/src/doctor.rs:348]

`server/diagnostics` 是 app-server experimental client method；handler 直接调用 `codex_diagnostics::snapshot()` 并拷贝 process/gauge 字段。[E: codex-rs/app-server-protocol/src/protocol/common.rs:503][E: codex-rs/app-server-protocol/src/protocol/common.rs:505][E: codex-rs/app-server/src/message_processor.rs:970][E: codex-rs/app-server/src/request_processors/diagnostics.rs:5]
...
- `codex-rs/cli/src/doctor.rs`：`run_doctor`、并行检查编排、state/storage 检查。[E: codex-rs/cli/src/doctor.rs:328][E: codex-rs/cli/src/doctor.rs:355][E: codex-rs/cli/src/doctor.rs:2186]
...
- `DoctorCheck` 是 doctor 行：id、类别、status、summary、details、issues、remediation；JSON/human 渲染共用同一结构。[E: codex-rs/cli/src/doctor.rs:239]
...
2. `run_doctor` 先 `build_report`，再按 `--json` 打印 JSON 或人读报告；`overall_status == Fail` 时 `exit(1)`。[E: codex-rs/cli/src/doctor.rs:334][E: codex-rs/cli/src/doctor.rs:336][E: codex-rs/cli/src/doctor.rs:348]
3. `build_report` 先跑 system、endpoint protection、installation、runtime、search；加载 config 后再跑 disk，Windows 上再跑 Dev Drive；config 成功时并行跑 config/auth/updates/network/websocket/MCP/sandbox/terminal/git/title/state/thread inventory/app-server/provider reachability。[E: codex-rs/cli/src/doctor.rs:363][E: codex-rs/cli/src/doctor.rs:383][E: codex-rs/cli/src/doctor.rs:386][E: codex-rs/cli/src/doctor.rs:416]
...
7. `state_check` 检查 `CODEX_HOME`、log dir、sqlite home 与每个 runtime DB 路径，并对存在的 SQLite 做 integrity check；失败建议把损坏库挪开后让 CLI/app-server 重建。[E: codex-rs/cli/src/doctor.rs:2186][E: codex-rs/cli/src/doctor.rs:2192][E: codex-rs/cli/src/doctor.rs:2219]
8. app-server 收到 `ClientRequest::ServerDiagnostics` 后调用 `read_server_diagnostics()`；该方法拷贝 `snapshot()` 的 pid/内存/gauge 列表。[E: codex-rs/app-server/src/message_processor.rs:970][E: codex-rs/app-server/src/request_processors/diagnostics.rs:5][E: codex-rs/app-server/src/request_processors/diagnostics.rs:8]

## 设计动机与权衡

doctor 刻意只读：同一套 redacted 行同时喂人读报告和 `--json`，避免诊断命令改用户状态。[I][E: codex-rs/cli/src/doctor.rs:328][E: codex-rs/cli/src/doctor.rs:336]

`server/diagnostics` 标 `#[experimental("server/diagnostics")]`，响应不含用户内容或日志正文，只给进程计数与内存足迹，降低把会话数据暴露给 IDE/client 的风险。[E: codex-rs/app-server-protocol/src/protocol/common.rs:503][E: codex-rs/app-server-protocol/src/protocol/v2/diagnostics.rs:14][I]
...
- `server/diagnostics` 需要 experimental API capability；测试断言缺 capability 时拒绝该方法。[E: codex-rs/app-server-protocol/src/protocol/common.rs:503][E: codex-rs/app-server/tests/suite/v2/server_diagnostics.rs:106]
- doctor 的 state 检查失败不会自动修复 SQLite；其它 entry point 也不保证会重建。[E: codex-rs/cli/src/doctor.rs:2219]
