...
`lib.rs` re-export `AnalyticsEventsClient`、guardian/app/hook/skill/plugin/turn 相关 public input types、`TrackEventsContext` 和 `build_track_events_context`。[E: codex-rs/analytics/src/lib.rs:16][E: codex-rs/analytics/src/lib.rs:19][E: codex-rs/analytics/src/lib.rs:22][E: codex-rs/analytics/src/lib.rs:31][E: codex-rs/analytics/src/lib.rs:36][E: codex-rs/analytics/src/lib.rs:49][E: codex-rs/analytics/src/lib.rs:59][E: codex-rs/analytics/src/lib.rs:62][E: codex-rs/analytics/src/lib.rs:64][E: codex-rs/analytics/src/lib.rs:73]
...
`AnalyticsFact` 是内部 fact enum，顶层覆盖 Initialize、ClientRequest、ClientResponse、ErrorResponse、ServerRequest、ServerResponse、EffectivePermissionsApprovalResponse、ServerRequestAborted、Notification 和 Custom；Custom facts 覆盖 subagent、compaction、goal、guardian、turn config/token/profile/error、skill/app/hook/plugin、plugin install failure 和 external-agent import events。[E: codex-rs/analytics/src/facts.rs:479][E: codex-rs/analytics/src/facts.rs:480][E: codex-rs/analytics/src/facts.rs:487][E: codex-rs/analytics/src/facts.rs:498][E: codex-rs/analytics/src/facts.rs:503][E: codex-rs/analytics/src/facts.rs:510][E: codex-rs/analytics/src/facts.rs:513][E: codex-rs/analytics/src/facts.rs:518][E: codex-rs/analytics/src/facts.rs:523][E: codex-rs/analytics/src/facts.rs:527][E: codex-rs/analytics/src/facts.rs:530][E: codex-rs/analytics/src/facts.rs:533][E: codex-rs/analytics/src/facts.rs:535][E: codex-rs/analytics/src/facts.rs:536][E: codex-rs/analytics/src/facts.rs:540][E: codex-rs/analytics/src/facts.rs:542][E: codex-rs/analytics/src/facts.rs:551][E: codex-rs/analytics/src/facts.rs:552][E: codex-rs/analytics/src/facts.rs:553]
...
`ingest_initialize` 把 connection metadata 存入 `connections`；普通 thread start/resume/fork response 通过 `emit_thread_initialized` 写入 thread->connection 和 thread metadata，并 emit `ThreadInitialized` event。[E: codex-rs/analytics/src/reducer.rs:1055][E: codex-rs/analytics/src/reducer.rs:1064][E: codex-rs/analytics/src/reducer.rs:1067][E: codex-rs/analytics/src/reducer.rs:1076][E: codex-rs/analytics/src/reducer.rs:1417][E: codex-rs/analytics/src/reducer.rs:1418][E: codex-rs/analytics/src/reducer.rs:1427][E: codex-rs/analytics/src/reducer.rs:1438][E: codex-rs/analytics/src/reducer.rs:1928][E: codex-rs/analytics/src/reducer.rs:1941][E: codex-rs/analytics/src/reducer.rs:1952][E: codex-rs/analytics/src/reducer.rs:1956][E: codex-rs/analytics/src/reducer.rs:1957][E: codex-rs/analytics/src/reducer.rs:1959][E: codex-rs/analytics/src/reducer.rs:1961]

subagent thread started 没有独立 `TrackEventRequest` variant；reducer 把 `CustomAnalyticsFact::SubAgentThreadStarted` 转成 `TrackEventRequest::ThreadInitialized`，其 event params 设置 `thread_source: Some("subagent")`。[E: codex-rs/analytics/src/reducer.rs:1080][E: codex-rs/analytics/src/reducer.rs:1099][E: codex-rs/analytics/src/reducer.rs:1105][E: codex-rs/analytics/src/reducer.rs:1105][E: codex-rs/analytics/src/events.rs:1373][E: codex-rs/analytics/src/events.rs:1389]
...
Code Mode runtime 使用 RAII guard：默认 terminal status 为 interrupted，正常返回时显式改成 completed/failed，Drop 必然发出 completed fact；sampling `response.completed` 再把 response id 与该轮 tool call ids 关联。[E: codex-rs/core/src/tools/code_mode/telemetry.rs:25][E: codex-rs/core/src/tools/code_mode/telemetry.rs:44][E: codex-rs/core/src/tools/code_mode/telemetry.rs:49][E: codex-rs/core/src/tools/code_mode/telemetry.rs:82][E: codex-rs/core/src/session/turn.rs:2787][E: codex-rs/core/src/session/turn.rs:2795][E: codex-rs/core/src/session/turn.rs:2799]

Reducer 维护 call→response、child call→cell 与 cell→parent/response state，把 `cell_id`、`parent_call_id`、originating/subsequent response ids 补到 tool events。每类 correlation map/pending queue 上限 256；pending 满时先 flush oldest，避免无界等待下一次 sampling response。[E: codex-rs/analytics/src/events.rs:574][E: codex-rs/analytics/src/events.rs:583][E: codex-rs/analytics/src/events.rs:586][E: codex-rs/analytics/src/reducer.rs:163][E: codex-rs/analytics/src/reducer.rs:172][E: codex-rs/analytics/src/reducer.rs:409][E: codex-rs/analytics/src/reducer.rs:869][E: codex-rs/analytics/src/reducer.rs:917][E: codex-rs/analytics/src/reducer.rs:1018]
...
analytics 使用 fact reducer，而不是每个调用点直接 POST event，是为了让 thread/request/turn context 可以在 reducer 中统一补齐，并允许 queue 层做去重与 backpressure drop。[I] 该设计由 `AnalyticsFact`、`AnalyticsReducer` state 和 `AnalyticsEventsQueue` 共同体现。[E: codex-rs/analytics/src/facts.rs:479][E: codex-rs/analytics/src/facts.rs:527][E: codex-rs/analytics/src/reducer.rs:166][E: codex-rs/analytics/src/client.rs:151][E: codex-rs/analytics/src/client.rs:187][E: codex-rs/analytics/src/client.rs:193][E: codex-rs/analytics/src/client.rs:209]
...
- plugin install failure 现在同时记录 `PluginInstallSource`（manual 或 external-agent migration）与 optional sub-error type。[E: codex-rs/analytics/src/facts.rs:623][E: codex-rs/analytics/src/facts.rs:625][E: codex-rs/analytics/src/facts.rs:630][E: codex-rs/analytics/src/facts.rs:630][E: codex-rs/analytics/src/facts.rs:634][E: codex-rs/analytics/src/client.rs:514][E: codex-rs/analytics/src/client.rs:516][E: codex-rs/analytics/src/client.rs:516]
...
- hook event/source mapping 使用固定字符串；hook status 只把 unexpected `Running` 归一化为 `Failed`，不是字符串 mapping。[E: codex-rs/analytics/src/events.rs:1316][E: codex-rs/analytics/src/events.rs:1325][E: codex-rs/analytics/src/events.rs:1326][E: codex-rs/analytics/src/events.rs:1327][E: codex-rs/analytics/src/events.rs:1328][E: codex-rs/analytics/src/events.rs:1347][E: codex-rs/analytics/src/events.rs:1355][E: codex-rs/analytics/src/events.rs:1356][E: codex-rs/analytics/src/events.rs:1406][E: codex-rs/analytics/src/events.rs:1408]
