...
- `ConnectionState` now retains `ConnectionOrigin` together with outbound flags, opted-out notification methods, and the per-connection session. The processor loop uses the retained origin to distinguish a closed stdio client from WebSocket/remote-control connections [E: codex-rs/app-server/src/transport.rs:39][E: codex-rs/app-server/src/transport.rs:40][E: codex-rs/app-server/src/lib.rs:1015][E: codex-rs/app-server/src/lib.rs:1019]。
- `ConnectionSessionState` gates initialized requests with an RPC gate and `OnceLock`; initialized state includes experimental API flag, notification opt-outs, client name/version, request-attestation 与 MCP extension flags [E: codex-rs/app-server/src/message_processor.rs:167][E: codex-rs/app-server/src/message_processor.rs:174][E: codex-rs/app-server/src/message_processor.rs:175][E: codex-rs/app-server/src/message_processor.rs:180]。
...
- 创建 `historyMode=paginated` thread 时，底层 store 必须同时支持 turns/items 分页列表，否则直接返回 invalid request。paginated `thread/read(includeTurns=true)` 不再读完整 rollout，而是走 `paginated_thread_full_turns` 兼容 hydration。[E: codex-rs/app-server/src/request_processors/thread_processor.rs:1145][E: codex-rs/app-server/src/request_processors/thread_processor.rs:1147][E: codex-rs/app-server/src/request_processors/thread_processor.rs:1150][E: codex-rs/app-server/src/request_processors/thread_processor.rs:2898][E: codex-rs/app-server/src/request_processors/thread_processor.rs:2902]
...
1. On `ConnectionOpened`, processor loop creates outbound initialized/experimental/opt-out flags, sends `OutboundControlEvent::Opened`, and inserts a `ConnectionState` keyed by connection id [E: codex-rs/app-server/src/lib.rs:981][E: codex-rs/app-server/src/lib.rs:988][E: codex-rs/app-server/src/lib.rs:1005]。
2. On incoming request, processor calls `MessageProcessor::process_request`, mirrors session opt-outs/experimental flag back to outbound state, then if initialize just completed sends connection-scoped initialize notifications, emits current remote-control status, calls `connection_initialized`, and finally marks outbound initialized [E: codex-rs/app-server/src/lib.rs:1047][E: codex-rs/app-server/src/lib.rs:1058][E: codex-rs/app-server/src/lib.rs:1078][E: codex-rs/app-server/src/lib.rs:1092][E: codex-rs/app-server/src/lib.rs:1100]。
3. On `ConnectionClosed`, processor removes local connection state, closes the RPC gate, notifies outbound loop, spawns processor cleanup, and exits in stdio single-client mode [E: codex-rs/app-server/src/lib.rs:1015][E: codex-rs/app-server/src/lib.rs:1019][E: codex-rs/app-server/src/lib.rs:1021][E: codex-rs/app-server/src/lib.rs:1027][E: codex-rs/app-server/src/lib.rs:1034]。
...
5. When a core thread is created, the processor gathers initialized connection ids and calls `try_attach_thread_listener`; the thread processor upserts watch state and calls `ensure_conversation_listener` best-effort for each initialized connection [E: codex-rs/app-server/src/lib.rs:1155][E: codex-rs/app-server/src/request_processors/thread_processor.rs:3536][E: codex-rs/app-server/src/request_processors/thread_processor.rs:3544][E: codex-rs/app-server/src/request_processors/thread_processor.rs:3560]。
6. Idle unload watches both subscriber presence and thread active status after `THREAD_UNLOADING_DELAY`；`UnloadingState` 同时订阅 connections 与 `ThreadStatus`。[E: codex-rs/app-server/src/request_processors/thread_lifecycle.rs:11][E: codex-rs/app-server/src/request_processors/thread_lifecycle.rs:27][E: codex-rs/app-server/src/request_processors/thread_lifecycle.rs:63]

## Notification media omission

`Feature::OmitAppServerNotificationMedia`（key `omit_app_server_notification_media`，UnderDevelopment，默认关）打开时，thread listener 在发出 `ItemStarted`、`ItemCompleted`、`RawResponseItemCompleted` 前调用 `without_notification_media`。该函数丢掉 inline `InputImage`/`InputAudio`、function/dynamic/MCP 结果里的 image/audio/blob，并清空 image-generation `result`；`LocalImage`/`LocalAudio` 路径引用保留。MCP 已被 core 折叠成 truncated text preview 的 oversized 结果仍可能含 inline media（代码里有 TODO）。其它 notification 原样转发。[E: codex-rs/features/src/lib.rs:273][E: codex-rs/features/src/lib.rs:1405][E: codex-rs/app-server/src/bespoke_event_handling.rs:1078][E: codex-rs/app-server/src/bespoke_event_handling.rs:1106][E: codex-rs/app-server/src/bespoke_event_handling.rs:1147][E: codex-rs/app-server/src/notification_media.rs:10][E: codex-rs/app-server/src/notification_media.rs:23][E: codex-rs/app-server/src/notification_media.rs:139][E: codex-rs/app-server/src/notification_media.rs:178]

## 设计动机与权衡

- `lib.rs` keeps outbound writing in a separate router task; this isolates slow writes from request dispatch while still allowing per-connection disconnect for transports that provide a disconnect token [E: codex-rs/app-server/src/lib.rs:861][E: codex-rs/app-server/src/transport.rs:156][E: codex-rs/app-server/src/transport.rs:159][E: codex-rs/app-server/src/transport.rs:163][I]。
- WebSocket JSON-RPC initialize readiness is finalized outside the shared handler: `process_request` passes `None`, then `lib.rs` mirrors session state, sends initialize notifications, calls `connection_initialized`, and stores outbound initialized true [E: codex-rs/app-server/src/message_processor.rs:636][E: codex-rs/app-server/src/lib.rs:1078][E: codex-rs/app-server/src/lib.rs:1092][E: codex-rs/app-server/src/lib.rs:1100][I]。
...
- paginated JSONL 是 canonical history；paginated resume 在发送前会重新取 backwards cursors。[E: codex-rs/app-server/src/request_processors/thread_processor.rs:3940][E: codex-rs/app-server/src/request_processors/thread_processor.rs:3941][I]
