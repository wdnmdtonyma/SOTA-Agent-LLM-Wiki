...
- `ThreadState` stores pending interrupt/rollback state, turn summary/history, listener cancel/command channel, raw-events opt-in, listener generation, listener thread weak handle, and watch registration [E: codex-rs/app-server/src/thread_state.rs:99][E: codex-rs/app-server/src/thread_state.rs:100][E: codex-rs/app-server/src/thread_state.rs:98][E: codex-rs/app-server/src/thread_state.rs:107][E: codex-rs/app-server/src/thread_state.rs:108][E: codex-rs/app-server/src/thread_state.rs:111][E: codex-rs/app-server/src/thread_state.rs:113][E: codex-rs/app-server/src/thread_state.rs:114]。
...
- `ThreadStateManagerInner` maps live connections, thread entries, and reverse connection-to-thread subscriptions; `ConnectionCapabilities` currently carries request attestation into thread listener state [E: codex-rs/app-server/src/thread_state.rs:347][E: codex-rs/app-server/src/thread_state.rs:348][E: codex-rs/app-server/src/thread_state.rs:350][E: codex-rs/app-server/src/thread_state.rs:354]。
...
- `thread/searchOccurrences` 校验非空 search term，把 cursor/limit 交给 store，并返回 turn/item id、snippet、匹配范围和 turn cursor。[E: codex-rs/app-server/src/message_processor.rs:1494][E: codex-rs/app-server/src/request_processors/thread_processor.rs:3194][E: codex-rs/app-server/src/request_processors/thread_processor.rs:3206][E: codex-rs/app-server/src/request_processors/thread_processor.rs:3217]
- resume 的兼容层可把所有 turn 分页 materialize 给旧客户端，也可只构造 initial turns page；running resume 会给 active turn 预留一个 page slot。最终 listener 合并 durable page 与 active turn、规范化状态，并在发送前从 store 重新取得 turns/items backwards cursors。[E: codex-rs/app-server/src/request_processors/thread_processor.rs:3069][E: codex-rs/app-server/src/request_processors/thread_processor.rs:3090][E: codex-rs/app-server/src/request_processors/thread_processor.rs:3114][E: codex-rs/app-server/src/request_processors/thread_processor.rs:3132][E: codex-rs/app-server/src/request_processors/thread_lifecycle.rs:574][E: codex-rs/app-server/src/request_processors/thread_lifecycle.rs:584][E: codex-rs/app-server/src/request_processors/thread_lifecycle.rs:601][E: codex-rs/app-server/src/request_processors/thread_lifecycle.rs:610][E: codex-rs/app-server/src/request_processors/thread_lifecycle.rs:615][E: codex-rs/app-server/src/request_processors/thread_lifecycle.rs:676]
...
`Feature::OmitAppServerNotificationMedia`（key `omit_app_server_notification_media`，UnderDevelopment，默认关）打开时，thread listener 在发出 `ItemStarted`、`ItemCompleted`、`RawResponseItemCompleted` 前调用 `without_notification_media`。该函数丢掉 inline `InputImage`/`InputAudio`、function/dynamic/MCP 结果里的 image/audio/blob，并清空 image-generation `result`；`LocalImage`/`LocalAudio` 路径引用保留。MCP 已被 core 折叠成 truncated text preview 的 oversized 结果仍可能含 inline media（代码里有 TODO）。其它 notification 原样转发。[E: codex-rs/features/src/lib.rs:285][E: codex-rs/features/src/lib.rs:1480][E: codex-rs/app-server/src/bespoke_event_handling.rs:1125][E: codex-rs/app-server/src/bespoke_event_handling.rs:1153][E: codex-rs/app-server/src/bespoke_event_handling.rs:1194][E: codex-rs/app-server/src/notification_media.rs:10][E: codex-rs/app-server/src/notification_media.rs:23][E: codex-rs/app-server/src/notification_media.rs:141][E: codex-rs/app-server/src/notification_media.rs:180]
...
- WebSocket JSON-RPC initialize readiness is finalized outside the shared handler: `process_request` passes `None`, then `lib.rs` mirrors session state, sends initialize notifications, calls `connection_initialized`, and stores outbound initialized true [E: codex-rs/app-server/src/message_processor.rs:665][E: codex-rs/app-server/src/lib.rs:1197][E: codex-rs/app-server/src/lib.rs:1211][E: codex-rs/app-server/src/lib.rs:1219][I]。
...
- paginated JSONL 是 canonical history；paginated resume 在发送前会重新取 backwards cursors。[E: codex-rs/app-server/src/request_processors/thread_processor.rs:4115][E: codex-rs/app-server/src/request_processors/thread_processor.rs:4116][I]
...
- `itemsView=full` 和 resume/`thread/read(includeTurns=true)` 仍会分页读完整历史，是明确的旧客户端 slow path；新客户端应使用 turns/items 分页与 initial page bootstrap。[E: codex-rs/app-server/src/request_processors/thread_processor.rs:3365][E: codex-rs/app-server/src/request_processors/thread_processor.rs:3365]
