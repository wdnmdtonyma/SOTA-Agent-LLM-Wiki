...
- 75/90/95% warning 与 backend `rate_limit_upsell` banner 怎样分层？
...
plan label 不是 protocol variant 的直接 title-case：`EnterpriseCbpAutomation` 显示为 `Enterprise (Automation)`，team-like variants 显示为 `Business`，business-like variants 显示为 `Enterprise`，`ProLite` 显示为 `Pro Lite`。[E: codex-rs/tui/src/status/helpers.rs:99][E: codex-rs/tui/src/status/helpers.rs:100][E: codex-rs/tui/src/status/helpers.rs:104][E: codex-rs/tui/src/status/helpers.rs:106][E: codex-rs/tui/src/status/helpers.rs:108]
...
`RateLimitWarningState` 在 primary/secondary 用量跨过 75/90/95% 时各发一条 “Heads up … Run /status” history warning；任一窗口到 100% 则不再发这些 threshold warning。90% 以上还可弹出切到 `gpt-5.6-luna` 的 switch prompt（view id `rate-limit-switch-prompt`），但 **已有 applicable backend banner 时不弹**。[E: codex-rs/tui/src/chatwidget/rate_limits.rs:18][E: codex-rs/tui/src/chatwidget/rate_limits.rs:29][E: codex-rs/tui/src/chatwidget/rate_limits.rs:39][E: codex-rs/tui/src/chatwidget/rate_limits.rs:15][E: codex-rs/tui/src/chatwidget/rate_limits.rs:377]

`GetAccountRateLimitsResponse.rate_limit_upsell` 解析成 `BackendBanner`：occurrence 由 account/plan/`reset_at`/blocked model 决定，不是文案；换模型只隐藏不 dismiss，新 turn 只 dismiss 已经 shown 且显式 dismissible 的 occurrence。CTA 不蕴含 recovery。[E: codex-rs/tui/src/chatwidget/backend_banners.rs:66][E: codex-rs/tui/src/chatwidget/backend_banners.rs:66][E: codex-rs/tui/src/chatwidget/backend_banners.rs:157]
...
running-task inline status 属于 bottom pane：`set_task_running` 更新 composer task state，首次 running 时创建 `StatusIndicatorWidget`、显示 interrupt hint、同步 inline message，结束时 hide status indicator。[E: codex-rs/tui/src/bottom_pane/mod.rs:1071][E: codex-rs/tui/src/bottom_pane/mod.rs:1074][E: codex-rs/tui/src/bottom_pane/mod.rs:1078][E: codex-rs/tui/src/bottom_pane/mod.rs:1097]
...
- invalid status-line/title warnings 有 once-only guard；测试或排障时重复配置错误可能不会重复弹 warning。[E: codex-rs/tui/src/chatwidget/status_surfaces.rs:117]
