...
rate-limit refresh 有明确 origin：startup prefetch、`/status` command、`/usage` menu、reset picker 和 reset-credit consume；`RateLimitsLoaded` 带 origin、hard-stop generation 与结果，旧 generation 的 response 不会覆盖更新后的 hard-stop snapshot。[E: codex-rs/tui/src/app_event.rs:156][E: codex-rs/tui/src/app_event.rs:161][E: codex-rs/tui/src/app/event_dispatch.rs:871][E: codex-rs/tui/src/app/event_dispatch.rs:878][E: codex-rs/tui/src/app/event_dispatch.rs:925]
...
persistent-history batch path 使用 `LookupMessageHistoryBatch { thread_id, cursor, log_id }`，结果以 `HistoryLookupResponse::{Batch,BatchError}` 路由回原 thread channel；single-entry lookup 仍走同一 response enum 的 `Entry` variant。[E: codex-rs/tui/src/app/event_dispatch.rs:459][E: codex-rs/tui/src/app/event_dispatch.rs:467][E: codex-rs/tui/src/app/event_dispatch.rs:482]

transcript prompt edit 是显式 `ForkSessionForPromptEdit` route：dispatcher 读取完整 thread，在选中 user message 之前 fork（若它是首条则 start fresh），替换 widget 后把原 prompt 放回 composer；失败时也恢复 prompt。[E: codex-rs/tui/src/app/event_dispatch.rs:233][E: codex-rs/tui/src/app/event_dispatch.rs:249][E: codex-rs/tui/src/app/event_dispatch.rs:258][E: codex-rs/tui/src/app/event_dispatch.rs:269][E: codex-rs/tui/src/app/event_dispatch.rs:280][E: codex-rs/tui/src/app/event_dispatch.rs:293]

usage reset 由 picker/confirmation/consume/result AppEvents 串行驱动：confirmation 生成 idempotency key，consume 的 `Reset`/`AlreadyRedeemed` 结果触发一次 rate-limit refresh，再更新剩余 credits。[E: codex-rs/tui/src/app/event_dispatch.rs:981][E: codex-rs/tui/src/app/event_dispatch.rs:1005][E: codex-rs/tui/src/app/event_dispatch.rs:1021][E: codex-rs/tui/src/app/event_dispatch.rs:1037]
...
- UI 退出默认应走 `ExitMode::ShutdownFirst`，`Immediate` 是 last-resort escape hatch，注释明确可能丢背景任务、rollout flush 或 child cleanup。[E: codex-rs/tui/src/app_event.rs:270][E: codex-rs/tui/src/app_event.rs:273][E: codex-rs/tui/src/app_event.rs:1140][E: codex-rs/tui/src/app_event.rs:1142][E: codex-rs/tui/src/app_event.rs:1147][I]
