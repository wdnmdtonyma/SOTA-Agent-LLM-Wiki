...
`Tui` 是 terminal wrapper：它持有 `FrameRequester`、draw channel、shared `EventBroker`、terminal backend、alt-screen state、focus state、notification backend、Zellij detection 和 `alt_screen_enabled` flag。[E: codex-rs/tui/src/tui.rs:559][E: codex-rs/tui/src/tui.rs:560][E: codex-rs/tui/src/tui.rs:561][E: codex-rs/tui/src/tui.rs:562][E: codex-rs/tui/src/tui.rs:563][E: codex-rs/tui/src/tui.rs:567][E: codex-rs/tui/src/tui.rs:571][E: codex-rs/tui/src/tui.rs:573][E: codex-rs/tui/src/tui.rs:575][E: codex-rs/tui/src/tui.rs:578][E: codex-rs/tui/src/tui.rs:580]

`TuiEvent` 当前只有四类：terminal key、paste payload、resize 和 scheduled draw；raw crossterm event fan-out 被 `TuiEventStream`/`EventBroker` 封装。[E: codex-rs/tui/src/tui.rs:544][E: codex-rs/tui/src/tui.rs:547][E: codex-rs/tui/src/tui.rs:549][E: codex-rs/tui/src/tui.rs:554][E: codex-rs/tui/src/tui.rs:556][E: codex-rs/tui/src/tui/event_stream.rs:51][E: codex-rs/tui/src/tui/event_stream.rs:51][E: codex-rs/tui/src/tui/event_stream.rs:153][E: codex-rs/tui/src/tui/event_stream.rs:153]
...
startup resume/fork 现在在真正 attach 前调用 `resolve_startup_resume_or_fork_cwd`，把 `tui.resume_cwd`、显式 `--cd`、session cwd 和 state-db remembered choice 收敛为 fallback cwd，再以它重载最终 config。remote workspace 的 `current` 模式必须显式给 `--cd`；remote workspace 本身则沿用 app-server config cwd，不弹 local cwd prompt。[E: codex-rs/tui/src/lib.rs:789][E: codex-rs/tui/src/lib.rs:798][E: codex-rs/tui/src/lib.rs:809][E: codex-rs/tui/src/lib.rs:810][E: codex-rs/tui/src/lib.rs:818][E: codex-rs/tui/src/lib.rs:822][E: codex-rs/tui/src/lib.rs:1652][E: codex-rs/tui/src/lib.rs:1688]
...
`Tui::event_stream` 创建 `TuiEventStream`，共享 `EventBroker` 以避免多个 crossterm readers 争抢 stdin；`pause_events`/`resume_events` 通过 drop/recreate underlying event stream 让外部交互程序临时接管终端输入。[E: codex-rs/tui/src/tui.rs:665][E: codex-rs/tui/src/tui.rs:665][E: codex-rs/tui/src/tui.rs:671][E: codex-rs/tui/src/tui.rs:748][E: codex-rs/tui/src/tui/event_stream.rs:51][E: codex-rs/tui/src/tui/event_stream.rs:51]

当前 alternate-screen 控制是代码事实：CLI 计算 `determine_alt_screen_mode(no_alt_screen, config.tui_alternate_screen)` 后调用 `tui.set_alt_screen_enabled`；`--no-alt-screen` 直接禁用，除此之外只有 `AltScreenMode::Never` 禁用。不要沿用旧文档里“auto 在 Zellij 禁用”的说法。[E: codex-rs/tui/src/lib.rs:1752][E: codex-rs/tui/src/lib.rs:1753][E: codex-rs/tui/src/lib.rs:1895][E: codex-rs/tui/src/lib.rs:1895][E: codex-rs/tui/src/lib.rs:1896][E: codex-rs/tui/src/lib.rs:1900]

`enter_alt_screen` 会在 enabled 时发送 `EnterAlternateScreen` 和 `EnableAlternateScroll`，保存 inline viewport 并把 viewport 扩到 terminal size；`leave_alt_screen` 反向禁用 alternate scroll、离开 alternate screen 并恢复 saved viewport。[E: codex-rs/tui/src/tui.rs:768][E: codex-rs/tui/src/tui.rs:769][E: codex-rs/tui/src/tui.rs:772][E: codex-rs/tui/src/tui.rs:774][E: codex-rs/tui/src/tui.rs:775][E: codex-rs/tui/src/tui.rs:776][E: codex-rs/tui/src/tui.rs:777][E: codex-rs/tui/src/tui.rs:785][E: codex-rs/tui/src/tui.rs:790][E: codex-rs/tui/src/tui.rs:795][E: codex-rs/tui/src/tui.rs:796][E: codex-rs/tui/src/tui.rs:797][E: codex-rs/tui/src/tui.rs:800]
...
- syntax theme 要在 resume/fork 可能触发的最后一次 config reload 之后设置；提前设置会拿到错误 cwd 下的 theme config。[E: codex-rs/tui/src/lib.rs:1716]
