...
`AuthModeWidget` 保存 frame requester、highlighted sign-in option、error、sign-in state、login status、app-server request handle、auth config 和 animation flags。[E: codex-rs/tui/src/onboarding/auth.rs:253][E: codex-rs/tui/src/onboarding/auth.rs:255][E: codex-rs/tui/src/onboarding/auth.rs:257][E: codex-rs/tui/src/onboarding/auth.rs:259]

browser/device-code state 会 suppress animations；取消 active browser login 会通过 app-server handle 异步 `cancel_login_attempt`。[E: codex-rs/tui/src/onboarding/auth.rs:271][E: codex-rs/tui/src/onboarding/auth.rs:278]

API key path 有三段：paste/edit 会填充或追加 `ApiKeyEntry` state，start entry 会从 env 预填，save 会发送 `ClientRequest::LoginAccount { LoginAccountParams::ApiKey }`。[E: codex-rs/tui/src/onboarding/auth.rs:812][E: codex-rs/tui/src/onboarding/auth.rs:820][E: codex-rs/tui/src/onboarding/auth.rs:836][E: codex-rs/tui/src/onboarding/auth.rs:842]
