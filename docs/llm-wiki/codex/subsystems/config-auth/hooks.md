...
每个 `MatcherGroup` 有 optional matcher 和 handler list；`HookHandlerConfig` 当前可反序列化 `command`、`prompt`、`agent` 三类，但 discovery 只把 command handler 加入 runnable handler set，prompt/agent handler 会产生 unsupported warning。[E: codex-rs/config/src/hook_config.rs:140][E: codex-rs/config/src/hook_config.rs:140][E: codex-rs/config/src/hook_config.rs:144][E: codex-rs/config/src/hook_config.rs:149][E: codex-rs/config/src/hook_config.rs:151][E: codex-rs/config/src/hook_config.rs:173][E: codex-rs/config/src/hook_config.rs:175][E: codex-rs/hooks/src/engine/discovery.rs:590]
...
`load_hooks_json` 只读取存在的 `hooks.json`，parse 失败记录 warning 并返回 None；`load_toml_hooks_from_layer` 从 layer config 的 `hooks` 字段反序列化 `HookEventsToml`，空 hooks 不产生 source。[E: codex-rs/hooks/src/engine/discovery.rs:307][E: codex-rs/hooks/src/engine/discovery.rs:311][E: codex-rs/hooks/src/engine/discovery.rs:327][E: codex-rs/hooks/src/engine/discovery.rs:330][E: codex-rs/hooks/src/engine/discovery.rs:350][E: codex-rs/hooks/src/engine/discovery.rs:355][E: codex-rs/hooks/src/engine/discovery.rs:367]

Discovery 会按 event 计算 matcher pattern、validate matcher 并拒绝空 command。普通 event 仍拒绝 async、timeout 缺省 600 秒且最小 1 秒；`SessionEnd` 可接受 async 配置但当前会 warning 后同步运行，timeout 缺省 1 秒并强制 cap 为 3 秒。[E: codex-rs/hooks/src/engine/discovery.rs:455][E: codex-rs/hooks/src/engine/discovery.rs:457][E: codex-rs/hooks/src/engine/discovery.rs:480][E: codex-rs/hooks/src/engine/discovery.rs:501][E: codex-rs/hooks/src/engine/discovery.rs:611][E: codex-rs/hooks/src/engine/discovery.rs:615]

每个 command handler 都会生成 `HookListEntry`；只有 enabled 且 trust status 是 Managed/Trusted，或 source 启用了 bypass hook trust，才会 push 到 runnable `ConfiguredHandler` list。[E: codex-rs/hooks/src/engine/discovery.rs:535][E: codex-rs/hooks/src/engine/discovery.rs:544][E: codex-rs/hooks/src/engine/discovery.rs:546][E: codex-rs/hooks/src/engine/discovery.rs:548][E: codex-rs/hooks/src/engine/discovery.rs:566][E: codex-rs/hooks/src/engine/discovery.rs:573]
...
Command runner 使用 configured shell 或默认 shell，设置 cwd、stdin、stdout/stderr pipe、kill_on_drop，并将 hook request JSON 写入 stdin；timeout 会被映射成 `"hook timed out after ..."` error。[E: codex-rs/hooks/src/engine/command_runner.rs:50][E: codex-rs/hooks/src/engine/command_runner.rs:60][E: codex-rs/hooks/src/engine/command_runner.rs:62][E: codex-rs/hooks/src/engine/command_runner.rs:85][E: codex-rs/hooks/src/engine/command_runner.rs:103][E: codex-rs/hooks/src/engine/command_runner.rs:93]

## Gotchas

- `include_disabled=false` 意味着 disabled project layers 不会贡献 layer-local hooks；managed requirements 和 plugin sources 走单独路径。[E: codex-rs/hooks/src/engine/discovery.rs:101][E: codex-rs/hooks/src/engine/discovery.rs:90][E: codex-rs/hooks/src/engine/discovery.rs:163]
- `HookHandlerConfig::Prompt` 和 `HookHandlerConfig::Agent` 是 schema 可读类型，但当前 discovery 会跳过并 warning；不能把它们当作 runnable handlers。[E: codex-rs/config/src/hook_config.rs:173][E: codex-rs/config/src/hook_config.rs:175][E: codex-rs/hooks/src/engine/discovery.rs:590][E: codex-rs/hooks/src/engine/discovery.rs:594]
...
- 配置里把 `SessionEnd` 标成 async 不会让进程 fire-and-forget；当前实现明确同步等待，只有超时预算被压到 1–3 秒。[E: codex-rs/hooks/src/engine/discovery.rs:501]
