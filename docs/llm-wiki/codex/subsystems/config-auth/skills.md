...
Loaded metadata 会经过 length validation，最终 `SkillMetadata` 保存 resolved canonical skill path、scope、plugin_id 和 remote_plugin_id。[E: codex-rs/ext/skills/src/loader/host.rs:317][E: codex-rs/ext/skills/src/loader/host.rs:376][E: codex-rs/ext/skills/src/loader/host.rs:384]

## Prompt rendering 与 injection

available-skills rendering 在 `ext/skills`。`render_available_skills` 只取 model-visible entries，支持 core-compatible/extension-compatible description 与排序策略；发生预算压力时会比较 absolute path 与 aliased root 两种布局并选择信息保留更好的版本。[E: codex-rs/ext/skills/src/render.rs:31][E: codex-rs/ext/skills/src/render.rs:38][E: codex-rs/ext/skills/src/render.rs:492][E: codex-rs/ext/skills/src/render.rs:501][E: codex-rs/ext/skills/src/render.rs:518]
...
`skills.list` 按 authority 列出 model-visible entries，每页最多 20 项、serialized response 最多 512 KiB，并返回后续 `skills.read` 必须原样使用的 exact `authority`、`package` 和 `main_resource`。catalog warnings 只在第一页返回；单项 metadata 自身超限时该项会被省略并附 warning。[E: codex-rs/ext/skills/src/tools/list.rs:32][E: codex-rs/ext/skills/src/tools/list.rs:32][E: codex-rs/ext/skills/src/tools/list.rs:90][E: codex-rs/ext/skills/src/tools/list.rs:125][E: codex-rs/ext/skills/src/tools/list.rs:144][E: codex-rs/ext/skills/src/tools/list.rs:162]

`skills.read` 校验 authority/package/resource handles，确认 package 对当前 authority 可用，再从 provider 读取指定 resource。它同样把单页 response 限到 512 KiB，用 opaque cursor 按 UTF-8 boundary 分页；orchestrator authority 的读取还记录 shadow selection invocation。[E: codex-rs/ext/skills/src/tools/read.rs:29][E: codex-rs/ext/skills/src/tools/read.rs:77][E: codex-rs/ext/skills/src/tools/read.rs:109][E: codex-rs/ext/skills/src/tools/read.rs:158][E: codex-rs/ext/skills/src/tools/read.rs:160]

`ExecutorSkillProvider` 从 selected capability roots 或 executor discovery snapshot 构造 `Executor` authority。snapshot 若内嵌 skill instructions，main resource 可携带该内容；否则 `skills.read` 通过 environment filesystem streaming 读取。它不实现 executor-side search，`search()` 当前返回空结果。[E: codex-rs/ext/skills/src/provider/executor.rs:34][E: codex-rs/ext/skills/src/provider/executor.rs:71][E: codex-rs/ext/skills/src/provider/executor.rs:187][E: codex-rs/ext/skills/src/provider/executor.rs:214][E: codex-rs/ext/skills/src/provider/executor.rs:259]

## Embedded system skills

Bundled system skills 用 `include_dir!` 嵌入，安装目标是 `CODEX_HOME/skills/.system`；安装时写 marker fingerprint，marker 匹配则跳过，否则清理旧目录并写入 embedded dir。[E: codex-rs/skills/src/lib.rs:55][E: codex-rs/skills/src/lib.rs:63][E: codex-rs/skills/src/lib.rs:77][E: codex-rs/skills/src/lib.rs:85][E: codex-rs/skills/src/lib.rs:87][E: codex-rs/skills/src/lib.rs:93]
