...
> Current Codex profile handling is split: legacy `ConfigToml.profile`/`ConfigToml.profiles` still deserialize, but runtime config rejects `profile = "..."`; profile-v2 uses `--profile <name>` to load `<name>.config.toml` as an additional User config layer, while project trust still comes from `[projects]` entries and influences default approval/sandbox behavior。[E: codex-rs/config/src/config_toml.rs:313][E: codex-rs/core/src/config/mod.rs:3260][E: codex-rs/core/src/config/mod.rs:1896][E: codex-rs/config/src/config_toml.rs:563][E: codex-rs/core/src/config/mod.rs:3597]
...
`ConfigToml.profile` 和 `ConfigToml.profiles` 仍存在于 TOML schema，但 `ConfigBuilder` 在看到 `cfg.profile` 时直接返回错误，提示使用 `--profile <name>` 和 `<name>.config.toml`。[E: codex-rs/config/src/config_toml.rs:313][E: codex-rs/core/src/config/mod.rs:3260][E: codex-rs/core/src/config/mod.rs:3264]

Profile-v2 的路径由 `resolve_profile_v2_config_path(codex_home, profile_name)` 解析为 `codex_home/<profile_name>.config.toml`；loader 先加入 base user config，再在 active profile file 不同于 base file 时加入第二个 User layer。[E: codex-rs/core/src/config/mod.rs:1896][E: codex-rs/core/src/config/mod.rs:1900][E: codex-rs/core/src/config/mod.rs:1901][E: codex-rs/config/src/loader/mod.rs:248][E: codex-rs/config/src/loader/mod.rs:280][E: codex-rs/config/src/loader/mod.rs:282]
...
Approval policy priority 是 CLI override 高于 config `approval_policy`，否则按 active project trust default：trusted project 默认 `OnRequest`，untrusted project 默认 `UnlessTrusted`，unknown project 使用 `AskForApproval::default()`；requirements 可在默认值不允许时强制 fallback。[E: codex-rs/core/src/config/mod.rs:3597][E: codex-rs/core/src/config/mod.rs:3599][E: codex-rs/core/src/config/mod.rs:3601][E: codex-rs/core/src/config/mod.rs:3602][E: codex-rs/core/src/config/mod.rs:3604][E: codex-rs/core/src/config/mod.rs:3607][E: codex-rs/core/src/config/mod.rs:3610]
...
Because profile-v2 is already materialized as a layer before `ConfigToml` reaches `ConfigBuilder`, runtime feature resolution uses effective `cfg.features` as base and passes an empty profile source to `Features::from_sources`.[E: codex-rs/core/src/config/mod.rs:3269][E: codex-rs/core/src/config/mod.rs:3274][E: codex-rs/core/src/config/mod.rs:3276][E: codex-rs/core/src/config/mod.rs:3279]
...
- Project trust affects defaults, but explicit approval/sandbox/permission-profile settings and requirements constraints can override or constrain those defaults。[E: codex-rs/core/src/config/mod.rs:3599][E: codex-rs/config/src/config_toml.rs:759][E: codex-rs/core/src/config/mod.rs:3610]
