...
本地主机 discovery 会先检查 plugin root 的 `plugin.json`；只要 `$schema` 属于 Agent Plugins schema family，它就优先于 nested legacy manifests，随后 supported 1.0 schema 走 Agent Plugins parser。若 regular root file 与 Agent Plugins 无关，才回退 `.codex-plugin/.claude-plugin/.cursor-plugin`；若 root `plugin.json` 是 symlink 或非 regular file，host discovery 直接返回无 manifest。executor provider 与 `PathUri` namespace helper 只遍历 nested legacy paths，不检查 root `plugin.json`。[E: codex-rs/utils/plugins/src/plugin_namespace.rs:11][E: codex-rs/utils/plugins/src/plugin_namespace.rs:26][E: codex-rs/utils/plugins/src/plugin_namespace.rs:42][E: codex-rs/utils/plugins/src/plugin_namespace.rs:45][E: codex-rs/utils/plugins/src/plugin_namespace.rs:52][E: codex-rs/utils/plugins/src/plugin_namespace.rs:62][E: codex-rs/utils/plugins/src/plugin_namespace.rs:89][E: codex-rs/core-plugins/src/provider.rs:179][E: codex-rs/core-plugins/src/provider.rs:180][E: codex-rs/core-plugins/src/manifest.rs:145][E: codex-rs/core-plugins/src/manifest.rs:148]

root manifest 必须是 JSON object，并声明受支持的 `$schema`；当前 canonical schema 是 `https://agent-plugins.org/schemas/1.0.0/plugin.schema.json`。parser 识别 `$schema`、name/version/description/author/homepage/repository/license/keywords/extensions，未知字段只 warning 后忽略。[E: codex-rs/core-plugins/src/agent_plugin_manifest.rs:16][E: codex-rs/core-plugins/src/agent_plugin_manifest.rs:27][E: codex-rs/core-plugins/src/agent_plugin_manifest.rs:63][E: codex-rs/core-plugins/src/agent_plugin_manifest.rs:80][E: codex-rs/utils/plugins/src/plugin_namespace.rs:16]
...
legacy manifest path 列表仍按 `.codex-plugin/plugin.json`、`.claude-plugin/plugin.json`、`.cursor-plugin/plugin.json` 排序；但本地 `find_plugin_manifest_path` 先执行上述 root portable-manifest 检查。namespace helper 仍可沿 ancestor manifests 读取 name。[E: codex-rs/exec-server-protocol/src/protocol.rs:47][E: codex-rs/exec-server-protocol/src/protocol.rs:50][E: codex-rs/utils/plugins/src/plugin_namespace.rs:42][E: codex-rs/utils/plugins/src/plugin_namespace.rs:62]
...
`remote_plugin` 已是 stable、default-on feature；ChatGPT backend auth 可启用 PS-backed global catalog，而其它 auth/provider 继续使用 local curated repository。manager 用 auth mode 选择 `OpenAiWithRemote` 或 `OpenAiApi` target marketplace，并在 load cache key 中包含 remote-global-active 状态。[E: codex-rs/features/src/lib.rs:1439][E: codex-rs/features/src/lib.rs:1442][E: codex-rs/core-plugins/src/manager.rs:597][E: codex-rs/core-plugins/src/manager.rs:624][E: codex-rs/core-plugins/src/manager.rs:626]
...
Auth-selected marketplace eligibility applies to hooks-only loading too：loader removes curated variants that do not match the selected target before it exports active hook sources/warnings, preventing an API-auth runtime from accidentally executing ChatGPT/remote curated hooks and vice versa。[E: codex-rs/core-plugins/src/loader.rs:209][E: codex-rs/core-plugins/src/loader.rs:214][E: codex-rs/core-plugins/src/loader.rs:226][E: codex-rs/core-plugins/src/loader.rs:296][E: codex-rs/core-plugins/src/loader.rs:303]

## Gotchas

- inactive plugin 仍保留在 `PluginLoadOutcome::plugins()` 中，但不会贡献 capability summary、effective skill roots、MCP servers、apps 或 hook sources。[E: codex-rs/plugin/src/load_outcome.rs:92][E: codex-rs/plugin/src/load_outcome.rs:200][E: codex-rs/plugin/src/load_outcome.rs:205][E: codex-rs/plugin/src/load_outcome.rs:48][E: codex-rs/plugin/src/load_outcome.rs:119][E: codex-rs/plugin/src/load_outcome.rs:167][E: codex-rs/plugin/src/load_outcome.rs:185]
- root `plugin.json` 只有声明 Agent Plugins schema family 才会优先；普通无关 `plugin.json` 不会遮蔽 legacy manifest。声明未来 Agent Plugins schema 会被选中但随后因 unsupported schema parse 失败，这是显式 fail-closed 行为。[E: codex-rs/utils/plugins/src/plugin_namespace.rs:26][E: codex-rs/utils/plugins/src/plugin_namespace.rs:33][E: codex-rs/utils/plugins/src/plugin_namespace.rs:35][E: codex-rs/utils/plugins/src/plugin_namespace.rs:52][E: codex-rs/core-plugins/src/agent_plugin_manifest.rs:63][E: codex-rs/core-plugins/src/agent_plugin_manifest.rs:80]
...
- Plugin hooks 是 manifest surface 与 `PluginHookSource` load outcome surface；真正是否运行还受 hooks engine trust/policy 控制。[E: codex-rs/core-plugins/src/manifest.rs:54][E: codex-rs/plugin/src/load_outcome.rs:32][E: codex-rs/hooks/src/engine/discovery.rs:166]
