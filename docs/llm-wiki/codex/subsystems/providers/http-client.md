...
MCP Apps 的 `openai/fileParams` 路径使用 session-owned、关闭 request logging 的 `RouteAwareClientPool`；该 pool 跨 turn 复用，并显式保留 transport-default route 的 legacy custom-CA fallback。[E: codex-rs/core/src/state/service.rs:62][E: codex-rs/core/src/session/session.rs:1316][E: codex-rs/core/src/session/session.rs:1320][E: codex-rs/core/src/mcp_openai_file.rs:47][E: codex-rs/core/src/mcp_openai_file.rs:49]
...
`Feature::Psp`（key `psp`，UnderDevelopment，默认关）让 `Config::http_client_factory()` 给 factory 加上 `oai-chat-psp=true` cookie；只有打开该 feature 的 ChatGPT cookie-store clients 才会带这条 PSP routing cookie。[E: codex-rs/features/src/lib.rs:1299][E: codex-rs/core/src/config/mod.rs:1654][E: codex-rs/core/src/config/mod.rs:1655]

delegated/local MCP HTTP 与部分 CLI MCP login 路径显式调用 `with_tls_backend_fallback()`，因此这些任意目标请求保留 rustls 协议协商回退，而不是只给固定 OpenAI endpoint 用。[E: codex-rs/codex-mcp/src/runtime.rs:774][E: codex-rs/cli/src/mcp_cmd.rs:430]
