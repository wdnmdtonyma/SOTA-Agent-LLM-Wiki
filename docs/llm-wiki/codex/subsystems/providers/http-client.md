...
kind: subsystem
tier: T2
source: [codex-rs/http-client/src/client.rs, codex-rs/http-client/src/client_builder.rs, codex-rs/http-client/src/outbound_proxy.rs, codex-rs/http-client/src/route_aware_client_pool.rs, codex-rs/http-client/src/route_aware_redirect.rs, codex-rs/http-client/src/tls_backend_fallback.rs, codex-rs/http-client/src/request.rs, codex-rs/http-client/src/transport.rs, codex-rs/core/src/config/mod.rs, codex-rs/features/src/lib.rs, codex-rs/codex-api/src/files.rs, codex-rs/core/src/mcp_openai_file.rs, codex-rs/core/src/state/service.rs, codex-rs/core/src/session/session.rs, codex-rs/codex-client/src/retry.rs, codex-rs/backend-client/src/client.rs]
symbols: [HttpClient, HttpClientBuilder, HttpClientFactory, OutboundProxyPolicy, OutboundProxyRoute, ClientRouteClass, RouteAwareClientPool, RouteAwareRequestBuilder, RouteAwareRequestError, upload_openai_file, HttpTransport, with_tls_backend_fallback, should_retry_with_rustls]
...
| `HttpClientBuilder` | TLS、redirect、timeout、UA 等构造选项，并针对已解析 route 构造 client。[E: codex-rs/http-client/src/client_builder.rs:19][E: codex-rs/http-client/src/client_builder.rs:61] |
| `HttpClientFactory` | 保存一次解析出的 outbound proxy policy，并可附加 ChatGPT cookie store（含 PSP cookie）。[E: codex-rs/http-client/src/outbound_proxy.rs:166][E: codex-rs/http-client/src/outbound_proxy.rs:205] |
| `RouteAwareClientPool` | 按 `OutboundProxyRoute` 缓存最多 16 个 client；每个 request URL 与每个 redirect hop 独立解析 route。[E: codex-rs/http-client/src/route_aware_client_pool.rs:59] |
...
`HttpClientFactory` 对 WebSocket URL 用对应 HTTP scheme 解析 proxy，使 `ws/wss` 与 `http/https` 能复用平台 PAC/system proxy 规则；system resolution 不可用时，显式 environment 设置优先，最后 direct。[E: codex-rs/http-client/src/outbound_proxy.rs:176][E: codex-rs/http-client/src/outbound_proxy.rs:190]
...
5. 一个 request timeout deadline 覆盖 route resolution、建连、所有 hops 与最终响应。[E: codex-rs/http-client/src/route_aware_client_pool.rs:512][E: codex-rs/http-client/src/route_aware_client_pool.rs:552][E: codex-rs/http-client/src/route_aware_client_pool.rs:594][E: codex-rs/http-client/src/route_aware_client_pool.rs:614]

若 `Location` 缺失/invalid，或 307/308 等保留 body 的 redirect 无法 `try_clone()` 原请求，`redirect_request` 返回 `None`，pool 会把原 redirect response 交给 caller，而不是强行继续下一 hop。[E: codex-rs/http-client/src/route_aware_redirect.rs:44][E: codex-rs/http-client/src/route_aware_redirect.rs:49][E: codex-rs/http-client/src/route_aware_redirect.rs:93][E: codex-rs/http-client/src/route_aware_client_pool.rs:587][E: codex-rs/http-client/src/route_aware_client_pool.rs:600]
...
MCP Apps 的 `openai/fileParams` 路径使用 session-owned、关闭 request logging 的 `RouteAwareClientPool`；该 pool 跨 turn 复用，并显式保留 transport-default route 的 legacy custom-CA fallback。[E: codex-rs/core/src/state/service.rs:62][E: codex-rs/core/src/session/session.rs:1392][E: codex-rs/core/src/session/session.rs:1396][E: codex-rs/core/src/mcp_openai_file.rs:47][E: codex-rs/core/src/mcp_openai_file.rs:49]
...
legacy custom-CA fallback 不是 system-proxy route 的通用兜底：只有显式 opt-in pool 且 outbound policy 为 `ReqwestDefault` 时，custom CA 构建失败才回退 system roots；`RespectSystemProxy` 仍按 resolved route 构建并传播错误。fallback 本身发出 log-only warning event。[E: codex-rs/http-client/src/route_aware_client_pool.rs:411]

## TLS backend fallback 与 PSP routing

`RouteAwareClientPool::with_tls_backend_fallback` 打开 per-origin rustls 回退：Native TLS 仍是默认；只有 recognized connection-time protocol negotiation failure 才会为该 HTTPS origin + outbound route 记住 rustls client，最多缓存 16 个 destination。[E: codex-rs/http-client/src/tls_backend_fallback.rs:15][E: codex-rs/http-client/src/route_aware_client_pool.rs:390][E: codex-rs/http-client/src/tls_backend_fallback.rs:104]

`should_retry_with_rustls` 要求 `is_connect()`、非 timeout，并识别 macOS “bad protocol version”、Linux “tlsv1 alert protocol version”、Windows Schannel `0x80090302`；证书/hostname/过期类错误不会回退。[E: codex-rs/http-client/src/tls_backend_fallback.rs:104][E: codex-rs/http-client/src/tls_backend_fallback.rs:132][E: codex-rs/http-client/src/tls_backend_fallback.rs:134][E: codex-rs/http-client/src/tls_backend_fallback.rs:136]
...
`Feature::Psp`（key `psp`，UnderDevelopment，默认关）让 `Config::http_client_factory()` 给 factory 加上 `oai-chat-psp=true` cookie；只有打开该 feature 的 ChatGPT cookie-store clients 才会带这条 PSP routing cookie。[E: codex-rs/features/src/lib.rs:1299][E: codex-rs/core/src/config/mod.rs:1643][E: codex-rs/core/src/config/mod.rs:1644]

delegated/local MCP HTTP 与部分 CLI MCP login 路径显式调用 `with_tls_backend_fallback()`，因此这些任意目标请求保留 rustls 协议协商回退，而不是只给固定 OpenAI endpoint 用。[E: codex-rs/codex-mcp/src/runtime.rs:774][E: codex-rs/cli/src/mcp_cmd.rs:431]
