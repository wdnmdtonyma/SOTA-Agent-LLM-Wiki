...
| `HttpClientFactory` | 保存一次解析出的 outbound proxy policy，并可附加 ChatGPT cookie store（含 PSP cookie）。[E: codex-rs/http-client/src/outbound_proxy.rs:146][E: codex-rs/http-client/src/outbound_proxy.rs:185] |
| `RouteAwareClientPool` | 按 `OutboundProxyRoute` 缓存最多 16 个 client；每个 request URL 与每个 redirect hop 独立解析 route。[E: codex-rs/http-client/src/route_aware_client_pool.rs:58] |
...
legacy custom-CA fallback 不是 system-proxy route 的通用兜底：只有显式 opt-in pool 且 outbound policy 为 `ReqwestDefault` 时，custom CA 构建失败才回退 system roots；`RespectSystemProxy` 仍按 resolved route 构建并传播错误。fallback 本身发出 log-only warning event。[E: codex-rs/http-client/src/route_aware_client_pool.rs:364]

## TLS backend fallback 与 PSP routing

`RouteAwareClientPool::with_tls_backend_fallback` 打开 per-origin rustls 回退：Native TLS 仍是默认；只有 recognized connection-time protocol negotiation failure 才会为该 HTTPS origin + outbound route 记住 rustls client，最多缓存 16 个 destination。[E: codex-rs/http-client/src/tls_backend_fallback.rs:15][E: codex-rs/http-client/src/route_aware_client_pool.rs:343][E: codex-rs/http-client/src/tls_backend_fallback.rs:104]
...
成功回退后 pool 记录 `codex.http_client.tls_backend_fallback` 并复用该 route 的 rustls client；其它 destination 仍用 transport-default backend。[E: codex-rs/http-client/src/route_aware_client_pool.rs:658][E: codex-rs/http-client/src/route_aware_client_pool.rs:660]

`Feature::Psp`（key `psp`，UnderDevelopment，默认关）让 `Config::http_client_factory()` 给 factory 加上 `oai-chat-psp=true` cookie；只有打开该 feature 的 ChatGPT cookie-store clients 才会带这条 PSP routing cookie。[E: codex-rs/features/src/lib.rs:1124][E: codex-rs/core/src/config/mod.rs:1568][E: codex-rs/core/src/config/mod.rs:1569]

delegated/local MCP HTTP 与部分 CLI MCP login 路径显式调用 `with_tls_backend_fallback()`，因此这些任意目标请求保留 rustls 协议协商回退，而不是只给固定 OpenAI endpoint 用。[E: codex-rs/codex-mcp/src/runtime.rs:501][E: codex-rs/cli/src/mcp_cmd.rs:446]
...
- backend-client 仍有自己的 typed API/path/auth layer；本节点不把它等同于 provider generic transport。[E: codex-rs/backend-client/src/client.rs:126][I]
