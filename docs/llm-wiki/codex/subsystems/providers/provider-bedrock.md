...
> Amazon Bedrock provider 是专用 runtime provider：provider info 携带 AWS auth config，runtime 按 region 生成 Bedrock Mantle 或 Runtime OpenAI-compatible base URL，并在 `auth_source` 里按 command auth、configured AWS profile、Codex-managed Bedrock API key / access keys、`AWS_BEARER_TOKEN_BEDROCK`、env AWS credentials、AWS SDK/SigV4 的顺序选择认证方式。[E: codex-rs/model-provider-info/src/lib.rs:156][E: codex-rs/model-provider/src/provider.rs:324][E: codex-rs/model-provider/src/amazon_bedrock/mod.rs:150][E: codex-rs/model-provider/src/amazon_bedrock/auth.rs:51][E: codex-rs/model-provider/src/amazon_bedrock/auth.rs:85]
...
- `ModelProviderAwsAuthInfo` 包含 optional profile 和 optional region；env bearer path 的 region fallback 是 configured region、`AWS_REGION`、`AWS_DEFAULT_REGION`。[E: codex-rs/model-provider-info/src/lib.rs:156][E: codex-rs/model-provider-info/src/lib.rs:158][E: codex-rs/model-provider-info/src/lib.rs:160][E: codex-rs/model-provider/src/amazon_bedrock/auth.rs:216][E: codex-rs/model-provider/src/amazon_bedrock/auth.rs:220]
...
9. `merge_configured_model_providers` 对 Bedrock 可合并 `base_url`、command `auth`、`http_headers` 和整组 AWS profile/region；command-auth path 走 configured provider auth 并可配 base URL，非 command-auth path 才走 Bedrock bearer/SigV4 resolution。[E: codex-rs/model-provider-info/src/lib.rs:550][E: codex-rs/model-provider-info/src/lib.rs:555][E: codex-rs/model-provider-info/src/lib.rs:572][E: codex-rs/model-provider/src/amazon_bedrock/auth.rs:56][E: codex-rs/model-provider/src/amazon_bedrock/mod.rs:137]
