...
symbols: [start_memories_startup_task, MemoriesExtension, build_memory_tool_developer_instructions, MemoriesConfig, claim_stage1_jobs_for_startup, memories::phase1::output_schema, sync_rollout_summaries_from_memories, validate_consolidation_artifacts, parse_memory_citation, memories_usage_kinds_from_command]
...
`MemoriesToml` 暴露 `disable_on_external_context`、`generate_memories`、`use_memories`、`dedicated_tools`、retention/selection limits、rate-limit threshold 和 extraction/consolidation model overrides；`MemoriesConfig::default` 默认生成和使用 memories，但 dedicated tools 默认关闭。[E: codex-rs/config/src/types.rs:290][E: codex-rs/config/src/types.rs:338][E: codex-rs/config/src/types.rs:342][E: codex-rs/config/src/types.rs:343][E: codex-rs/config/src/types.rs:344]

`generate_memories` 决定 thread persistence metadata 的 initial `memory_mode`: 新建和 resume 时为 true 则 `Enabled`，否则 `Disabled`；state DB 的 startup claim 再过滤掉 `threads.memory_mode != 'enabled'` 的线程。[E: codex-rs/core/src/session/session.rs:648][E: codex-rs/core/src/session/session.rs:651][E: codex-rs/core/src/session/session.rs:652][E: codex-rs/core/src/session/session.rs:654][E: codex-rs/core/src/session/session.rs:678][E: codex-rs/core/src/session/session.rs:678]
...
1. App-server turn path 在成功提交有 input 的 turn 后调用 `codex_memories_write::start_memories_startup_task`，把 thread manager、auth manager、thread id、thread、当前 config 和 session source 传入。[E: codex-rs/app-server/src/request_processors/turn_processor.rs:590][E: codex-rs/app-server/src/request_processors/turn_processor.rs:594][E: codex-rs/app-server/src/request_processors/turn_processor.rs:595][E: codex-rs/app-server/src/request_processors/turn_processor.rs:599][E: codex-rs/app-server/src/request_processors/turn_processor.rs:601]
2. Startup gate 跳过 ephemeral session、未开启 `Feature::MemoryTool` 的 session、以及 non-root agent；缺少 state DB 时直接跳过。[E: codex-rs/memories/write/src/start.rs:32][E: codex-rs/memories/write/src/start.rs:33][E: codex-rs/memories/write/src/start.rs:34][E: codex-rs/memories/write/src/start.rs:48][E: codex-rs/memories/write/src/start.rs:49]
...
4. Rate-limit guard 只在 backend auth 下查询 rate limits；如果 primary/secondary window used percent 超过 `100 - min_rate_limit_remaining_percent` 或 snapshot 标记 reached，则跳过 startup。[E: codex-rs/memories/write/src/guard.rs:15][E: codex-rs/memories/write/src/guard.rs:17][E: codex-rs/memories/write/src/guard.rs:27][E: codex-rs/memories/write/src/guard.rs:33][E: codex-rs/memories/write/src/guard.rs:38][E: codex-rs/memories/write/src/guard.rs:51][E: codex-rs/memories/write/src/guard.rs:56]

## Phase 1

Phase 1 claim 从 state DB 的 active/allowed-source threads 中选择 jobs，排除当前 thread、超过 age/idle window 的 thread 和 `memory_mode != 'enabled'` 的 thread，并受 scan limit、max claimed、lease seconds 控制。[E: codex-rs/memories/write/src/phase1.rs:164][E: codex-rs/memories/write/src/phase1.rs:169][E: codex-rs/memories/write/src/phase1.rs:170][E: codex-rs/memories/write/src/phase1.rs:174]
...
Phase 1 过滤 rollout item 时丢弃 developer messages；非 user messages 保留；user messages 会移除 AGENTS.md instructions 和 `<skill>` 这两类 memory-excluded contextual user fragment，其他 user content 可以保留。[E: codex-rs/memories/write/src/phase1.rs:439][E: codex-rs/memories/write/src/phase1.rs:443][E: codex-rs/memories/write/src/phase1.rs:447][E: codex-rs/memories/write/src/phase1.rs:450][E: codex-rs/memories/write/src/phase1.rs:470][E: codex-rs/memories/write/src/phase1.rs:470]

## Phase 2

Phase 2 先 claim global phase2 job，再确保 memory workspace git baseline，构建 locked-down consolidation agent config，读取 selected raw memories，计算 watermark，同步 `rollout_summaries/` 和 `raw_memories.md`，再用 workspace diff 判断是否需要 agent。[E: codex-rs/memories/write/src/phase2.rs:64][E: codex-rs/memories/write/src/phase2.rs:73][E: codex-rs/memories/write/src/phase2.rs:86][E: codex-rs/memories/write/src/phase2.rs:104][E: codex-rs/memories/write/src/phase2.rs:123][E: codex-rs/memories/write/src/phase2.rs:126][E: codex-rs/memories/write/src/phase2.rs:139][E: codex-rs/memories/write/src/phase2.rs:153]

Workspace sync 使用 DB-backed Stage1 outputs 写 rollout summaries 和 raw memories，并 prune old extension resources；如果 sync 后没有 workspace changes，Phase 2 会标记 `succeeded_no_workspace_changes` 后返回。[E: codex-rs/memories/write/src/phase2.rs:213][E: codex-rs/memories/write/src/phase2.rs:218][E: codex-rs/memories/write/src/phase2.rs:219][E: codex-rs/memories/write/src/phase2.rs:220][E: codex-rs/memories/write/src/phase2.rs:143][E: codex-rs/memories/write/src/phase2.rs:156][E: codex-rs/memories/write/src/phase2.rs:162]

Consolidation agent config 把 cwd 设为 memory root，标记 ephemeral，关闭 generate/use memories、apps、MCP servers 和多种 delegation/tool features，并固定 approval policy 为 Never。sandbox 会尊重 parent 明确选择的 Disabled/External profile；只有 parent 使用 Managed profile 时，才收紧为仅允许 memory root write 且无 network。[E: codex-rs/memories/write/src/phase2.rs:313][E: codex-rs/memories/write/src/phase2.rs:319][E: codex-rs/memories/write/src/phase2.rs:338][E: codex-rs/memories/write/src/phase2.rs:345][E: codex-rs/memories/write/src/phase2.rs:355]

Agent model 来自 `config.memories.consolidation_model` 或 provider 的 preferred consolidation model，reasoning effort 是 stage-two Medium。Agent final status 到达后，Phase 2 必须先在 10 秒超时内 `shutdown_and_wait` 并从 `ThreadManager` 移除；shutdown 失败则保留 lease 到自然过期并停止，避免另一个 worker 与仍存活 agent 竞态。只有 shutdown 成功后才验证 artifacts、确认 lock、reset baseline 并标记成功；非完成状态走 `failed_agent`。[E: codex-rs/memories/write/src/phase2.rs:366][E: codex-rs/memories/write/src/phase2.rs:397][E: codex-rs/memories/write/src/phase2.rs:410][E: codex-rs/memories/write/src/phase2.rs:417][E: codex-rs/memories/write/src/phase2.rs:420][E: codex-rs/memories/write/src/phase2.rs:434][E: codex-rs/memories/write/src/phase2.rs:461][E: codex-rs/memories/write/src/phase2.rs:480][E: codex-rs/memories/write/src/runtime.rs:364][E: codex-rs/memories/write/src/runtime.rs:370]

## Read path 与 tools

App-server 的 `thread_extensions` 安装 `codex_memories_extension`；extension 在 thread start/config changed 时把 `MemoriesExtensionConfig` 存入 thread store，config 的 `enabled` 条件是 `Feature::MemoryTool && config.memories.use_memories`，`dedicated_tools` 独立跟随 config。[E: codex-rs/app-server/src/extensions.rs:50][E: codex-rs/app-server/src/extensions.rs:87][E: codex-rs/ext/memories/src/extension.rs:41][E: codex-rs/ext/memories/src/extension.rs:43][E: codex-rs/ext/memories/src/extension.rs:44][E: codex-rs/ext/memories/src/extension.rs:73][E: codex-rs/ext/memories/src/extension.rs:86]
...
Memory citation parsing 接收多个 citation strings，提取 `<citation_entries>` 和 `<rollout_ids>`；legacy `<thread_ids>` 也作为 ids block fallback。session 侧 `record_memory_citation_for_turn` 只把 turn state 的 `has_memory_citation` 标记为 true。[E: codex-rs/memories/read/src/citations.rs:6][E: codex-rs/memories/read/src/citations.rs:12][E: codex-rs/memories/read/src/citations.rs:22][E: codex-rs/memories/read/src/citations.rs:38][E: codex-rs/memories/read/src/citations.rs:78][E: codex-rs/core/src/session/mod.rs:4033][E: codex-rs/core/src/session/mod.rs:4041]
...
外部上下文可以把 thread memory mode 标成 `polluted`：state DB update 把 threads row 的 `memory_mode` 改为 `polluted`，并保留 selected-for-phase2 查询用于后续队列行为。[E: codex-rs/state/src/runtime/memories.rs:606][E: codex-rs/state/src/runtime/memories.rs:611][E: codex-rs/state/src/runtime/memories.rs:623][E: codex-rs/state/src/runtime/memories.rs:626][E: codex-rs/state/src/runtime/memories.rs:627]
...
Memory 生成被放在 turn 提交后的 background startup task，而不是同步塞进当前 turn sampling；Phase 1/Phase 2 还受 state DB、rate-limit guard、lease 和 global lock 约束，这把慢速 extraction/consolidation 从当前交互延迟中隔离出来。[E: codex-rs/app-server/src/request_processors/turn_processor.rs:590][E: codex-rs/memories/write/src/start.rs:53][E: codex-rs/memories/write/src/start.rs:67][E: codex-rs/memories/write/src/phase1.rs:166][E: codex-rs/memories/write/src/phase2.rs:231][I]

`use_memories` 与 `dedicated_tools` 的拆分让 Codex 可以只注入 memory summary 而不暴露 dedicated tools，或在配置允许时额外暴露 tools；生成侧 `generate_memories` 又独立控制 future extraction eligibility。[E: codex-rs/ext/memories/src/extension.rs:43][E: codex-rs/ext/memories/src/extension.rs:107][I]

## gotcha

- 当前 Memory 的 write/read/extension 三层分别由 write crate 的 startup/storage exports、read crate 的 `memory_root` 与 memories extension installer 暴露。[E: codex-rs/memories/write/src/lib.rs:27][E: codex-rs/memories/write/src/lib.rs:28][E: codex-rs/memories/read/src/lib.rs:13][E: codex-rs/ext/memories/src/extension.rs:119][E: codex-rs/ext/memories/src/extension.rs:124]
...
- Phase 1 不会把 developer messages 写入 stage-one prompt input；AGENTS.md 和 `<skill>` user fragments 也会从 user messages 中过滤掉。[E: codex-rs/memories/write/src/phase1.rs:439][E: codex-rs/memories/write/src/phase1.rs:470][E: codex-rs/memories/write/src/phase1.rs:470]
