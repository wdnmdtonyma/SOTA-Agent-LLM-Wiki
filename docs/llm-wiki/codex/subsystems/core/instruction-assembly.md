...
1. `build_initial_context_with_world_state_and_mcp` 先创建 `developer_sections`、`contextual_user_sections` 和 `separate_developer_sections`，并从 session state 取 `reference_context_item`、previous settings、collaboration mode、base instructions、session source 和 auto-compact window ids。[E: codex-rs/core/src/session/mod.rs:3186][E: codex-rs/core/src/session/mod.rs:3192][E: codex-rs/core/src/session/mod.rs:3193][E: codex-rs/core/src/session/mod.rs:3194][E: codex-rs/core/src/session/mod.rs:3161][E: codex-rs/core/src/session/mod.rs:3201]
2. model switch instructions 最先加入 developer sections；permissions 不再由这段函数单独 push，而是在 world state 中构造后由 `render_full()` 按 developer role 合并。[E: codex-rs/core/src/session/mod.rs:3204][E: codex-rs/core/src/session/mod.rs:3210][E: codex-rs/core/src/session/world_state.rs:45][E: codex-rs/core/src/session/mod.rs:3385]
...
7. token budget 是 full-context developer metadata；`world_state.render_full()` 生成的 developer fragments 进入 developer sections，user fragments 进入 contextual user sections。[E: codex-rs/core/src/session/mod.rs:3337][E: codex-rs/core/src/session/mod.rs:3364][E: codex-rs/core/src/session/mod.rs:3385][E: codex-rs/core/src/session/mod.rs:3386][E: codex-rs/core/src/session/mod.rs:3387][E: codex-rs/core/src/session/mod.rs:3388]
...
- steady-state path 调用 `build_settings_update_items`，其中 developer update 只组合 model instructions、multi-agent mode 与 personality 的变化。[E: codex-rs/core/src/session/mod.rs:3570][E: codex-rs/core/src/context_manager/updates.rs:144][E: codex-rs/core/src/context_manager/updates.rs:147][E: codex-rs/core/src/context_manager/updates.rs:148][E: codex-rs/core/src/context_manager/updates.rs:149] 因而它不是 initial context 的 full replay，其他 model-visible input 是否都有 deterministic diff/replay 覆盖仍需另证。[I]
