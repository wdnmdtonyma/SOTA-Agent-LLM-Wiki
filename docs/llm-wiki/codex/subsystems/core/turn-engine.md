...
> Turn 引擎是 regular task 内部的 model-turn 状态机：`RegularTask::run` 先发 `TurnStarted`，再循环调用 `run_turn`；`run_turn` 做 pre-sampling compaction、captured step context、skill/plugin 注入、prompt history materialization、sampling request 和 follow-up 判断；真正的 stream/tool 处理在 `run_sampling_request`/`try_run_sampling_request` 中完成。[E: codex-rs/core/src/tasks/regular.rs:50][E: codex-rs/core/src/tasks/regular.rs:74][E: codex-rs/core/src/session/turn.rs:144][E: codex-rs/core/src/session/turn.rs:158][E: codex-rs/core/src/session/turn.rs:177][E: codex-rs/core/src/session/turn.rs:280][E: codex-rs/core/src/session/turn.rs:322][E: codex-rs/core/src/session/turn.rs:1123][E: codex-rs/core/src/session/turn.rs:1948]
...
`try_run_sampling_request` 的 stream-local state 包括 `in_flight: FuturesOrdered<...ResponseInputItem>`、`needs_follow_up`、`last_agent_message`、active turn item、tool argument diff consumer、turn diff/token emit flags 和 plan-mode parsers。[E: codex-rs/core/src/session/turn.rs:1992][E: codex-rs/core/src/session/turn.rs:1994][E: codex-rs/core/src/session/turn.rs:1995][E: codex-rs/core/src/session/turn.rs:1996][E: codex-rs/core/src/session/turn.rs:1997][E: codex-rs/core/src/session/turn.rs:2001][E: codex-rs/core/src/session/turn.rs:2002][E: codex-rs/core/src/session/turn.rs:2005][E: codex-rs/core/src/session/turn.rs:2006]

## 控制流

1. `run_turn` 创建或复用 `ModelClientSession`，先跑 `run_pre_sampling_compact`；失败时发 turn error lifecycle 并返回 `None`。[E: codex-rs/core/src/session/turn.rs:152][E: codex-rs/core/src/session/turn.rs:158][E: codex-rs/core/src/session/turn.rs:163]
2. pre-sampling 后，`run_turn` 捕获第一份 `StepContext`，记录 context update/reference context baseline，构建 skill/plugin 注入，运行 session-start hooks 和 user-prompt hooks，然后把 injection items 写入 conversation history。[E: codex-rs/core/src/session/turn.rs:170][E: codex-rs/core/src/session/turn.rs:172][E: codex-rs/core/src/session/turn.rs:173][E: codex-rs/core/src/session/turn.rs:177][E: codex-rs/core/src/session/turn.rs:188][E: codex-rs/core/src/session/turn.rs:192][E: codex-rs/core/src/session/turn.rs:204]
3. 主 loop 在允许时 drain pending input，确保 context、advertised tools 和 tool calls 共享同一个 request view，然后 clone session history 并调用 `for_prompt` 生成 model input。[E: codex-rs/core/src/session/turn.rs:227][E: codex-rs/core/src/session/turn.rs:231][E: codex-rs/core/src/session/turn.rs:250][E: codex-rs/core/src/session/turn.rs:268][E: codex-rs/core/src/session/turn.rs:270]
...
6. `try_run_sampling_request` 打开 model stream；`OutputItemDone` 完成当前 diff consumer，调用 `handle_output_item_done`，把产生的 `tool_future` 推入 `in_flight`，并 OR 合并 `needs_follow_up`。[E: codex-rs/core/src/session/turn.rs:1978][E: codex-rs/core/src/session/turn.rs:2059][E: codex-rs/core/src/session/turn.rs:2129][E: codex-rs/core/src/session/turn.rs:2137][E: codex-rs/core/src/session/turn.rs:2143]
...
turn engine 把 context update、tool construction、sampling retry、stream parsing、tool future drain 和 follow-up loop 收束在 `session/turn.rs`，便于沿一次 turn 追踪状态，但也让该文件成为多个子系统的 glue 层。[E: codex-rs/core/src/session/turn.rs:164][E: codex-rs/core/src/session/turn.rs:1948][I]
