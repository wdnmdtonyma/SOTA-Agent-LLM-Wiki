...
| 实体 | 当前形态 |
|---|---|
| `ModeKind` | 变体只有 `Plan` 与 default `Default`。`Default` 兼容旧 alias `code`、`pair_programming`、`execute`、`custom`。`PairProgramming` / `Execute` 变体已从 enum 删除。[E: codex-rs/protocol/src/config_types.rs:674] |
| TUI visibility | `TUI_VISIBLE_COLLABORATION_MODES` 只包含 `Default` 和 `Plan`；`ModeKind::is_tui_visible` 同样只匹配这两者。[E: codex-rs/protocol/src/config_types.rs:686][E: codex-rs/protocol/src/config_types.rs:696] |
| `CollaborationMode` | 字段是 `mode` 和 `settings`。[E: codex-rs/protocol/src/config_types.rs:708] |
| `Settings` | 包含 `model`、可选 `reasoning_effort`、可选 `developer_instructions`。[E: codex-rs/protocol/src/config_types.rs:780] |
| `CollaborationModeMask` | 可选覆盖 mode/model/reasoning/developer instructions；`name` 是 metadata，不参与 `apply_mask` 的 runtime output。[E: codex-rs/protocol/src/config_types.rs:789] |
