...
| `codex-rs/protocol/src/protocol.rs` | collaboration mode prompt tags 与 `ThreadSettingsOverrides.collaboration_mode`。[E: codex-rs/protocol/src/protocol.rs:116][E: codex-rs/protocol/src/protocol.rs:117][E: codex-rs/protocol/src/protocol.rs:451] |
...
| `codex-rs/core/src/session/world_state.rs` | 每 step 从当前 turn context 构造 typed world state，并按 feature gate 加入 collaboration section。[E: codex-rs/core/src/session/world_state.rs:19][E: codex-rs/core/src/session/world_state.rs:38][E: codex-rs/core/src/session/world_state.rs:72] |
| `codex-rs/core/src/session/mod.rs` | session 默认 collaboration mode 与 initial context 注入。[E: codex-rs/core/src/session/mod.rs:667][E: codex-rs/core/src/session/mod.rs:3346] |
| `codex-rs/core/src/session/handlers.rs` | thread settings update 中 collaboration mode 的 precedence 和 model/effort fallback 更新。[E: codex-rs/core/src/session/handlers.rs:117][E: codex-rs/core/src/session/handlers.rs:134][E: codex-rs/core/src/session/handlers.rs:137][E: codex-rs/core/src/session/handlers.rs:140] |
...
1. session start 时构造 `ModeKind::Default` 的 `CollaborationMode`，settings 取当前 model、config reasoning effort，developer instructions 为 None。[E: codex-rs/core/src/session/mod.rs:667][E: codex-rs/core/src/session/mod.rs:668][E: codex-rs/core/src/session/mod.rs:669][E: codex-rs/core/src/session/mod.rs:670][E: codex-rs/core/src/session/mod.rs:671][E: codex-rs/core/src/session/mod.rs:672]
2. `Session::collaboration_mode()` 从 session state 返回当前 mode clone，tools handler 通过它读取实时 mode。[E: codex-rs/core/src/session/mod.rs:3253][E: codex-rs/core/src/session/mod.rs:3254][E: codex-rs/core/src/session/mod.rs:3255][E: codex-rs/core/src/tools/handlers/request_user_input.rs:65]
3. `ThreadSettingsOverrides.collaboration_mode` 的注释说明该字段优先于 model、effort 和 developer instructions。[E: codex-rs/protocol/src/protocol.rs:498]
4. thread settings update 如果带 `collaboration_mode` 就直接使用；否则在当前 collaboration mode 上调用 `with_updates(model, effort, None)`，保留当前 mode 和 developer instructions，只刷新 model/effort。[E: codex-rs/core/src/session/handlers.rs:121][E: codex-rs/core/src/session/handlers.rs:134][E: codex-rs/core/src/session/handlers.rs:137][E: codex-rs/core/src/session/handlers.rs:140][E: codex-rs/core/src/session/handlers.rs:145][E: codex-rs/core/src/session/handlers.rs:146][E: codex-rs/protocol/src/config_types.rs:702][E: codex-rs/protocol/src/config_types.rs:709][E: codex-rs/protocol/src/config_types.rs:712]
5. `build_world_state_for_step` 仅在 collaboration instructions gate 开启且 developer instructions 非空时加入 `CollaborationModeState`；section snapshot 是 `ModeKind`。[E: codex-rs/core/src/session/world_state.rs:72][E: codex-rs/core/src/session/world_state.rs:74][E: codex-rs/core/src/context/world_state/collaboration_mode.rs:17][E: codex-rs/core/src/context/world_state/collaboration_mode.rs:32]
...
- thread settings fallback 使用 `with_updates(model, effort, None)`，避免只改 model/effort 时意外清掉 active mode 或 developer instructions。[E: codex-rs/core/src/session/handlers.rs:140][E: codex-rs/core/src/session/handlers.rs:146][E: codex-rs/protocol/src/config_types.rs:712][I]
- runtime hard gate 主要体现在 tool handlers 和 prompt instructions；Plan template 禁止 mutating work，但 `ModeKind` enum 本身并不是全局 filesystem write lock。[E: codex-rs/core/src/tools/handlers/plan.rs:84][E: codex-rs/core/src/tools/handlers/request_user_input.rs:65][E: codex-rs/protocol/src/config_types.rs:668][I]
