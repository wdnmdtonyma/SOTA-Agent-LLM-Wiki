...
`Session` 是状态和服务容器，`SessionIo` 是可丢弃的 queue/lifecycle endpoints；public `CodexThread` 组合二者成为 bidirectional conduit。[E: codex-rs/core/src/session/mod.rs:399][E: codex-rs/core/src/session/session.rs:50]
...
- `codex-rs/core/src/session/mod.rs`: `SessionIo`、`SessionSpawnArgs`、`Session::spawn`、submit/event receive queue endpoints。[E: codex-rs/core/src/session/mod.rs:399][E: codex-rs/core/src/session/mod.rs:426][E: codex-rs/core/src/session/mod.rs:502][E: codex-rs/core/src/session/mod.rs:887]
- `codex-rs/core/src/session/session.rs`: `Session` 和 `SessionConfiguration` fields。[E: codex-rs/core/src/session/session.rs:46][E: codex-rs/core/src/session/session.rs:86]
- `codex-rs/core/src/session/handlers.rs`: `submission_loop` 与 per-op dispatch handler。[E: codex-rs/core/src/session/handlers.rs:538][E: codex-rs/core/src/session/handlers.rs:553]
...
- `codex-rs/core/src/state/session.rs` 与 `codex-rs/core/src/state/turn.rs`: session-scoped state、active turn 和 turn-local waiters/queues。[E: codex-rs/core/src/state/session.rs:69][E: codex-rs/core/src/state/turn.rs:35][E: codex-rs/core/src/state/turn.rs:92]

## 数据模型

`Session` 持有 thread/session identity、event sender、agent status sender、`SessionState` mutex、realtime conversation manager、`active_turn` mutex、`InputQueue`、`SessionServices` 和 internal sub-id counter。[E: codex-rs/core/src/session/session.rs:46][E: codex-rs/core/src/session/session.rs:47][E: codex-rs/core/src/session/session.rs:49][E: codex-rs/core/src/session/session.rs:50][E: codex-rs/core/src/session/session.rs:51][E: codex-rs/core/src/session/session.rs:73][E: codex-rs/core/src/session/session.rs:75][E: codex-rs/core/src/session/session.rs:77][E: codex-rs/core/src/session/session.rs:78][E: codex-rs/core/src/session/session.rs:82]

`SessionState` 保存 session configuration、`ContextManager` history、rate limit/server reasoning state、additional context、previous turn settings、auto-compact window、startup prewarm、active connector selection、pending session-start sources、granted permissions 和 next-turn-is-first flag。[E: codex-rs/core/src/state/session.rs:69][E: codex-rs/core/src/state/session.rs:70][E: codex-rs/core/src/state/session.rs:75][E: codex-rs/core/src/state/session.rs:76][E: codex-rs/core/src/state/session.rs:78][E: codex-rs/core/src/state/session.rs:80][E: codex-rs/core/src/state/session.rs:84][E: codex-rs/core/src/state/session.rs:86][E: codex-rs/core/src/state/session.rs:90][E: codex-rs/core/src/state/session.rs:94][E: codex-rs/core/src/state/session.rs:95][E: codex-rs/core/src/state/session.rs:96][E: codex-rs/core/src/state/session.rs:97]

`ActiveTurn` 持有当前 `RunningTask` 与共享 `TurnState`；`TurnState` 保存 approval/request-permissions/user-input/elicitation/dynamic-tool waiters、pending input、mailbox delivery phase、turn-scoped granted permissions、strict auto-review flag、tool call count、memory citation flag 和 turn-start token usage。[E: codex-rs/core/src/state/turn.rs:34][E: codex-rs/core/src/state/turn.rs:36][E: codex-rs/core/src/state/turn.rs:91][E: codex-rs/core/src/state/turn.rs:92][E: codex-rs/core/src/state/turn.rs:93][E: codex-rs/core/src/state/turn.rs:94][E: codex-rs/core/src/state/turn.rs:95][E: codex-rs/core/src/state/turn.rs:97][E: codex-rs/core/src/state/turn.rs:98][E: codex-rs/core/src/state/turn.rs:99][E: codex-rs/core/src/state/turn.rs:100][E: codex-rs/core/src/state/turn.rs:101][E: codex-rs/core/src/state/turn.rs:102][E: codex-rs/core/src/state/turn.rs:103][E: codex-rs/core/src/state/turn.rs:104]
...
`ElicitationService` 用 reference-counted registration 统一表示 session 是否处于用户交互暂停：并发 MCP elicitation、approval 等 holder 全部释放后 watch 才回到 false。Unified exec 与 code-mode result delivery 都订阅/等待这一状态，因此不再只针对某一种 out-of-band elicitation。[E: codex-rs/core/src/elicitation.rs:66][E: codex-rs/core/src/elicitation.rs:70][E: codex-rs/core/src/session/mod.rs:1276]
...
2. spawn 后启动后台 `session_loop` task，内部运行 `submission_loop`；返回 `(Arc<Session>, SessionIo)`。[E: codex-rs/core/src/session/mod.rs:869][E: codex-rs/core/src/session/mod.rs:881]
3. `submit`/`submit_with_trace` 创建 `Submission` 并把它送入 `tx_sub`；`next_event` 从 `rx_event` 读取 user-visible `Event`。[E: codex-rs/core/src/session/mod.rs:887][E: codex-rs/core/src/session/mod.rs:894][E: codex-rs/core/src/session/mod.rs:986]
4. `submission_loop` 从 `rx_sub.recv()` 取 submission，按 `Op` match；`Op::Review` 进入 review handler，`Op::Shutdown` 返回 should-exit。[E: codex-rs/core/src/session/handlers.rs:538][E: codex-rs/core/src/session/handlers.rs:545][E: codex-rs/core/src/session/handlers.rs:718][E: codex-rs/core/src/session/handlers.rs:719]
5. `Op::TurnInput` 交给 `turn_input::handle`：先 `steer_input`，没有 active turn 才 start `RegularTask`。persistent `ThreadSettingsOverrides` 在 Started/Steered 后才 apply。[E: codex-rs/core/src/session/handlers.rs:598][E: codex-rs/core/src/session/turn_input.rs:202][E: codex-rs/core/src/session/turn_input.rs:270][E: codex-rs/core/src/session/turn_input.rs:280]
6. `Op::ThreadSettings` 走 `thread_settings::update`，不启动 turn；`Op::TurnSettings` 走 `apply_turn_settings`；`Op::SuspendTurnAndShutdown` 在成功 Suspended 后退出 loop；`Op::ThreadRollback` 走 `thread_rollback`，要求无 active turn。[E: codex-rs/core/src/session/handlers.rs:635][E: codex-rs/core/src/session/handlers.rs:639][E: codex-rs/core/src/session/handlers.rs:622][E: codex-rs/core/src/session/handlers.rs:692][E: codex-rs/core/src/session/handlers.rs:269]
7. reserved thread ID：`New`/`Cleared`/`Forked` 使用预分配 id；`Resumed` 带 reserved id 是错误。[E: codex-rs/core/src/session/session.rs:787][E: codex-rs/core/src/session/session.rs:796]
...
9. `Op::Shutdown` 调用 `shutdown()`，内部先跑 `shutdown_session_runtime`：abort prewarm/tasks、关 realtime/unified exec/code mode/MCP/guardian。[E: codex-rs/core/src/session/handlers.rs:718][E: codex-rs/core/src/session/handlers.rs:454][E: codex-rs/core/src/session/handlers.rs:411]
...
`SessionIo` 暴露 queue-pair API，`Session` 持 mutable state/services，`submission_loop` 统一分发 ops；分离 endpoints 还允许所有 submission sender 被 drop 后终止 loop。[E: codex-rs/core/src/session/mod.rs:399][E: codex-rs/core/src/session/handlers.rs:538][I]
...
- `SessionIo::submit` 只返回 submission id；用户可见结果来自 `next_event`。[E: codex-rs/core/src/session/mod.rs:887][E: codex-rs/core/src/session/mod.rs:910][E: codex-rs/core/src/session/mod.rs:986]
...
- `shutdown_and_wait` 是提交 `Op::Shutdown` 后等待 session-loop termination，不是直接同步调用 runtime teardown。[E: codex-rs/core/src/session/mod.rs:975][E: codex-rs/core/src/session/mod.rs:977]
