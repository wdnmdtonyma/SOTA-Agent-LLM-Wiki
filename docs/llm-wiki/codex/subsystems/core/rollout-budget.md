...
`RolloutBudget` 是 under-development、default-off feature。Structured TOML 包含 `limit_tokens`、`reminder_at_remaining_tokens`、`sampling_token_weight` 与 `prefill_token_weight`。[E: codex-rs/features/src/lib.rs:1536][E: codex-rs/features/src/feature_configs.rs:335][E: codex-rs/features/src/feature_configs.rs:340][E: codex-rs/features/src/feature_configs.rs:343]

Feature 开启时 `limit_tokens` 和 reminder thresholds 必填；limit 必须为正，threshold 必须为正且小于 limit，weights 必须 finite/non-negative，默认都为 1.0。校验失败会阻断 config resolve，而不是静默回退。[E: codex-rs/core/src/config/mod.rs:2801][E: codex-rs/core/src/config/mod.rs:2817][E: codex-rs/core/src/config/mod.rs:2820][E: codex-rs/core/src/config/mod.rs:2826][E: codex-rs/core/src/config/mod.rs:2836][E: codex-rs/core/src/config/mod.rs:2845][E: codex-rs/core/src/config/mod.rs:2846][E: codex-rs/core/src/config/mod.rs:2851][E: codex-rs/core/src/config/mod.rs:2858]

## Accounting data source

Responses SSE 将 optional `codex_rollout_budget_units` 解析进 `TokenUsage`。这个字段明确跳过 serialization、JSON schema 和 TypeScript export，所以只服务 provider-to-core accounting，不扩展 public token-usage wire schema。[E: codex-rs/codex-api/src/sse/responses.rs:128][E: codex-rs/codex-api/src/sse/responses.rs:135][E: codex-rs/codex-api/src/sse/responses.rs:151][E: codex-rs/protocol/src/protocol.rs:2207][E: codex-rs/protocol/src/protocol.rs:2208][E: codex-rs/protocol/src/protocol.rs:2209][E: codex-rs/protocol/src/protocol.rs:2210]
...
`AgentControl` 在一个 root session tree 中只创建一次并共享给所有 subagents；它持有同一个 `Arc<RolloutBudget>`。Root thread 由 effective config 初始化 budget，普通无 config handle 则不另建独立 budget。[E: codex-rs/core/src/agent/control.rs:118][E: codex-rs/core/src/agent/control.rs:132][E: codex-rs/core/src/agent/control.rs:164][E: codex-rs/core/src/thread_manager.rs:1411][E: codex-rs/core/src/thread_manager.rs:1412][E: codex-rs/core/src/thread_manager.rs:1419][E: codex-rs/core/src/thread_manager.rs:1423]

普通 response token usage 在更新 session token info 后记 budget；remote compaction 的 sampling usage 也记入同一 budget。达到上限时 `record_rollout_budget_usage` 返回 `SessionBudgetExceeded`，compaction 对该错误直接 emit/return，不进入 context-window retry trimming。[E: codex-rs/core/src/session/mod.rs:4243][E: codex-rs/core/src/session/rollout_budget.rs:26][E: codex-rs/core/src/session/rollout_budget.rs:33][E: codex-rs/core/src/compact_remote_v2.rs:305][E: codex-rs/core/src/compact.rs:308][E: codex-rs/core/src/compact.rs:312]
...
Turn loop 在每次 sampling 前读取 current window id 并尝试注入 reminder；thread rollback 会 rearm 当前 thread 的 delivery，让恢复后的 prompt 重述当前 remainder，但不会退还已经累计的 shared usage。[E: codex-rs/core/src/session/turn.rs:328][E: codex-rs/core/src/session/turn.rs:329][E: codex-rs/core/src/session/handlers.rs:348][E: codex-rs/core/src/session/handlers.rs:351][E: codex-rs/core/src/rollout_budget.rs:113][E: codex-rs/core/src/rollout_budget.rs:117]
