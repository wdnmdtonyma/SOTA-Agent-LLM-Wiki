...
Feature 开启时 `limit_tokens` 和 reminder thresholds 必填；limit 必须为正，threshold 必须为正且小于 limit，weights 必须 finite/non-negative，默认都为 1.0。校验失败会阻断 config resolve，而不是静默回退。[E: codex-rs/core/src/config/mod.rs:2859][E: codex-rs/core/src/config/mod.rs:2862][E: codex-rs/core/src/config/mod.rs:2866][E: codex-rs/core/src/config/mod.rs:2878][E: codex-rs/core/src/config/mod.rs:2882][E: codex-rs/core/src/config/mod.rs:2888][E: codex-rs/core/src/config/mod.rs:2896][E: codex-rs/core/src/config/mod.rs:2903][E: codex-rs/core/src/config/mod.rs:2911]

## Accounting data source

Responses SSE 将 optional `codex_rollout_budget_units` 解析进 `TokenUsage`。这个字段明确跳过 serialization、JSON schema 和 TypeScript export，所以只服务 provider-to-core accounting，不扩展 public token-usage wire schema。[E: codex-rs/codex-api/src/sse/responses.rs:128][E: codex-rs/codex-api/src/sse/responses.rs:135][E: codex-rs/codex-api/src/sse/responses.rs:151][E: codex-rs/protocol/src/protocol.rs:2193][E: codex-rs/protocol/src/protocol.rs:2205][E: codex-rs/protocol/src/protocol.rs:2207][E: codex-rs/protocol/src/protocol.rs:2207]
...
`AgentControl` 在一个 root session tree 中只创建一次并共享给所有 subagents；它持有同一个 `Arc<RolloutBudget>`。Root thread 由 effective config 初始化 budget，普通无 config handle 则不另建独立 budget。[E: codex-rs/core/src/agent/control.rs:108][E: codex-rs/core/src/agent/control.rs:117][E: codex-rs/core/src/agent/control.rs:121][E: codex-rs/core/src/thread_manager.rs:1343][E: codex-rs/core/src/thread_manager.rs:1347][E: codex-rs/core/src/thread_manager.rs:1313]
...
Turn loop 在每次 sampling 前读取 current window id 并尝试注入 reminder；thread rollback 会 rearm 当前 thread 的 delivery，让恢复后的 prompt 重述当前 remainder，但不会退还已经累计的 shared usage。[E: codex-rs/core/src/session/turn.rs:308][E: codex-rs/core/src/session/turn.rs:309][E: codex-rs/core/src/session/handlers.rs:554][E: codex-rs/core/src/session/handlers.rs:558][E: codex-rs/core/src/rollout_budget.rs:113][E: codex-rs/core/src/rollout_budget.rs:117]
