...
> Unified Exec 是 Codex 的交互式 shell/process 执行面：`spec_plan.rs` 在 unified shell type 下暴露 `exec_command` 与 `write_stdin`，handler 解析模型参数，`UnifiedExecProcessManager` 管进程生命周期，`UnifiedExecRuntime` 经 `ToolOrchestrator` 接入 approval/sandbox/retry 协议。[E: codex-rs/core/src/tools/spec_plan.rs:646][E: codex-rs/core/src/tools/spec_plan.rs:657][E: codex-rs/core/src/unified_exec/process_manager.rs:382][E: codex-rs/core/src/unified_exec/process_manager.rs:424][E: codex-rs/core/src/unified_exec/process_manager.rs:1114]
...
`add_shell_tools` 只有在 turn 有 environment 时继续；当 `shell_type_for_model_and_features(...)` 返回 `ConfigShellToolType::UnifiedExec`，计划器添加 `ExecCommandHandler`、`WriteStdinHandler`，并 hidden 注册 legacy `ShellCommandHandler`。[E: codex-rs/core/src/tools/spec_plan.rs:628][E: codex-rs/core/src/tools/spec_plan.rs:657]

`ExecCommandHandler` 的 wire name 是 `exec_command`，spec 来自 `create_exec_command_tool_with_environment_id`，并声明支持 parallel tool calls。[E: codex-rs/core/src/tools/handlers/unified_exec/exec_command.rs:78][E: codex-rs/core/src/tools/handlers/unified_exec/exec_command.rs:96]
...
`ExecCommandHandler` 只接受 Function payload；它解析 tool environment、根据 sandbox 初始选择判断是否能用 native cwd 解析参数，然后分配 process id、解析 shell command。[E: codex-rs/core/src/tools/handlers/unified_exec/exec_command.rs:117][E: codex-rs/core/src/tools/handlers/unified_exec/exec_command.rs:186][E: codex-rs/core/src/tools/handlers/unified_exec/exec_command.rs:198][E: codex-rs/core/src/tools/handlers/unified_exec/exec_command.rs:214]

如果命令被识别为 apply_patch，handler 释放 process id 并返回 synthetic `ExecCommandToolOutput`，`process_id` 为 `None`，不进入 process manager。[E: codex-rs/core/src/tools/handlers/unified_exec/exec_command.rs:290][E: codex-rs/core/src/tools/handlers/unified_exec/exec_command.rs:317]

普通命令会构造 `ExecCommandRequest`，字段包括 command、shell type、process id、yield/max output、cwd、turn environment、network、TTY、sandbox/additional permissions、justification 和 prefix rule，然后调用 `manager.exec_command(...)`。[E: codex-rs/core/src/tools/handlers/unified_exec/exec_command.rs:321][E: codex-rs/core/src/tools/handlers/unified_exec/exec_command.rs:340][E: codex-rs/core/src/unified_exec/mod.rs:92][E: codex-rs/core/src/unified_exec/mod.rs:110]

`UnifiedExecProcessManager::exec_command` 调 `open_session_with_sandbox` 启动进程，开始 output streaming，并在 initial yield wait 前把仍活着的 process 存进 process store，避免 turn interrupt drop 最后一个 `Arc` 终止后台进程。[E: codex-rs/core/src/unified_exec/process_manager.rs:382][E: codex-rs/core/src/unified_exec/process_manager.rs:390][E: codex-rs/core/src/unified_exec/process_manager.rs:424][E: codex-rs/core/src/unified_exec/process_manager.rs:428][E: codex-rs/core/src/unified_exec/process_manager.rs:444]

initial response collection 会 clamp `yield_time_ms`，从 output handles 收集到 deadline；后续 `refresh_process_state` 会把已退出进程从 store 移除，或返回仍 alive 的 process id。[E: codex-rs/core/src/unified_exec/process_manager.rs:452][E: codex-rs/core/src/unified_exec/process_manager.rs:470][E: codex-rs/core/src/unified_exec/process_manager.rs:770][E: codex-rs/core/src/unified_exec/process_manager.rs:793]
...
process manager 通过 `prepare_process_handles` 从 process store 找到 process/output handles；找不到时返回 `UnknownProcessId`。[E: codex-rs/core/src/unified_exec/process_manager.rs:796][E: codex-rs/core/src/unified_exec/process_manager.rs:804]

## 5 Approval / Sandbox

`open_session_with_sandbox` 构造 shell env、`CODEX_THREAD_ID` env、exec-server env config 和 `ExecApprovalRequest`，再创建 `UnifiedExecRuntime` 并调用 `ToolOrchestrator::run`。[E: codex-rs/core/src/unified_exec/process_manager.rs:1047][E: codex-rs/core/src/unified_exec/process_manager.rs:1064][E: codex-rs/core/src/unified_exec/process_manager.rs:1069][E: codex-rs/core/src/unified_exec/process_manager.rs:1121]

`UnifiedExecRuntime` 的 sandbox preference 是 `Auto`，`escalate_on_failure()` 返回 true；runtime run 会根据 attempt permissions 生成 launch sandbox permissions、network env，并在本地/remote/ZshFork 路径间分流。[E: codex-rs/core/src/tools/runtimes/unified_exec.rs:143][E: codex-rs/core/src/tools/runtimes/unified_exec.rs:150][E: codex-rs/core/src/tools/runtimes/unified_exec.rs:289][E: codex-rs/core/src/tools/runtimes/unified_exec.rs:323][E: codex-rs/core/src/tools/runtimes/unified_exec.rs:347][E: codex-rs/core/src/tools/runtimes/unified_exec.rs:389]

`ToolOrchestrator::run` 负责 approval 与 sandbox attempt；sandbox denial 后，只有 runtime 允许 escalate 且 approval/network 条件允许，才会进入 retry 路径；只有 `unsandboxed_allowed` 为真时 retry sandbox 才是 `SandboxType::None`。[E: codex-rs/core/src/tools/orchestrator.rs:132][E: codex-rs/core/src/tools/orchestrator.rs:149][E: codex-rs/core/src/tools/orchestrator.rs:282][E: codex-rs/core/src/tools/orchestrator.rs:349][E: codex-rs/core/src/tools/orchestrator.rs:404]
