...
> Ghost snapshot/undo is no longer an active runtime subsystem in current Codex. The old `ghost_snapshot` config and `undo` feature key are compatibility-only, legacy rollout `ghost_snapshot` items deserialize as `Other`, and current rollback semantics are represented by `Op::ThreadRollback`, which explicitly does not revert local filesystem edits.[E: codex-rs/features/src/lib.rs:363][E: codex-rs/features/src/lib.rs:909][E: codex-rs/protocol/src/models.rs:3730][E: codex-rs/protocol/src/models.rs:3743][E: codex-rs/protocol/src/protocol.rs:742]
...
- Do not document legacy ghost/undo task structs or git ghost-commit helper functions as current runtime symbols for HEAD `a9519cbcdd`; current `TaskKind` exposes only regular/review/compact task classes, and `Feature::GhostCommit` is only a removed compatibility feature flag.[E: codex-rs/core/src/state/turn.rs:69][E: codex-rs/core/src/state/turn.rs:70][E: codex-rs/core/src/state/turn.rs:71][E: codex-rs/core/src/state/turn.rs:72][E: codex-rs/features/src/lib.rs:363][E: codex-rs/features/src/lib.rs:911]
- The `undo` feature key still parsing does not mean undo is active; the feature registry marks `Feature::GhostCommit` as `Stage::Removed` and disabled by default.[E: codex-rs/features/src/lib.rs:582][E: codex-rs/features/src/lib.rs:911][E: codex-rs/features/src/lib.rs:912]
- `ThreadRollback` loads persisted history, appends a rollback event, reconstructs session state, and persists that marker。[E: codex-rs/core/src/session/handlers.rs:313][E: codex-rs/core/src/session/handlers.rs:334][E: codex-rs/core/src/session/handlers.rs:337][E: codex-rs/core/src/session/handlers.rs:348] 该路径没有执行 filesystem rollback；需要磁盘撤销的 UI 必须另行实现。[I]
