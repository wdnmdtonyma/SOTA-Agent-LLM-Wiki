...
> Ghost snapshot/undo is no longer an active runtime subsystem in current Codex. The old `ghost_snapshot` config and `undo` feature key are compatibility-only, legacy rollout `ghost_snapshot` items deserialize as `Other`, and current rollback semantics are represented by `Op::ThreadRollback`, which explicitly does not revert local filesystem edits.[E: codex-rs/config/src/config_toml.rs:465][E: codex-rs/core/src/config/mod.rs:183][E: codex-rs/features/src/lib.rs:255][E: codex-rs/features/src/lib.rs:806][E: codex-rs/protocol/src/models.rs:3316][E: codex-rs/protocol/src/protocol.rs:661][E: codex-rs/protocol/src/protocol.rs:663]
...
The session dispatch loop now exposes `Op::Compact`, `Op::ThreadRollback`, and `Op::RunUserShellCommand` around this area, but no `Op::Undo` branch. `SessionTask` 注释仍举例提到 ghost snapshots；当前实际 `TaskKind` variants 只有 `Regular`、`Review`、`Compact`。[E: codex-rs/core/src/session/handlers.rs:810][E: codex-rs/core/src/session/handlers.rs:814][E: codex-rs/core/src/session/handlers.rs:822][E: codex-rs/core/src/tasks/mod.rs:206][E: codex-rs/core/src/tasks/mod.rs:208][E: codex-rs/core/src/state/turn.rs:65][E: codex-rs/core/src/state/turn.rs:66][E: codex-rs/core/src/state/turn.rs:67][E: codex-rs/core/src/state/turn.rs:68][E: codex-rs/core/src/state/turn.rs:69]

`ConfigToml` still accepts `ghost_snapshot`, but the comment says it is retained so legacy config still loads. `GhostSnapshotToml` fields are all documented as legacy no-op settings.[E: codex-rs/config/src/config_toml.rs:465][E: codex-rs/config/src/config_toml.rs:466][E: codex-rs/config/src/config_toml.rs:449][E: codex-rs/config/src/config_toml.rs:468][E: codex-rs/config/src/config_toml.rs:715][E: codex-rs/config/src/config_toml.rs:732][E: codex-rs/config/src/config_toml.rs:735][E: codex-rs/config/src/config_toml.rs:738][E: codex-rs/config/src/config_toml.rs:740]

`core::config::GhostSnapshotConfig` is also compatibility-only; its comment says legacy settings continue to load even though snapshots are no longer produced. The loaded config field remains present on `Config` and is populated from the legacy TOML fields with default thresholds.[E: codex-rs/core/src/config/mod.rs:183][E: codex-rs/core/src/config/mod.rs:184][E: codex-rs/core/src/config/mod.rs:186][E: codex-rs/core/src/config/mod.rs:192][E: codex-rs/core/src/config/mod.rs:195][E: codex-rs/core/src/config/mod.rs:196][E: codex-rs/core/src/config/mod.rs:1045][E: codex-rs/core/src/config/mod.rs:1047][E: codex-rs/core/src/config/mod.rs:3648][E: codex-rs/core/src/config/mod.rs:3650][E: codex-rs/core/src/config/mod.rs:3659][E: codex-rs/core/src/config/mod.rs:3665]

`Feature::GhostCommit` is retained as a removed compatibility flag. The registry entry has key `undo`, stage `Removed`, and `default_enabled: false`; feature-source parsing also skips the old `undo` key.[E: codex-rs/features/src/lib.rs:252][E: codex-rs/features/src/lib.rs:253][E: codex-rs/features/src/lib.rs:254][E: codex-rs/features/src/lib.rs:255][E: codex-rs/features/src/lib.rs:466][E: codex-rs/features/src/lib.rs:467][E: codex-rs/features/src/lib.rs:806][E: codex-rs/features/src/lib.rs:807][E: codex-rs/features/src/lib.rs:808][E: codex-rs/features/src/lib.rs:809]
...
- Do not document legacy ghost/undo task structs or git ghost-commit helper functions as current runtime symbols for HEAD `4d7a5c7c73`; current `TaskKind` exposes only regular/review/compact task classes, and `Feature::GhostCommit` is only a removed compatibility feature flag.[E: codex-rs/core/src/tasks/mod.rs:214][E: codex-rs/core/src/state/turn.rs:66][E: codex-rs/core/src/state/turn.rs:67][E: codex-rs/core/src/state/turn.rs:68][E: codex-rs/core/src/state/turn.rs:69][E: codex-rs/features/src/lib.rs:252]
...
- `ThreadRollback` is history-context rollback only; any UI that wants disk undo must implement or invoke disk-level undo outside this protocol op.[E: codex-rs/protocol/src/protocol.rs:661][E: codex-rs/protocol/src/protocol.rs:663][E: codex-rs/protocol/src/protocol.rs:664]
