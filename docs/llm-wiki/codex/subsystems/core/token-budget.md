...
> Token budget 管理当前模型 context window 临近 rollover 时的 guidance、reminder 和 auto-compact fallback。它按每个 turn 选中的 model/config 生效；它不是 root agent tree 的累计 inference quota。[E: codex-rs/core/src/session/token_budget.rs:81][E: codex-rs/core/src/session/token_budget.rs:161][E: codex-rs/core/src/session/turn_context.rs:746][E: codex-rs/core/src/session/context_window.rs:83]
...
`TokenBudget` 是 under-development、default-off feature。Structured config 包含 reminder threshold/template、guidance message、auto-compact fallback prompt、fallback buffer，以及是否暴露 history/notes extension。[E: codex-rs/features/src/lib.rs:1595][E: codex-rs/features/src/feature_configs.rs:316][E: codex-rs/features/src/feature_configs.rs:321][E: codex-rs/features/src/feature_configs.rs:325][E: codex-rs/features/src/feature_configs.rs:330][E: codex-rs/features/src/feature_configs.rs:334][E: codex-rs/features/src/feature_configs.rs:338][E: codex-rs/features/src/feature_configs.rs:342]
...
`TurnContext` 构造时保存 `configured_token_budget` 与 `use_model_token_budget_defaults`（feature 开且无 explicit settings），再对当前 `model_info` 调用 `resolve_token_budget`。因此每次 model switch 都重新解析当前模型 metadata；user explicit config 保持优先。[E: codex-rs/core/src/session/turn_context.rs:742][E: codex-rs/core/src/session/turn_context.rs:743][E: codex-rs/core/src/session/turn_context.rs:746][E: codex-rs/core/src/session/turn_context.rs:789]

## Reminder 与 fallback

sampling 完成后，turn loop 取得 base/unbuffered remaining tokens。若低于 reminder threshold，session 通过 auto-compact-window claim state 保证一个 context window 只注入一次 reminder fragment，并用 `{n_remaining}` 渲染当前剩余 token；window advance 会重置该 claim。[E: codex-rs/core/src/session/turn.rs:512][E: codex-rs/core/src/session/token_budget.rs:178][E: codex-rs/core/src/session/token_budget.rs:184][E: codex-rs/core/src/state/auto_compact_window.rs:77][E: codex-rs/core/src/state/auto_compact_window.rs:82][E: codex-rs/core/src/state/auto_compact_window.rs:87][E: codex-rs/core/src/state/session.rs:231]

Fallback buffer 只有配置了 fallback prompt 才预留，并加到 auto-compaction scope limit；模型 full context window 仍是不可越过的 hard cap。base remaining 保持按未加 buffer 的 threshold 计算，便于 reminder/fallback 判断。[E: codex-rs/core/src/config/mod.rs:1226][E: codex-rs/core/src/session/context_window.rs:83][E: codex-rs/core/src/session/context_window.rs:88][E: codex-rs/core/src/session/context_window.rs:97][E: codex-rs/core/src/session/context_window.rs:101][E: codex-rs/core/src/session/context_window.rs:105]

只有没有立即 rollover、也尚未撞到 hard/buffered limit，且 base remaining 正好为 0 时，fallback prompt 才可能通过 per-window one-shot claim 写入 history；window advance 会重置该 claim。它为 compaction threshold 后的 note-taking 留出一次 developer-message sampling 机会。[E: codex-rs/core/src/session/turn.rs:509][E: codex-rs/core/src/session/turn.rs:511][E: codex-rs/core/src/session/token_budget.rs:197][E: codex-rs/core/src/session/token_budget.rs:200][E: codex-rs/core/src/session/token_budget.rs:204][E: codex-rs/core/src/state/auto_compact_window.rs:83][E: codex-rs/core/src/state/auto_compact_window.rs:91][E: codex-rs/core/src/state/session.rs:235]
