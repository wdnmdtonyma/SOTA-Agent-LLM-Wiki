...
> Token budget 管理当前模型 context window 临近 rollover 时的 guidance、reminder 和 auto-compact fallback。它按每个 turn 选中的 model/config 生效；它不是 root agent tree 的累计 inference quota。[E: codex-rs/core/src/session/token_budget.rs:30][E: codex-rs/core/src/session/token_budget.rs:71][E: codex-rs/core/src/session/turn_context.rs:745][E: codex-rs/core/src/session/context_window.rs:83]
...
`TokenBudget` 是 under-development、default-off feature。Structured config 包含 reminder threshold/template、guidance message、auto-compact fallback prompt、fallback buffer，以及是否暴露 history/notes extension。[E: codex-rs/features/src/lib.rs:1530][E: codex-rs/features/src/feature_configs.rs:298][E: codex-rs/features/src/feature_configs.rs:303][E: codex-rs/features/src/feature_configs.rs:307][E: codex-rs/features/src/feature_configs.rs:312][E: codex-rs/features/src/feature_configs.rs:316][E: codex-rs/features/src/feature_configs.rs:320][E: codex-rs/features/src/feature_configs.rs:324]

Model catalog 可在 `ModelMessages.token_budget` 携带同一组 model-owned defaults（`ModelTokenBudgetConfig`）。[E: codex-rs/protocol/src/openai_models.rs:559][E: codex-rs/protocol/src/openai_models.rs:596]

旧的 `apply_model_defaults` 已拆成两步：`has_explicit_settings` 判断用户是否写过除开关以外的偏好；`resolve_token_budget` 在允许用 model defaults 时复制并校验 catalog 值。TOML 表里只有 `enabled` 或 `use_history_notes_extension` 不算 explicit；已解析 config 在把 `use_history_notes_extension` 清成 false 后若仍不等于 default，也算 explicit。无效 model defaults warning 后回退用户配置。[E: codex-rs/core/src/session/token_budget.rs:10][E: codex-rs/core/src/session/token_budget.rs:20][E: codex-rs/core/src/session/token_budget.rs:30][E: codex-rs/core/src/session/token_budget.rs:35][E: codex-rs/core/src/session/token_budget.rs:47][E: codex-rs/core/src/session/token_budget.rs:59]

`TurnContext` 构造时保存 `configured_token_budget` 与 `use_model_token_budget_defaults`（feature 开且无 explicit settings），再对当前 `model_info` 调用 `resolve_token_budget`。因此每次 model switch 都重新解析当前模型 metadata；user explicit config 保持优先。[E: codex-rs/core/src/session/turn_context.rs:741][E: codex-rs/core/src/session/turn_context.rs:742][E: codex-rs/core/src/session/turn_context.rs:745][E: codex-rs/core/src/session/turn_context.rs:788]

## Reminder 与 fallback

sampling 完成后，turn loop 取得 base/unbuffered remaining tokens。若低于 reminder threshold，session 通过 auto-compact-window claim state 保证一个 context window 只注入一次 reminder fragment，并用 `{n_remaining}` 渲染当前剩余 token；window advance 会重置该 claim。[E: codex-rs/core/src/session/turn.rs:464][E: codex-rs/core/src/session/token_budget.rs:88][E: codex-rs/core/src/session/token_budget.rs:94][E: codex-rs/core/src/state/auto_compact_window.rs:77][E: codex-rs/core/src/state/auto_compact_window.rs:82][E: codex-rs/core/src/state/auto_compact_window.rs:87][E: codex-rs/core/src/state/session.rs:184]

Fallback buffer 只有配置了 fallback prompt 才预留，并加到 auto-compaction scope limit；模型 full context window 仍是不可越过的 hard cap。base remaining 保持按未加 buffer 的 threshold 计算，便于 reminder/fallback 判断。[E: codex-rs/core/src/config/mod.rs:1211][E: codex-rs/core/src/session/context_window.rs:83][E: codex-rs/core/src/session/context_window.rs:88][E: codex-rs/core/src/session/context_window.rs:97][E: codex-rs/core/src/session/context_window.rs:101][E: codex-rs/core/src/session/context_window.rs:105]

只有没有立即 rollover、也尚未撞到 hard/buffered limit，且 base remaining 正好为 0 时，fallback prompt 才可能通过 per-window one-shot claim 写入 history；window advance 会重置该 claim。它为 compaction threshold 后的 note-taking 留出一次 developer-message sampling 机会。[E: codex-rs/core/src/session/turn.rs:461][E: codex-rs/core/src/session/turn.rs:463][E: codex-rs/core/src/session/token_budget.rs:107][E: codex-rs/core/src/session/token_budget.rs:110][E: codex-rs/core/src/session/token_budget.rs:114][E: codex-rs/core/src/state/auto_compact_window.rs:83][E: codex-rs/core/src/state/auto_compact_window.rs:91][E: codex-rs/core/src/state/session.rs:188]

## World-state guidance

Feature enabled、模型有 context window 且 guidance 非空时，session 将 `ContextWindowGuidanceState` 加入 typed world state。[E: codex-rs/core/src/session/world_state.rs:119][E: codex-rs/core/src/session/world_state.rs:138][E: codex-rs/core/src/session/world_state.rs:143]
...
`account/usage/read` 带 `thread_id` 时不读 account-wide profile，而是 60s timeout 调 backend `get_thread_usage`。403/404 返回 `thread_usage: None` 而不是失败；其它错误才是 internal error。此时 `summary` 与 `daily_usage_buckets` 为空。[E: codex-rs/app-server-protocol/src/protocol/common.rs:1241][E: codex-rs/app-server-protocol/src/protocol/v2/account.rs:417][E: codex-rs/app-server/src/request_processors/account_processor.rs:20][E: codex-rs/app-server/src/request_processors/account_processor.rs:1247][E: codex-rs/app-server/src/request_processors/account_processor.rs:1278][E: codex-rs/app-server/src/request_processors/account_processor.rs:1288]
...
- 在 `[features.token_budget]` 里只写 `enabled` 或 `use_history_notes_extension` 不算 explicit settings，所以 model defaults 仍可应用；任何其它 key 都会阻止 model override。[E: codex-rs/core/src/session/token_budget.rs:10][E: codex-rs/core/src/session/token_budget.rs:20]
