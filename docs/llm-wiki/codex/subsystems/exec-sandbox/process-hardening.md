...
> process hardening 是 Codex 进程 main 之前的 best-effort defense layer:Linux 关闭 dumpability、禁 core dump、移除 `LD_` env；FreeBSD/OpenBSD 禁 core dump 并移除 `LD_` env；macOS deny attach、禁 core dump、移除 `DYLD_` 和 malloc stack logging env；Windows 当前是 no-op。[E: codex-rs/process-hardening/src/lib.rs:13][E: codex-rs/process-hardening/src/lib.rs:45][E: codex-rs/process-hardening/src/lib.rs:56][E: codex-rs/process-hardening/src/lib.rs:61][E: codex-rs/process-hardening/src/lib.rs:64][E: codex-rs/process-hardening/src/lib.rs:67][E: codex-rs/process-hardening/src/lib.rs:69][E: codex-rs/process-hardening/src/lib.rs:73][E: codex-rs/process-hardening/src/lib.rs:75][E: codex-rs/process-hardening/src/lib.rs:115][E: codex-rs/process-hardening/src/lib.rs:118]
...
- 哪些 dynamic loader / diagnostic env vars 会被清掉？
...
- `codex-rs/process-hardening/src/lib.rs`: 单文件 crate，导出 `pre_main_hardening` 并按 target OS dispatch。[E: codex-rs/process-hardening/src/lib.rs:13][E: codex-rs/process-hardening/src/lib.rs:14][E: codex-rs/process-hardening/src/lib.rs:17][E: codex-rs/process-hardening/src/lib.rs:20][E: codex-rs/process-hardening/src/lib.rs:24]
...
4. macOS path 调用 `ptrace(PT_DENY_ATTACH, 0, NULL, 0)`；失败时打印错误并以 `PTRACE_DENY_ATTACH_FAILED_EXIT_CODE` 退出，该常量值为 6。[E: codex-rs/process-hardening/src/lib.rs:31][E: codex-rs/process-hardening/src/lib.rs:75][E: codex-rs/process-hardening/src/lib.rs:81]
5. macOS path 调用 `set_core_file_size_limit_to_zero()`，删除 `DYLD_` 前缀环境变量，并移除 `MallocStackLogging` 与 `MallocLogFile` 前缀环境变量。[E: codex-rs/process-hardening/src/lib.rs:84][E: codex-rs/process-hardening/src/lib.rs:89][E: codex-rs/process-hardening/src/lib.rs:94][E: codex-rs/process-hardening/src/lib.rs:95]
...
7. `remove_env_vars_with_prefix` 遍历 `env_keys_with_prefix(std::env::vars_os(), prefix)` 的结果并调用 `remove_var`；prefix matching 在 helper 中以 raw bytes 执行。[E: codex-rs/process-hardening/src/lib.rs:121][E: codex-rs/process-hardening/src/lib.rs:122][E: codex-rs/process-hardening/src/lib.rs:124][E: codex-rs/process-hardening/src/lib.rs:130]
8. Windows `apply` 当前只有 TODO 和 no-op body。[E: codex-rs/process-hardening/src/lib.rs:115][E: codex-rs/process-hardening/src/lib.rs:118]

## 设计动机与权衡

- Linux/macOS 对 attach/dump/core-limit hardening failure 都选择显式退出，退出码分别由 `PRCTL_FAILED_EXIT_CODE`、`PTRACE_DENY_ATTACH_FAILED_EXIT_CODE`、`SET_RLIMIT_CORE_FAILED_EXIT_CODE` 常量定义。[E: codex-rs/process-hardening/src/lib.rs:29][E: codex-rs/process-hardening/src/lib.rs:31][E: codex-rs/process-hardening/src/lib.rs:42][E: codex-rs/process-hardening/src/lib.rs:53][E: codex-rs/process-hardening/src/lib.rs:81][E: codex-rs/process-hardening/src/lib.rs:111]
- 删除 `LD_`/`DYLD_` env vars 是为了缩小 dynamic loader 注入面；macOS 额外删除 malloc stack logging 变量以降低 diagnostic data 泄露面。[I]
- Windows no-op 明确保留 TODO，表示 Windows hardening 尚未与 Unix path 对齐。[E: codex-rs/process-hardening/src/lib.rs:115]

## gotcha

- `remove_env_vars_with_prefix` 通过 `env_keys_with_prefix` 做 byte-prefix matching；测试覆盖了 non-UTF8 key 与 prefix filtering 行为。[E: codex-rs/process-hardening/src/lib.rs:122][E: codex-rs/process-hardening/src/lib.rs:130][E: codex-rs/process-hardening/src/lib.rs:152]
- `pre_main_hardening` 返回 `()`，调用者不能从 public API 获取哪些 hardening step 成功或失败。[E: codex-rs/process-hardening/src/lib.rs:13]
- Windows path 当前不执行防护逻辑；不要把该 crate 误读为跨平台等价 hardening。[E: codex-rs/process-hardening/src/lib.rs:115][E: codex-rs/process-hardening/src/lib.rs:118]
