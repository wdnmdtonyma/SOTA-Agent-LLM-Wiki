...
4. `try_parse_word_only_commands_sequence` 拒绝 tree-sitter parse error，只允许 `program/list/pipeline/command/command_name/word/string/raw_string/number/concatenation` 等 named node，并只允许 `&&`、`||`、`;`、`|`、quote tokens。[E: codex-rs/shell-command/src/bash.rs:29][E: codex-rs/shell-command/src/bash.rs:36]
...
- PowerShell AST parser 留作 test oracle，不再进入 production classification，避免把 subprocess parser 当成安全证明。[E: codex-rs/shell-command/src/command_safety/mod.rs:5]
