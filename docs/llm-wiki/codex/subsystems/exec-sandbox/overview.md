...
`should_require_platform_sandbox` 在 managed network 生效时返回 true，在网络关闭且不是 `ExternalSandbox` 时返回 true，在网络开启且 filesystem 是 restricted 且不存在 `full_disk_write_access` 时也返回 true。[E: codex-rs/sandboxing/src/policy_transforms.rs:512][E: codex-rs/sandboxing/src/policy_transforms.rs:517][E: codex-rs/sandboxing/src/policy_transforms.rs:521][E: codex-rs/sandboxing/src/policy_transforms.rs:522][E: codex-rs/sandboxing/src/policy_transforms.rs:528][E: codex-rs/sandboxing/src/policy_transforms.rs:529]
...
- `codex-rs/sandboxing/src/policy_transforms.rs`: normalizes additional permissions and, before backend transform, merges `PermissionProfile`, filesystem policy, network policy, and additional permissions into effective policies.[E: codex-rs/sandboxing/src/policy_transforms.rs:448][E: codex-rs/sandboxing/src/policy_transforms.rs:481][E: codex-rs/sandboxing/src/policy_transforms.rs:496]
- `codex-rs/sandboxing/src/lib.rs`: 按 `cfg` 声明 Linux bubblewrap module、policy transforms、macOS Seatbelt module，重导出 manager types，并把 sandbox transform errors 映射到 protocol errors。[E: codex-rs/sandboxing/src/lib.rs:1][E: codex-rs/sandboxing/src/lib.rs:4][E: codex-rs/sandboxing/src/lib.rs:6][E: codex-rs/sandboxing/src/lib.rs:8][E: codex-rs/sandboxing/src/lib.rs:18][E: codex-rs/sandboxing/src/lib.rs:21][E: codex-rs/sandboxing/src/lib.rs:48]
...
- `effective_permission_profile`: `policy_transforms` 里的归一化函数先从 `PermissionProfile` 派生 runtime filesystem/network policies，再分别应用 `effective_file_system_sandbox_policy` 和 `effective_network_sandbox_policy`，最后用原 enforcement 重建 effective `PermissionProfile`。[E: codex-rs/sandboxing/src/policy_transforms.rs:496][E: codex-rs/sandboxing/src/policy_transforms.rs:500][E: codex-rs/sandboxing/src/policy_transforms.rs:501][E: codex-rs/sandboxing/src/policy_transforms.rs:503][E: codex-rs/sandboxing/src/policy_transforms.rs:505][E: codex-rs/sandboxing/src/manager.rs:343][E: codex-rs/sandboxing/src/manager.rs:351]
...
- `Auto` 模式把“需要 OS sandbox”的判断集中在 `should_require_platform_sandbox`，因为 managed network、禁用 network、restricted filesystem 都需要平台能力补足普通 spawn 无法表达的限制。[E: codex-rs/sandboxing/src/policy_transforms.rs:512][E: codex-rs/sandboxing/src/policy_transforms.rs:517][E: codex-rs/sandboxing/src/policy_transforms.rs:521][E: codex-rs/sandboxing/src/policy_transforms.rs:528][E: codex-rs/sandboxing/src/policy_transforms.rs:529]
...
- Linux helper `arg0` override 不是用户命令 argv[0]，而是为了让 helper 以 `codex-linux-sandbox` 这个 argv0 分支重新进入 arg0 dispatch。[E: codex-rs/sandboxing/src/manager.rs:410][E: codex-rs/sandboxing/src/manager.rs:705][E: codex-rs/sandboxing/src/manager.rs:706][E: codex-rs/sandboxing/src/manager.rs:709]
