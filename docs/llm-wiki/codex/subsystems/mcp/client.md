...
Codex Apps 的 explicit hard refresh 先等 dirty refresh，再独占 refresh gate，使用 `replace_fresh` 发布新 clients 后读取完整 refreshed catalog；该返回值与 hard-refresh 完成时的 publication 一致，但后续 ordinary call 仍按 call-time current binding 解析。[E: codex-rs/core/src/session/mcp.rs:261][E: codex-rs/core/src/session/mcp.rs:316][E: codex-rs/codex-mcp/src/runtime.rs:263][E: codex-rs/codex-mcp/src/runtime.rs:264][E: codex-rs/core/src/session/mcp_runtime.rs:66][E: codex-rs/core/src/session/mcp_runtime.rs:69]

## 3 Step binding 与 call authority

每个 step 在 `mcp_runtime_for_step` 比较 selected capability roots，必要时标 dirty、完成 refresh，然后调用 `current_binding_with_required_servers`；拿不到 published binding 才返回空 binding。[E: codex-rs/core/src/session/mcp.rs:326][E: codex-rs/core/src/session/mcp.rs:340][E: codex-rs/core/src/session/mcp.rs:358][E: codex-rs/core/src/session/mcp.rs:367][E: codex-rs/core/src/session/mcp.rs:373]

`McpBinding::tools` 是冻结目录；step planner 据此创建只保存 `ToolInfo/spec` 的 `McpHandler`，handler 不保存 step binding 或 prepared call。[E: codex-rs/codex-mcp/src/binding.rs:81][E: codex-rs/core/src/tools/handlers/mcp.rs:51][E: codex-rs/core/src/tools/handlers/mcp.rs:58][E: codex-rs/core/src/tools/handlers/mcp.rs:84]

真正调用时，`McpHandler::handle_call` 走 `Session::prepare_mcp_call`：先 `refresh_mcp_if_dirty()`，再从 runtime 的 `current_binding_for_call(server)` 按 `(server, tool)` 查 `PreparedMcpCall`，然后把这个 call-time handle 传给 `handle_mcp_tool_call`。若 tool 已从最新 catalog 消失，调用直接返回 “not available to the model”；若仍存在，approval metadata、config、plugin provenance 与 exact client 来自这个 call-time binding，而非广告时 binding。[E: codex-rs/core/src/tools/handlers/mcp.rs:180][E: codex-rs/core/src/session/mcp_runtime.rs:61][E: codex-rs/core/src/session/mcp_runtime.rs:66][E: codex-rs/core/src/session/mcp_runtime.rs:69][E: codex-rs/core/src/session/mcp_runtime.rs:71][E: codex-rs/core/src/mcp_tool_call.rs:157][E: codex-rs/core/src/mcp_tool_call.rs:166]

真正发送前，call-time `PreparedMcpCall::call_with_preparation` 获取 catalog revision read guard：prepared call 建立后、guard 获取前 revision 已变化则拒绝；匹配时在 guard 持有期间完成不可逆参数准备与 exact-client call，因此 catalog replacement 被阻塞到发送结束。这个 guard 保护 “call-time prepare → send” 窗口，不把 model 广告时 binding 延长到执行时。[E: codex-rs/codex-mcp/src/binding.rs:313][E: codex-rs/codex-mcp/src/binding.rs:328][E: codex-rs/codex-mcp/src/binding.rs:329][E: codex-rs/codex-mcp/src/binding.rs:366][I]
...
- model-advertised schema 来自 step binding，但 ordinary MCP execution follow call-time current binding；因此 refresh 后同名 tool 的 execution authority 可以变化，删除则得到 unavailable。不要把 revision guard描述成整步冻结。[E: codex-rs/core/src/session/mcp_runtime.rs:66][E: codex-rs/core/src/mcp_tool_call.rs:157][I]
- tool-facing list/read resource handlers 从 step binding 进入；capture 只把 exact ready clients 插入 client map。指定 server 不在这组 step-ready clients 中时会 live fallback；extension-facing `McpResourceClient` 每次 follow latest runtime；ordinary MCP call 则在执行前主动 refresh 后取 current binding。[E: codex-rs/codex-mcp/src/connection_manager/tool_catalog.rs:255][E: codex-rs/codex-mcp/src/binding.rs:109][E: codex-rs/core/src/session/mcp_runtime.rs:66][E: codex-rs/codex-mcp/src/resource_client.rs:200][I]
