...
- `codex-rs/codex-mcp/src/mcp/auth.rs`: login support gate, scope resolution, discovered-scope retry policy, auth-status aggregation.[E: codex-rs/codex-mcp/src/mcp/auth.rs:91][E: codex-rs/codex-mcp/src/mcp/auth.rs:152][E: codex-rs/codex-mcp/src/mcp/auth.rs:186][E: codex-rs/codex-mcp/src/mcp/auth.rs:188]

## Auth status

- If a streamable HTTP config uses `bearer_token_env_var`, status is `BearerToken` without probing OAuth.[E: codex-rs/rmcp-client/src/auth_status.rs:152]
- Static or env-derived default HTTP headers containing `Authorization` also classify as `BearerToken`.[E: codex-rs/rmcp-client/src/auth_status.rs:156][E: codex-rs/rmcp-client/src/auth_status.rs:157][E: codex-rs/rmcp-client/src/auth_status.rs:158]
- Stored usable OAuth tokens classify as `OAuth`; stored but unusable/refresh-needed-without-valid-refresh-token classify as `LoggedOut(Reauthentication)`, which maps to protocol `NotLoggedIn`; missing tokens fall through to discovery.[E: codex-rs/rmcp-client/src/auth_status.rs:59][E: codex-rs/rmcp-client/src/auth_status.rs:161][E: codex-rs/rmcp-client/src/auth_status.rs:163][E: codex-rs/rmcp-client/src/oauth.rs:156][E: codex-rs/rmcp-client/src/oauth.rs:206]
- Discovery returns `LoggedOut(Login)` when OAuth metadata is found, `Unsupported` when none is found, and debug-logs discovery errors before returning `Unsupported`; `LoggedOut` maps to protocol `NotLoggedIn`.[E: codex-rs/rmcp-client/src/auth_status.rs:59][E: codex-rs/rmcp-client/src/auth_status.rs:175][E: codex-rs/rmcp-client/src/auth_status.rs:182][E: codex-rs/rmcp-client/src/auth_status.rs:183][E: codex-rs/rmcp-client/src/auth_status.rs:184]

## Login support and scopes

- `oauth_login_support` supports only `StreamableHttp` transports and rejects configs that use bearer-token env vars.[E: codex-rs/codex-mcp/src/mcp/auth.rs:91][E: codex-rs/codex-mcp/src/mcp/auth.rs:119][E: codex-rs/codex-mcp/src/mcp/auth.rs:129]
...
- Credential identity is environment-aware: local servers retain legacy names (with `local:` escaping for names that already look executor-owned), while non-local servers use `executor:<base64url environment id>:<base64url server name>`。The fallback file also stores an `executor_owned` marker；executor lookups require it and host lookups reject it, so legacy host credentials do not silently authorize an executor server。[E: codex-rs/config/src/mcp_types.rs:243][E: codex-rs/config/src/mcp_types.rs:250][E: codex-rs/rmcp-client/src/oauth.rs:672][E: codex-rs/rmcp-client/src/oauth.rs:681][E: codex-rs/rmcp-client/src/oauth.rs:685][E: codex-rs/rmcp-client/src/oauth.rs:689]
- `StoredOAuthTokens` stores server name, URL, client id, wrapped token response, and optional `expires_at` timestamp.[E: codex-rs/rmcp-client/src/oauth.rs:89]
