...
| EQ / Event Queue | wiki 对 core 输出队列的简称[I]；源码 `Event` 注释为 "Event Queue Entry - events from agent"，字段是 correlated submission id 与 `EventMsg` payload。 | [E: codex-rs/protocol/src/protocol.rs:1346][E: codex-rs/protocol/src/protocol.rs:1348][E: codex-rs/protocol/src/protocol.rs:1350] |
| `Op` | 客户端提交给 core 的 `#[non_exhaustive]` operation enum，当前 29 个变体；turn 输入走 `TurnInput` / `RecoverTurn`，不再有 `Op::UserInput`。 | [E: codex-rs/protocol/src/protocol.rs:599][E: codex-rs/protocol/src/protocol.rs:600] |
| `EventMsg` | agent 发给客户端的 response event tagged enum，当前 83 个变体；serde 与 strum 使用 snake_case，TS 只声明 `tag = "type"`。ServerNotification 另有 84 个变体（83 个 `=> "wire"` + `AccountLoginCompleted`），不是同一份 enum。 | [E: codex-rs/protocol/src/protocol.rs:1361][E: codex-rs/protocol/src/protocol.rs:1363][E: codex-rs/protocol/src/protocol.rs:1364][E: codex-rs/app-server-protocol/src/protocol/common.rs:1907][E: codex-rs/app-server-protocol/src/protocol/common.rs:2025] |
...
| `TurnContext` | 单轮上下文，聚合 sub/trace id、realtime/code-mode state、config/auth/model/provider/telemetry、environment、instructions、collaboration/personality、permission/network/sandbox、dynamic tools 和 extension/timing state。 | [E: codex-rs/core/src/session/turn_context.rs:282][E: codex-rs/core/src/session/turn_context.rs:283][E: codex-rs/core/src/session/turn_context.rs:285][E: codex-rs/core/src/session/turn_context.rs:289][E: codex-rs/core/src/session/turn_context.rs:304][E: codex-rs/core/src/session/turn_context.rs:327][E: codex-rs/core/src/session/turn_context.rs:329][E: codex-rs/core/src/session/turn_context.rs:330] |
...
| `Session` / `SessionIo` | `Session` 拥有运行时状态；`SessionIo` 单独持有 submission sender、event receiver、agent-status watch 与 session-loop completion future。`Session::spawn` 返回二者，外层 `CodexThread` 再把它们组合成客户端句柄。 | [E: codex-rs/core/src/session/mod.rs:399][E: codex-rs/core/src/session/mod.rs:502][E: codex-rs/core/src/codex_thread.rs:179] |
...
| `SessionMeta` | session-level rollout metadata；除 base instructions 外，还记录 `history_mode`、可继承的 `history_base`、subagent 自有投影起点和 initial context-window identity。 | [E: codex-rs/protocol/src/protocol.rs:3061][E: codex-rs/protocol/src/protocol.rs:3115][E: codex-rs/protocol/src/protocol.rs:3118][E: codex-rs/protocol/src/protocol.rs:3124] |
| `ThreadHistoryMode` | thread history 的兼容模式：默认 `Legacy`，或使用 ordinal/SQLite 投影的 `Paginated`。 | [E: codex-rs/protocol/src/protocol.rs:784][E: codex-rs/protocol/src/protocol.rs:786][E: codex-rs/protocol/src/protocol.rs:787] |
...
| `ResponseItem` | model/history response item 的 18-variant union；`FunctionCall` 可带 encrypted function args，可携 metadata 的 variants 复用 internal passthrough，其中 attempted-tool calls 是 warehouse-only，不进入 public schema/TS。 | [E: codex-rs/protocol/src/models.rs:999][E: codex-rs/protocol/src/models.rs:1061][E: codex-rs/protocol/src/models.rs:1075][E: codex-rs/protocol/src/models.rs:949][E: codex-rs/protocol/src/models.rs:974] |
| compaction | `ContextCompactedEvent` is an empty event struct; `CompactedItem` stores summary message, optional replacement history and optional context-window identity fields。 | [E: codex-rs/protocol/src/protocol.rs:2146][E: codex-rs/history/src/lib.rs:190][E: codex-rs/history/src/lib.rs:191][E: codex-rs/history/src/lib.rs:192] |
...
| seatbelt | macOS sandbox marker sets `CODEX_SANDBOX=seatbelt`; non-macOS seatbelt transform errors with "seatbelt sandbox is only available on macOS"。 | [E: codex-rs/core/src/sandboxing/mod.rs:182][E: codex-rs/sandboxing/src/lib.rs:84] |
...
| Term | 中文解释 | Evidence |
|---|---|---|
| feature flag | `Feature` 是 config-toggled identity enum；`FEATURES` 把 142 个 ids 映射到 keys/stages/defaults，removed compatibility flags 仍留在 registry。`UnifiedExec` 默认全平台 `true`。 | [E: codex-rs/features/src/lib.rs:93][E: codex-rs/features/src/lib.rs:904][E: codex-rs/features/src/lib.rs:943][E: codex-rs/features/src/lib.rs:946] |
