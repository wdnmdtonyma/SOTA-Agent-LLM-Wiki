...
| Term | 中文解释 | Evidence |
|---|---|---|
| SQ / Submission Queue | wiki 对 core 输入队列的简称[I]；源码 `Submission` struct 的字段是 correlation `id`、`op`、client user message id、W3C trace carrier 与 optional core-generated parent turn id。 | [E: codex-rs/protocol/src/protocol.rs:176][E: codex-rs/protocol/src/protocol.rs:178][E: codex-rs/protocol/src/protocol.rs:180][E: codex-rs/protocol/src/protocol.rs:182][E: codex-rs/protocol/src/protocol.rs:184][E: codex-rs/protocol/src/protocol.rs:186] |
| `parent_turn_id` | core 内部 submission provenance：标识直接发起该 submission 的 parent turn；submission loop 把它交给 user-input 或 inter-agent communication handler，公共 client submission path 则清空该值。[I] | [E: codex-rs/protocol/src/protocol.rs:186][E: codex-rs/core/src/session/mod.rs:794][E: codex-rs/core/src/session/mod.rs:802][E: codex-rs/core/src/session/handlers.rs:769][E: codex-rs/core/src/session/handlers.rs:775][E: codex-rs/core/src/session/handlers.rs:784][E: codex-rs/core/src/session/handlers.rs:789] |
| EQ / Event Queue | wiki 对 core 输出队列的简称[I]；源码 `Event` 注释为 "Event Queue Entry - events from agent"，字段是 correlated submission id 与 `EventMsg` payload。 | [E: codex-rs/protocol/src/protocol.rs:1269][E: codex-rs/protocol/src/protocol.rs:1272] |
| `Op` | 客户端提交给 core 的 operation enum；current variants start with interrupt/background-terminal/realtime/user-input families and are documented under `Submission.op`。 | [E: codex-rs/protocol/src/protocol.rs:530] |
| `EventMsg` | agent 发给客户端的 response event tagged enum；serde/TS/strum tag 都使用 snake_case。 | [E: codex-rs/protocol/src/protocol.rs:1285][E: codex-rs/protocol/src/protocol.rs:1286][E: codex-rs/protocol/src/protocol.rs:1287][E: codex-rs/protocol/src/protocol.rs:1288] |
| Turn | wiki 对一次模型/工具工作单元的简称[I]；源码 `TurnContext` 注释为 single turn context，`SessionTask` 注释为驱动 session turn 的 async task。 | [E: codex-rs/core/src/session/turn_context.rs:98][E: codex-rs/core/src/tasks/mod.rs:184] |
| `TurnContext` | 单轮上下文，聚合 sub/trace id、realtime/code-mode state、config/auth/model/provider/telemetry、environment、instructions、collaboration/personality、permission/network/sandbox、dynamic tools 和 extension/timing state。 | [E: codex-rs/core/src/session/turn_context.rs:99][E: codex-rs/core/src/session/turn_context.rs:100][E: codex-rs/core/src/session/turn_context.rs:103][E: codex-rs/core/src/session/turn_context.rs:104][E: codex-rs/core/src/session/turn_context.rs:107][E: codex-rs/core/src/session/turn_context.rs:113][E: codex-rs/core/src/session/turn_context.rs:115][E: codex-rs/core/src/session/turn_context.rs:124][E: codex-rs/core/src/session/turn_context.rs:128][E: codex-rs/core/src/session/turn_context.rs:130][E: codex-rs/core/src/session/turn_context.rs:131][E: codex-rs/core/src/session/turn_context.rs:136][E: codex-rs/core/src/session/turn_context.rs:138][E: codex-rs/core/src/session/turn_context.rs:139] |
| `SessionTask` | core 内部驱动 turn/workflow 的 async trait，要求 `kind`、`span_name`、`run`，并提供 optional `abort`。 | [E: codex-rs/core/src/tasks/mod.rs:184][E: codex-rs/core/src/tasks/mod.rs:187][E: codex-rs/core/src/tasks/mod.rs:190][E: codex-rs/core/src/tasks/mod.rs:202][E: codex-rs/core/src/tasks/mod.rs:215] |
...
| `McpBinding` | 单次 model sampling request 捕获的不可变 MCP catalog/authority；包含 exact connection set、clients、config、tools 与 prepared calls。 | [E: codex-rs/codex-mcp/src/binding.rs:30][E: codex-rs/codex-mcp/src/binding.rs:30][E: codex-rs/codex-mcp/src/binding.rs:36][E: codex-rs/core/src/session/step_context.rs:20] |

## Persistence / history

| Term | 中文解释 | Evidence |
|---|---|---|
| rollout | Codex 会话 JSONL transcript / replay persistence 的 wiki 术语[I]；recorder 把记录序列化为逐行 JSON，policy 会按 `ThreadHistoryMode` 决定持久化 canonical `ItemCompleted` 还是 legacy event。 | [E: codex-rs/rollout/src/recorder.rs:1922][E: codex-rs/rollout/src/recorder.rs:1923][E: codex-rs/rollout/src/recorder.rs:1924][E: codex-rs/rollout/src/recorder.rs:1925][E: codex-rs/rollout/src/policy.rs:9][E: codex-rs/rollout/src/policy.rs:14][E: codex-rs/rollout/src/policy.rs:87][E: codex-rs/rollout/src/policy.rs:89][E: codex-rs/rollout/src/policy.rs:92][E: codex-rs/rollout/src/policy.rs:108][E: codex-rs/rollout/src/policy.rs:119] |
| `RolloutItem` | rollout item tagged union，包含 `SessionMeta`、`ResponseItem`、inter-agent communication、inter-agent communication metadata、compacted item、turn context item、world state item 和 event msg。 | [E: codex-rs/protocol/src/protocol.rs:3207][E: codex-rs/protocol/src/protocol.rs:3208][E: codex-rs/protocol/src/protocol.rs:3209][E: codex-rs/protocol/src/protocol.rs:3211][E: codex-rs/protocol/src/protocol.rs:3213][E: codex-rs/protocol/src/protocol.rs:3216][E: codex-rs/protocol/src/protocol.rs:3217][E: codex-rs/protocol/src/protocol.rs:3218][E: codex-rs/protocol/src/protocol.rs:3219] |
| `RolloutLine` | single JSONL line schema：timestamp、paginated history 使用的 optional ordinal，以及 flattened rollout item。 | [E: codex-rs/protocol/src/protocol.rs:3402][E: codex-rs/protocol/src/protocol.rs:3405][E: codex-rs/protocol/src/protocol.rs:3406] |
| `SessionMeta` | session-level rollout metadata；除 base instructions 外，还记录 `history_mode`、可继承的 `history_base`、subagent 自有投影起点和 initial context-window identity。 | [E: codex-rs/protocol/src/protocol.rs:3120][E: codex-rs/protocol/src/protocol.rs:3138] |
| `ThreadHistoryMode` | thread history 的兼容模式：默认 `Legacy`，或使用 ordinal/SQLite 投影的 `Paginated`。 | [E: codex-rs/protocol/src/protocol.rs:697][E: codex-rs/protocol/src/protocol.rs:700][E: codex-rs/protocol/src/protocol.rs:703][E: codex-rs/protocol/src/protocol.rs:706] |
...
| `ResponseItem` | model/history response item 的 17-variant union；`FunctionCall` 可带 encrypted function args，可携 metadata 的 variants 复用 internal passthrough，其中 attempted-tool calls 是 warehouse-only，不进入 public schema/TS。 | [E: codex-rs/protocol/src/models.rs:811][E: codex-rs/protocol/src/models.rs:813][E: codex-rs/protocol/src/models.rs:875][E: codex-rs/protocol/src/models.rs:889][E: codex-rs/protocol/src/models.rs:791][E: codex-rs/protocol/src/models.rs:792][E: codex-rs/protocol/src/models.rs:793][E: codex-rs/protocol/src/models.rs:794][E: codex-rs/protocol/src/models.rs:1044][I] |
| compaction | `ContextCompactedEvent` is an empty event struct; `CompactedItem` stores summary message, optional replacement history and optional context-window identity fields。 | [E: codex-rs/protocol/src/protocol.rs:1992][E: codex-rs/protocol/src/protocol.rs:3241][E: codex-rs/protocol/src/protocol.rs:3242][E: codex-rs/protocol/src/protocol.rs:3244][E: codex-rs/protocol/src/protocol.rs:3247][E: codex-rs/protocol/src/protocol.rs:3250][E: codex-rs/protocol/src/protocol.rs:3253][E: codex-rs/protocol/src/protocol.rs:3256] |
...
| managed network proxy | proxy code lists URL env keys and applies managed HTTP/WS/no-proxy overrides plus `CODEX_NETWORK_PROXY_ACTIVE` and local-binding marker。 | [E: codex-rs/network-proxy/src/proxy.rs:502][E: codex-rs/network-proxy/src/proxy.rs:503][E: codex-rs/network-proxy/src/proxy.rs:522][E: codex-rs/network-proxy/src/proxy.rs:523][E: codex-rs/network-proxy/src/proxy.rs:672][E: codex-rs/network-proxy/src/proxy.rs:674][E: codex-rs/network-proxy/src/proxy.rs:683][E: codex-rs/network-proxy/src/proxy.rs:704][E: codex-rs/network-proxy/src/proxy.rs:708][E: codex-rs/network-proxy/src/proxy.rs:581] |
| exec-server environment | `CODEX_EXEC_SERVER_URL` is the legacy environment selector; `none` disables default environment access, and `Environment::create_inner()` normalizes the raw value before choosing remote/local environment。 | [E: codex-rs/exec-server/src/environment.rs:45][E: codex-rs/exec-server/src/environment.rs:708][E: codex-rs/exec-server/src/environment.rs:713][E: codex-rs/exec-server/src/environment.rs:714][E: codex-rs/exec-server/src/environment.rs:720][E: codex-rs/exec-server/src/environment.rs:661][E: codex-rs/exec-server/src/environment.rs:729] |
| `PathUri` | 以 `url::Url` 为内核的 host-neutral absolute path URI；跨 executor/host 的协议与 approval identity 用它代替平台原生 path serialization。 | [E: codex-rs/utils/path-uri/src/lib.rs:59] |
| `HttpClientFactory` / `RouteAwareClientPool` | factory 按目标 URL 解析 direct/system/managed outbound route；pool 为请求及每次 redirect 重新选路并缓存 bounded route-specific clients。 | [E: codex-rs/http-client/src/outbound_proxy.rs:142][E: codex-rs/http-client/src/route_aware_client_pool.rs:50][E: codex-rs/http-client/src/route_aware_client_pool.rs:260] |

## Feature / collaboration / UI shorthand

| Term | 中文解释 | Evidence |
|---|---|---|
| feature flag | `Feature` 是 config-toggled identity enum；`FEATURES` 把 102 个 ids 映射到 keys/stages/defaults，removed compatibility flags 仍留在 registry。本轮新增 attempted-tool metadata、recommended plugins 与 in-app updates。[I] | [E: codex-rs/features/src/lib.rs:84][E: codex-rs/features/src/lib.rs:838][E: codex-rs/features/src/lib.rs:895][E: codex-rs/features/src/lib.rs:1175][E: codex-rs/features/src/lib.rs:1205] |
| code mode | feature `CodeMode` 开启 JavaScript Code Mode；app-server 进程默认选择 local host transport，也可用 `--code-mode-host ws://...`/`wss://...` 共享远端 host。 | [E: codex-rs/features/src/lib.rs:98][E: codex-rs/app-server/src/code_mode_host.rs:6][E: codex-rs/app-server/src/code_mode_host.rs:9][E: codex-rs/app-server/src/code_mode_host.rs:13][E: codex-rs/app-server/src/code_mode_host.rs:18][E: codex-rs/app-server/src/code_mode_host.rs:21][E: codex-rs/app-server/src/code_mode_host.rs:23] |
| review mode | `ReviewTask` 是 `SessionTask` with `TaskKind::Review`，span 为 `session_task.review`；它启动 sub-codex review conversation，并在未取消时退出 review mode。 | [E: codex-rs/core/src/tasks/review.rs:43][E: codex-rs/core/src/tasks/review.rs:45][E: codex-rs/core/src/tasks/review.rs:49][E: codex-rs/core/src/tasks/review.rs:73][E: codex-rs/core/src/tasks/review.rs:81][E: codex-rs/core/src/tasks/review.rs:84][E: codex-rs/core/src/tasks/review.rs:85] |
| app-server | workspace includes `app-server` and adjacent app-server crates，包括新加入的 no-op protocol derive macro crate；app-server CLI accepts a transport endpoint URL via `--listen`。 | [E: codex-rs/Cargo.toml:11][E: codex-rs/Cargo.toml:12][E: codex-rs/Cargo.toml:13][E: codex-rs/Cargo.toml:14][E: codex-rs/Cargo.toml:15][E: codex-rs/Cargo.toml:16][E: codex-rs/app-server/src/main.rs:22][E: codex-rs/app-server/src/main.rs:32] |
| TUI | workspace includes `tui`; TUI session logging is enabled through `CODEX_TUI_RECORD_SESSION` truthy values。 | [E: codex-rs/Cargo.toml:92][E: codex-rs/tui/src/session_log.rs:84][E: codex-rs/tui/src/session_log.rs:85][E: codex-rs/tui/src/session_log.rs:86] |
