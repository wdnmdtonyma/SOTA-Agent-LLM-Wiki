...
| Term | 中文解释 | Evidence |
|---|---|---|
| SQ / Submission Queue | wiki 对 core 输入队列的简称[I]；源码 `Submission` struct 的字段是 correlation `id`、`op`、optional W3C `trace`、optional core-generated `parent_turn_id` 与 `root_turn_id`。`client_user_message_id` 已从 `Submission` 移除，用户消息 id 现在挂在 `TurnInput::UserInput.client_id`。 | [E: codex-rs/protocol/src/protocol.rs:190][E: codex-rs/protocol/src/protocol.rs:192][E: codex-rs/protocol/src/protocol.rs:194][E: codex-rs/protocol/src/protocol.rs:196][E: codex-rs/protocol/src/turn_input.rs:19][E: codex-rs/protocol/src/turn_input.rs:21] |
...
| `EventMsg` | agent 发给客户端的 response event tagged enum，当前 83 个变体；serde/TS/strum tag 都使用 snake_case。 | [E: codex-rs/protocol/src/protocol.rs:1334][E: codex-rs/protocol/src/protocol.rs:1337] |
...
| `TurnContext` | 单轮上下文，聚合 sub/trace id、realtime/code-mode state、config/auth/model/provider/telemetry、environment、instructions、collaboration/personality、permission/network/sandbox、dynamic tools 和 extension/timing state。 | [E: codex-rs/core/src/session/turn_context.rs:99][E: codex-rs/core/src/session/turn_context.rs:99][E: codex-rs/core/src/session/turn_context.rs:103][E: codex-rs/core/src/session/turn_context.rs:104][E: codex-rs/core/src/session/turn_context.rs:107][E: codex-rs/core/src/session/turn_context.rs:109][E: codex-rs/core/src/session/turn_context.rs:113][E: codex-rs/core/src/session/turn_context.rs:125][E: codex-rs/core/src/session/turn_context.rs:128][E: codex-rs/core/src/session/turn_context.rs:132][E: codex-rs/core/src/session/turn_context.rs:132][E: codex-rs/core/src/session/turn_context.rs:136][E: codex-rs/core/src/session/turn_context.rs:138][E: codex-rs/core/src/session/turn_context.rs:139] |
| `SessionTask` | core 内部驱动 turn/workflow 的 async trait，要求 `kind`、`span_name`、`run`，并提供 optional `abort`。 | [E: codex-rs/core/src/tasks/mod.rs:197][E: codex-rs/core/src/tasks/mod.rs:197][E: codex-rs/core/src/tasks/mod.rs:197][E: codex-rs/core/src/tasks/mod.rs:210][E: codex-rs/core/src/tasks/mod.rs:216] |
| `Session` / `SessionIo` | `Session` 拥有运行时状态；`SessionIo` 单独持有 submission sender、event receiver、agent-status watch 与 session-loop completion future。`Session::spawn` 返回二者，外层 `CodexThread` 再把它们组合成客户端句柄。 | [E: codex-rs/core/src/session/mod.rs:392][E: codex-rs/core/src/session/mod.rs:488][E: codex-rs/core/src/session/mod.rs:514] |
| `McpRuntime` | thread-owned MCP publication owner；用 `ArcSwap` 原子替换包含 connection set、config 和 auth context 的 published runtime。 | [E: codex-rs/codex-mcp/src/runtime.rs:64][E: codex-rs/codex-mcp/src/runtime.rs:85] |
| `McpBinding` | 单次 model sampling request 捕获的不可变 MCP catalog/authority；包含 exact connection set、clients、config、tools 与 prepared calls。 | [E: codex-rs/codex-mcp/src/binding.rs:29][E: codex-rs/codex-mcp/src/binding.rs:29][E: codex-rs/codex-mcp/src/binding.rs:36][E: codex-rs/core/src/session/step_context.rs:22] |

## Persistence / history

| Term | 中文解释 | Evidence |
|---|---|---|
| rollout | Codex 会话 JSONL transcript / replay persistence 的 wiki 术语[I]；recorder 把记录序列化为逐行 JSON，policy 会按 `ThreadHistoryMode` 决定持久化 canonical `ItemCompleted` 还是 legacy event。 | [E: codex-rs/rollout/src/recorder.rs:1922][E: codex-rs/rollout/src/recorder.rs:1923][E: codex-rs/rollout/src/recorder.rs:1923][E: codex-rs/rollout/src/recorder.rs:1925][E: codex-rs/rollout/src/policy.rs:10][E: codex-rs/rollout/src/policy.rs:14][E: codex-rs/rollout/src/policy.rs:84][E: codex-rs/rollout/src/policy.rs:89][E: codex-rs/rollout/src/policy.rs:96][E: codex-rs/rollout/src/policy.rs:109][E: codex-rs/rollout/src/policy.rs:119] |
| `RolloutItem` | rollout item tagged union，现定义在 `history` crate：`SessionMeta`、`ResponseItem`、inter-agent communication、inter-agent communication metadata、compacted item、turn context item、world state item、`SecurityRiskScore` 和 event msg。 | [E: codex-rs/history/src/lib.rs:94][E: codex-rs/history/src/lib.rs:95][E: codex-rs/history/src/lib.rs:95][E: codex-rs/history/src/lib.rs:95][E: codex-rs/history/src/lib.rs:95][E: codex-rs/history/src/lib.rs:100][E: codex-rs/history/src/lib.rs:100][E: codex-rs/history/src/lib.rs:101][E: codex-rs/history/src/lib.rs:102][E: codex-rs/history/src/lib.rs:103] |
...
| `SessionMeta` | session-level rollout metadata；除 base instructions 外，还记录 `history_mode`、可继承的 `history_base`、subagent 自有投影起点和 initial context-window identity。 | [E: codex-rs/protocol/src/protocol.rs:2864] |
| `ThreadHistoryMode` | thread history 的兼容模式：默认 `Legacy`，或使用 ordinal/SQLite 投影的 `Paginated`。 | [E: codex-rs/protocol/src/protocol.rs:706][E: codex-rs/protocol/src/protocol.rs:711][E: codex-rs/protocol/src/protocol.rs:711][E: codex-rs/protocol/src/protocol.rs:711] |
...
| compaction | `ContextCompactedEvent` is an empty event struct; `CompactedItem` stores summary message, optional replacement history and optional context-window identity fields。 | [E: codex-rs/protocol/src/protocol.rs:1990][E: codex-rs/protocol/src/protocol.rs:3241][E: codex-rs/protocol/src/protocol.rs:3242][E: codex-rs/protocol/src/protocol.rs:3244][E: codex-rs/protocol/src/protocol.rs:3244][E: codex-rs/protocol/src/protocol.rs:3250][E: codex-rs/protocol/src/protocol.rs:3253][E: codex-rs/protocol/src/protocol.rs:3254] |
...
| Term | 中文解释 | Evidence |
|---|---|---|
| sandbox mode | config-facing `SandboxMode`: `read-only`, `workspace-write`, `danger-full-access`; default variant is `ReadOnly`。 | [E: codex-rs/protocol/src/config_types.rs:86][E: codex-rs/protocol/src/config_types.rs:88][E: codex-rs/protocol/src/config_types.rs:88][E: codex-rs/protocol/src/config_types.rs:92][E: codex-rs/protocol/src/config_types.rs:95] |
| network sandbox | `NetworkSandboxPolicy` has `Restricted` and `Enabled`; `is_enabled()` checks the enabled variant。 | [E: codex-rs/protocol/src/permissions.rs:78][E: codex-rs/protocol/src/permissions.rs:79][E: codex-rs/protocol/src/permissions.rs:79][E: codex-rs/protocol/src/permissions.rs:88][E: codex-rs/protocol/src/permissions.rs:88] |
...
| exec-server environment | `CODEX_EXEC_SERVER_URL` is the legacy environment selector; `none` disables default environment access, and `Environment::create_inner()` normalizes the raw value before choosing remote/local environment。 | [E: codex-rs/exec-server/src/environment.rs:45][E: codex-rs/exec-server/src/environment.rs:707][E: codex-rs/exec-server/src/environment.rs:713][E: codex-rs/exec-server/src/environment.rs:713][E: codex-rs/exec-server/src/environment.rs:722][E: codex-rs/exec-server/src/environment.rs:663][E: codex-rs/exec-server/src/environment.rs:727] |
| `PathUri` | 以 `url::Url` 为内核的 host-neutral absolute path URI；跨 executor/host 的协议与 approval identity 用它代替平台原生 path serialization。 | [E: codex-rs/utils/path-uri/src/lib.rs:62] |
| `HttpClientFactory` / `RouteAwareClientPool` | factory 按目标 URL 解析 direct/system/managed outbound route；pool 为请求及每次 redirect 重新选路并缓存 bounded route-specific clients。 | [E: codex-rs/http-client/src/outbound_proxy.rs:142][E: codex-rs/http-client/src/route_aware_client_pool.rs:49][E: codex-rs/http-client/src/route_aware_client_pool.rs:259] |

## Feature / collaboration / UI shorthand

| Term | 中文解释 | Evidence |
|---|---|---|
| feature flag | `Feature` 是 config-toggled identity enum；`FEATURES` 把 133 个 ids 映射到 keys/stages/defaults，removed compatibility flags 仍留在 registry。`UnifiedExec` 默认全平台 `true`。[I] | [E: codex-rs/features/src/lib.rs:92][E: codex-rs/features/src/lib.rs:869][E: codex-rs/features/src/lib.rs:908][E: codex-rs/features/src/lib.rs:911] |
