...
|---|---|---|---|
| `SessionIo` | struct | queue/lifecycle endpoints: `tx_sub`, `rx_event`, agent-status receiver, shared session-loop termination future；runtime state 不再混在 queue handle 中。[E: codex-rs/core/src/session/mod.rs:387][E: codex-rs/core/src/session/mod.rs:392][E: codex-rs/core/src/session/mod.rs:399] | `core/src/session/mod.rs` |
| `Session` / `SessionSpawnArgs` | struct | `Session::spawn(args)` 返回 `(Arc<Session>, SessionIo)`；`Session` 持 state/services，spawn args 汇集 config、history、provider/services 与 thread-store inputs。[E: codex-rs/core/src/session/mod.rs:404][E: codex-rs/core/src/session/mod.rs:469][E: codex-rs/core/src/session/mod.rs:471][E: codex-rs/core/src/session/mod.rs:495] | `core/src/session/mod.rs` |
| `CodexThread` | struct | public thread conduit，组合 `Arc<Session>` 与 `SessionIo`；submit/next_event 代理到 `io`，thread-specific reads/control 代理到 session。[E: codex-rs/core/src/codex_thread.rs:162][E: codex-rs/core/src/codex_thread.rs:163][E: codex-rs/core/src/codex_thread.rs:164][E: codex-rs/core/src/codex_thread.rs:205][E: codex-rs/core/src/codex_thread.rs:414] | `core/src/codex_thread.rs` |
| `TurnContext` | struct | single-turn context: sub id, trace id, realtime flag, config/auth/model/provider/telemetry, session source, environments, deprecated cwd, instructions, collaboration/personality, approval/profile/network/sandbox, models, unified exec, dynamic tools, metadata/extension/skills/timing state。[E: codex-rs/core/src/session/turn_context.rs:112][E: codex-rs/core/src/session/turn_context.rs:112][E: codex-rs/core/src/session/turn_context.rs:113][E: codex-rs/core/src/session/turn_context.rs:115][E: codex-rs/core/src/session/turn_context.rs:116][E: codex-rs/core/src/session/turn_context.rs:117][E: codex-rs/core/src/session/turn_context.rs:118][E: codex-rs/core/src/session/turn_context.rs:120][E: codex-rs/core/src/session/turn_context.rs:123][E: codex-rs/core/src/session/turn_context.rs:129][E: codex-rs/core/src/session/turn_context.rs:130][E: codex-rs/core/src/session/turn_context.rs:126][E: codex-rs/core/src/session/turn_context.rs:137][E: codex-rs/core/src/session/turn_context.rs:140][E: codex-rs/core/src/session/turn_context.rs:141][E: codex-rs/core/src/session/turn_context.rs:142][E: codex-rs/core/src/session/turn_context.rs:143][E: codex-rs/core/src/session/turn_context.rs:144][E: codex-rs/core/src/session/turn_context.rs:145][E: codex-rs/core/src/session/turn_context.rs:146][E: codex-rs/core/src/session/turn_context.rs:148][E: codex-rs/core/src/session/turn_context.rs:149][E: codex-rs/core/src/session/turn_context.rs:150][E: codex-rs/core/src/session/turn_context.rs:151][E: codex-rs/core/src/session/turn_context.rs:152] | `core/src/session/turn_context.rs` |
| `ModelClient` | struct | session-scoped API client; comments say it shares auth/provider/thread id/transport fallback across turns while turn-scoped settings are passed explicitly。[E: codex-rs/core/src/client.rs:242][E: codex-rs/core/src/client.rs:244][E: codex-rs/core/src/client.rs:247][E: codex-rs/core/src/client.rs:250][E: codex-rs/core/src/client.rs:254][E: codex-rs/core/src/client.rs:256] | `core/src/client.rs` |
| `ModelClientSession` | struct | turn-scoped streaming session; comment says create a fresh session for each Codex turn to avoid replaying sticky-routing token across turns。[E: codex-rs/core/src/client.rs:261][E: codex-rs/core/src/client.rs:263][E: codex-rs/core/src/client.rs:268][E: codex-rs/core/src/client.rs:271][E: codex-rs/core/src/client.rs:274][E: codex-rs/core/src/client.rs:275][E: codex-rs/core/src/client.rs:276][E: codex-rs/core/src/client.rs:287] | `core/src/client.rs` |
| `ThreadManager` | struct | `state: Arc<ThreadManagerState>` plus test guard; `StartThreadOptions` carries config, initial history, source, dynamic tools, metrics and parent trace inputs。[E: codex-rs/core/src/thread_manager.rs:184][E: codex-rs/core/src/thread_manager.rs:185][E: codex-rs/core/src/thread_manager.rs:186][E: codex-rs/core/src/thread_manager.rs:190][E: codex-rs/core/src/thread_manager.rs:193][E: codex-rs/core/src/thread_manager.rs:195][E: codex-rs/core/src/thread_manager.rs:196][E: codex-rs/core/src/thread_manager.rs:198][E: codex-rs/core/src/thread_manager.rs:199][E: codex-rs/core/src/thread_manager.rs:200] | `core/src/thread_manager.rs` |
...
| Symbol | Kind | 当前字段/方法/变体 | 定义处 |
|---|---|---|---|
| `ToolSpec` | enum | Responses API tool spec union: `Function`, `Namespace`, `ToolSearch`, `WebSearch`, `Freeform`；hosted image generation 已不在该 enum。[E: codex-rs/tools/src/tool_spec.rs:17][E: codex-rs/tools/src/tool_spec.rs:19][E: codex-rs/tools/src/tool_spec.rs:21][E: codex-rs/tools/src/tool_spec.rs:23][E: codex-rs/tools/src/tool_spec.rs:35][E: codex-rs/tools/src/tool_spec.rs:50] | `tools/src/tool_spec.rs` |
...
| `NetworkSandboxPolicy` | enum | `Restricted` or `Enabled`; `is_enabled()` matches `Enabled`。[E: codex-rs/protocol/src/permissions.rs:83][E: codex-rs/protocol/src/permissions.rs:85][E: codex-rs/protocol/src/permissions.rs:86][E: codex-rs/protocol/src/permissions.rs:89][E: codex-rs/protocol/src/permissions.rs:90] | `protocol/src/permissions.rs` |
| `FileSystemAccessMode` | enum | `Read`, `Write`, `Deny`; `none` remains a legacy alias for `Deny`。[E: codex-rs/protocol/src/permissions.rs:117][E: codex-rs/protocol/src/permissions.rs:118][E: codex-rs/protocol/src/permissions.rs:119][E: codex-rs/protocol/src/permissions.rs:120][E: codex-rs/protocol/src/permissions.rs:122] | `protocol/src/permissions.rs` |
| `FileSystemSandboxPolicy` | struct | `kind`, optional `glob_scan_max_depth`, and `entries`。[E: codex-rs/protocol/src/permissions.rs:197][E: codex-rs/protocol/src/permissions.rs:198][E: codex-rs/protocol/src/permissions.rs:201][E: codex-rs/protocol/src/permissions.rs:203] | `protocol/src/permissions.rs` |
| `Feature` / `FEATURES` | enum/registry | feature enum includes active feature flags plus removed compatibility flags; `FEATURES` maps ids to keys, stages and defaults。[E: codex-rs/features/src/lib.rs:84][E: codex-rs/features/src/lib.rs:84][E: codex-rs/features/src/lib.rs:86][E: codex-rs/features/src/lib.rs:94][E: codex-rs/features/src/lib.rs:241][E: codex-rs/features/src/lib.rs:250][E: codex-rs/features/src/lib.rs:253][E: codex-rs/features/src/lib.rs:256][E: codex-rs/features/src/lib.rs:258][E: codex-rs/features/src/lib.rs:260][E: codex-rs/features/src/lib.rs:804][E: codex-rs/features/src/lib.rs:807][E: codex-rs/features/src/lib.rs:809][E: codex-rs/features/src/lib.rs:855][E: codex-rs/features/src/lib.rs:857] | `features/src/lib.rs` |
...
| `CodexAuth` | enum | `ApiKey`, `Chatgpt`, `ChatgptAuthTokens`, `AgentIdentity`, `PersonalAccessToken`, `BedrockApiKey`。[E: codex-rs/login/src/auth/manager.rs:72][E: codex-rs/login/src/auth/manager.rs:73][E: codex-rs/login/src/auth/manager.rs:74][E: codex-rs/login/src/auth/manager.rs:76][E: codex-rs/login/src/auth/manager.rs:77][E: codex-rs/login/src/auth/manager.rs:78][E: codex-rs/login/src/auth/manager.rs:79] | `login/src/auth/manager.rs` |
| `AuthManager` | struct | auth home, cached auth lock, change notifier, env API key flag, credential store mode, keyring backend kind, forced workspace id, ChatGPT base URL, refresh lock, external auth。[E: codex-rs/login/src/auth/manager.rs:1767][E: codex-rs/login/src/auth/manager.rs:1768][E: codex-rs/login/src/auth/manager.rs:1769][E: codex-rs/login/src/auth/manager.rs:1770][E: codex-rs/login/src/auth/manager.rs:1771][E: codex-rs/login/src/auth/manager.rs:1772][E: codex-rs/login/src/auth/manager.rs:1773][E: codex-rs/login/src/auth/manager.rs:1774][E: codex-rs/login/src/auth/manager.rs:1777][E: codex-rs/login/src/auth/manager.rs:1780][E: codex-rs/login/src/auth/manager.rs:1782] | `login/src/auth/manager.rs` |

## Gotchas

- The deleted registry-plan structs and old handler-kind enum are not current key types for core routing; runtime registration is now centered on `ToolExecutor` / `CoreToolRuntime` and built from `core/src/tools/spec_plan.rs`.[E: codex-rs/tools/src/tool_executor.rs:49][E: codex-rs/core/src/tools/registry.rs:51][E: codex-rs/core/src/tools/router.rs:65]
...
- Removed compatibility feature flags such as `GhostCommit`/`JsRepl` may still parse old config keys, but their registry stage is `Removed`.[E: codex-rs/features/src/lib.rs:256][E: codex-rs/features/src/lib.rs:258][E: codex-rs/features/src/lib.rs:809][E: codex-rs/features/src/lib.rs:857]
