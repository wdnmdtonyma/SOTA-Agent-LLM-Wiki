...
| `CODEX_CONNECTORS_TOKEN` | MCP apps connector auth | non-empty or non-Unicode value marks Codex apps MCP bearer token env var as present。[E: codex-rs/codex-mcp/src/mcp/mod.rs:67][E: codex-rs/codex-mcp/src/mcp/mod.rs:575][E: codex-rs/codex-mcp/src/mcp/mod.rs:576] | `codex-mcp/src/mcp/mod.rs` |
...
| `CODEX_SQLITE_HOME` | state DB | overrides SQLite state database home directory when `config.toml` `sqlite_home` is absent; relative env value is resolved against cwd, otherwise default is `$CODEX_HOME`。[E: codex-rs/state/src/lib.rs:124][E: codex-rs/core/src/config/mod.rs:267][E: codex-rs/config/src/config_toml.rs:353] | `state/src/lib.rs`, `core/src/config/mod.rs`, `config/src/config_toml.rs` |
