...
|---|---|---|---|
| `OPENAI_API_KEY` | Auth/API key | `read_openai_api_key_from_env()` 读取并 trim，空字符串被过滤。[E: codex-rs/login/src/auth/manager.rs:926][E: codex-rs/login/src/auth/manager.rs:930][E: codex-rs/login/src/auth/manager.rs:931][E: codex-rs/login/src/auth/manager.rs:933][E: codex-rs/login/src/auth/manager.rs:934] | `login/src/auth/manager.rs` |
| `CODEX_API_KEY` | Auth/API key | `read_codex_api_key_from_env()` 读取 Codex-specific API key。[E: codex-rs/login/src/auth/manager.rs:927][E: codex-rs/login/src/auth/manager.rs:937][E: codex-rs/login/src/auth/manager.rs:938] | `login/src/auth/manager.rs` |
| `CODEX_ACCESS_TOKEN` | Auth/API key | `read_codex_access_token_from_env()` 读取非空 Codex access token。[E: codex-rs/login/src/auth/manager.rs:928][E: codex-rs/login/src/auth/manager.rs:941][E: codex-rs/login/src/auth/manager.rs:942] | `login/src/auth/manager.rs` |
| `CODEX_REFRESH_TOKEN_URL_OVERRIDE` | Auth debug/config override | OAuth refresh token endpoint override constant。[E: codex-rs/login/src/auth/manager.rs:202][E: codex-rs/login/src/auth/manager.rs:1734] | `login/src/auth/manager.rs` |
| `CODEX_REVOKE_TOKEN_URL_OVERRIDE` | Auth debug/config override | OAuth revoke token endpoint override constant。[E: codex-rs/login/src/auth/manager.rs:203] | `login/src/auth/manager.rs` |
| `CODEX_APP_SERVER_LOGIN_CLIENT_ID` | Auth/app-server login override | overrides the OAuth client id when set to a non-empty value。[E: codex-rs/login/src/auth/manager.rs:204][E: codex-rs/login/src/auth/manager.rs:1727][E: codex-rs/login/src/auth/manager.rs:1729] | `login/src/auth/manager.rs` |
| `OPENAI_ORGANIZATION` | OpenAI provider headers | mapped to `OpenAI-Organization` HTTP header in provider info。[E: codex-rs/model-provider-info/src/lib.rs:483] | `model-provider-info/src/lib.rs` |
| `OPENAI_PROJECT` | OpenAI provider headers | mapped to `OpenAI-Project` HTTP header in provider info。[E: codex-rs/model-provider-info/src/lib.rs:485] | `model-provider-info/src/lib.rs` |
| `CODEX_OSS_PORT` | OSS provider experimental | used to build default OSS base URL port when non-empty and parseable; comment marks `CODEX_OSS_` variables experimental。[E: codex-rs/model-provider-info/src/lib.rs:671] | `model-provider-info/src/lib.rs` |
| `CODEX_OSS_BASE_URL` | OSS provider experimental | overrides default OSS base URL when non-empty。[E: codex-rs/model-provider-info/src/lib.rs:678] | `model-provider-info/src/lib.rs` |
| `AWS_BEARER_TOKEN_BEDROCK` | Amazon Bedrock provider | 在 managed Bedrock auth 未提供时读取非空 env bearer token；若两者都没有则回退 AWS SDK auth。[E: codex-rs/model-provider/src/amazon_bedrock/auth.rs:29][E: codex-rs/model-provider/src/amazon_bedrock/auth.rs:82][E: codex-rs/model-provider/src/amazon_bedrock/auth.rs:135] | `model-provider/src/amazon_bedrock/auth.rs` |
| `AWS_REGION` | Amazon Bedrock provider | env bearer-token region fallback；优先级在 `model_providers.amazon-bedrock.aws.region` 之后、`AWS_DEFAULT_REGION` 之前。[E: codex-rs/model-provider/src/amazon_bedrock/auth.rs:32][E: codex-rs/model-provider/src/amazon_bedrock/auth.rs:247] | `model-provider/src/amazon_bedrock/auth.rs` |
| `AWS_DEFAULT_REGION` | Amazon Bedrock provider | env bearer-token region 的末级 fallback；三处都缺失时返回 fatal configuration error。[E: codex-rs/model-provider/src/amazon_bedrock/auth.rs:33][E: codex-rs/model-provider/src/amazon_bedrock/auth.rs:248][E: codex-rs/model-provider/src/amazon_bedrock/auth.rs:252] | `model-provider/src/amazon_bedrock/auth.rs` |
...
| `CODEX_APP_SERVER_MANAGED_CONFIG_PATH` | app-server debug-only test hook | debug builds can point app-server at a temporary managed config path。[E: codex-rs/app-server/src/main.rs:27][E: codex-rs/app-server/src/main.rs:94][E: codex-rs/app-server/src/main.rs:97][E: codex-rs/app-server/src/main.rs:98][E: codex-rs/app-server/src/main.rs:150][E: codex-rs/app-server/src/main.rs:151][E: codex-rs/app-server/src/main.rs:153][E: codex-rs/app-server/src/main.rs:157] | `app-server/src/main.rs` |
| `CODEX_APP_SERVER_DISABLE_MANAGED_CONFIG` | app-server debug-only test hook | debug builds treat `1/true/TRUE/yes/YES` as disabling managed config。[E: codex-rs/app-server/src/main.rs:28][E: codex-rs/app-server/src/main.rs:94][E: codex-rs/app-server/src/main.rs:139][E: codex-rs/app-server/src/main.rs:140][E: codex-rs/app-server/src/main.rs:142][E: codex-rs/app-server/src/main.rs:143] | `app-server/src/main.rs` |
...
| `CODEX_CONNECTORS_TOKEN` | MCP apps connector auth | non-empty or non-Unicode value marks Codex apps MCP bearer token env var as present。[E: codex-rs/codex-mcp/src/mcp/mod.rs:67][E: codex-rs/codex-mcp/src/mcp/mod.rs:573][E: codex-rs/codex-mcp/src/mcp/mod.rs:574] | `codex-mcp/src/mcp/mod.rs` |
...
| `CODEX_SQLITE_HOME` | state DB | overrides SQLite state database home directory when `config.toml` `sqlite_home` is absent; relative env value is resolved against cwd, otherwise default is `$CODEX_HOME`。[E: codex-rs/state/src/lib.rs:124][E: codex-rs/core/src/config/mod.rs:256][E: codex-rs/config/src/config_toml.rs:348] | `state/src/lib.rs`, `core/src/config/mod.rs`, `config/src/config_toml.rs` |
| `CODEX_ABSOLUTE_PATH_REMOVED_CWD_CHILD` | test-only absolute path utility | ignored test child process uses this variable in the removed-current-directory absolute path test。[E: codex-rs/utils/absolute-path/src/lib.rs:434][E: codex-rs/utils/absolute-path/src/lib.rs:445] | `utils/absolute-path/src/lib.rs` |
