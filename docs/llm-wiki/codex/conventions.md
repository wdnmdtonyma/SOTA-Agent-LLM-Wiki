...
`1 Identity`(wire name 给模型看的名 / aliases / `CoreToolRuntime` handler / 所属 crate)· `2 用途定位` · `3 输入 schema 表`(字段·类型·必填·默认·说明·校验,源自 `JsonSchema`/struct)· `4 输出 schema & 截断` · `5 ToolSpec 类型`(`Function`/`Freeform`/`Namespace`/`ToolSearch`/`WebSearch`/`ImageGeneration`——及为何选它;`LocalShell` 已不存在)· `6 注册与门控`(在 `codex-rs/core/src/tools/spec_plan.rs` 中找到它被写入 `ToolRegistry` 的条件,并记录 `ToolEnvironmentMode` / config 字段 / feature flag / provider capability / model capability / 始终开;effective exposure 以 `finalize_tool_router` 后的 registry 为准)· `7 parallel-safe`(`supports_parallel_tool_calls` 实际值 + 为什么)· `8 handler 走读`(该 `CoreToolRuntime` 在 core 的 dispatch/执行路径,handler 通常位于 `codex-rs/core/src/tools/handlers/`)· `9 设计动机·edge·历史`(V1/V2、freeform vs json、hosted vs extension 等)· `## Sources` · `## 相关`
