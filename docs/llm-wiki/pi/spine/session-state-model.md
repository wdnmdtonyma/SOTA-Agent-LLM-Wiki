...
> `spine.session-state-model` 串起 `pi-agent-core` 的 `SessionRepo` → `Storage` → `Session` / `Branch` → `commit(Write[])` 流程：tree 只存 `Entry`，branch tip / lane 配置 / 操作态走 `values.ts` 地址；并明确它与 `pi-coding-agent` 产品级 `SessionManager`（`CURRENT_SESSION_VERSION = 3`）是两套相邻但独立的状态系统。

## 能回答的问题

- harness `Entry` 现在有哪些 type？lane 配置还是不是 tree entry？
- `SessionRepo`、`Storage`、`Session`、`Branch` 各自负责什么？
- `JsonlSessionRepo` 与 `MemorySessionRepo` 共享什么语义？
...
2. `NewEntry` 是写入前的 entry：调用方提供 `id` / `parentId` / type / payload，省略 `seq` 与 `timestamp`。storage 在 `commitWrite` 里补这两项。[E: packages/agent/src/harness/session/types.ts:59] [E: packages/agent/src/harness/session/commit.ts:61]
...
4. `Storage` 是单会话 persistence 原语：`commit(writes)`、按 id 读 entry、value/list 读写、`scanBranch` / `scanEntries` / `scanUsage`、`getStats`、`close`。[E: packages/agent/src/harness/session/types.ts:454] [E: packages/agent/src/harness/session/types.ts:455]
5. `SessionRepo` 管 collection lifetime：`create` / `open` / `list` / `delete` / `fork`。每个方法都带 `Context`。[E: packages/agent/src/harness/session/types.ts:591] [E: packages/agent/src/harness/session/types.ts:596]
6. `Session` 是面向调用方的 facade：实现 `SessionReader`，再提供 `branch` / `createBranch`、`beginMutation` / `mutate`、value/list writer 与 name/label。包内实现是 `StorageBackedSession`。[E: packages/agent/src/harness/session/types.ts:529] [E: packages/agent/src/harness/session/session.ts:224]
...
8. 新建会话**没有**隐式 `main` branch。conformance 断言 `create` 之后 `branch("main")` 与 `laneConfig("main")` 都是 `undefined`，必须 `createBranch`。[E: packages/agent/src/harness/session/testing/conformance/session-repo.ts:158] [E: packages/agent/src/harness/session/session.ts:355]
9. memory / JSONL 共用 `InMemoryStorageState`：从 `nextSeq = 1` 起步，`prepareCommit` 赋 seq/timestamp，`applyValidated` 更新 entries / values / usage / stats。[E: packages/agent/src/harness/session/in-memory-storage-state.ts:62] [E: packages/agent/src/harness/session/in-memory-storage-state.ts:78] [E: packages/agent/src/harness/session/in-memory-storage-state.ts:81]
10. branch 查询从显式 `start` 沿 `parentId` 走向 root；缺 parent 抛 `Corrupt branch: missing parent`。`Session.scanBranch` 要求 `StorageBranchScan.start`；`Branch.findEntries` 用 tip 补默认 start。[E: packages/agent/src/harness/session/in-memory-storage-state.ts:185] [E: packages/agent/src/harness/session/in-memory-storage-state.ts:195] [E: packages/agent/src/harness/session/session.ts:198]
...
12. `JsonlSessionRepo` 把新会话写成 `JsonlStorageHeader`（`kind: "header"`、`v: 4`、`storageVersion: 1`），再交给 `JsonlStorage.create`。公开入口是 `jsonl/index.ts`。[E: packages/agent/src/harness/session/jsonl/types.ts:4] [E: packages/agent/src/harness/session/jsonl/repo.ts:78] [E: packages/agent/src/harness/session/jsonl/index.ts:1]
13. `MemorySessionRepo` 用进程内 `Map<id, MemorySessionRecord>` 实现同一 `SessionRepo` contract；`create` / `open` / `fork` 返回带 close facade 的 `Session`。[E: packages/agent/src/harness/session/memory.ts:344] [E: packages/agent/src/harness/session/memory.ts:355] [E: packages/agent/src/harness/session/memory.ts:450]
14. harness session 的 public barrel 是 `packages/agent/src/harness/session/index.ts`：导出 `JsonlSessionRepo`、`MemorySessionRepo`、`StorageBackedSession`、types 与 values。[E: packages/agent/src/harness/session/index.ts:20] [E: packages/agent/src/harness/session/index.ts:24]

## 关键决策点

### Branch 是 parent path，不是 append-order history

`Session.findEntries()` 按 seq 扫 session-wide log（`scanEntries`）；branch traversal 从 tip 沿 `parentId` 向上。[E: packages/agent/src/harness/session/session.ts:318] [E: packages/agent/src/harness/session/in-memory-storage-state.ts:185] 导航通过 `setValue(branchTip(name), id)` 改 tip，不改写旧 entries。[E: packages/agent/src/harness/session/session.ts:365] [E: packages/agent/src/harness/session/values.ts:158]
...
`pi-agent-core` 的 `Session` 是 async、`SessionRepo`-backed、`Context`-scoped 的可复用 harness API；JSONL 是 `v: 4` header + commit 事务行，`storageVersion` 为 1。`pi-coding-agent` 的 `SessionManager` 自己维护 `fileEntries`、`byId`、labels 与 `leafId`，`CURRENT_SESSION_VERSION = 3`，并同步实现 `getBranch()` / `buildSessionContext()`。[E: packages/coding-agent/src/core/session-manager.ts:30] [E: packages/coding-agent/src/core/session-manager.ts:856] [E: packages/coding-agent/src/core/session-manager.ts:1274] [E: packages/coding-agent/src/core/session-manager.ts:1298] 两层概念相似，但类型、持久化入口与恢复逻辑不是同一个实现。[I]

产品 `SessionManager.appendMessage()` 自己把 `parentId` 写成 `this.leafId` 再 `_appendEntry` 推进 leaf；harness `Branch.appendMessage` 在 mutation 里读 `branchTip` 再 `commit`。[E: packages/coding-agent/src/core/session-manager.ts:1071] [E: packages/agent/src/harness/session/session.ts:433]

## Gotcha

- `create()` 不会自动建 `main`。调用方或 `AgentHarness.lane()` 必须 `createBranch`。[E: packages/agent/src/harness/session/testing/conformance/session-repo.ts:158]
- `Storage.scanBranch` 强制 `start`；缺省 start 是 `Branch.findEntries` 的糖。[E: packages/agent/src/harness/session/types.ts:431] [E: packages/agent/src/harness/session/session.ts:198]
- `InMemoryStorageState.scanBranch` 遇到 missing parent 抛普通 `Error`，不是旧的 `SessionError`。[E: packages/agent/src/harness/session/in-memory-storage-state.ts:195]
- `JsonlSessionRepo.list` 对空文件或 `parseJsonlSessionHeader` 失败的 `.jsonl` 选择 skip，不会因单个 malformed header 让整个 list 失败。[E: packages/agent/src/harness/session/jsonl/repo.ts:250] [E: packages/agent/src/harness/session/jsonl/repo.ts:252]
- JSONL header 的格式字段是 `v: 4`，另有 `storageVersion`（当前必须为 `JSONL_STORAGE_VERSION = 1`）。coding-agent 产品 JSONL 仍是 `type: "session", version: 3`，不能把两种文件互相当同一 schema 打开；harness JSONL 可以把 v3-legacy 只读投影进来，第一次 write 再升级。[E: packages/agent/src/harness/session/jsonl/types.ts:4] [E: packages/agent/src/harness/session/jsonl/codec.ts:38] [E: packages/coding-agent/src/core/session-manager.ts:30]
