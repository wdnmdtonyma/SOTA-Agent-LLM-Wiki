...
9. A finalized tool result becomes a provider-visible `ToolResultMessage` with `role: "toolResult"`, `toolCallId`, `toolName`, `content`, `details`, `usage`, optional `addedToolNames`, `isError`, and a timestamp; the main loop appends those result messages to `currentContext.messages` before the next assistant turn [E: packages/agent/src/agent-loop.ts:775] [E: packages/agent/src/agent-loop.ts:777] [E: packages/agent/src/agent-loop.ts:778] [E: packages/agent/src/agent-loop.ts:782] [E: packages/agent/src/agent-loop.ts:783] [E: packages/agent/src/agent-loop.ts:784] [E: packages/agent/src/agent-loop.ts:785] [E: packages/agent/src/agent-loop.ts:786] [E: packages/agent/src/agent-loop.ts:787].
...
`AgentToolResult` is the common result envelope for final and partial tool output: model-visible `content`, structured `details`, optional `usage`, optional `addedToolNames`, and optional `terminate` [E: packages/agent/src/types.ts:362] [E: packages/agent/src/types.ts:364] [E: packages/agent/src/types.ts:366] [E: packages/agent/src/types.ts:368] [E: packages/agent/src/types.ts:370] [E: packages/agent/src/types.ts:375]. This means the reusable core knows how to validate, execute, stream updates, emit lifecycle events, and append tool results, but it does not know coding-agent-specific prompt snippets, renderers, shell settings, or extension metadata [I].
...
`AgentHarnessTool` 保留 `AgentTool` 的静态字段，但把 `execute` 扩成第五个应用 context 参数 [E: packages/agent/src/harness/types.ts:81] [E: packages/agent/src/harness/types.ts:85] [E: packages/agent/src/harness/types.ts:87] [E: packages/agent/src/harness/types.ts:92]。当前 `AgentHarness` 实现是 stub：`prompt`/`resume`/`steer`/`runToCompletion` 等走 `unavailable()`，`toolContext` 只出现在 `AgentHarnessOptions` 类型上，不会被存储或按 turn 绑定进 tools。[E: packages/agent/src/harness/agent-harness.ts:250] [E: packages/agent/src/harness/agent-harness.ts:380] [E: packages/agent/src/harness/agent-harness.ts:419] [E: packages/agent/src/harness/agent-harness.ts:437] 四个内置 harness execution tools 的 I/O 见 `subsys.agent-core.execution-tools`。
