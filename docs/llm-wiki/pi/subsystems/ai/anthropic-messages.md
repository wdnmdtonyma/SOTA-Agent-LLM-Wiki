...
启用 `supportsToolReferences` 时，request builder 将 deferred tools 从立即 `tools` 列表拆出；`ToolResultMessage.addedToolNames` 会在对应 `tool_result` 后追加 Anthropic `tool_reference { tool_name }` blocks，并只引用实际存在于 deferred map 的名称 [E: packages/ai/src/api/anthropic-messages.ts:194] [E: packages/ai/src/api/anthropic-messages.ts:985] [E: packages/ai/src/api/anthropic-messages.ts:994] [E: packages/ai/src/api/anthropic-messages.ts:1127] [E: packages/ai/src/api/anthropic-messages.ts:1130] [E: packages/ai/src/api/anthropic-messages.ts:1133] [E: packages/ai/src/api/anthropic-messages.ts:1134]。
