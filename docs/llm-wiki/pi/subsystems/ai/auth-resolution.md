...
本节点只覆盖 auth resolution 的调度和边界: provider-specific api-key 解析在 `apiKey.resolve({ ctx, credential, signal })` 中执行,而 OAuth token 刷新与 auth 派生分别由 provider 的 `oauth.refresh(current, refreshSignal)` 和 `oauth.toAuth(...)` 执行 [E: packages/ai/src/auth/resolve.ts:189] [E: packages/ai/src/auth/resolve.ts:153] [E: packages/ai/src/auth/resolve.ts:175]。[ref.ai.auth-types](../../reference/auth-types.md) 应定义 `AuthResult`、credential 和 provider auth 类型的完整字段语义 [I]。
...
`AuthResolutionOverrides` 有 request 级 `apiKey?: string`、`env?: ProviderEnv`、`minOAuthValidityMs?: number` 与 `signal?: AbortSignal`;`env` 会先被叠加到 `AuthContext`,`apiKey` 只在 provider 声明 `auth.apiKey` 时触发 direct api-key resolution，`minOAuthValidityMs` 则向 stored OAuth path 传递 caller 所需的最小剩余有效期。`resolveProviderAuth()` 用 `operationSignal(overrides?.signal)` 得到 operation signal，再 `raceWithAbortSignal` 与 inner `resolveProviderAuthWithSignal(...)` 竞速。[E: packages/ai/src/auth/resolve.ts:18] [E: packages/ai/src/auth/resolve.ts:19] [E: packages/ai/src/auth/resolve.ts:20] [E: packages/ai/src/auth/resolve.ts:22] [E: packages/ai/src/auth/resolve.ts:23] [E: packages/ai/src/auth/resolve.ts:56] [E: packages/ai/src/auth/resolve.ts:57] [E: packages/ai/src/auth/resolve.ts:71] [E: packages/ai/src/auth/resolve.ts:73] [E: packages/ai/src/auth/resolve.ts:96]。
...
4. 读到 OAuth credential 且 provider 支持 `auth.oauth` 时,解析进入 `resolveStoredOAuth(credentials, provider.id, provider.auth.oauth, stored, signal, overrides?.minOAuthValidityMs)`;第五参是 `AbortSignal`,这条路径不会把 `AuthContext` 传给 OAuth refresh 或 toAuth [E: packages/ai/src/auth/resolve.ts:88] [E: packages/ai/src/auth/resolve.ts:89] [E: packages/ai/src/auth/resolve.ts:90] [E: packages/ai/src/auth/resolve.ts:91] [E: packages/ai/src/auth/resolve.ts:95] [E: packages/ai/src/auth/resolve.ts:127] [E: packages/ai/src/auth/resolve.ts:132] [E: packages/ai/src/auth/resolve.ts:175]。
...
即将过期的 OAuth credential 通过 `credentials.modify(providerId, async (current) => ...)` 刷新；modify 回调里会再次检查当前 credential 仍是 OAuth 且仍落在同一 validity window，满足条件时回调内唯一刷新调用点是 `oauth.refresh(current, refreshSignal)`，其中 `refreshSignal = AbortSignal.any([signal, AbortSignal.timeout(DEFAULT_OAUTH_REFRESH_TIMEOUT_MS)])` 且超时为 15s，形成 double-checked locking 边界。[E: packages/ai/src/auth/resolve.ts:139] [E: packages/ai/src/auth/resolve.ts:149] [E: packages/ai/src/auth/resolve.ts:151] [E: packages/ai/src/auth/resolve.ts:153] [E: packages/ai/src/auth/resolve.ts:120]。
...
api-key provider 的具体环境变量、文件探测或 ambient credential 判断不在 `resolveProviderAuth()` 中硬编码;该入口只把 `{ ctx: authContext, credential, signal }` 传给 provider 的 `apiKey.resolve(...)` [E: packages/ai/src/auth/resolve.ts:181] [E: packages/ai/src/auth/resolve.ts:189] [I]。
