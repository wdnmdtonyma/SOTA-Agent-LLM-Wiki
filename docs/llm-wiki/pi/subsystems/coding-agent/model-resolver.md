...
本节点的 index authority symbols 是 `findInitialModel` 与 `getModelSearchText`; 正文仍会说明同文件里的 `resolveCliModel`、`resolveModelScopeWithDiagnostics`、`resolveModelScope`、`parseModelPattern` 和 `restoreModelFromSession`, 因为它们是 `findInitialModel` 的前置/相邻控制流, 但这些 helper 未被 index 标为本节点权威 symbol [E: packages/coding-agent/src/core/model-resolver.ts:195] [E: packages/coding-agent/src/core/model-resolver.ts:273] [E: packages/coding-agent/src/core/model-resolver.ts:355] [E: packages/coding-agent/src/core/model-resolver.ts:385] [E: packages/coding-agent/src/core/model-resolver.ts:601] [E: packages/coding-agent/src/core/model-resolver.ts:686] [I]。
...
- `packages/coding-agent/src/core/model-resolver.ts`: 默认模型表 `defaultModelPerProvider`、exact/reference match、pattern/thinking parser、scope resolver with diagnostics、CLI resolver、initial model selection、session restore fallback [E: packages/coding-agent/src/core/model-resolver.ts:14] [E: packages/coding-agent/src/core/model-resolver.ts:79] [E: packages/coding-agent/src/core/model-resolver.ts:195] [E: packages/coding-agent/src/core/model-resolver.ts:273] [E: packages/coding-agent/src/core/model-resolver.ts:355] [E: packages/coding-agent/src/core/model-resolver.ts:385] [E: packages/coding-agent/src/core/model-resolver.ts:601] [E: packages/coding-agent/src/core/model-resolver.ts:686]。
...
`ScopedModel` 是 model scope 的最小数据结构: 一个 `Model<Api>` 加一个可选 `thinkingLevel`; undefined 表示该 pattern 没有显式指定 thinking level, 后续 cycling 时可继承当前 session thinking level [E: packages/coding-agent/src/core/model-resolver.ts:55] [E: packages/coding-agent/src/core/model-resolver.ts:58] [E: packages/coding-agent/src/main.ts:478] [E: packages/coding-agent/src/main.ts:480]。
...
CLI resolver 使用 strict colon fallback: `resolveCliModel()` 调 `parseModelPattern(pattern, candidates, { allowInvalidThinkingLevelFallback: false })`, 因此 `--model` 中的 invalid `:suffix` 不会被静默剥成 thinking level; 测试覆盖 `gpt-4o:extended` 与 custom `:banana` 作为 raw model id 的行为 [E: packages/coding-agent/src/core/model-resolver.ts:494] [E: packages/coding-agent/src/core/model-resolver.ts:495] [E: packages/coding-agent/test/model-resolver.test.ts:384] [E: packages/coding-agent/test/model-resolver.test.ts:655]。
...
3. `resolveModelScope()` 调用 `resolveModelScopeWithDiagnostics()` 并打印 diagnostics；真正 scope 解析只在 `modelRuntime.getAvailable()` 返回的 auth-configured models 内进行，因此 Ctrl+P cycling scope 不会包含当前无 auth 的模型 [E: packages/coding-agent/src/core/model-resolver.ts:273] [E: packages/coding-agent/src/core/model-resolver.ts:275] [E: packages/coding-agent/src/core/model-resolver.ts:277] [E: packages/coding-agent/src/core/model-resolver.ts:355] [E: packages/coding-agent/src/core/model-resolver.ts:356] [E: packages/coding-agent/src/core/model-runtime.ts:331] [E: packages/coding-agent/src/core/model-runtime.ts:344] [E: packages/coding-agent/src/core/model-runtime.ts:345]。
...
8. `findInitialModel()` 的 fallback 顺序是: `cliProvider && cliModel` 时解析 CLI 并可 `process.exit(1)`, 非 continuing 时取第一个 scoped model, settings default provider/model, auth-configured available models 中的 known-provider default, 最后 first available 或 undefined [E: packages/coding-agent/src/core/model-resolver.ts:626] [E: packages/coding-agent/src/core/model-resolver.ts:642] [E: packages/coding-agent/src/core/model-resolver.ts:651] [E: packages/coding-agent/src/core/model-resolver.ts:663] [E: packages/coding-agent/src/core/model-resolver.ts:667] [E: packages/coding-agent/src/core/model-resolver.ts:676] [E: packages/coding-agent/src/core/model-resolver.ts:680]。
9. `restoreModelFromSession()` 不是 `findInitialModel()` 内部步骤；它单独用 `modelRuntime.getModel()` 和 `hasConfiguredAuth()` 验证 saved provider/modelId，失败时可回退到 current model 或 available models [E: packages/coding-agent/src/core/model-resolver.ts:686] [E: packages/coding-agent/src/core/model-resolver.ts:691] [E: packages/coding-agent/src/core/model-resolver.ts:693] [E: packages/coding-agent/src/core/model-resolver.ts:696] [E: packages/coding-agent/src/core/model-resolver.ts:698] [E: packages/coding-agent/src/core/model-resolver.ts:713] [E: packages/coding-agent/src/core/model-resolver.ts:724]。

## CLI provider/model 解析细节

`resolveCliModel()` 特意用 `modelRuntime.getModels()` 而不是 `getAvailable()`，因为 CLI 可以配合 `--api-key` 做 first-time setup；代码在读取 all models 后才处理 provider map、provider inference 与 fallback custom model [E: packages/coding-agent/src/core/model-resolver.ts:389] [E: packages/coding-agent/src/core/model-resolver.ts:391] [E: packages/coding-agent/src/core/model-resolver.ts:399] [E: packages/coding-agent/src/core/model-resolver.ts:409] [E: packages/coding-agent/src/core/model-resolver.ts:493]。

无显式 `--provider` 时, `resolveCliModel()` 先尝试把 `--model` 的第一个 slash 前缀当 known provider; 这让 `zai/glm-5` 优先解析为 provider `zai` + model `glm-5`, 而不是 gateway provider 下 id 为 `zai/glm-5` 的 raw model [E: packages/coding-agent/src/core/model-resolver.ts:431] [E: packages/coding-agent/src/core/model-resolver.ts:435] [E: packages/coding-agent/src/core/model-resolver.ts:437] [E: packages/coding-agent/test/model-resolver.test.ts:392] [E: packages/coding-agent/test/model-resolver.test.ts:514] [E: packages/coding-agent/test/model-resolver.test.ts:515]。

如果 slash provider inference 命中了一个 unauthenticated provider/model, 但完整 raw model id 存在且唯一 authenticated, resolver 会偏向 authenticated raw id; 这个例外用于处理 raw id 恰好以 known provider name 开头的模型 [E: packages/coding-agent/src/core/model-resolver.ts:504] [E: packages/coding-agent/src/core/model-resolver.ts:508] [E: packages/coding-agent/src/core/model-resolver.ts:510] [E: packages/coding-agent/test/model-resolver.test.ts:508] [E: packages/coding-agent/test/model-resolver.test.ts:554] [E: packages/coding-agent/test/model-resolver.test.ts:555]。

显式 provider 下找不到注册 model 时, `buildFallbackModel()` 会拿该 provider 的 default model 或第一个 provider model 作 base, 再替换成 requested custom model id/name; 如果 model pattern 带 valid `:thinking` 且没有显式 `--thinking`, fallback path 会剥掉 suffix 并在 requested thinking 不是 `off` 时把 fallback model 标为 `reasoning: true` [E: packages/coding-agent/src/core/model-resolver.ts:166] [E: packages/coding-agent/src/core/model-resolver.ts:170] [E: packages/coding-agent/src/core/model-resolver.ts:175] [E: packages/coding-agent/src/core/model-resolver.ts:566] [E: packages/coding-agent/src/core/model-resolver.ts:568] [E: packages/coding-agent/src/core/model-resolver.ts:570] [E: packages/coding-agent/test/model-resolver.test.ts:597] [E: packages/coding-agent/test/model-resolver.test.ts:604] [E: packages/coding-agent/test/model-resolver.test.ts:605] [E: packages/coding-agent/test/model-resolver.test.ts:606]。
...
`findInitialModel()` 的 docblock 仍写有“Restored from session”优先级 [I: packages/coding-agent/src/core/model-resolver.ts:520], 但当前函数参数没有 saved session model, 实际 restore helper 是 `restoreModelFromSession()` 且 SDK 里也在调用 `findInitialModel()` 之前单独检查 existing session model; 因此应把 session restore 视为相邻流程而非 `findInitialModel()` 内部步骤 [E: packages/coding-agent/src/core/model-resolver.ts:601] [E: packages/coding-agent/src/core/model-resolver.ts:686] [E: packages/coding-agent/src/core/sdk.ts:196] [E: packages/coding-agent/src/core/sdk.ts:208] [I]。

## Gotcha

- `findInitialModel()` 的 CLI branch 只有 `cliProvider && cliModel` 同时存在才触发; app startup 的无 provider `--model provider/id` 解析由 `main.ts` 里的 `buildSessionOptions()` 直接调用 `resolveCliModel()` 完成 [E: packages/coding-agent/src/core/model-resolver.ts:626] [E: packages/coding-agent/src/main.ts:423] [E: packages/coding-agent/src/main.ts:424]。
