...
本节点不覆盖 edit diff、write mkdir、find/grep 外部命令、ls 排序或输出截断; 那些工具只把本子系统给出的绝对路径当作执行前置输入 [E: packages/coding-agent/src/core/tools/edit.ts:316] [E: packages/coding-agent/src/core/tools/write.ts:208] [E: packages/coding-agent/src/core/tools/find.ts:164] [E: packages/coding-agent/src/core/tools/ls.ts:129] [E: packages/coding-agent/src/core/tools/grep.ts:183]。
...
`resolveReadPathAsync(filePath, cwd)` 与同步版本同构, 但存在性检查用 async `pathExists()`; `read` tool 使用异步版本, CLI `@file` 使用同步版本 [E: packages/coding-agent/src/core/tools/path-utils.ts:86] [E: packages/coding-agent/src/core/tools/path-utils.ts:89] [E: packages/coding-agent/src/core/tools/read.ts:245] [E: packages/coding-agent/src/cli/file-processor.ts:31]。
...
read-specific fallback 是保守设计: `read` 和 CLI `@file` 面向用户手打或粘贴的已有文件名, 所以当原路径不存在时额外尝试 macOS screenshot/Unicode 变体; mutation/search tools 使用 `resolveToCwd()` 后由各自 access/stat/external command 报错, 避免把写入或搜索悄悄重定向到另一个相似路径 [E: packages/coding-agent/src/core/tools/read.ts:245] [E: packages/coding-agent/src/cli/file-processor.ts:31] [E: packages/coding-agent/src/core/tools/edit.ts:316] [E: packages/coding-agent/src/core/tools/write.ts:208] [E: packages/coding-agent/src/core/tools/find.ts:164] [E: packages/coding-agent/src/core/tools/ls.ts:129] [E: packages/coding-agent/src/core/tools/grep.ts:183] [I]。

`resolveReadPath()` 和 `resolveReadPathAsync()` 返回 unresolved absolute path 而不是抛错, 把“是否存在/可读”的错误格式留给调用方: `read` 随后调用 `ops.access(absolutePath)`, CLI `@file` 随后 `access(absolutePath)` 并打印 `File not found` [E: packages/coding-agent/src/core/tools/path-utils.ts:83] [E: packages/coding-agent/src/core/tools/path-utils.ts:117] [E: packages/coding-agent/src/core/tools/read.ts:248] [E: packages/coding-agent/src/cli/file-processor.ts:35] [E: packages/coding-agent/src/cli/file-processor.ts:37]。
...
- Leading `@` 会被 `expandPath()`/`resolveToCwd()` 剥掉, 但 CLI parser 已经把 `@file` 的 `@` 去掉后再传给 `processFileArguments()`; 因此剥 `@` 是兼容多入口输入, 不是 CLI `@file` 唯一去前缀步骤 [E: packages/coding-agent/src/utils/paths.ts:80] [E: packages/coding-agent/src/cli/args.ts:211] [E: packages/coding-agent/src/cli/args.ts:212] [E: packages/coding-agent/src/cli/file-processor.ts:29]。
...
- `edit` 和 `write` 在 resolved absolute path 上进入 `withFileMutationQueue()`, 但该队列的 realpath/symlink 行为属于文件变更串行化节点, 不是本节点的 path variant 解析 [E: packages/coding-agent/src/core/tools/edit.ts:316] [E: packages/coding-agent/src/core/tools/edit.ts:318] [E: packages/coding-agent/src/core/tools/write.ts:208] [E: packages/coding-agent/src/core/tools/write.ts:210] [I]。
