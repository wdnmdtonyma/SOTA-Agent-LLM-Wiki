...
- `packages/coding-agent/src/core/agent-session.ts`: 调用点背景, `AgentSession.executeBash()` 把 settings prefix/shellPath 转成 `executeBashWithOperations()` 调用,把每个 chunk 发成带可选 command id 的 `bash_execution_update`，并把结果写成 bash execution history [E: packages/coding-agent/src/core/agent-session.ts:3006] [E: packages/coding-agent/src/core/agent-session.ts:3015] [E: packages/coding-agent/src/core/agent-session.ts:3027] [E: packages/coding-agent/src/core/agent-session.ts:3033]。
- `packages/coding-agent/src/modes/rpc/rpc-mode.ts`: 调用点背景, RPC `"bash"` 先 `emitUserBash`；若 handler 返回 `result` 则 `recordBashResult` 并 success，不再 `executeBash`。否则 `session.executeBash(..., { operations: eventResult?.operations })`。[E: packages/coding-agent/src/modes/rpc/rpc-mode.ts:564] [E: packages/coding-agent/src/modes/rpc/rpc-mode.ts:571] [E: packages/coding-agent/src/modes/rpc/rpc-mode.ts:575] [E: packages/coding-agent/src/modes/rpc/rpc-mode.ts:578] [E: packages/coding-agent/src/modes/rpc/rpc-mode.ts:581]。
- `packages/coding-agent/src/core/extensions/loader.ts`: 调用点背景, extension runtime 的 `exec()` 包装 `execCommand()` [E: packages/coding-agent/src/core/extensions/loader.ts:395] [E: packages/coding-agent/src/core/extensions/loader.ts:397]。
...
9. `AgentSession.executeBash()` 包装 `onChunk`:先调用调用方 callback，再 emit `{ type: "bash_execution_update", id: options?.id, delta }` [E: packages/coding-agent/src/core/agent-session.ts:3023] [E: packages/coding-agent/src/core/agent-session.ts:3027]。RPC handler 把 command `id` 传入该 options，所以 host 能把并发/连续 direct bash 的 chunk 与 response 关联 [E: packages/coding-agent/src/modes/rpc/rpc-mode.ts:578] [E: packages/coding-agent/src/modes/rpc/rpc-mode.ts:580]。
