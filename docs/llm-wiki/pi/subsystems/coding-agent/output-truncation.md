...
`AgentSession.recordBashResult()` 把 direct bash helper 的 `BashResult.fullOutputPath` 写入 coding-agent history message, `bashExecutionToText()` 只在 `truncated && fullOutputPath` 时把完整输出路径加入 LLM context [E: packages/coding-agent/src/core/agent-session.ts:2801] [E: packages/coding-agent/src/core/agent-session.ts:2809] [E: packages/coding-agent/src/core/messages.ts:82] [E: packages/coding-agent/src/core/messages.ts:94]。
