...
`$1`、`$2` 这类 positional placeholder 用 1-based index 访问 `args[parseInt(num, 10) - 1]`;目标参数不存在时替换为空字符串 [E: packages/agent/src/harness/prompt-templates.ts:246]。

`${@:N}` 使用 1-based start, 代码会把 start 转成 zero-based 且小于 0 时夹到 0;没有 length 时返回 `args.slice(start).join(" ")`, 有 `${@:N:L}` length 时返回 `args.slice(start, start + L).join(" ")` [E: packages/agent/src/harness/prompt-templates.ts:247] [E: packages/agent/src/harness/prompt-templates.ts:248] [E: packages/agent/src/harness/prompt-templates.ts:249] [E: packages/agent/src/harness/prompt-templates.ts:250] [E: packages/agent/src/harness/prompt-templates.ts:251]。

`$ARGUMENTS` 与 `$@` 都使用同一个 `allArgs = args.join(" ")`;这两个 placeholder 不保留原始 quoting 信息, 也不重新 escape 参数 [E: packages/agent/src/harness/prompt-templates.ts:253] [E: packages/agent/src/harness/prompt-templates.ts:254] [E: packages/agent/src/harness/prompt-templates.ts:255]。

`formatPromptTemplateInvocation()` 是薄包装: 它默认 `args` 为空数组, 并把 `template.content` 与 args 交给 `substituteArgs()` [E: packages/agent/src/harness/prompt-templates.ts:260] [E: packages/agent/src/harness/prompt-templates.ts:261]。
