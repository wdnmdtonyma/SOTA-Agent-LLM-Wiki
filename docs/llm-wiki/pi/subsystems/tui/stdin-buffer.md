...
incomplete CSI/mouse sequence 的默认等待是 `options.timeout ?? 50`（`DEFAULT_SEQUENCE_TIMEOUT_MS`）。lone ESC 另有 `options.escapeTimeout ?? 10`。`process()` 按当前 buffer 是否等于 ESC 选择 `escapeTimeoutMs` 还是 `timeoutMs`。[E: packages/tui/src/stdin-buffer.ts:23][E: packages/tui/src/stdin-buffer.ts:24][E: packages/tui/src/stdin-buffer.ts:290][E: packages/tui/src/stdin-buffer.ts:292][E: packages/tui/src/stdin-buffer.ts:293]
...
CSI sequence 需要 `ESC [` 后至少还有一个字符，并且 payload 最后一个字符码落在 `0x40..0x7e` 才能完成 [E: packages/tui/src/stdin-buffer.ts:92][E: packages/tui/src/stdin-buffer.ts:93][E: packages/tui/src/stdin-buffer.ts:96][E: packages/tui/src/stdin-buffer.ts:101][E: packages/tui/src/stdin-buffer.ts:103]。SGR mouse sequence 是 CSI 的特殊分支：payload 以 `<` 开头时先按 `/^<\\d+;\\d+;\\d+[Mm]$/` 检查；若不匹配但最后一个字符是 `M`/`m`，再看 `slice(1,-1).split(';')` 是否恰好 3 段数字，是则也 complete。[E: packages/tui/src/stdin-buffer.ts:106][E: packages/tui/src/stdin-buffer.ts:108][E: packages/tui/src/stdin-buffer.ts:113][E: packages/tui/src/stdin-buffer.ts:115][E: packages/tui/src/stdin-buffer.ts:116][E: packages/tui/src/stdin-buffer.ts:121]。
