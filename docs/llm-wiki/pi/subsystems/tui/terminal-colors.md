...
- 默认 `ProcessTerminal` 会经 `StdinBuffer` 把一个 raw chunk 中的完整 CSI sequences 分别 emit；所以 parser 的 batch 分支主要覆盖其他 `Terminal` adapter 或直接把合并字符串交给 parser/consumer 的调用路径，不能据此推断默认 stdin path 只回调一次。[E: packages/tui/src/stdin-buffer.ts:194] [E: packages/tui/src/stdin-buffer.ts:209] [E: packages/tui/src/stdin-buffer.ts:233] [E: packages/tui/src/terminal.ts:217] [E: packages/tui/src/terminal.ts:227] [I]
