...
RPC mode 的 event 一般不带 request `id`,但 `bash_execution_update` 是明确例外：`bash` command 的 envelope id 会传入 `executeBash`,后续 update event 可用同一可选 `id` 做 streaming correlation。[E: packages/coding-agent/docs/rpc.md:855][E: packages/coding-agent/docs/rpc.md:857][E: packages/coding-agent/src/modes/rpc/rpc-mode.ts:578][E: packages/coding-agent/src/modes/rpc/rpc-mode.ts:580][E: packages/coding-agent/src/core/agent-session.ts:186] 其它异步生命周期仍由 response `id` 关联 command 接受/失败，再由无 id 的 event stream 观察。[I]
