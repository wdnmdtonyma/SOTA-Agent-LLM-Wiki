...
`createEditToolDefinition` 的 returned object 设置了 `parameters`、`renderShell: "self"` 和 `prepareArguments`, 未出现 `executionMode` 字段 [E: packages/coding-agent/src/core/tools/edit.ts:305] [E: packages/coding-agent/src/core/tools/edit.ts:306] [E: packages/coding-agent/src/core/tools/edit.ts:307] [I]。`ToolDefinition.executionMode` 是可选字段; `edit` 省略它时不会触发 agent-loop 的 per-tool sequential branch [E: packages/coding-agent/src/core/extensions/types.ts:465] [E: packages/agent/src/agent-loop.ts:421] [E: packages/agent/src/agent-loop.ts:422] [E: packages/agent/src/agent-loop.ts:424] [E: packages/agent/src/agent-loop.ts:427]。agent 的全局默认 `toolExecution` 是 `"parallel"` [E: packages/agent/src/agent.ts:228]。

因此, `edit` 不是靠 per-tool `executionMode: "sequential"` 阻止并发 [I]。agent-loop 只有在全局 sequential 或 batch 中存在某个 `executionMode === "sequential"` 的 tool call 时才切到 sequential, 否则走 parallel 执行路径 [E: packages/agent/src/agent-loop.ts:421] [E: packages/agent/src/agent-loop.ts:422] [E: packages/agent/src/agent-loop.ts:424] [E: packages/agent/src/agent-loop.ts:425] [E: packages/agent/src/agent-loop.ts:427]。同文件写入安全由 `withFileMutationQueue(absolutePath, ...)` 提供; 不同文件仍可并行这一点由 queue 单测覆盖 [E: packages/coding-agent/src/core/tools/edit.ts:312] [E: packages/coding-agent/test/file-mutation-queue.test.ts:56] [E: packages/coding-agent/test/file-mutation-queue.test.ts:74]。

## 6 注册与装配

`AgentSessionConfig.initialActiveToolNames` 可覆盖初始 active tool names; 未覆盖时 `_buildRuntime()` 使用 `["read", "bash", "edit", "write"]` 作为默认 active built-ins [E: packages/coding-agent/src/core/agent-session.ts:191] [E: packages/coding-agent/src/core/agent-session.ts:2571] [E: packages/coding-agent/src/core/agent-session.ts:2573]。constructor 保存 allowlist/denylist/initial active names 后立即调用 `_buildRuntime({ activeToolNames, includeAllExtensionTools: true })` [E: packages/coding-agent/src/core/agent-session.ts:366] [E: packages/coding-agent/src/core/agent-session.ts:378] [E: packages/coding-agent/src/core/agent-session.ts:380]。

`_buildRuntime()` 默认调用 `createAllToolDefinitions(this._cwd, { read: { autoResizeImages }, bash: { commandPrefix, shellPath } })`, 其中没有给 `edit` 注入特殊 options, 所以内置 `edit` 使用 local filesystem 的 default operations [E: packages/coding-agent/src/core/agent-session.ts:2542] [E: packages/coding-agent/src/core/agent-session.ts:2543] [E: packages/coding-agent/src/core/agent-session.ts:2544] [E: packages/coding-agent/src/core/tools/edit.ts:291]。随后 `_buildRuntime()` 把 object entries 放入 `_baseToolDefinitions`, 创建 `ExtensionRunner`, 绑定扩展, 再用默认 active names `["read", "bash", "edit", "write"]` 刷新 tool registry [E: packages/coding-agent/src/core/agent-session.ts:2547] [E: packages/coding-agent/src/core/agent-session.ts:2548] [E: packages/coding-agent/src/core/agent-session.ts:2558] [E: packages/coding-agent/src/core/agent-session.ts:2573] [E: packages/coding-agent/src/core/agent-session.ts:2575]。

`_refreshToolRegistry()` 会先按 `allowedToolNames`/`excludedToolNames` 过滤 built-in 与 custom tool definitions, 再把 built-ins 和 extension/custom tools 都交给 `wrapRegisteredTools` 包装成 `AgentTool` [E: packages/coding-agent/src/core/agent-session.ts:2437] [E: packages/coding-agent/src/core/agent-session.ts:2449] [E: packages/coding-agent/src/core/agent-session.ts:2451] [E: packages/coding-agent/src/core/agent-session.ts:2485] [E: packages/coding-agent/src/core/agent-session.ts:2486]。`wrapToolDefinition` 会把 `prepareArguments` 与 `executionMode` 原样转发给 agent runtime [E: packages/coding-agent/src/core/tools/tool-definition-wrapper.ts:14] [E: packages/coding-agent/src/core/tools/tool-definition-wrapper.ts:15]。
...
built-in `edit` 与 `write` 共用同一个 queue helper; 扩展文档要求 custom mutating tool 也使用 `withFileMutationQueue()` 加入同一 per-file queue, 因为 tool calls 默认并行, 否则两个工具可能读到同一份旧内容并出现 last write wins [E: packages/coding-agent/docs/extensions.md:1858] [E: packages/coding-agent/docs/extensions.md:1864]。file-mutation-queue test 覆盖了并发两个 `edit` 保留两处修改, 以及 `edit` 与 `write` 共享 queue [E: packages/coding-agent/test/file-mutation-queue.test.ts:101] [E: packages/coding-agent/test/file-mutation-queue.test.ts:128] [E: packages/coding-agent/test/file-mutation-queue.test.ts:131] [E: packages/coding-agent/test/file-mutation-queue.test.ts:173]。
