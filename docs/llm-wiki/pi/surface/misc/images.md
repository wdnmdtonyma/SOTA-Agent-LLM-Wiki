...
`renderImage(base64Data, imageDimensions, options)` 是 TUI 终端图像渲染的 surface 入口: 它读取 `getCapabilities().images`,在没有 image protocol 时返回 `null`,在 Kitty 或 iTerm2 能力下返回 escape sequence、占用 columns/rows 与可选 image id [E: packages/tui/src/terminal-image.ts:571] [E: packages/tui/src/terminal-image.ts:575] [E: packages/tui/src/terminal-image.ts:576] [E: packages/tui/src/terminal-image.ts:578] [E: packages/tui/src/terminal-image.ts:601] [E: packages/tui/src/terminal-image.ts:610]。
...
usage 文档把 file arguments 定义为用 `@` 前缀把文件包含进消息,并给出 `pi -p @screenshot.png "What's in this image?"` 的图像示例 [E: packages/coding-agent/docs/usage.md:256] [E: packages/coding-agent/docs/usage.md:258] [E: packages/coding-agent/docs/usage.md:262]。主入口在非 RPC 模式读取 stdin 后调用 `prepareInitialMessage(parsed, settingsManager.getImageAutoResize(), stdinContent)`,并把产出的 `initialMessage`、`initialImages` 传给 interactive 或 print 模式 [E: packages/coding-agent/src/main.ts:874] [E: packages/coding-agent/src/main.ts:875] [E: packages/coding-agent/src/main.ts:882] [E: packages/coding-agent/src/main.ts:884] [E: packages/coding-agent/src/main.ts:931] [E: packages/coding-agent/src/main.ts:936] [E: packages/coding-agent/src/main.ts:964] [E: packages/coding-agent/src/main.ts:968]。

`prepareInitialMessage()` 只在 `parsed.fileArgs.length > 0` 时调用 `processFileArguments()`,并把返回的 `text` 和 `images` 作为 `fileText`、`fileImages` 交给 `buildInitialMessage()` [E: packages/coding-agent/src/main.ts:225] [E: packages/coding-agent/src/main.ts:229] [E: packages/coding-agent/src/main.ts:232] [E: packages/coding-agent/src/main.ts:233]。`buildInitialMessage()` 会拼接 stdin、`@file` text 和第一条 CLI message,并且只有 `fileImages.length > 0` 时才设置 `initialImages` [E: packages/coding-agent/src/cli/initial-message.ts:26] [E: packages/coding-agent/src/cli/initial-message.ts:27] [E: packages/coding-agent/src/cli/initial-message.ts:30] [E: packages/coding-agent/src/cli/initial-message.ts:34] [E: packages/coding-agent/src/cli/initial-message.ts:40] [E: packages/coding-agent/src/cli/initial-message.ts:41]。
...
interactive startup 直接调用 `session.prompt(initialMessage, { images: initialImages })`,print/json 模式也用同样的 `{ images: initialImages }` 发送初始 prompt [E: packages/coding-agent/src/modes/interactive/interactive-mode.ts:1095] [E: packages/coding-agent/src/modes/interactive/interactive-mode.ts:1097] [E: packages/coding-agent/src/modes/print-mode.ts:131] [E: packages/coding-agent/src/modes/print-mode.ts:132]。`AgentSession` 在构造 user message 时先放 `{ type: "text", text: expandedText }`,然后把当前 images 追加到同一个 user content array [E: packages/coding-agent/src/core/agent-session.ts:1216] [E: packages/coding-agent/src/core/agent-session.ts:1217] [E: packages/coding-agent/src/core/agent-session.ts:1218] [E: packages/coding-agent/src/core/agent-session.ts:1221] [E: packages/coding-agent/src/core/agent-session.ts:1222]。

steering 和 follow-up 也能携带 images: `_queueSteer()`、`_queueFollowUp()` 都把 text part 和 optional images 放进 user content,再调用 `agent.steer()` 或 `agent.followUp()` [E: packages/coding-agent/src/core/agent-session.ts:1379] [E: packages/coding-agent/src/core/agent-session.ts:1382] [E: packages/coding-agent/src/core/agent-session.ts:1383] [E: packages/coding-agent/src/core/agent-session.ts:1386] [E: packages/coding-agent/src/core/agent-session.ts:1396] [E: packages/coding-agent/src/core/agent-session.ts:1399] [E: packages/coding-agent/src/core/agent-session.ts:1400] [E: packages/coding-agent/src/core/agent-session.ts:1403]。

`images.blockImages` 是 provider 发送前的防线:SDK wrapper 先 `convertToLlm(messages)`,若 `settingsManager.getBlockImages()` 为 false 就原样返回;若为 true,它把 user/toolResult content array 里的 image part 替换为文本 `"Image reading is disabled."` [E: packages/coding-agent/src/core/sdk.ts:259] [E: packages/coding-agent/src/core/sdk.ts:260] [E: packages/coding-agent/src/core/sdk.ts:262] [E: packages/coding-agent/src/core/sdk.ts:266] [E: packages/coding-agent/src/core/sdk.ts:267] [E: packages/coding-agent/src/core/sdk.ts:270] [E: packages/coding-agent/src/core/sdk.ts:274]。这意味着 blockImages 不阻止 CLI/TUI 先构造 `ImageContent`,而是在 LLM conversion 边界过滤图片 [I]。

## 5 用户设置与显示开关

`TerminalSettings.showImages` 默认 true,只在终端支持图片时有意义;`TerminalSettings.imageWidthCells` 默认 60,是 inline image 的首选 terminal cell 宽度 [E: packages/coding-agent/src/core/settings-manager.ts:39] [E: packages/coding-agent/src/core/settings-manager.ts:40] [E: packages/coding-agent/src/core/settings-manager.ts:41]。settings selector 只有在 `getCapabilities().images` 为真时才展示 `show-images` 和 `image-width-cells`,说明这两个开关控制终端显示,不是控制是否把图片送给模型 [E: packages/coding-agent/src/modes/interactive/components/settings-selector.ts:667] [E: packages/coding-agent/src/modes/interactive/components/settings-selector.ts:670] [E: packages/coding-agent/src/modes/interactive/components/settings-selector.ts:677] [I]。

`ImageSettings.autoResize` 默认 true,`ImageSettings.blockImages` 默认 false;settings selector 把 auto resize 标注为影响 attached/read images,把 block images 标注为阻止图片发送给 LLM providers [E: packages/coding-agent/src/core/settings-manager.ts:46] [E: packages/coding-agent/src/core/settings-manager.ts:47] [E: packages/coding-agent/src/core/settings-manager.ts:48] [E: packages/coding-agent/src/core/settings-manager.ts:1162] [E: packages/coding-agent/src/core/settings-manager.ts:1163] [E: packages/coding-agent/src/core/settings-manager.ts:1175] [E: packages/coding-agent/src/core/settings-manager.ts:1176] [E: packages/coding-agent/src/modes/interactive/components/settings-selector.ts:686] [E: packages/coding-agent/src/modes/interactive/components/settings-selector.ts:689] [E: packages/coding-agent/src/modes/interactive/components/settings-selector.ts:695] [E: packages/coding-agent/src/modes/interactive/components/settings-selector.ts:699]。
...
AgentSession 的顺序是先发 extension `tool_result` hook，再 normalize hook 产出的最终 content；因此 built-in、SDK/custom tool 与 extension 修改后的图片都共享这条进入 history 前的边界 [E: packages/coding-agent/src/core/agent-session.ts:501] [E: packages/coding-agent/src/core/agent-session.ts:503] [E: packages/coding-agent/src/core/agent-session.ts:514] [E: packages/coding-agent/src/core/agent-session.ts:516] [E: packages/coding-agent/src/core/agent-session.ts:518] [E: packages/coding-agent/src/core/agent-session.ts:526] [E: packages/coding-agent/src/core/agent-session.ts:527]。

## 6 TUI 终端渲染

`detectCapabilities()` 在 tmux 或 screen 下把 `images` 设为 `null`;Kitty、Ghostty、WezTerm、Warp 走 `"kitty"`,iTerm2 走 `"iterm2"`,Windows Terminal、VS Code、Alacritty、JetBrains terminal 和未知终端都不声明 image protocol [E: packages/tui/src/terminal-image.ts:68] [E: packages/tui/src/terminal-image.ts:78] [E: packages/tui/src/terminal-image.ts:79] [E: packages/tui/src/terminal-image.ts:83] [E: packages/tui/src/terminal-image.ts:84] [E: packages/tui/src/terminal-image.ts:87] [E: packages/tui/src/terminal-image.ts:91] [E: packages/tui/src/terminal-image.ts:95] [E: packages/tui/src/terminal-image.ts:100] [E: packages/tui/src/terminal-image.ts:104] [E: packages/tui/src/terminal-image.ts:108] [E: packages/tui/src/terminal-image.ts:112] [E: packages/tui/src/terminal-image.ts:116] [E: packages/tui/src/terminal-image.ts:120] [E: packages/tui/src/terminal-image.ts:135]。

Kitty encoding chunks base64 payloads at 4096 characters and can include columns, rows, imageId and cursor-movement options;delete helpers emit Kitty delete sequences for one image id or all visible images [E: packages/tui/src/terminal-image.ts:176] [E: packages/tui/src/terminal-image.ts:186] [E: packages/tui/src/terminal-image.ts:191] [E: packages/tui/src/terminal-image.ts:192] [E: packages/tui/src/terminal-image.ts:193] [E: packages/tui/src/terminal-image.ts:195] [E: packages/tui/src/terminal-image.ts:203] [E: packages/tui/src/terminal-image.ts:208] [E: packages/tui/src/terminal-image.ts:211] [E: packages/tui/src/terminal-image.ts:213] [E: packages/tui/src/terminal-image.ts:226] [E: packages/tui/src/terminal-image.ts:234]。iTerm2 encoding emits OSC 1337 `File=` with inline flag, optional width/height/name and preserveAspectRatio flag [E: packages/tui/src/terminal-image.ts:243] [E: packages/tui/src/terminal-image.ts:245] [E: packages/tui/src/terminal-image.ts:258] [E: packages/tui/src/terminal-image.ts:259] [E: packages/tui/src/terminal-image.ts:260] [E: packages/tui/src/terminal-image.ts:264] [E: packages/tui/src/terminal-image.ts:268]。

`Image` component 先从 image bytes 解析 dimensions,否则默认 800x600;render 时根据可用 width、配置 maxWidthCells 和 terminal cell dimensions 计算 max height,能渲染就调用 `renderImage()`,否则使用 `imageFallback()` 输出 `[Image: ...]` 文本 [E: packages/tui/src/components/image.ts:25] [E: packages/tui/src/components/image.ts:47] [E: packages/tui/src/components/image.ts:66] [E: packages/tui/src/components/image.ts:67] [E: packages/tui/src/components/image.ts:68] [E: packages/tui/src/components/image.ts:74] [E: packages/tui/src/components/image.ts:78] [E: packages/tui/src/components/image.ts:113] [E: packages/tui/src/components/image.ts:114] [E: packages/tui/src/components/image.ts:118] [E: packages/tui/src/terminal-image.ts:644] [E: packages/tui/src/terminal-image.ts:656]。

fullscreen 使用应用自有 viewport scrolling：Kitty graphics 支持 placement 删除、重放与裁剪，因而可继续显示；iTerm2 inline-image protocol 不能删除/裁剪 placement，所以 fullscreen 中降级为 text placeholder。regular mode 仍使用 terminal scrollback，iTerm2 inline images 继续正常渲染 [E: packages/coding-agent/docs/usage.md:249] [E: packages/coding-agent/docs/usage.md:250]。

tool result image rendering 还受 `showImages` 与 terminal capability 共同门控;当前代码在 Kitty 下跳过非 PNG image mime,这说明 tool result display 与 message input attachment 的支持格式不是同一层能力 [E: packages/coding-agent/src/modes/interactive/components/tool-execution.ts:342] [E: packages/coding-agent/src/modes/interactive/components/tool-execution.ts:346] [E: packages/coding-agent/src/modes/interactive/components/tool-execution.ts:349] [E: packages/coding-agent/src/modes/interactive/components/tool-execution.ts:350] [I]。
