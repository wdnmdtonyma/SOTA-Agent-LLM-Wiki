...
Provider docs 把配置分成 OAuth subscription 和 API key 两类：交互模式用 `/login`/`/logout` 管理 credential，API-key provider 也能直接从环境变量启动 [E: packages/coding-agent/docs/providers.md:3] [E: packages/coding-agent/docs/providers.md:17] [E: packages/coding-agent/docs/providers.md:25] [E: packages/coding-agent/docs/providers.md:54] [E: packages/coding-agent/docs/providers.md:58]。用户再通过 `/model`、CLI `--provider`/`--model` 或 embedding API 选择具体模型；完整登录与 credential precedence 由 [surface.providers.auth](auth.md) 解释。[I]

## 内置集合与 static catalog

runtime ground truth 是 `builtinProviders()`：当前有 36 个 provider objects，包含位于 OpenRouter 与 Together 之间的动态 Radius [E: packages/ai/src/providers/all.ts:78] [E: packages/ai/src/providers/all.ts:105] [E: packages/ai/src/providers/all.ts:106] [E: packages/ai/src/providers/all.ts:115]。`builtinModels()` 把这 36 个 objects 全部写入 `Models` collection [E: packages/ai/src/providers/all.ts:120] [E: packages/ai/src/providers/all.ts:122] [E: packages/ai/src/providers/all.ts:123]。

generated `MODELS` 是另一套 35-bucket static catalog：`BuiltinProvider` 取 `keyof typeof MODELS`，`getBuiltinProviders()` 只返回 `Object.keys(MODELS)`，而 Radius 只出现在 runtime array [E: packages/ai/src/providers/all.ts:48] [E: packages/ai/src/providers/all.ts:64] [E: packages/ai/src/providers/all.ts:65] [E: packages/ai/src/providers/all.ts:106]。所以 provider catalog 按 runtime array 计 36，model catalog 按 committed structural shards 计 35。[I]

图片生成 provider 另走 `builtinImagesProviders()`/`builtinImagesModels()`，当前只注册 OpenRouter Images，不进入 chat/text `Models` [E: packages/ai/src/providers/all.ts:129] [E: packages/ai/src/providers/all.ts:130] [E: packages/ai/src/providers/all.ts:134] [E: packages/ai/src/providers/all.ts:139]。
...
Docs 给出的用户可见 credential precedence 是 CLI `--api-key`、`auth.json`、环境变量、`models.json` provider key [E: packages/coding-agent/docs/providers.md:290] [E: packages/coding-agent/docs/providers.md:297]。Provider-scoped credential env 能覆盖进程环境，并承载 Cloudflare、Azure、Vertex、Bedrock 等附加配置 [E: packages/coding-agent/docs/providers.md:119] [E: packages/coding-agent/docs/providers.md:137]。

## Custom provider 的两条路

`models.json` 适合复用现有 wire protocol 的 base URL、headers、auth 与 model list；extension 适合新 stream implementation、OAuth 或自定义生命周期 [E: packages/coding-agent/docs/providers.md:284] [I]。`createProvider()` 支持 static baseline `models`、可选 `fetchModels()` dynamic overlay、credential filter，以及单一 API 或按 `model.api` 的 map [E: packages/ai/src/models.ts:533] [E: packages/ai/src/models.ts:542] [E: packages/ai/src/models.ts:544] [E: packages/ai/src/models.ts:545] [E: packages/ai/src/models.ts:547]。
...
- Radius 属于 `pi-ai` 的 36 个 runtime built-ins，但没有 static shard；它从 stored/gateway config 产生 `pi-messages` models [E: packages/ai/src/providers/radius.ts:20] [E: packages/ai/src/providers/radius.ts:35] [E: packages/ai/src/providers/radius.ts:54] [E: packages/ai/src/providers/radius.ts:56]。
...
- `getBuiltinProviders()` 名字指 generated catalog keys，不是 `builtinProviders()` runtime objects [E: packages/ai/src/providers/all.ts:64] [E: packages/ai/src/providers/all.ts:78]。
...
- [ref.ai.provider-catalog](../../reference/provider-catalog.md): 36 个 runtime built-in provider 目录。
