...
- Pi monorepo 里 `chord`、`pi-ai`、`pi-agent-core`、`pi-coding-agent`、`pi-tui`、`pi-server` 各是什么意思？`pi-client` 的公开类名是什么？
...
| Pi monorepo | README 的 All Packages 表现在列出 `chord`、`pi-telemetry`、`pi-ai`、`pi-agent-core`、`pi-coding-agent`、`pi-tui` 六个公开包；“monorepo”是本 wiki 对该源码树的组织性叫法。[I] | [E: README.md:26] [E: README.md:30] [E: README.md:31] [E: README.md:32] [E: README.md:33] [E: README.md:34] [E: README.md:35] |
| `chord` / `@earendil-works/chord` | README 定义为 standalone application-composition runtime，覆盖 services、replicated state、RPC 与 plugins。 | [E: README.md:31] |
| `pi-telemetry` / `@earendil-works/pi-telemetry` | README 定义为 vendor-neutral telemetry contracts、reference adapter、conformance tests 与 typed schemas。 | [E: README.md:32] |
| `pi-ai` / `@earendil-works/pi-ai` | README 定义为 Unified multi-provider LLM API，并例举 OpenAI、Anthropic、Google 等 provider 语境。 | [E: README.md:33] |
| `pi-agent-core` / `@earendil-works/pi-agent-core` | README 定义为带 tool calling 和 state management 的 agent runtime。 | [E: README.md:34] |
| `pi-coding-agent` / `@earendil-works/pi-coding-agent` | README 定义为 Interactive coding agent CLI。 | [E: README.md:34] |
| `pi-tui` / `@earendil-works/pi-tui` | README 定义为带 differential rendering 的 Terminal UI library。 | [E: README.md:35] |
| `pi-client` / `@earendil-works/pi-client` | 公开面是 Chord 风格的 `Client` + `createClientServiceTransport`，不是 `PiClient` / `PiSessionHandle`。README All Packages 短表不列此包。[I] | [I] |
...
| 术语 | 中文解释 | 源码锚点 |
|---|---|---|
| Agent | README 把 `pi-agent-core` 定位为带 tool calling 和 state management 的 agent runtime；`Agent` 类和状态细节由 [spine.agent-loop](../spine/agent-loop.md) 与 [subsys.agent-core.turn-control](../subsystems/agent-core/turn-control.md) 证明。[I] | [E: README.md:34] |
| Agent state | agent runtime 的 state management 属于 README 公开定位；具体 state 字段与重置行为由 [subsys.agent-core.message-model](../subsystems/agent-core/message-model.md) 和 [subsys.agent-core.turn-control](../subsystems/agent-core/turn-control.md) 覆盖。[I] | [E: README.md:33] |
...
| Tool call | README 只公开说明 `pi-agent-core` 支持 tool calling；tool call 的消息块、校验、hook 与执行分派由 [spine.tool-call-anatomy](../spine/tool-call-anatomy.md) 和 [subsys.agent-core.tool-invocation](../subsystems/agent-core/tool-invocation.md) 证明。[I] | [E: README.md:33] |
...
| 术语 | 中文解释 | 源码锚点 |
|---|---|---|
| Provider | README 把 `pi-ai` 定义为 multi-provider LLM API；provider 的源码形态、built-in 集合和注册细节看 [surface.providers.overview](../surface/providers/overview.md) 与 [subsys.ai.provider-registry](../subsystems/ai/provider-registry.md)。[I] | [E: README.md:33] |
...
| Built-in providers | README 只例举 OpenAI、Anthropic、Google 等 provider 语境；当前 built-in provider catalog 以 [ref.ai.provider-catalog](provider-catalog.md) 为准。[I] | [E: README.md:33] |
...
| 术语 | 中文解释 | 源码锚点 |
|---|---|---|
| Built-in tools | README 公开说明 `pi-agent-core` 支持 tool calling；`pi-coding-agent` 的具体 built-in tool 清单由 [ref.tools-catalog](tools-catalog.md) 与 `surface.tools.*` 节点逐项覆盖。[I] | [E: README.md:33] |
...
| telemetry | vendor-neutral telemetry contracts 包,以及 coding-agent 的 install ping / analytics settings;包级见 README,产品实现见 [subsys.coding-agent.telemetry](../subsystems/coding-agent/telemetry.md)。[I] | [E: README.md:32] |
...
- README 明确说明 Pi 默认没有内置 permission system；需要更强边界时应 containerize 或 sandbox Pi，并列出 Gondolin extension、Plain Docker、OpenShell 三种方向。[E: README.md:41] [E: README.md:43] [E: README.md:46] [E: README.md:47] [E: README.md:47]
- README 的 Development 段列出 `npm install --ignore-scripts`、`npm run build`、`npm run check`、`./test.sh`、`./pi-test.sh` 等开发入口；更细的测试/命令策略由 `AGENTS.md` 的 Development Rules 约束。[E: README.md:57] [E: README.md:57] [E: README.md:60] [E: README.md:61] [E: README.md:61] [E: AGENTS.md:1]
- package-internal、RPC/`PiServer`、provider/model 等术语在本页标 `[I]`，其源码证明责任属于链接到的 `spine.*`、`surface.*`、`subsys.*` 或 `ref.*` 节点。
- README 的 All Packages 表现在直接列出 `chord`、`pi-telemetry`、`pi-ai`、`pi-agent-core`、`pi-coding-agent`、`pi-tui`；`pi-client` 的公开类名是 `Client` 不是 `PiClient`。`pi-server` 与 `session-backends` 的 experimental/stability 口径不在本 glossary 节点用 README/AGENTS 直证。[E: README.md:26] [E: README.md:30] [E: README.md:31] [E: README.md:32] [E: README.md:33] [E: README.md:34] [E: README.md:35] [I]
- 已删除术语:`ServerSupervisor`、legacy JSONL IPC、`ServerRequest`、RPC stream bridge。这些对象已不在 `packages/server` 源码中。Radius 不是已删除术语：`radiusProvider` 与 `loadRadiusOAuth` 仍在 `packages/ai`。[I]
