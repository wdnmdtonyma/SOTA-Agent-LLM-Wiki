...
- `instances.json`、`auth.json`、`machine.json`、`server.sock` 的路径 helper 如何派生文件名?
- server directory 当前优先读哪个环境变量,没有时回退到哪里?
...
本节点不逐条证明状态写入点、状态迁移、JSON 读写、restart recovery 或 supervisor lifecycle;这些属于 [subsys.server.storage](../subsystems/server/storage.md) 与 supervisor 相关节点的行为范围。[I]
...
- [subsys.server.storage](../subsystems/server/storage.md): `instances.json` 的读写、upsert/remove 与 instance record 持久化行为。
