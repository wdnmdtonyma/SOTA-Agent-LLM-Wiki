...
server 包稳定性是 experimental:包 README 明确说该 package 处于 active development,CLI、API 和 behavior 尚不稳定,可能无通知变更或移除 [E: packages/server/README.md:3]。本节点只记录当前 `a8ee03b815` 源码快照的 message shape,不把这些 IPC 字段描述成稳定外部 API [I]。
