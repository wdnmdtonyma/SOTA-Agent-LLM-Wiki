...
- 已按目标 `builtinProviders()` 覆盖 39 个 runtime provider，包括新增 Baseten 与没有 structural model shard 的 Radius；catalog title 与 index group 已同步。本轮未留下 `[U]`。

## ai-provider-registry

# uncertainty-ai-provider-registry

- 已区分 39 个 runtime built-in providers 与 38 个 generated structural buckets；Baseten 同时属于两者，Radius 仍是 runtime-only。本轮未留下 `[U]`。
