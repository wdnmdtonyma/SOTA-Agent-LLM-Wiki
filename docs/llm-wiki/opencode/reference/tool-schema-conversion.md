...
| number + non-finite enum flatten | `anyOf` 含一个 `type: "number"` 以及其余全是 `NaN`/`Infinity`/`-Infinity` enum。[E: packages/opencode/src/tool/json-schema.ts:57] [E: packages/opencode/src/tool/json-schema.ts:62] | 丢掉 union，只保留 number schema。 | Effect JSON schema 会把非有限数字编成 enum 字符串；provider 不需要这些 branch。[I] |
| empty struct union flatten | `anyOf` 恰好两项：无 properties 的 object 与无 items 的 array。[E: packages/opencode/src/tool/json-schema.ts:67] [E: packages/opencode/src/tool/json-schema.ts:102] | 变成 `{ type: "object", properties: {} }`。 | 压平 Effect empty-struct 编码。[I] |
