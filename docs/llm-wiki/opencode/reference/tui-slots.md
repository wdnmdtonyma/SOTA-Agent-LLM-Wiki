...
| `session_prompt` | `{ session_id, visible?, disabled?, on_submit?, ref? }` [E: packages/plugin/src/tui.ts:464] [E: packages/plugin/src/tui.ts:468] | session route [E: packages/tui/src/routes/session/index.tsx:1314] | `replace` [E: packages/tui/src/routes/session/index.tsx:1315] | 替换 session prompt，并继承可见性、禁用态和提交回调。 |
| `session_prompt_right` | `{ session_id }` [E: packages/plugin/src/tui.ts:471] | session prompt right prop [E: packages/tui/src/routes/session/index.tsx:1330] | default | session prompt 右侧附加 UI。 |
