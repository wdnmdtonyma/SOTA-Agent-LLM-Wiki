...
5. registry 中 `lsp` tool 只在 `flags.experimentalLspTool` 开启时加入模型工具集合。[E: packages/opencode/src/tool/registry.ts:247]
...
`experimentalLspTool` gating 说明暴露给模型的 `lsp` tool 仍是实验功能；LSP service 作为 V1 runtime layer 初始化，但模型不一定能主动调用 `lsp` tool。[E: packages/opencode/src/tool/registry.ts:224] [E: packages/opencode/src/tool/registry.ts:247] [I]
