...
V1 远程 skill discovery 在 `packages/opencode/src/skill/discovery.ts`，它从 URL 拉取 index、下载文件到 cache，并返回含 `SKILL.md` 的本地目录。[E: packages/opencode/src/skill/discovery.ts:49] [E: packages/opencode/src/skill/discovery.ts:88]
...
1. V1 `skill` tool id 是 `skill`。[E: packages/opencode/src/tool/skill.ts:13]
2. 参数只有 `name`，工具会先用 `skill.require(name)` 读取完整 skill；service 层找不到时会产生 `Skill.NotFoundError`。[E: packages/opencode/src/tool/skill.ts:8] [E: packages/opencode/src/tool/skill.ts:23] [E: packages/opencode/src/skill/index.ts:294] [E: packages/opencode/src/skill/index.ts:298]
3. tool 在读取 skill 后请求 permission，permission action 是 `skill`，pattern 和 always 都是 skill name。[E: packages/opencode/src/tool/skill.ts:23] [E: packages/opencode/src/tool/skill.ts:27]
4. tool 输出包含 `skill_content`、`base directory`、相对路径提示，以及最多 10 个非 `SKILL.md` 文件样本。[E: packages/opencode/src/tool/skill.ts:46] [E: packages/opencode/src/tool/skill.ts:53]
5. 文件采样通过 ripgrep `find`，隐藏文件开启，排除 `**/SKILL.md`，limit 是 10。[E: packages/opencode/src/tool/skill.ts:36]

## V2

### 职责

V2 `SkillV2.Service` 把 skill 抽象为 source 列表、transform hook、list cache；source 可以来自 Directory、Url、Embedded。[E: packages/core/src/skill.ts:49] [E: packages/schema/src/skill.ts:8] [E: packages/schema/src/skill.ts:14] [E: packages/schema/src/skill.ts:29] [E: packages/core/src/skill.ts:109] 这是 V2 core 的 Effect-native service，不是 V1 `packages/opencode/src/skill` 的复用实现。[I]

V2 `SkillGuidance.Service` 不把完整 skill body塞进 system prompt；它只为当前 agent 可用且有 description 的 skill 渲染 guidance，并通过 `SystemContext` 做 baseline/update/removed 增量。[E: packages/core/src/skill/guidance.ts:49] [E: packages/core/src/skill/guidance.ts:57]

### 数据模型

V2 `Source` 有三种 tag：Directory 含 `path`，Url 含 `url`，Embedded 含完整 `SkillV2.Info`。[E: packages/schema/src/skill.ts:8] [E: packages/schema/src/skill.ts:14] [E: packages/schema/src/skill.ts:29] source key 是 `directory:<path>`、`url:<url>`、或 `embedded:<name>`。[E: packages/schema/src/skill.ts:48]

V2 `Info` 包含 `name`、`description`、可选 `slash`、`location`、`content`。[E: packages/schema/src/skill.ts:20] [E: packages/schema/src/skill.ts:21] [E: packages/schema/src/skill.ts:22] [E: packages/schema/src/skill.ts:23] [E: packages/schema/src/skill.ts:24] [E: packages/schema/src/skill.ts:25] frontmatter 允许 `name`、`description`、`slash`。[E: packages/core/src/skill.ts:34] [E: packages/core/src/skill.ts:35] [E: packages/core/src/skill.ts:36]

V2 config 层暴露 `skills` string array，描述是 "Additional paths or URLs to discover skills from"。[E: packages/core/src/config.ts:90] [E: packages/core/src/config.ts:91]

### 加载流程

1. `SkillV2.Service` 的 state 保存 source list，当前 layer 的初始 sources 是空数组。[E: packages/core/src/skill.ts:40] [E: packages/core/src/skill.ts:63]
2. `load(source)` 对 Embedded 直接返回 embedded skill；Directory/Url source 先经过 discovery，再 glob `{*.md,**/SKILL.md}`。[E: packages/core/src/skill.ts:75] [E: packages/core/src/skill.ts:78]
3. frontmatter name 优先；没有 frontmatter name 时，只有位于 source root 目录下的 markdown file 会用 basename 作为 skill name，nested file 没有 fallback name 会被跳过。[E: packages/core/src/skill.ts:88] [E: packages/core/src/skill.ts:91] [E: packages/core/src/skill.ts:94]
4. list 有 cache map；`list()` 会对每个 source key load/cache，按 skill name 去重后返回 values。[E: packages/core/src/skill.ts:109] [E: packages/core/src/skill.ts:113] [E: packages/core/src/skill.ts:116]
5. V2 `list()` 使用 local cache map，并在 source key 命中时复用 loaded skills；当前可见 list 逻辑里未看到 filesystem watch invalidation。[E: packages/core/src/skill.ts:109] [E: packages/core/src/skill.ts:113] [I]

### V2 remote discovery

1. V2 discovery 对 URL path segment 和 relative path 做安全过滤，防止 unsafe segment 或 path traversal。[E: packages/core/src/skill/discovery.ts:15] [E: packages/core/src/skill/discovery.ts:31]
2. 每个 remote skill 必须存在 `SKILL.md` 或 `${skill.name}.md`，否则跳过。[E: packages/core/src/skill/discovery.ts:119]
3. 下载文件前会校验 skill root 仍在 source root 内，后续 destination containment 校验防止恶意 index 写出 skill root。[E: packages/core/src/skill/discovery.ts:123] [E: packages/core/src/skill/discovery.ts:124]
4. HTTP fetch 使用 `HttpClient.retryTransient({ retryOn: "errors-and-responses", times: 2 })`。[E: packages/core/src/skill/discovery.ts:76] [E: packages/core/src/skill/discovery.ts:79]
...
两代都避免把所有 skill body 默认塞进 prompt：V1 用 `skill` tool 按 name 加载完整内容，V2 更明确地把 guidance 和 content loading 分离。[I] V1 tool 输出完整 content 的行为在 `packages/opencode/src/tool/skill.ts`，V2 guidance 只渲染 name/description 的行为在 `packages/core/src/skill/guidance.ts`。[E: packages/opencode/src/tool/skill.ts:46] [E: packages/core/src/skill/guidance.ts:52]

V2 source abstraction 是为了让 plugin/config/embedded skill 都能进入同一 list pipeline。[I] `SkillV2.Source` 的 equals/key 实现把不同来源标准化成 identity，state transform 用 equals 去重，list cache 用 key 缓存。[E: packages/schema/src/skill.ts:41] [E: packages/schema/src/skill.ts:48] [E: packages/core/src/skill.ts:66] [E: packages/core/src/skill.ts:113]
