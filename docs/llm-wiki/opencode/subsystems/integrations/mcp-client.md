...
`packages/opencode/src/mcp/index.ts` 定义 `MCP.Service`，接口暴露 status、tools、prompts、resources、prompt/resource lookup、OAuth auth/removeAuth、动态 add/remove 等方法。[E: packages/opencode/src/mcp/index.ts:159] service state 从 `cfg.mcp ?? {}` 初始化并连接 server；tool-list-change notification 会重新拉取 defs 并发布 `mcp.tools.changed`。[E: packages/opencode/src/mcp/index.ts:477] [E: packages/opencode/src/mcp/index.ts:485] [E: packages/opencode/src/mcp/index.ts:443] [E: packages/opencode/src/mcp/index.ts:446] [E: packages/opencode/src/mcp/index.ts:451] 服务状态枚举区分 `connected`、`disabled`、`failed`、`needs_auth`、`needs_client_registration`，因此调用方可以把连接失败、显式禁用、OAuth 待处理分开呈现。[E: packages/opencode/src/mcp/index.ts:95]

`packages/opencode/src/mcp/catalog.ts` 是 MCP catalog adapter：`defs` 读取 tools，`prompts/resources` 读取对应 capability，`fetch` 会把 server name 和 item name sanitize/qualify。[E: packages/opencode/src/mcp/catalog.ts:38] [E: packages/opencode/src/mcp/catalog.ts:112] [E: packages/opencode/src/mcp/catalog.ts:120] [E: packages/opencode/src/mcp/catalog.ts:100] `packages/opencode/src/mcp/oauth-provider.ts` 是 MCP SDK `OAuthClientProvider` 的本地实现，负责 redirect URI、client metadata、动态注册信息、token、PKCE code verifier、CSRF state。[E: packages/opencode/src/mcp/oauth-provider.ts:35] [E: packages/opencode/src/mcp/oauth-provider.ts:43] [E: packages/opencode/src/mcp/oauth-provider.ts:82] [E: packages/opencode/src/mcp/oauth-provider.ts:113] [E: packages/opencode/src/mcp/oauth-provider.ts:132] [E: packages/opencode/src/mcp/oauth-provider.ts:144]
...
`MCP.Status` 是以 `status` 为 discriminator 的 union：`connected`、`disabled`、`failed`、`needs_auth`、`needs_client_registration`；只有 `failed` 与 `needs_client_registration` 在 schema 上带 `error` 字段。[E: packages/opencode/src/mcp/index.ts:95] [E: packages/opencode/src/mcp/index.ts:101] [E: packages/opencode/src/mcp/index.ts:107] [E: packages/opencode/src/mcp/index.ts:118]
...
V1 config schema 中 local MCP server 有 `type: "local"`、`command`、`environment`、`enabled`、`timeout` 字段。[E: packages/core/src/v1/config/mcp.ts:6] remote MCP server 有 `type: "remote"`、`url`、`enabled`、`headers`、`oauth`、`timeout` 字段。[E: packages/core/src/v1/config/mcp.ts:44]

## 控制流

### 启动与连接

1. `MCP.Service` 启动时读取 `cfg.mcp ?? {}` 并遍历静态配置项，初始化 `status`、`clients`、`defs`；state 里的 `config` 初始为空，后续动态 `add()` 路径会写入 `s.config[name]`，静态配置通过 `cfg` fallback 读取。[E: packages/opencode/src/mcp/index.ts:475] [E: packages/opencode/src/mcp/index.ts:478] [E: packages/opencode/src/mcp/index.ts:485] [E: packages/opencode/src/mcp/index.ts:501] [E: packages/opencode/src/mcp/index.ts:605] [E: packages/opencode/src/mcp/index.ts:624]
2. disabled server 直接进入 `disabled` 状态，不创建 transport。[E: packages/opencode/src/mcp/index.ts:494]
3. local server 把 config 中的 `command` 数组解构成 `cmd` 和 `args`，再创建 `StdioClientTransport`。[E: packages/opencode/src/mcp/index.ts:331] [E: packages/opencode/src/mcp/index.ts:334]
4. local stdio transport 会继承并扩展环境变量；当命令本体是 `opencode` 时，额外设置 `BUN_BE_BUN=1`。[E: packages/opencode/src/mcp/index.ts:341]
5. remote server 先 parse URL，再构造 `StreamableHTTPClientTransport` 和 `SSEClientTransport` 两个候选 transport，尝试顺序是 StreamableHTTP 再 SSE。[E: packages/opencode/src/mcp/index.ts:229] [E: packages/opencode/src/mcp/index.ts:256] [E: packages/opencode/src/mcp/index.ts:265]
...
7. server 连接成功后，如果 server capability 包含 tools，service 只拉取 `McpCatalog.defs` 作为 cached tool defs；prompts/resources 由 `MCP.prompts()` 和 `MCP.resources()` 后续按 connected clients 懒收集。[E: packages/opencode/src/mcp/index.ts:377] [E: packages/opencode/src/mcp/index.ts:501] [E: packages/opencode/src/mcp/index.ts:674] [E: packages/opencode/src/mcp/index.ts:678]
8. finalizer 关闭所有 MCP clients；如果 transport 是 stdio，还会杀掉 stdio 进程的 descendant processes。[E: packages/opencode/src/mcp/index.ts:510] [E: packages/opencode/src/mcp/index.ts:519] [E: packages/opencode/src/mcp/index.ts:528]

### Tool listing 与执行

1. `McpCatalog.defs` 先调用 `client.listTools`，如果标准 SDK schema 因 `outputSchema` 不兼容而失败，会退回 tolerant request schema。[E: packages/opencode/src/mcp/catalog.ts:38] [E: packages/opencode/src/mcp/catalog.ts:135] [E: packages/opencode/src/mcp/catalog.ts:138]
...
4. catalog `fetch` helper 对 prompts/resources 这类 named items 使用 `${sanitize(client)}:${sanitize(item.name)}` key，并在返回对象上保留原始 `client` 字段。[E: packages/opencode/src/mcp/catalog.ts:101] [E: packages/opencode/src/mcp/catalog.ts:103]
5. `MCP.Service.tools()` 注入到 session 时使用 `${sanitize(clientName)}_${sanitize(toolName)}` 作为最终 MCP tool key。[E: packages/opencode/src/mcp/index.ts:646]
6. `sanitize` 只保留 `a-zA-Z0-9_-`，其它字符都替换成 `_`。[E: packages/opencode/src/mcp/catalog.ts:110]
...
5. `startAuth` 只对 remote MCP server 有效；local server 或显式 `oauth: false` 的 remote server 会报错；未提供 object OAuth config 时，`oauthConfig` 为 undefined 且流程继续构造 auth provider。[E: packages/opencode/src/mcp/index.ts:748] [E: packages/opencode/src/mcp/index.ts:750] [E: packages/opencode/src/mcp/index.ts:751] [E: packages/opencode/src/mcp/index.ts:756] [E: packages/opencode/src/mcp/index.ts:771]
6. `startAuth` 启动 callback server、生成 state，并仅用 `StreamableHTTPClientTransport` 发起 auth discovery。[E: packages/opencode/src/mcp/index.ts:764] [E: packages/opencode/src/mcp/index.ts:766] [E: packages/opencode/src/mcp/index.ts:788]
7. 如果 unauthorized error 携带 captured authorization URL，service 保存 pending transport 并返回 auth URL 给调用端。[E: packages/opencode/src/mcp/index.ts:803]
8. `authenticate` 在无 auth URL 时会直接尝试保存 client；有 auth URL 时会打开浏览器、等待 callback、校验 state、再进入 `finishAuth`。[E: packages/opencode/src/mcp/index.ts:813] [E: packages/opencode/src/mcp/index.ts:836] [E: packages/opencode/src/mcp/index.ts:861]
9. `finishAuth` 从 pending transport 取回 transport，调用 MCP SDK `finishAuth(code)`，随后清理 verifier 和 pending transport，再重建 server client。[E: packages/opencode/src/mcp/index.ts:872] [E: packages/opencode/src/mcp/index.ts:876] [E: packages/opencode/src/mcp/index.ts:886]

## 设计动机与权衡

V1 MCP client 明确偏向“尽量接上”：remote 传输同时支持 StreamableHTTP 和 SSE fallback，这是为了兼容不同 MCP server 的传输实现。[E: packages/opencode/src/mcp/index.ts:256] [I] OAuth client 信息支持 dynamic registration，本地 auth 读写也通过 `flock.withLock(lockKey)` 包裹，这让用户能长期复用 MCP server 登录状态。[E: packages/opencode/src/mcp/oauth-provider.ts:82] [E: packages/opencode/src/mcp/auth.ts:74] [E: packages/opencode/src/mcp/auth.ts:82] [I]

工具 catalog 使用 tolerant schema 是兼容性取舍：一些 MCP server 返回的 `outputSchema` 不满足 SDK validator，opencode 会先走标准 SDK，失败后用更宽松 schema 再列 tools。[E: packages/opencode/src/mcp/catalog.ts:135] [E: packages/opencode/src/mcp/catalog.ts:138] 这让工具可见性优先于严格 schema 校验[I]，但执行阶段仍通过 MCP SDK `callTool`。[E: packages/opencode/src/mcp/catalog.ts:54]

MCP 工具名同时携带 client name 和 tool name，是为了避免多个 server 暴露同名工具时发生冲突。[I] prompts/resources catalog 用冒号，最终 session tool key 用下划线，说明“内部资源标识”和“模型可见工具名”是两个命名层。[E: packages/opencode/src/mcp/catalog.ts:103] [E: packages/opencode/src/mcp/index.ts:646]
...
- remote 连接顺序是 StreamableHTTP 然后 SSE fallback，不是只支持 SSE。[E: packages/opencode/src/mcp/index.ts:256] [E: packages/opencode/src/mcp/index.ts:265]
...
- MCP server notification `ToolListChanged` 会触发重新 fetch defs 并发布 `mcp.tools.changed`，所以工具集合可能在 session 运行期间变化。[E: packages/opencode/src/mcp/index.ts:443] [E: packages/opencode/src/mcp/index.ts:446] [E: packages/opencode/src/mcp/index.ts:451]
