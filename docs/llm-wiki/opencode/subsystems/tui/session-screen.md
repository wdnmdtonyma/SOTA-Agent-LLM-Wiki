...
| `foregroundTasks()` | 仅当 `sync.data.capabilities.experimentalBackgroundSubagents` 为真时，收集 running `task` 且 `metadata.background !== true` 的 parts；否则恒为空数组 | 决定 `session.background` command 是否 enabled。[E: packages/tui/src/routes/session/index.tsx:220] [E: packages/tui/src/routes/session/index.tsx:221] [E: packages/tui/src/routes/session/index.tsx:226] [E: packages/tui/src/routes/session/index.tsx:228] |
...
`UserMessage` 从 parts 里只拼接 non-synthetic text parts，并把 file parts 按 mime 显示 `Directory`/`File` badge 加 filename；queued user message 会显示 `QUEUED`，普通 message 可按 timestamp toggle 显示时间。[E: packages/tui/src/routes/session/index.tsx:1374] [E: packages/tui/src/routes/session/index.tsx:1377] [E: packages/tui/src/routes/session/index.tsx:1385] [E: packages/tui/src/routes/session/index.tsx:1425] [E: packages/tui/src/routes/session/index.tsx:1429] [E: packages/tui/src/routes/session/index.tsx:1451]
