...
| `packages/tui/src/app.tsx` | `Effect.fn("Tui.run")`、OpenTUI renderer lifecycle、Solid provider stack、global command palette、route switch。[E: packages/tui/src/app.tsx:184] [E: packages/tui/src/app.tsx:240] [E: packages/tui/src/app.tsx:554] [E: packages/tui/src/app.tsx:1099] |
...
| `packages/tui/src/context/sync.tsx` | legacy/current TUI server-state mirror；bootstrap 后进入 `partial`/`complete` 状态。[E: packages/tui/src/context/sync.tsx:64] [E: packages/tui/src/context/sync.tsx:512] [E: packages/tui/src/context/sync.tsx:531] [I] |
| `packages/tui/src/context/data.tsx` | V2 `session.next.*`/location data mirror，和 `SyncProvider` 在 root provider stack 中并存。[E: packages/tui/src/context/data.tsx:47] [E: packages/tui/src/context/data.tsx:132] [E: packages/tui/src/app.tsx:302] [E: packages/tui/src/app.tsx:303] |
...
`TuiInput` 把 TUI 的 host dependence 显式化：transport (`url`/`fetch`/`headers`/`events`)、launch args (`args`)、resolved TUI config (`config`)、heap snapshot hook (`onSnapshot`) 和 host-owned plugin bridge (`pluginHost`) 都从外部传入。[E: packages/tui/src/app.tsx:138] [E: packages/tui/src/app.tsx:146] `SDKProvider` 接收同一批 transport 字段并传给 `createOpencodeClient({ baseUrl, signal, directory, fetch, headers })`。[E: packages/tui/src/context/sdk.tsx:24] [E: packages/tui/src/context/sdk.tsx:29]

`App` 内部同时消费 `useRoute`、`useSync`、`useProject`、`usePluginRuntime`、`useTheme`、`useKV`、`useSDK`、`useDialog`、`usePromptRef` 等 contexts，说明根组件是 orchestration layer，而不是纯展示组件。[E: packages/tui/src/app.tsx:360] [E: packages/tui/src/app.tsx:379]

## 控制流

1. `run` 进入 `Effect.scoped`，通过 `Effect.acquireRelease` 创建 `CliRenderer`；renderer 使用 `externalOutputMode: "passthrough"`、`targetFps: 60`、`exitOnCtrlC: false`、`useKittyKeyboard: {}`、`autoFocus: false`，mouse 由 `Flag.OPENCODE_DISABLE_MOUSE` 和 `input.config.mouse` 共同决定。[E: packages/tui/src/app.tsx:184] [E: packages/tui/src/app.tsx:184] [E: packages/tui/src/app.tsx:190] [E: packages/tui/src/app.tsx:190] [E: packages/tui/src/app.tsx:197]
2. renderer release path 调 `destroyRenderer(renderer)`；Windows input guard、OpenTUI default keymap、OpenCode keymap 注册、pluginHost dispose、audio dispose、SIGHUP destroy 都在 scoped finalizer 范围内。[E: packages/tui/src/app.tsx:206] [E: packages/tui/src/app.tsx:209] [E: packages/tui/src/app.tsx:210] [E: packages/tui/src/app.tsx:212] [E: packages/tui/src/app.tsx:218] [E: packages/tui/src/app.tsx:224] [E: packages/tui/src/app.tsx:226]
3. 首次 render 前预热 terminal palette，等待 theme mode，避免 `system` theme 第一帧 fallback flash。[E: packages/tui/src/app.tsx:236] [E: packages/tui/src/app.tsx:237]
4. Solid `render()` 把 provider stack 挂到 renderer：`ExitProvider`、`EpilogueProvider`、`ErrorBoundary`、runtime paths/env/startup providers、clipboard、keymap、args、KV、toast、route、config、plugin runtime、SDK、project、sync、data、theme、local、prompt stash/dialog/frecency/history/ref/editor context 都在 root 组合。[E: packages/tui/src/app.tsx:240] [E: packages/tui/src/app.tsx:251] [E: packages/tui/src/app.tsx:276] [E: packages/tui/src/app.tsx:291] [E: packages/tui/src/app.tsx:292] [E: packages/tui/src/app.tsx:293] [E: packages/tui/src/app.tsx:302] [E: packages/tui/src/app.tsx:304] [E: packages/tui/src/app.tsx:313] [E: packages/tui/src/app.tsx:313]
5. `App` 创建 plugin API adapters，把 version/config/dialog/keymap/route/event/sdk/sync/theme/toast/renderer/attention/Slot 注入 `createTuiApi`，再调用 `pluginHost.start({ api, config, runtime, dispose })`；失败只写 console，不阻断 ready。[E: packages/tui/src/app.tsx:383] [E: packages/tui/src/app.tsx:404] [E: packages/tui/src/app.tsx:410] [E: packages/tui/src/app.tsx:414]
6. `App` 根据 `route.data.type` 切换 `Home` 或 `Session`，plugin route 则通过 `pluginRuntime.routes.get(route.data.id)` 渲染；缺失 plugin route 渲染 `PluginRouteMissing`。[E: packages/tui/src/app.tsx:1068] [E: packages/tui/src/app.tsx:1069] [E: packages/tui/src/app.tsx:1099] [E: packages/tui/src/app.tsx:1102] [E: packages/tui/src/app.tsx:1108]
7. `run` 等待 renderer destroy 对应的 `shutdown` deferred，收尾时 flush Windows input buffer，并把 exit reason/epilogue 写到 stderr/stdout。[E: packages/tui/src/app.tsx:231] [E: packages/tui/src/app.tsx:348] [E: packages/tui/src/app.tsx:353] [E: packages/tui/src/app.tsx:355]
...
因此 TUI root 的架构选择是“host-agnostic UI package + host-injected transport/plugin host”。当前源码还没有完全达到 spec 的 `@opencode-ai/core` 去依赖目标，但它已经把 server startup、config loading、plugin loading 留给 V1 host adapter；主 UI root 通过 `TuiInput` 消费 resolved inputs。[E: packages/tui/src/app.tsx:137] [E: specs/tui-package.md:529] [I]

## Gotcha

- `packages/tui` 是 terminal UI package，不是 V2 core；节点 `v: na` 表示它是 client/presentation layer，但它同时消费 legacy SDK events 和 V2 `session.next.*` events。[E: packages/tui/src/context/sync.tsx:170] [E: packages/tui/src/context/data.tsx:132] [I]
...
- `packages/tui/src/runtime.tsx` 不是 renderer runtime，只包含 `abbreviateHome()` helper；主 renderer lifecycle 在 `packages/tui/src/app.tsx`。[E: packages/tui/src/runtime.tsx:3] [E: packages/tui/src/app.tsx:184]
