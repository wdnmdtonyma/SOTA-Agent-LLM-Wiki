...
- stored messages / home sessions 按什么时间排序。
- 哪些迁移边界仍未完成。
...
bootstrap 已把 legacy config read 变成显式 V1-only 分支：global config 在 protocol 不是 V1 时直接返回空对象，directory config 同样跳过 `sdk.config.get()`。global bootstrap 原本就用 `Promise.allSettled` 聚合 slow reads，单个 legacy global-config rejection 不会直接抛出整个 bootstrap；directory bootstrap 则会收集 slow errors，只有零错误时才把 status 设为 `complete`，所以跳过缺失 endpoint 主要消除了 directory completion 的阻断。UI store 的 legacy `config` 字段在 V2 下也不是 current config 的镜像。[E: packages/app/src/context/global-sync/bootstrap.ts:88][E: packages/app/src/context/global-sync/bootstrap.ts:89][E: packages/app/src/context/global-sync/bootstrap.ts:108][E: packages/app/src/context/global-sync/bootstrap.ts:112][E: packages/app/src/context/global-sync/bootstrap.ts:113][E: packages/app/src/context/global-sync/bootstrap.ts:154][E: packages/app/src/context/global-sync/bootstrap.ts:166][E: packages/app/src/context/global-sync/bootstrap.ts:383][E: packages/app/src/context/global-sync/bootstrap.ts:384][E: packages/app/src/context/global-sync/bootstrap.ts:541][E: packages/app/src/context/global-sync/bootstrap.ts:552]
...
stream clean close或失败后会重连；current transport 被测试为 volatile，重连不会发送 `Last-Event-ID`，所以不能把这个客户端队列描述成 durable replay 或无损恢复。[E: packages/app/src/context/server-sdk.tsx:293][E: packages/app/src/context/server-sdk.tsx:307][E: packages/app/src/context/server-sdk.tsx:308][E: packages/app/e2e/regression/session-timeline-transport.spec.ts:73][E: packages/app/e2e/regression/session-timeline-transport.spec.ts:86][E: packages/app/e2e/regression/session-timeline-transport.spec.ts:100][E: packages/app/e2e/regression/session-timeline-transport.spec.ts:111]

## Session projection

`ServerSyncProvider` 为 session helper 同时传入 legacy client、compatible session/message API 和 protocol。收到 event 时，带 `current` 的 event 先走 `applyV2()`，随后仍走 legacy-shaped `apply()`，让 current reducer 与既有 UI store 共存。[E: packages/app/src/context/server-sync.tsx:227][E: packages/app/src/context/server-sync.tsx:228][E: packages/app/src/context/server-sync.tsx:531][E: packages/app/src/context/server-sync.tsx:538][E: packages/app/src/context/server-sync.tsx:539]

current server 的消息列表使用 cursor pagination（`order: "desc"`）；结果先 `toReversed()` 成 oldest-first `SessionMessageInfo[]` source，再由 `normalizeSessionMessages()` 投影成 legacy `Message[]` 与按 message ID 分组的 `Part[]`。写入 store 前 `session` 数组再按 `compareMessages` 排序。V1 server 则继续使用 legacy `session.messages()` response，同样 sort。[E: packages/app/src/context/server-session.ts:537][E: packages/app/src/context/server-session.ts:542][E: packages/app/src/context/server-session.ts:552][E: packages/app/src/context/server-session.ts:553][E: packages/app/src/context/server-session.ts:555][E: packages/app/src/context/server-session.ts:566][E: packages/app/src/context/server-session.ts:572]

`compareMessages` / `messageKey` 的主键是 `time.created + id`，不是 durable seq 或 arrival order。[E: packages/app/src/utils/session-message.ts:15][E: packages/app/src/utils/session-message.ts:21] 分页边界 `needsOlderTurnRoot()` 看 oldest-first source 的第一条 user/shell/assistant/synthetic-description：若边界是 assistant，再向前翻一页，避免 turn 从半截 assistant 开始。[E: packages/app/src/context/server-session.ts:35][E: packages/app/src/context/server-session.ts:43][E: packages/app/src/context/server-session.ts:546] event 到达时也按 `compareMessages` 插入/重排，不按 id 字典序。[E: packages/app/src/context/server-session.ts:930][E: packages/app/src/context/server-session.ts:1343] 这是 store 内的 creation-time 合同，不是对并发 hydrate/SSE 竞态的全覆盖保证。[I]
...
`projectV2()` 保存 current source，只重新投影 touched messages/parts，并通过 legacy `message.updated` / `message.part.updated` 形状喂给原 store；`applyV2()` 同时维护 session rename/move/usage 和 execution status。只有 reducer 报告缺失 promoted input 时，上层才按 message ID 做一次 hydrate，而且失败被静默忽略；这不是通用 event-gap recovery。[E: packages/app/src/context/server-session.ts:883][E: packages/app/src/context/server-session.ts:901][E: packages/app/src/context/server-session.ts:911][E: packages/app/src/context/server-session.ts:924][E: packages/app/src/context/server-session.ts:936][E: packages/app/src/context/server-session.ts:939][E: packages/app/src/context/server-session.ts:941][E: packages/app/src/context/server-session.ts:946][E: packages/app/src/context/server-session.ts:957][E: packages/app/src/context/server-session.ts:963]

## Timeline

`normalizeSessionMessages()` 维护当前 agent/model/parent 上下文，把 user、synthetic、shell、assistant 和 compaction source 转成 legacy messages/parts。[E: packages/app/src/utils/session-message.ts:48][E: packages/app/src/utils/session-message.ts:55][E: packages/app/src/utils/session-message.ts:64][E: packages/app/src/utils/session-message.ts:70][E: packages/app/src/utils/session-message.ts:83][E: packages/app/src/utils/session-message.ts:90][E: packages/app/src/utils/session-message.ts:107][E: packages/app/src/utils/session-message.ts:120]

`MessageTimeline` 同时读取 legacy message store 与 current `session_message` source。timeline projection 以 current source 的输入顺序建立 turns，再用 projected legacy messages/parts 渲染；shell 生成成对 user/assistant turn，孤立 assistant 会按 parent 查回 user。尚未进入 current source 的 optimistic user messages 按 `compareMessages`（creation time + id）插入 turn 列表，而不是一律 append。[E: packages/app/src/pages/session/timeline/message-timeline.tsx:283][E: packages/app/src/pages/session/timeline/message-timeline.tsx:289][E: packages/app/src/pages/session/timeline/projection.ts:32][E: packages/app/src/pages/session/timeline/projection.ts:40][E: packages/app/src/pages/session/timeline/rows.ts:36][E: packages/app/src/pages/session/timeline/rows.ts:45][E: packages/app/src/pages/session/timeline/rows.ts:75][E: packages/app/src/pages/session/timeline/rows.ts:78][E: packages/app/src/pages/session/timeline/rows-current.test.ts:17][E: packages/app/src/pages/session/timeline/rows-current.test.ts:162] current source 本身的 turn 顺序仍不按 timestamp 或 durable sequence 重排；App 层没有证明该输入顺序总等于 aggregate seq。[U]
...
Terminal component 的 resize 同样分流：V1 调 legacy update，current 调 `api.pty.update` 并传 `location.directory`。[E: packages/app/src/components/terminal.tsx:244][E: packages/app/src/components/terminal.tsx:246][E: packages/app/src/components/terminal.tsx:255][E: packages/app/src/components/terminal.tsx:257] `gone()` 不是持续 liveness probe，而是 abnormal-close retry 前的检查：V1 以 get 的 404 判 gone；current 以 `status === "exited"` 或 `PtyNotFoundError` 判 gone。[E: packages/app/src/components/terminal.tsx:539][E: packages/app/src/components/terminal.tsx:542][E: packages/app/src/components/terminal.tsx:543][E: packages/app/src/components/terminal.tsx:549][E: packages/app/src/components/terminal.tsx:550][E: packages/app/src/components/terminal.tsx:551][E: packages/app/src/components/terminal.tsx:553][E: packages/app/src/components/terminal.tsx:588][E: packages/app/src/components/terminal.tsx:596][E: packages/app/src/components/terminal.tsx:686][E: packages/app/src/components/terminal.tsx:695]
...
connect-token wiring 仍有迁移张力：active `connectToken()` 只在 protocol 为 V1 时调用 legacy endpoint；current path 会得到 `undefined` ticket。[E: packages/app/src/components/terminal.tsx:560][E: packages/app/src/components/terminal.tsx:562][I] `open()` 随后解析 protocol 并继续创建 WebSocket，把 optional ticket 传给 URL builder，没有 active no-ticket barrier。[E: packages/app/src/components/terminal.tsx:611][E: packages/app/src/components/terminal.tsx:615][E: packages/app/src/components/terminal.tsx:620][E: packages/app/src/components/terminal.tsx:627][I] 迁移 checklist 虽把 current connect-token 标为完成，但 App 源码不能证明 ticketless current handshake 能成功，也不能证明其预期 authorization contract；因此不能把 current ticket acquisition 写成 live behavior。[E: packages/app/V1_API_MIGRATION.md:193][U]
