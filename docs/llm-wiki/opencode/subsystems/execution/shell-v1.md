...
输入 schema 由 `ShellPrompt.parameterSchema` 生成：`command` 必填，`timeout` 可选正整数毫秒，`workdir` 可选，`description` 必填 [E: packages/opencode/src/tool/shell/prompt.ts:22]。tool runtime 使用 configured shell：`Shell.acceptable(cfg.shell)` 会拒绝不合适 shell 并选择 fallback [E: packages/opencode/src/tool/shell.ts:611] [E: packages/opencode/src/shell/shell.ts:204] [E: packages/opencode/src/shell/shell.ts:205] [E: packages/opencode/src/shell/shell.ts:107] [E: packages/opencode/src/shell/shell.ts:108] [E: packages/opencode/src/shell/shell.ts:113]。
...
- `Shell.args()` 在 `packages/opencode/src/shell/shell.ts` 中存在，但本工具实际执行使用 `tool/shell.ts` 的 `cmd()` helper；不要把两个路径混成一个控制流 [E: packages/opencode/src/shell/shell.ts:159] [E: packages/opencode/src/tool/shell.ts:299]。
