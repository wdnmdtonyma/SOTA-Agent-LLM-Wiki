...
- `ZEN_LITE_PRICE` 在 `dev` 与其它 stage 分别绑哪组 Stripe product/price?
...
Linkable 是跨资源配置模型。Console database Linkable 暴露 host/database/username/password/port [E: infra/console.ts:36] [E: infra/console.ts:38] [E: infra/console.ts:39] [E: infra/console.ts:40] [E: infra/console.ts:41] [E: infra/console.ts:42]。`ConsoleMigration` 只在 production/`dev` 填 `consoleUrl` / `inferenceUrl`（`https://{domain}/console` 与 `https://{domain}/inference`）；其它 preview stage 是空字符串，避免把独立 DB 的 workspace 送到 shared dev。[E: infra/console.ts:226][E: infra/console.ts:228][E: infra/console.ts:230][E: infra/console.ts:231] `ZEN_LITE_PRICE`：`$app.stage === "dev"` 时 `product`/`price` 用已有 Stripe `prod_U1tUscpmwtV2bG` / `price_1T3phhE7fOCwHSD4zS6w2NPy`；其它 stage 用新建 `zenLiteProduct.id` / `zenLitePrice.id`。[E: infra/console.ts:154][E: infra/console.ts:155] Lake ingest Linkable 暴露 ingest service URL 和 secret [E: infra/lake.ts:274] [E: infra/lake.ts:276] [E: infra/lake.ts:277]。Secret 模块还把 random password 包装成 Linkable property, 并集中声明 R2、Honeycomb、`SUPPORT_API_KEY`、Upstash secrets [E: infra/secret.ts:1] [E: infra/secret.ts:7] [E: infra/secret.ts:8] [E: infra/secret.ts:9] [E: infra/secret.ts:10] [E: infra/secret.ts:12] [E: infra/secret.ts:13] [E: infra/secret.ts:14]。
...
6. Console SolidStart 的 link 除 buckets/database/Upstash/auth/Stripe/SES/Salesforce 外，还包含 `consoleMigration` 与 `SECRET.SupportApiKey`。[E: infra/console.ts:269][E: infra/console.ts:271]
...
- `ConsoleMigration.inferenceUrl` 在非 production/dev stage 为空；`proxyInference` 仍会在有 `migratedAt` 时构造 destination，空 URL 会在运行时失败而不是静默回本地 handler。[E: infra/console.ts:231][I]
