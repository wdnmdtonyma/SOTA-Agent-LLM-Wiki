...
V2 `ToolOutputStore.Service` 是 core registry settlement 的 model-output bounding 服务：它接收完整 `ToolOutput`，测量 provider-facing textual projection 或 structured JSON preview，超限时写 managed tool-output file，并返回 bounded `ToolOutput` 和 typed `outputPaths`。[E: packages/core/src/tool-output-store.ts:18][E: packages/core/src/tool-output-store.ts:136][E: packages/core/src/tool-output-store.ts:139][E: packages/core/src/tool-output-store.ts:143][E: packages/core/src/tool-output-store.ts:152][E: packages/core/src/tool-output-store.ts:157][E: packages/core/src/tool-output-store.ts:161][E: packages/core/src/tool-output-store.ts:166][E: specs/v2/tools.md:157]
...
| 实体 | V1 | V2 |
| --- | --- | --- |
| 默认行数 | `MAX_LINES = 2000`。[E: packages/opencode/src/tool/truncate.ts:15] | `MAX_LINES = 2_000`。[E: packages/core/src/tool-output-store.ts:12] |
| 默认字节 | `MAX_BYTES = 50 * 1024`。[E: packages/opencode/src/tool/truncate.ts:16] | `MAX_BYTES = 50 * 1024`。[E: packages/core/src/tool-output-store.ts:13] |
| 保留时间 | `RETENTION = Duration.days(7)`。[E: packages/opencode/src/tool/truncate.ts:13] | `RETENTION = Duration.days(7)`。[E: packages/core/src/tool-output-store.ts:14] |
| result | `{ content, truncated }` 或 `{ content, truncated, outputPath }`。[E: packages/opencode/src/tool/truncate.ts:20] | `{ output, outputPaths }`。[E: packages/core/src/tool-output-store.ts:24] |
| config | V1 从 config `tool_output.max_lines/max_bytes` 读，缺省用常量。[E: packages/opencode/src/tool/truncate.ts:79] | V2 config schema `max_lines/max_bytes`，store 从 document entries 合并配置。[E: packages/core/src/config/tool-output.ts:7][E: packages/core/src/config/tool-output.ts:8][E: packages/core/src/tool-output-store.ts:115][E: packages/core/src/tool-output-store.ts:118][E: packages/core/src/tool-output-store.ts:120] |
| storage dir | `TRUNCATION_DIR`。[E: packages/opencode/src/tool/truncate.ts:17] | `MANAGED_DIRECTORY = "tool-output"` under global data。[E: packages/core/src/tool-output-store.ts:16][E: packages/core/src/tool-output-store.ts:112] |
...
1. Registry settle 得到 leaf tool 的完整 `ToolOutput` 后调用 `resources.bound({ sessionID, toolCallID, output })`。[E: packages/core/src/tool/registry.ts:61][E: packages/core/src/tool/registry.ts:74]
2. `ToolOutputStore.bound` 读取 limits。[E: packages/core/src/tool-output-store.ts:132][E: packages/core/src/tool-output-store.ts:133]
3. 如果 output content 为空，V2 把 structured output JSON stringify 为 contextual text；否则只拼接 text content，并把 media/file content 单独保留。[E: packages/core/src/tool-output-store.ts:134][E: packages/core/src/tool-output-store.ts:136][E: packages/core/src/tool-output-store.ts:142]
4. contextual text 如果行数和字节都在 limit 内，返回原 output 和空 `outputPaths`。[E: packages/core/src/tool-output-store.ts:143][E: packages/core/src/tool-output-store.ts:147]
5. 超限时写完整 contextual text 到 shared `tool-output/tool_<id>` 文件。[E: packages/core/src/tool-output-store.ts:123][E: packages/core/src/tool-output-store.ts:152]
6. 构造 marker，把 bounded preview 放进 text content，同时保留 structured output 和 media parts，返回 `outputPaths: [outputPath]`。[E: packages/core/src/tool-output-store.ts:153][E: packages/core/src/tool-output-store.ts:155][E: packages/core/src/tool-output-store.ts:163][E: packages/core/src/tool-output-store.ts:166]
7. V2 cleanup 每小时扫描 managed directory，删除 mtime 超过 retention 的 `tool_` 文件。[E: packages/core/src/tool-output-store.ts:170][E: packages/core/src/tool-output-store.ts:172][E: packages/core/src/tool-output-store.ts:174][E: packages/core/src/tool-output-store.ts:181][E: packages/core/src/tool-output-store.ts:192][E: packages/core/src/tool-output-store.ts:195]

## 5 V1 / V2 对照

| 维度 | V1 | V2 |
| --- | --- | --- |
| bounding 位置 | leaf wrapper 和 MCP bridge 调用 `Truncate.output`；shell 使用 Truncate limits/write 加 shell-local tail bounding。[E: packages/opencode/src/tool/tool.ts:135][E: packages/opencode/src/session/tools.ts:176][E: packages/opencode/src/tool/shell.ts:445][E: packages/opencode/src/tool/shell.ts:579][E: packages/opencode/src/tool/shell.ts:582] | registry settlement 后统一调用 ToolOutputStore。[E: packages/core/src/tool/registry.ts:74] |
