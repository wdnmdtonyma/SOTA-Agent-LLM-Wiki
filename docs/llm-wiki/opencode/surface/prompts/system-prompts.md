...
`SystemPrompt.provider(model)` 根据 `model.api.id` 选择 prompt family，而不是根据 provider name 选择。[E: packages/opencode/src/session/system.ts:25]

| match 条件 | prompt family | source line |
| --- | --- | --- |
| `model.api.id` 包含 `gpt-4`、`o1` 或 `o3` | `beast.txt` | [E: packages/opencode/src/session/system.ts:26] |
| `model.api.id` 包含 `gpt` 且包含 `codex` | `codex.txt` | [E: packages/opencode/src/session/system.ts:28] |
| `model.api.id` 包含 `gpt` 但不包含 `codex` | `gpt.txt` | [E: packages/opencode/src/session/system.ts:32] |
| `model.api.id` 包含 `gemini-` | `gemini.txt` | [E: packages/opencode/src/session/system.ts:34] |
| `model.api.id` 包含 `claude` | `anthropic.txt` | [E: packages/opencode/src/session/system.ts:35] |
| lower-case `model.api.id` 包含 `trinity` | `trinity.txt` | [E: packages/opencode/src/session/system.ts:36] |
| lower-case `model.api.id` 包含 `kimi` | `kimi.txt` | [E: packages/opencode/src/session/system.ts:37] |
| 以上条件都不满足 | `default.txt` | [E: packages/opencode/src/session/system.ts:38] |

导入表显示这些 family prompt 的真实 `.txt` 文件：`anthropic.txt`、`default.txt`、`beast.txt`、`gemini.txt`、`gpt.txt`、`kimi.txt`、`codex.txt` 和 `trinity.txt`。[E: packages/opencode/src/session/system.ts:6]
...
environment prompt 会写入精确 model id、working directory、workspace root、git repo 状态、platform 和当天日期。[E: packages/opencode/src/session/system.ts:61] 如果 reference service 返回带 description 的 references，environment prompt 会附加 `<available_references>`，每条 reference 带 name、path 和可选 description。[E: packages/opencode/src/session/system.ts:73]

skills prompt 先检查 agent permission：如果 `skill` permission disabled，就直接不注入 skills 文字。[E: packages/opencode/src/session/system.ts:95] 未禁用时，源码调用 `skill.available(agent)`，并以 verbose 格式加入 skills 列表。[E: packages/opencode/src/session/system.ts:97]

## Reminder 注入

`SessionReminders` 不修改 system message；它把 reminder 作为 synthetic text part 推入最后一条 user message。[E: packages/opencode/src/session/reminders.ts:28]
...
| 开启 experimental plan mode，从 `plan` 切到非 plan | `build-switch.txt`，若存在 plan file 还附加 plan path | [E: packages/opencode/src/session/reminders.ts:51] |
| 开启 experimental plan mode，进入 `plan` 且上一条 assistant 不是 `plan` | `plan-mode.txt`，替换 `${planInfo}` | [E: packages/opencode/src/session/reminders.ts:70] |

session loop 还有两个独立 reminder 机制：step 大于 1 且存在上一条完成消息时，新 user text 会包进 `<system-reminder>`。[E: packages/opencode/src/session/prompt.ts:1307] 当 `isLastStep` 为真时，model messages 尾部追加来自 `max-steps.txt` 的 assistant message。[E: packages/opencode/src/session/prompt.ts:1343]
...
| `max-steps.txt` | imported by `prompt.ts` | step 超限时的 text-only reminder。[E: packages/opencode/src/session/prompt.ts:19] |
