...
V2 config service 名称是 `@opencode/v2/Config`。[E: packages/core/src/config.ts:133] loader 支持文件名 `opencode.json`、`opencode.jsonc`。[E: packages/core/src/config.ts:142] JSONC 解析允许 trailing comma。[E: packages/core/src/config.ts:152]

loader 同时准备 V2 `Info` decoder 与 V1 `ConfigV1.Info` decoder。[E: packages/core/src/config.ts:144] [E: packages/core/src/config.ts:145] 如果 `ConfigMigrateV1.isV1(input)` 判断为 true，就先 decode V1，再 migrate，再 decode V2；否则直接 decode V2。[E: packages/core/src/config.ts:156]

V2 discovery 最终把 `configs` 组装为 global supplementary、direct project configs、再到后续 `.opencode` supplementary entries。[E: packages/core/src/config.ts:203] policy loading 对这些 config 调用 `toReversed()` 后再取 `experimental.policies`。[E: packages/core/src/config.ts:209] [E: packages/core/src/config.ts:210]

## Top-level Catalog

| V2 key | type/default | 含义 | V1 来源 |
| --- | --- | --- | --- |
| `$schema` | optional string；schema default omitted | JSON schema reference。[E: packages/core/src/config.ts:30] | V1 `$schema` 原样迁移。[E: packages/core/src/v1/config/migrate.ts:37] |
| `shell` | optional string | default shell for terminal/shell tool。[E: packages/core/src/config.ts:33] | V1 `shell` 原样迁移。[E: packages/core/src/v1/config/migrate.ts:38] |
| `model` | optional string | default model when session/agent has none。[E: packages/core/src/config.ts:36] | V1 `model` 原样迁移。[E: packages/core/src/v1/config/migrate.ts:39] |
| `default_agent` | optional string | default primary agent。[E: packages/core/src/config.ts:39] | V1 `default_agent` 原样迁移。[E: packages/core/src/v1/config/migrate.ts:40] |
| `autoupdate` | optional boolean or `"notify"` | automatic update/notify behavior。[E: packages/core/src/config.ts:42] | V1 `autoupdate` 原样迁移。[E: packages/core/src/v1/config/migrate.ts:41] |
| `share` | `manual`/`auto`/`disabled` optional | sharing behavior。[E: packages/core/src/config.ts:47] | V1 `share` or `autoshare` alias。[E: packages/core/src/v1/config/migrate.ts:42] |
| `enterprise.url` | optional string | enterprise sharing service config。[E: packages/core/src/config.ts:50] | V1 `enterprise` 原样迁移。[E: packages/core/src/v1/config/migrate.ts:43] |
| `username` | optional string | displayed username / telemetry identity。[E: packages/core/src/config.ts:57] | V1 `username` 原样迁移。[E: packages/core/src/v1/config/migrate.ts:44] |
| `permissions` | optional V2 ruleset | ordered tool permission rules。[E: packages/core/src/config.ts:60] | V1 `permission` and `tools` migrate here。[E: packages/core/src/v1/config/migrate.ts:45] |
| `agents` | optional record agent info | named built-in overrides and custom agents。[E: packages/core/src/config.ts:63] | V1 `agent` plus deprecated `mode` migrate here。[E: packages/core/src/v1/config/migrate.ts:46] |
| `snapshots` | optional boolean | snapshots for undo/revert behavior。[E: packages/core/src/config.ts:66] | V1 `snapshot` renamed。[E: packages/core/src/v1/config/migrate.ts:47] |
| `watcher` | optional watcher info | filesystem watcher config。[E: packages/core/src/config.ts:69] | V1 `watcher` 原样迁移。[E: packages/core/src/v1/config/migrate.ts:48] |
| `formatter` | optional formatter info | built-in formatter enablement/overrides。[E: packages/core/src/config.ts:72] | V1 `formatter` 原样迁移。[E: packages/core/src/v1/config/migrate.ts:49] |
| `lsp` | optional LSP info | language server enablement/overrides。[E: packages/core/src/config.ts:75] | V1 `lsp` 原样迁移。[E: packages/core/src/v1/config/migrate.ts:50] |
| `attachments` | optional attachments info | attachment processing config。[E: packages/core/src/config.ts:78] | V1 `attachment` renamed。[E: packages/core/src/v1/config/migrate.ts:51] |
| `tool_output` | optional thresholds | tool output truncation thresholds。[E: packages/core/src/config.ts:81] | V1 `tool_output` 原样迁移。[E: packages/core/src/v1/config/migrate.ts:52] |
| `mcp` | optional MCP info | MCP server config。[E: packages/core/src/config.ts:84] | V1 `mcp` and `experimental.mcp_timeout` migrate here。[E: packages/core/src/v1/config/migrate.ts:53] |
| `compaction` | optional compaction info | conversation compaction behavior。[E: packages/core/src/config.ts:87] | V1 `compaction` reshaped。[E: packages/core/src/v1/config/migrate.ts:54] |
| `skills` | optional string array | local/remote skill discovery sources。[E: packages/core/src/config.ts:90] | V1 `skills.paths` + `skills.urls` concat。[E: packages/core/src/v1/config/migrate.ts:62] |
| `commands` | optional record command info | named slash command definitions。[E: packages/core/src/config.ts:93] | V1 `command` renamed by migration。[E: packages/core/src/v1/config/migrate.ts:63] |
| `instructions` | optional string array | ambient instruction sources。[E: packages/core/src/config.ts:96] | V1 `instructions` 原样迁移。[E: packages/core/src/v1/config/migrate.ts:64] |
| `references` | optional reference info | named local/git external context。[E: packages/core/src/config.ts:99] | V1 `references ?? reference`。[E: packages/core/src/v1/config/migrate.ts:65] |
| `plugins` | optional plugin array | ordered external plugin packages。[E: packages/core/src/config.ts:102] | V1 `plugin` renamed and tuple entries objectified。[E: packages/core/src/v1/config/migrate.ts:66] |
| `experimental` | optional experimental info | experimental V2-only subkeys; currently policies。[E: packages/core/src/config.ts:105] | V1 only migrates `experimental.policies`。[E: packages/core/src/v1/config/migrate.ts:69] |
| `providers` | optional record provider info | provider/model config overrides。[E: packages/core/src/config.ts:106] | V1 `provider` renamed。[E: packages/core/src/v1/config/migrate.ts:70] |

## Permission 与 Policy

V2 permission effect 允许 `allow`、`deny`、`ask`。[E: packages/schema/src/permission.ts:54] Permission rule 是 `{ action, resource, effect }`。[E: packages/schema/src/permission.ts:58] Ruleset 是 rule array。[E: packages/schema/src/permission.ts:64] V2 config describes permissions as ordered tool permission rules。[E: packages/core/src/config.ts:61]

V2 policy effect 只有 `allow` 和 `deny`。[E: packages/core/src/policy.ts:8] Policy info 同样有 `action`、`effect`、`resource`。[E: packages/core/src/policy.ts:12] [E: packages/core/src/policy.ts:13] [E: packages/core/src/policy.ts:14] policy evaluation 用 `findLast` 匹配 action/resource wildcard，因此同一加载序列里后面的 statement 覆盖前面的 statement。[E: packages/core/src/policy.ts:38]
...
| `agents.<name>.request.body` | optional record unknown | agent request body options。[E: packages/core/src/config/provider.ts:9] | V1 agent `options`/`temperature`/`top_p` migrate into request body when body is non-empty。[E: packages/core/src/v1/config/migrate.ts:108] [E: packages/core/src/v1/config/migrate.ts:109] [E: packages/core/src/v1/config/migrate.ts:110] [E: packages/core/src/v1/config/migrate.ts:115] |
...
| `watcher.ignore` | optional string array | filesystem ignore patterns。[E: packages/core/src/config/watcher.ts:6] | V1 `watcher.ignore`。 |
...
| `mcp.servers.<name>.url` | string | remote MCP URL。[E: packages/core/src/config/mcp.ts:36] | V1 same field。 |
| `mcp.servers.<name>.headers` | optional record string | remote headers。[E: packages/core/src/config/mcp.ts:37] | V1 same field。 |
| `mcp.servers.<name>.oauth.client_id` | optional string | OAuth client id。[E: packages/core/src/config/mcp.ts:27] | V1 `clientId`。 |
| `mcp.servers.<name>.oauth.client_secret` | optional string | OAuth secret。[E: packages/core/src/config/mcp.ts:28] | V1 `clientSecret`。 |
| `mcp.servers.<name>.oauth.scope` | optional string | OAuth scopes。[E: packages/core/src/config/mcp.ts:29] | V1 same field。 |
| `mcp.servers.<name>.oauth.callback_port` | optional int 1-65535 | OAuth callback port。[E: packages/core/src/config/mcp.ts:30] | V1 `callbackPort`。 |
| `mcp.servers.<name>.oauth.redirect_uri` | optional string | OAuth redirect URI。[E: packages/core/src/config/mcp.ts:31] | V1 `redirectUri`。 |
...
| `compaction.keep.tokens` | optional non-negative int | recent-history token budget。[E: packages/core/src/config/compaction.ts:7] | V1 `preserve_recent_tokens`。 |
