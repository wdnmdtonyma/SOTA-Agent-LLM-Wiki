...
| `provider.<name>.options.headerTimeout` | optional `PositiveInt` or `false`；schema 文档 default `300000` | wait for response headers。`false` 关闭 header timeout。V1 `resolveSDK` fetch wrapper 用 `headerTimeout ?? 300_000`。[E: packages/core/src/v1/config/provider.ts:108][E: packages/opencode/src/provider/provider.ts:1800][E: packages/opencode/src/provider/provider.ts:1808] | V2 通过 request body/options 表达。[I] |
| `provider.<name>.options.chunkTimeout` | optional `PositiveInt` or `false`；schema 文档 default `300000` | streamed SSE chunk timeout。`false` 或非正数不 wrap SSE。V1 `resolveSDK` fetch wrapper 用 `chunkTimeout ?? 300_000`。[E: packages/core/src/v1/config/provider.ts:117][E: packages/opencode/src/provider/provider.ts:1799][E: packages/opencode/src/provider/provider.ts:1807] | V2 通过 request body/options 表达。[I] |
...
| --- | --- | --- | --- |
| `mcp.<name>.type` | `local` or `remote` | MCP server connection kind。[E: packages/core/src/v1/config/mcp.ts:7] | V2 keeps explicit tagged local/remote under `mcp.servers`。[E: packages/core/src/config/mcp.ts:43] |
| `mcp.<name>.command` | string array | local server command/args。[E: packages/core/src/v1/config/mcp.ts:8] | V2 `mcp.servers.<name>.command`。[E: packages/core/src/config/mcp.ts:17] |
| `mcp.<name>.cwd` | optional string | local MCP process working directory；相对路径相对 workspace。[E: packages/core/src/v1/config/mcp.ts:11] | V2 `mcp.servers.<name>.cwd`；migration 原样复制。[E: packages/core/src/config/mcp.ts:18] [E: packages/core/src/v1/config/migrate.ts:144] |
| `mcp.<name>.environment` | optional record string | local server env vars。[E: packages/core/src/v1/config/mcp.ts:14] | V2 `environment`。[E: packages/core/src/config/mcp.ts:21] |
| `mcp.<name>.enabled` | optional boolean | enable/disable server on startup。[E: packages/core/src/v1/config/mcp.ts:17] | V2 renamed/inverted to `disabled`; migration writes `!enabled` when present。[E: packages/core/src/v1/config/migrate.ts:139] |
| `mcp.<name>.timeout` | optional positive int；description says default 5000 | request timeout in ms。[E: packages/core/src/v1/config/mcp.ts:20] | V2 reshapes per-server timeout into a `Timeout` object and adds top-level `mcp.timeout`; migration writes V1 int into `request` for local and remote servers。[E: packages/core/src/config/mcp.ts:23] [E: packages/core/src/config/mcp.ts:46] [E: packages/core/src/v1/config/migrate.ts:147] [E: packages/core/src/v1/config/migrate.ts:161] |
| `mcp.<name>.url` | string | remote MCP URL。[E: packages/core/src/v1/config/mcp.ts:46] | V2 `url`。[E: packages/core/src/config/mcp.ts:36] |
| `mcp.<name>.headers` | optional record string | remote headers。[E: packages/core/src/v1/config/mcp.ts:50] | V2 `headers`。[E: packages/core/src/config/mcp.ts:37] |
| `mcp.<name>.oauth` | optional OAuth object or false | OAuth config or opt-out。[E: packages/core/src/v1/config/mcp.ts:53] | V2 keeps object-or-false but snake_cases fields。[E: packages/core/src/config/mcp.ts:38] |
| `oauth.clientId` | optional string | OAuth client ID。[E: packages/core/src/v1/config/mcp.ts:27] | V2 `client_id`。[E: packages/core/src/config/mcp.ts:27] |
| `oauth.clientSecret` | optional string | OAuth client secret。[E: packages/core/src/v1/config/mcp.ts:30] | V2 `client_secret`。[E: packages/core/src/config/mcp.ts:28] |
| `oauth.scope` | optional string | OAuth scopes。[E: packages/core/src/v1/config/mcp.ts:33] | V2 `scope`。[E: packages/core/src/config/mcp.ts:29] |
| `oauth.callbackPort` | optional int 1-65535 | local OAuth callback port。[E: packages/core/src/v1/config/mcp.ts:34] | V2 `callback_port`。[E: packages/core/src/config/mcp.ts:30] |
| `oauth.redirectUri` | optional string | OAuth redirect URI。[E: packages/core/src/v1/config/mcp.ts:38] | V2 `redirect_uri`。[E: packages/core/src/config/mcp.ts:31] |
...
| custom LSP `extensions` requirement | validation rule | non-disabled custom servers must declare extensions because runtime cannot infer them；disabled entries are exempt。[E: packages/core/src/v1/config/lsp.ts:63] [E: packages/core/src/v1/config/lsp.ts:68] | V2 aggregate schema keeps only the broad boolean/record shape, and the design note says validation belongs with eventual V2 LSP integration。[E: packages/core/src/config/lsp.ts:18] [E: specs/v2/config.md:125] [I] |
