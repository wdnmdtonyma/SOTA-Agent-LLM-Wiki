...
V2 `apply_patch` 是 core 内核的 typed patch tool。description 明确限定 add/update/delete，说明路径会先 resolve/approve，然后 sequential apply；moves 和 atomic rollback unsupported。[E: packages/core/src/tool/apply-patch.ts:72]

## 3 输入 schema 表

| 字段 | V1 输入 | V2 输入 | 语义与迁移注意 |
| --- | --- | --- | --- |
| patchText | `patchText: string`。[E: packages/opencode/src/tool/apply_patch.ts:19] | `patchText: string`。[E: packages/core/src/tool/apply-patch.ts:20] | 两代都要求非空 patch text；空字符串失败。[E: packages/opencode/src/tool/apply_patch.ts:34][E: packages/core/src/tool/apply-patch.ts:91] |
...
- V2 输出是 `{ applied: Applied[], files: FileDiff.Info[] }`，每个 applied 项 type 只能是 `add/update/delete`，包含 target/resource。[E: packages/core/src/tool/apply-patch.ts:25][E: packages/core/src/tool/apply-patch.ts:31][E: packages/core/src/tool/apply-patch.ts:33]
- V2 `toModelOutput` 先输出标题行 `"Applied patch sequentially:"`，再逐行输出 `A|D|M <resource>`。[E: packages/core/src/tool/apply-patch.ts:37][E: packages/core/src/tool/apply-patch.ts:39][E: packages/core/src/tool/apply-patch.ts:41]
- V2 failure helper 会把已应用的 partial list 拼进 `ToolFailure.message`；当前 helper 没有把 partial list 作为 typed data 字段返回。[E: packages/core/src/tool/apply-patch.ts:78][E: packages/core/src/tool/apply-patch.ts:83]
...
1. 校验 `patchText` 非空，调用 `Patch.parse`。[E: packages/core/src/tool/apply-patch.ts:91][E: packages/core/src/tool/apply-patch.ts:93]
2. 空 patch 失败；任何 update hunk 带 `movePath` 都立即失败。[E: packages/core/src/tool/apply-patch.ts:96][E: packages/core/src/tool/apply-patch.ts:97][E: packages/core/src/tool/apply-patch.ts:98]
3. 解析所有 target 并做 external directory 权限。[E: packages/core/src/tool/apply-patch.ts:100][E: packages/core/src/tool/apply-patch.ts:108]
4. 对所有 target resources 申请 `edit` 权限。[E: packages/core/src/tool/apply-patch.ts:116]
5. prepare 阶段：add 不读旧文件，delete 只 stat 检查文件存在，update 读取旧内容并通过 `Patch.derive` 计算新内容。[E: packages/core/src/tool/apply-patch.ts:128][E: packages/core/src/tool/apply-patch.ts:138][E: packages/core/src/tool/apply-patch.ts:139][E: packages/core/src/tool/apply-patch.ts:146]
6. apply 阶段顺序执行：add 用 `files.create`，delete 用 `files.remove`，update 用 `writeIfUnchanged`。[E: packages/core/src/tool/apply-patch.ts:164][E: packages/core/src/tool/apply-patch.ts:175][E: packages/core/src/tool/apply-patch.ts:179]
7. 任一步出错时用 `fail(..., applied)` 风格的闭包返回错误消息，消息内列出此前已应用资源。[E: packages/core/src/tool/apply-patch.ts:78][E: packages/core/src/tool/apply-patch.ts:82][E: packages/core/src/tool/apply-patch.ts:185]
...
| gating | 只在 GPT patch 条件下暴露，并隐藏 edit/write。[E: packages/opencode/src/tool/registry.ts:273][E: packages/opencode/src/tool/registry.ts:274][E: packages/opencode/src/tool/registry.ts:275] | builtins 固定注册，无同等 modelID gate。[E: packages/core/src/tool/builtins.ts:35] |
...
- V1 apply_patch 是模型选择层的一部分：registry 根据 model id 决定是暴露 patch 工具，还是暴露 edit/write。这不是 patch parser 的能力判断，而是模型能力/提示策略判断。[E: packages/opencode/src/tool/registry.ts:273][E: packages/opencode/src/tool/registry.ts:274][E: packages/opencode/src/tool/registry.ts:275][I]
