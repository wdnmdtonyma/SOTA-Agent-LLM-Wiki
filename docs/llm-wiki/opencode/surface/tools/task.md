...
Task 先用 parent turn 的 `ctx.ask` 请求 `permission: "task"`，`patterns` 是 `subagent_type`，`always` 是 `["*"]`；`ctx.extra.bypassAgentCheck` 为 true 时跳过这一步，V1 subtask 内部调用会设置该 bypass。[E: packages/opencode/src/tool/task.ts:119][E: packages/opencode/src/tool/task.ts:120][E: packages/opencode/src/tool/task.ts:121][E: packages/opencode/src/tool/task.ts:122][E: packages/opencode/src/tool/task.ts:123][E: packages/opencode/src/session/prompt.ts:331]

child session 的 permission 不是简单复制 parent agent：`deriveSubagentSessionPermission()` 只继承 parent session 的 deny rules 与 `external_directory` rules，再按 subagent 自己的 permission 是否包含 `todowrite`/`task` 决定是否追加默认 deny。[E: packages/opencode/src/agent/subagent-permissions.ts:14][E: packages/opencode/src/agent/subagent-permissions.ts:18][E: packages/opencode/src/agent/subagent-permissions.ts:19][E: packages/opencode/src/agent/subagent-permissions.ts:21][E: packages/opencode/src/agent/subagent-permissions.ts:22][E: packages/opencode/src/agent/subagent-permissions.ts:24][E: packages/opencode/src/agent/subagent-permissions.ts:25] `TaskTool` 还会把 `cfg.experimental?.primary_tools` 逐项转成 child deny rule，防止 subagent 使用 primary-only tools。[E: packages/opencode/src/tool/task.ts:150][E: packages/opencode/src/tool/task.ts:151][E: packages/opencode/src/tool/task.ts:153]

## 6 execute() 走读

1. `TaskTool.execute` 读取 config、runtime flags 和 assistant message；如果当前 tool call 不属于 assistant message，直接失败。[E: packages/opencode/src/tool/task.ts:89][E: packages/opencode/src/tool/task.ts:96][E: packages/opencode/src/tool/task.ts:98][E: packages/opencode/src/tool/task.ts:174][E: packages/opencode/src/tool/task.ts:178]
2. V1 Task 校验 `subagent_type` 存在；没有 `task_id` 时创建 child session，child title 是 `${description} (@${next.name} subagent)`，child agent 是 `next.name`。[E: packages/opencode/src/tool/task.ts:131][E: packages/opencode/src/tool/task.ts:158][E: packages/opencode/src/tool/task.ts:160][E: packages/opencode/src/tool/task.ts:161]
3. child model 优先使用 subagent 自己的 `next.model`，否则继承 parent assistant message 的 `modelID/providerID`。[E: packages/opencode/src/tool/task.ts:181][E: packages/opencode/src/tool/task.ts:182][E: packages/opencode/src/tool/task.ts:183]
4. execute 要求 `ctx.extra.promptOps` 存在；该对象提供 cancel、resolvePromptParts 和 prompt，是 Task 调回 `SessionPrompt` 的桥。[E: packages/opencode/src/tool/task.ts:197][E: packages/opencode/src/tool/task.ts:198][E: packages/opencode/src/session/prompt.ts:128][E: packages/opencode/src/session/prompt.ts:133]
5. `runTask()` 解析 prompt parts 后调用 `ops.prompt()`，目标 session 是 child session，agent 是 subagent。若返回的 assistant 带 `info.error`，或最后一条 tool part 的 `state.status === "error"`，则 `Effect.fail`，消息含 `task_id: ${nextSession.id}`；否则返回最后一个 text part 的 text。[E: packages/opencode/src/tool/task.ts:200][E: packages/opencode/src/tool/task.ts:201][E: packages/opencode/src/tool/task.ts:202][E: packages/opencode/src/tool/task.ts:204][E: packages/opencode/src/tool/task.ts:210][E: packages/opencode/src/tool/task.ts:213][E: packages/opencode/src/tool/task.ts:218][E: packages/opencode/src/tool/task.ts:220][E: packages/opencode/src/tool/task.ts:222][E: packages/opencode/src/tool/task.ts:224]
6. 如果已有同 id background job 正在跑，`background.extend()` 会把新 run 排到旧 tail 之后，并返回 “Background task updated”。[E: packages/opencode/src/tool/task.ts:267][E: packages/opencode/src/tool/task.ts:278][E: packages/core/src/background-job.ts:262][E: packages/core/src/background-job.ts:263][E: packages/core/src/background-job.ts:281]
7. 新 job 通过 `background.start()` 注册；`onPromote` 会把 parent tool metadata 改成 background 并启动 completion notification。[E: packages/opencode/src/tool/task.ts:284][E: packages/opencode/src/tool/task.ts:289][E: packages/opencode/src/tool/task.ts:292][E: packages/opencode/src/tool/task.ts:294]
8. foreground 默认等待 `background.wait()` 或 `waitForPromotion()`；如果 job 被 promote 成 background，foreground 调用也返回 running background result。[E: packages/opencode/src/tool/task.ts:334][E: packages/opencode/src/tool/task.ts:335][E: packages/opencode/src/tool/task.ts:338]
9. foreground abort 会触发 child session cancel；Effect interrupt release 阶段还会 cancel background job。[E: packages/opencode/src/tool/task.ts:321][E: packages/opencode/src/tool/task.ts:325][E: packages/opencode/src/tool/task.ts:349][E: packages/opencode/src/tool/task.ts:350]
...
| 背景模式 | 可选 experimental；field 只有 flag 开启才暴露给模型。[E: packages/opencode/src/tool/task.ts:362][E: packages/opencode/src/tool/task.ts:366] | 没有 V2 equivalent，这是从 V2 builtins deps 枚举推断。[E: packages/core/src/tool/builtins.ts:35][E: packages/core/src/tool/builtins.ts:46][I] |
