...
tier: T1
v: shared
source: [packages/opencode/src/tool/skill.ts, packages/opencode/src/tool/skill.txt, packages/opencode/src/tool/registry.ts, packages/opencode/src/skill/index.ts, packages/core/src/tool/skill.ts, packages/core/src/skill.ts, packages/core/src/plugin/boot.ts, packages/core/src/tool/builtins.ts, CONTEXT.md]
...
- V2 `SkillTool` 为什么等待 `PluginBoot`？

## V1

### 1 Identity

V1 `SkillTool` 通过 `Tool.define("skill", ...)` 注册，registry 初始化 `skilltool` 并把 `tool.skill` 放入 builtin 列表。[E: packages/opencode/src/tool/skill.ts:13][E: packages/opencode/src/tool/skill.ts:14][E: packages/opencode/src/tool/registry.ts:107][E: packages/opencode/src/tool/registry.ts:210][E: packages/opencode/src/tool/registry.ts:232]
...
| 字段 | 类型 | 必填 | 默认 | 约束 | 说明 |
|---|---|---:|---|---|---|
| `name` | `string` | 是 | 无 | 必须能被 `Skill.Service.require(name)` 找到 | 要加载的 skill name。[E: packages/opencode/src/tool/skill.ts:9][E: packages/opencode/src/tool/skill.ts:10][E: packages/opencode/src/tool/skill.ts:25] |

### 4 输出 & 大小/截断限制

V1 输出是 `<skill_content name="...">` 包裹的文本，包含 `# Skill: <name>`、skill content、base directory file URL、relative path 说明、sampled `<skill_files>` 列表。[E: packages/opencode/src/tool/skill.ts:48][E: packages/opencode/src/tool/skill.ts:49][E: packages/opencode/src/tool/skill.ts:50][E: packages/opencode/src/tool/skill.ts:52][E: packages/opencode/src/tool/skill.ts:54][E: packages/opencode/src/tool/skill.ts:55][E: packages/opencode/src/tool/skill.ts:58] metadata 保存 `name` 和计算出的 `dir = path.dirname(info.location)`。[E: packages/opencode/src/tool/skill.ts:35][E: packages/opencode/src/tool/skill.ts:64][E: packages/opencode/src/tool/skill.ts:65]

文件列表来自 ripgrep：cwd 是 `path.dirname(info.location)`，disk skill 时这就是 skill directory；built-in skill 的 `location` 是 `"<built-in>"`，这是一个需要调用方理解的 edge。[E: packages/opencode/src/tool/skill.ts:35][E: packages/opencode/src/skill/index.ts:281] ripgrep pattern 是 `!**/SKILL.md`，hidden true，follow false，limit 10；输出中每个 file 都用 absolute path。[E: packages/opencode/src/tool/skill.ts:37][E: packages/opencode/src/tool/skill.ts:38][E: packages/opencode/src/tool/skill.ts:39][E: packages/opencode/src/tool/skill.ts:40][E: packages/opencode/src/tool/skill.ts:41][E: packages/opencode/src/tool/skill.ts:43][E: packages/opencode/src/tool/skill.ts:59]

### 5 权限

V1 在确认 skill 存在后调用 `ctx.ask`，`permission: "skill"`，`patterns` 与 `always` 都是该 skill name。[E: packages/opencode/src/tool/skill.ts:25][E: packages/opencode/src/tool/skill.ts:28][E: packages/opencode/src/tool/skill.ts:29][E: packages/opencode/src/tool/skill.ts:30][E: packages/opencode/src/tool/skill.ts:31]

### 6 execute() 走读

1. `SkillTool.execute` 调 `skill.require(params.name)`；NotFoundError 会变成 defect error，其 message 包含 available skill 列表。[E: packages/opencode/src/tool/skill.ts:25][E: packages/opencode/src/tool/skill.ts:26][E: packages/opencode/src/skill/index.ts:73][E: packages/opencode/src/skill/index.ts:78]
2. permission 通过后，V1 计算 `path.dirname(info.location)` 和 directory file URL。[E: packages/opencode/src/tool/skill.ts:35][E: packages/opencode/src/tool/skill.ts:36]
...
V2 `SkillTool` name 是 `"skill"`，通过 `[name]: Tool.make(...)` 注册；`BuiltInTools.locationLayer` 把 `SkillTool.layer` 放入 V2 内建工具层。[E: packages/core/src/tool/skill.ts:14][E: packages/core/src/tool/skill.ts:66][E: packages/core/src/tool/skill.ts:67][E: packages/core/src/tool/builtins.ts:39]
...
| 字段 | 类型 | 必填 | 默认 | 约束 | 说明 |
|---|---|---:|---|---|---|
| `name` | `string` | 是 | 无 | 必须匹配当前 `SkillV2.Service.list()` 中某个 skill name | 要加载的 skill name。[E: packages/core/src/tool/skill.ts:17][E: packages/core/src/tool/skill.ts:18][E: packages/core/src/tool/skill.ts:74][E: packages/core/src/tool/skill.ts:75] |

### 4 输出 & 大小/截断限制

V2 output schema 是 `{ name, directory, output }`，`toModelOutput` 只把 `output.output` 作为 text 发给模型。[E: packages/core/src/tool/skill.ts:21][E: packages/core/src/tool/skill.ts:22][E: packages/core/src/tool/skill.ts:23][E: packages/core/src/tool/skill.ts:24][E: packages/core/src/tool/skill.ts:71] `toModelOutput()` 的文本结构与 V1 同形[I]，也包含 `<skill_content>`、base directory file URL、sampled `<skill_files>`。[E: packages/core/src/tool/skill.ts:35][E: packages/core/src/tool/skill.ts:38][E: packages/core/src/tool/skill.ts:43][E: packages/core/src/tool/skill.ts:45][E: packages/core/src/tool/skill.ts:47][E: packages/core/src/tool/skill.ts:50]

### 5 权限

V2 在找到 skill 后调用 `PermissionV2.assert`，`action: "skill"`，`resources` 和 `save` 都是 `[skill.name]`，source 携带 message/call id。[E: packages/core/src/tool/skill.ts:14][E: packages/core/src/tool/skill.ts:78][E: packages/core/src/tool/skill.ts:79][E: packages/core/src/tool/skill.ts:80][E: packages/core/src/tool/skill.ts:81][E: packages/core/src/tool/skill.ts:84]
...
5. V2 `SkillV2.available()` 也按 `PermissionV2.evaluate("skill", skill.name, agent.permissions)` 过滤 deny。[E: packages/core/src/skill.ts:57][E: packages/core/src/skill.ts:58]
