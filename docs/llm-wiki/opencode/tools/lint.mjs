#!/usr/bin/env node
// L1 机械校验 —— opencode 源码 LLM wiki。无外部依赖,跑:node tools/lint.mjs
// 落地 conventions.md 第 5 节 10 条规则。errors → exit 1;warnings 不影响退出码。
...
const SRC = path.resolve(WIKI, "../../../opencode") // Best/opencode/
...
for (const n of nodes) {
  ...
  if (n.v && !VS.has(n.v)) err(w, `v 非法: ${n.v}`)
  ...
  // Rule 3 (index 侧): source 路径存在于 Best/opencode/
  for (const s of n.source || []) if (!srcExists(s)) err(w, `source 路径不存在于 opencode/: ${s}`)
}
...
const SRC_TOKEN = /(?:packages|specs|infra|nix|script|install|sdks|patches|\.github|\.opencode)\/[^\s`)\]]+|(?:AGENTS|CONTEXT|CONTRIBUTING|SECURITY|README)\.md|package\.json|turbo\.json|bunfig\.toml|sst\.config\.ts|flake\.nix/g
for (const n of nodes) {
  ...
  if (fm.v && !VS.has(fm.v)) err(w, `v 非法: ${fm.v}`)
  ...
  // Rule 10: updated 是合法 opencode 短 SHA
  if (fm.updated) {
    try { execSync(`git -C "${SRC}" cat-file -e ${fm.updated}^{commit}`, { stdio: "ignore" }) }
    catch { err(w, `updated 不是合法 opencode commit SHA: ${fm.updated}`) }
  }
}
